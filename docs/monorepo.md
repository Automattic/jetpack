...
* Some CI jobs (e.g. the WPCOMSH WP Cloud build) check out a project on a branchless, detached-HEAD commit and run a plain `composer update` with no `COMPOSER_ROOT_VERSION` override. There, Composer's version guesser can't label sibling path packages `dev-trunk`, so a third-party dependency with a caret constraint against one of those packages (e.g. `automattic/woocommerce-analytics` against `automattic/jetpack-assets`) fails to resolve — the path package's guessed version doesn't satisfy the constraint, and Composer refuses to fall back to Packagist because the path repo is canonical. If you hit this, pin the affected packages to `dev-trunk` explicitly via `options.versions` on the project's monorepo path repository in `composer.json` (see `projects/packages/premium-analytics/composer.json`, `projects/packages/jetpack-mu-wpcom/composer.json`, and `projects/plugins/wpcomsh/composer.json` for the pattern, added for `automattic/woocommerce-analytics`'s dependencies in Automattic/jetpack#50266). This mirrors what `tools/cli/commands/build.js` injects automatically for `--for-mirrors` builds — those pins just aren't needed for every other CI path, so add only what a specific failing job requires.
