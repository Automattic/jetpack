...
**Warning: This creates a tunnel to your local machine which means that if it is compromised, so is everything on your computer. Only `jetpack docker jt-up` when needed for testing things that require the site to be publicly accessible, and `jetpack docker jt-down` when completed.**
