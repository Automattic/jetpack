...
class Jetpack_Autoupdate {
	...
	function autoupdate_plugin( $update, $item ) {
		...
		if ( in_array( $item->plugin, $autoupdate_plugin_list ) ) {
			...
			// Logs that we are expecting this plugin to be updated.
			// The update could fail for various reasons and the item may be absent from the results.
			$this->autoupdate_expected['plugin'][] = $item->plugin;
 			return true;
		}
	...
	/**
	 * Parses the autoupdate results generated by WP_Automatic_Updater and returns
	 * a simple array of successful items
	 *
	 * @param string $item 'plugin' or 'theme'
	 *
	 * @return array
	 */
	private function get_successful_updates( $item = 'plugin' ) {
		$successful_updates = array();

		if ( ! isset( $this->autoupdate_results[ $item ] ) ) {
			return $successful_updates;
		}

		foreach( $this->autoupdate_results[ $item ] as $result ) {
			if ( $result->result ) {
				switch( $result ) {
					case 'theme':
	...
	function log_items( $items ) {
		...
		if ( $items_failed ) {
			$this->jetpack->stat( "autoupdates/$items-fail", $plugins_failed );
		}

	}

	function log_results() {
		// If we've run an autoupdate, lets compare the expected results to the actual results, and log our findings.
		if ( $this->is_updating ) {
