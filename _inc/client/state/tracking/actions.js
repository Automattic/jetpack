...
export const fetchTrackingSettings = () => {
	return ( dispatch ) => {
		...
		return restApi.fetchUserTrackingSettings().then( settings => {
			dispatch( {
...
export const updateTrackingSettings = ( newSettings ) => {
	return ( dispatch ) => {
		const messages = {
				...
				error: error => __( 'Error updating privacy settings. %(error)s', { args: { error: error.message || error.code || error.name || error } } )
			},
		...
		return restApi.updateUserTrackingSettings( newSettings ).then( success => {
			dispatch( {
			...
			dispatch( createNotice(
				'is-success',
				messages.success,
				{ id: 'tracking-settings-update-success', duration: 2000 }
			) );
		} ).catch( error => {
			dispatch( {
