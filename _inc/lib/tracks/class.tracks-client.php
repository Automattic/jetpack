...
class Jetpack_Tracks_Client {
	...
	static function record_event( $event ) {
		if ( ! $event instanceof Jetpack_Jetpack_Tracks_Client ) {
			$event = new Jetpack_Jetpack_Tracks_Client( $event );
		}
	 ...
	 * @deprecated          Call the `build_pixel_url` method on a Jetpack_Jetpack_Tracks_Client object instead.
	 * @param  array $event Event keys and values
	 * @return string       URL of a tracking pixel
	 */
	static function build_pixel_url( $event ) {
		$_event = new Jetpack_Jetpack_Tracks_Client( $event );
		return $_event->build_pixel_url();
	}

	/**
	 * Validate input for a tracks event.
	 * @deprecated          Instantiate a Jetpack_Jetpack_Tracks_Client object instead
	 * @param  array $event Event keys and values
