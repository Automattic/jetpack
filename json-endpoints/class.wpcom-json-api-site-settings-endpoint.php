...
class WPCOM_JSON_API_Site_Settings_Endpoint extends WPCOM_JSON_API_Endpoint {
		...
		foreach ( array_keys( $response_format ) as $key ) {
			switch ( $key ) {
				...
				case 'settings':
					...
					$jetpack_relatedposts_allowed = $this->jetpack_relatedposts_supported();
					$jetpack_relatedposts_options = Jetpack_Options::get_option( 'relatedposts' );

					if ( wpcom_is_vip() || Enterprise()->is_enabled() ) {
						if ( !is_array( $jetpack_relatedposts_options ) )
							$jetpack_relatedposts_options = array();
						if ( !isset( $jetpack_relatedposts_options['enabled'] ) ) {
							$jetpack_relatedposts_options['enabled'] = false;
						}
					}
					...
					$response[$key] = array(
						...
						'jetpack_relatedposts_allowed' => (bool) $jetpack_relatedposts_allowed,
	...
	public function update_settings() {
		...
		if ( count( $jetpack_relatedposts_options ) ) {
			Jetpack_Options::update_option( 'relatedposts', $jetpack_relatedposts_options );
		}
