	...
	public function find_invocation_warnings( $invocation, $warnings ) {
		if ( $invocation instanceof New_ ) {
			...
			if ( $invocation->class_name === $this->old_declaration->class_name ) {
				$warnings->add( new Warning( $invocation->path, $invocation->line, 'Class ' . $this->old_declaration->display_name() . ' was moved from ' . $this->old_declaration->path . ' to ' . $this->new_declaration->path ) );
			}
		}
	}
}
