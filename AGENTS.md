...
- **Reach for a `@wordpress/ui` component before writing CSS that reimplements one.** Custom flex/gap/margin CSS for spacing is usually a `<Stack gap="...">`. Custom font-size/color/line-height CSS is usually a `<Text>` — pick one of its documented variants (https://wordpress.github.io/gutenberg/?path=/docs/design-system-components-text--docs#all-variants) rather than sizing it manually. Also check for `Card`, `Badge`, `Notice`, `Icon`/`Tooltip`, `Link`, `VisuallyHidden`, `Popover`/`Modal`, and `Tabs` before hand-rolling equivalents — if a chunk of custom CSS looks like it's recreating one of these, replace it with the component. Known deprecated → recommended swaps: `Col`/`Container` → `Stack`; `IconTooltip` → `Icon` + `Tooltip` combined; `DonutMeter` → `PieChart` from `@automattic/charts`; ad hoc loading placeholders → `Spinner`/`ProgressBar`. (Jetpack is also rolling out an ESLint rule, `@wordpress/use-recommended-components`, to catch these automatically — once merged, defer to its list as canonical.)
...
- **Don't layer custom style overrides on top of a design-system component just to preserve old visual behavior.** If an override only exists because "that's how it looked before," prefer using the component as-is, or flag it as needing an explicit design decision — overrides on shared components are unwieldy to maintain long-term. If you find yourself overriding an existing override (an exception stacked on an exception), that's a signal the shared component or design needs to change centrally, not gain another one-off patch.
- **Prefer a component's own props over CSS overrides that depend on internal/unstable selectors** — e.g. use `<Page hasPadding={false}>` instead of overriding `Page`'s padding with CSS targeting its class names.
...
- **Favor shared dashboard scaffolding over one-off local dashboard layouts.** Use `<Page>` from `@wordpress/admin-ui` and other shared layout components instead of custom per-package admin-layout CSS, so dashboards stay visually consistent. Keep cross-cutting behavior (e.g. Tabs) centralized in the shared component rather than adjusting or reimplementing it per page — local tweaks fragment consistency and defeat the point of centralizing it in the first place. New pages should be scaffolded with current tooling (`wp-build`) from the start rather than older per-project setups. If shared scaffolding seems to be missing something, that's a signal to raise it with design-system owners rather than diverging locally.
- **Don't fork a shared component to get a variant** (e.g. duplicating `JetpackFooter` to drop a couple of links) — extend the shared component with a prop instead, so consistency doesn't drift and there's only one place to maintain.
...
- Known temporary limitation: `@wordpress/ui`'s `Button` can't yet render as a link (https://github.com/WordPress/gutenberg/issues/77098). Until that's fixed, workarounds like `render={ <a /> }`, `render={ <Link /> }`, or an `onClick` that navigates are acceptable — link the issue in a code comment so it reads as a deliberate, temporary workaround.
- Prefer importing icons directly from `@wordpress/icons` rather than re-exporting/wrapping them locally, unless there's a concrete reason (e.g. the package must stay bundled rather than externalized) — local re-export layers can silently drift from upstream renames.
