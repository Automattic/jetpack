...
- **Reimplemented components**: custom CSS that reproduces a `@wordpress/ui` primitive — flex/gap/margin layout instead of `<Stack gap="...">`, custom font-size/color/line-height instead of a `<Text>` variant, or hand-rolled `Card`, `Badge`, `Notice`, `Icon`/`Tooltip`, `Link`, `VisuallyHidden`, `Popover`/`Modal`, `Tabs`. `[suggestion]`: point at the matching component. Known deprecated → recommended swaps: `Col`/`Container` → `Stack`; `IconTooltip` → `Icon` + `Tooltip`; `DonutMeter` → `PieChart` from `@automattic/charts`; ad hoc loading placeholders → `Spinner`/`ProgressBar`.
...
- **Unjustified overrides on design-system components**: extra CSS layered on a `@wordpress/ui` component purely to preserve old visual behavior, or CSS overriding an existing override (an exception stacked on an exception). `[suggestion]`: ask if the override is still needed, or flag it as a sign the shared component/design needs a central fix rather than another local patch.
- **CSS overrides where a component prop would do the job** — e.g. overriding `Page`'s padding with CSS instead of `<Page hasPadding={false}>`. `[suggestion]`: use the prop; it's a stable API, the CSS selector isn't.
...
- **New one-off dashboard layout CSS or behavior** duplicating shared scaffolding (e.g. `<Page>` from `@wordpress/admin-ui`, or per-page Tabs tweaks that belong centralized in `Page`/`AdminPage`). `[suggestion]`: use the shared component/behavior for consistency across Jetpack dashboards. New pages should be scaffolded with current tooling (`wp-build`) from the start.
- **Forked/duplicated shared components** to get a small variant (e.g. copying `JetpackFooter` to drop a couple of links). `[suggestion]`: extend the shared component with a prop instead of forking it.
...
- **Local re-exports/wrappers around `@wordpress/icons`** with no concrete reason (bundling, stability). `[suggestion]`: import the icon directly.
