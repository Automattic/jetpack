...
class Jetpack
{
	 ...
	 */
	var $concatenated_style_handles = array( 'jetpack-carousel', 'grunion.css', 'the-neverending-homepage', 'jetpack_likes', 'jetpack_related-posts', 'sharedaddy', 'jetpack-slideshow', 'presentations', 'jetpack-subscriptions', 'tiled-gallery', 'widget-conditions', 'jetpack_display_posts_widget', 'gravatar-profile-widget', 'widget-grid-and-list', 'jetpack-widgets', 'goodreads-widget', );

	var $plugins_to_deactivate = array( 'stats' => array( 'stats/stats.php', 'WordPress.com Stats' ), 'shortlinks' => array( 'stats/stats.php', 'WordPress.com Stats' ), 'sharedaddy' => array( 'sharedaddy/sharedaddy.php', 'Sharedaddy' ), 'twitter-widget' => array( 'wickett-twitter-widget/wickett-twitter-widget.php', 'Wickett Twitter Widget' ), 'after-the-deadline' => array( 'after-the-deadline/after-the-deadline.php', 'After The Deadline' ), 'contact-form' => array( 'grunion-contact-form/grunion-contact-form.php', 'Grunion Contact Form' ), 'contact-form' => array( 'mullet/mullet-contact-form.php', 'Mullet Contact Form' ), 'custom-css' => array( 'safecss/safecss.php', 'WordPress.com Custom CSS' ), 'random-redirect' => array( 'random-redirect/random-redirect.php', 'Random Redirect' ), 'videopress' => array( 'video/video.php', 'VideoPress' ), 'widget-visibility' => array( 'jetpack-widget-visibility/widget-visibility.php', 'Jetpack Widget Visibility' ), 'widget-visibility' => array( 'widget-visibility-without-jetpack/widget-visibility-without-jetpack.php', 'Widget Visibility Without Jetpack' ), 'sharedaddy' => array( 'jetpack-sharing/sharedaddy.php', 'Jetpack Sharing' ), 'omnisearch' => array( 'jetpack-omnisearch/omnisearch.php', 'Jetpack Omnisearch' ), 'gravatar-hovercards' => array( 'jetpack-gravatar-hovercards/gravatar-hovercards.php', 'Jetpack Gravatar Hovercards' ), 'latex' => array( 'wp-latex/wp-latex.php', 'WP LaTeX' ) );

	var $capability_translations = array( 'administrator' => 'manage_options', 'editor' => 'edit_others_posts', 'author' => 'publish_posts', 'contributor' => 'edit_posts', 'subscriber' => 'read', );
	 ...
	 */
	private $open_graph_conflicting_plugins = array( '2-click-socialmedia-buttons/2-click-socialmedia-buttons.php', // 2 Click Social Media Buttons
	 ...
	 */
	private $twitter_cards_conflicting_plugins = array( //	'twitter/twitter.php',                       // The official one handles this on its own.
	...
	{
		if ( ! self::$instance ) {
			if ( did_action( 'plugins_loaded' ) ) self::plugin_textdomain(); else
	...
	private function Jetpack ()
	{
		...
		if ( is_multisite() ) {
			...
			if ( is_network_admin() ) return; // End here to prevent single site actions from firing
		}
		...
		$this->sync->options( JETPACK__PLUGIN_DIR . 'jetpack.php', 'home', 'siteurl', 'blogname', 'gmt_offset', 'timezone_string', 'security_report', 'stylesheet', "theme_mods_{$theme_slug}", 'jetpack_sync_non_public_post_stati', 'jetpack_options' );
	...
	{
		// Check for nonce
		if ( ! isset( $_REQUEST[ 'jumpstartNonce' ] ) || ! wp_verify_nonce( $_REQUEST[ 'jumpstartNonce' ], 'jetpack-jumpstart-nonce' ) ) wp_die( 'permissions check failed' );
	...
	public function register_assets ()
	{
		...
		if ( ! wp_style_is( 'jetpack-icons', 'registered' ) ) wp_register_style( 'jetpack-icons', plugins_url( 'css/jetpack-icons.min.css', JETPACK__PLUGIN_FILE ), false, JETPACK__VERSION );
	}
	...
	function user_role_change ( $user_id )
	{
		if ( Jetpack::is_active() && Jetpack::is_user_connected( $user_id ) ) {
			...
			if ( ! $master_user_id ) return; // this shouldn't happen
	...
	public static function load_modules ()
	{
		...
		if ( Jetpack::is_active() || Jetpack::is_development_mode() ) require_once( JETPACK__PLUGIN_DIR . 'modules/module-extras.php' );
	}
	...
	public function check_rest_api_compat ()
	{
		...
		if ( function_exists( 'bbpress' ) ) $_jetpack_rest_api_compat_includes[ ] = JETPACK__PLUGIN_DIR . 'class.jetpack-bbpress-json-api-compat.php';

		foreach ( $_jetpack_rest_api_compat_includes as $_jetpack_rest_api_compat_include ) require_once $_jetpack_rest_api_compat_include;
	}
	...
	public static function create_nonce ( $name )
	{
		...
		if ( empty( $secret_1 ) || empty( $secret_2 ) || $eol < time() ) return new Jetpack_Error( 'missing_secrets' );
	...
	public static function update_user_token ( $user_id, $token, $is_master_user )
	{
		...
		if ( ! is_array( $user_tokens ) ) $user_tokens = array();
	...
	public static function activate_new_modules ( $redirect = false )
	{
		...
		Jetpack_Options::update_options( array( 'version' => $new_version, 'old_version' => $jetpack_old_version, ) );
	...
	{
		$deprecated_modules = array( 'debug' => null,  // Closed out and moved to ./class.jetpack-debugger.php
	...
	public static function activate_module ( $module, $exit = true, $redirect = true )
	{
		...
		if ( ! strlen( $module ) ) return false;

		if ( ! Jetpack::is_module( $module ) ) return false;
		...
		if ( ! Jetpack::is_active() ) {
			if ( ! Jetpack::is_development_mode() ) return false;

			// If we're not connected but in development mode, make sure the module doesn't require a connection
			if ( Jetpack::is_development_mode() && $module_data[ 'requires_connection' ] ) return false;
		}
	...
	public static function plugin_activation ( $network_wide )
	{
		...
		if ( $network_wide ) Jetpack::state( 'network_nag', true );
	...
	{
		if ( ! $tokens = Jetpack_Options::get_option( 'user_tokens' ) ) return false;

		$user_id = empty( $user_id ) ? get_current_user_id() : intval( $user_id );

		if ( Jetpack_Options::get_option( 'master_user' ) == $user_id ) return false;

		if ( ! isset( $tokens[ $user_id ] ) ) return false;
	...
	function admin_init ()
	{
		...
		if ( // the plugin was auto-activated and needs its candy
			Jetpack_Options::get_option( 'do_activate' ) || // the plugin is active, but was never activated.  Probably came from a site-wide network activation
	...
	function prepare_connect_notice ()
	{
		...
		if ( Jetpack::state( 'network_nag' ) ) add_action( 'network_admin_notices', array( $this, 'network_connect_notice' ) );
	}
	...
	function admin_help ()
	{
		...
		// Help Sidebar
		$current_screen->set_help_sidebar( '<p><strong>' . __( 'For more information:', 'jetpack' ) . '</strong></p>' . '<p><a href="http://jetpack.me/faq/" target="_blank">' . __( 'Jetpack FAQ', 'jetpack' ) . '</a></p>' . '<p><a href="http://jetpack.me/support/" target="_blank">' . __( 'Jetpack Support', 'jetpack' ) . '</a></p>' . '<p><a href="' . Jetpack::admin_url( array( 'page' => 'jetpack-debugger' ) ) . '">' . __( 'Jetpack Debugging Center', 'jetpack' ) . '</a></p>' );
	}
	...
	function jetpack_menu_order ( $menu_order )
	{
		...
		foreach ( $menu_order as $index => $item ) {
			if ( $item != 'jetpack' ) $jp_menu_order[ ] = $item;

			if ( $index == 0 ) $jp_menu_order[ ] = 'jetpack';
		}
	...
	function admin_head ()
	{
		if ( isset( $_GET[ 'configure' ] ) && Jetpack::is_module( $_GET[ 'configure' ] ) && current_user_can( 'manage_options' ) ) do_action( 'jetpack_module_configuration_head_' . $_GET[ 'configure' ] );
	}
	...
	function plugin_action_links ( $actions )
	{
		...
		if ( current_user_can( 'jetpack_manage_modules' ) && ( Jetpack::is_active() || Jetpack::is_development_mode() ) ) {
			return array_merge( $jetpack_home, array( 'settings' => sprintf( '<a href="%s">%s</a>', Jetpack::admin_url( 'page=jetpack_modules' ), __( 'Settings', 'jetpack' ) ) ), $actions );
		}
	...
	function admin_connect_notice ()
	{
		...
		if ( empty( $_GET[ 'page' ] ) || 'jetpack' !== $_GET[ 'page' ] ) return;

		if ( ! current_user_can( 'jetpack_connect' ) ) return;
		...
		<div id="message" class="updated jetpack-message jp-banner" style="display:block !important;">
			...
			<?php if ( in_array( Jetpack_Options::get_option( 'activated' ), array( 1, 2, 3 ) ) ) : ?>
				...
				<div class="jp-banner__action-container is-connection">
					<a href="<?php echo $this->build_connect_url() ?>" class="jp-banner__button"
	...
	function admin_jetpack_manage_notice ()
	{
		...
		if ( ! in_array( $screen->base, array( 'dashboard' ) ) || $screen->is_network || $screen->action ) return;
	...
	function can_display_jetpack_manage_notice ()
	{
		...
		if ( ! current_user_can( 'jetpack_manage_modules' ) ) return false;
		...
		if ( ! Jetpack_Options::get_option( 'public' ) ) return false;
	...
	public static function jetpack_comment_notice ()
	{
		...
		return '<br /><br />' . sprintf( __( 'Jetpack now includes Jetpack Comments, which enables your visitors to use their WordPress.com, Twitter, or Facebook accounts when commenting on your site. To activate Jetpack Comments, <a href="%s">%s</a>.', 'jetpack' ), wp_nonce_url( Jetpack::admin_url( array( 'page' => 'jetpack', 'action' => 'activate', 'module' => 'comments', ) ), 'jetpack_activate-comments' ), __( 'click here', 'jetpack' ) );
	}
	...
	function admin_page_load ()
	{
		...
		if ( isset( $_GET[ 'connect_url_redirect' ] ) ) {
			...
			if ( ! Jetpack::is_user_connected() ) {
				...
				if ( isset( $_GET[ 'notes_iframe' ] ) ) $connect_url .= '&notes_iframe';
	...
	function admin_notices ()
	{
		...
		if ( $this->can_display_jetpack_manage_notice() && ! $this->error && ! $this->message && ! $this->privacy_checks ) {
			if ( isset( $_GET[ 'page' ] ) && 'jetpack' != $_GET[ 'page' ] ) $this->opt_in_jetpack_manage_notice();
		}
	}
	...
	{
		if ( ! isset( $this->stats[ $group ] ) ) $this->stats[ $group ] = array();
	...
	{
		$defaults = array( 'v' => 'wpcom2', 'rand' => md5( mt_rand( 0, 999 ) . time() ), );
		$args = wp_parse_args( $args, $defaults );
		$base_url = apply_filters( 'jetpack_stats_base_url', set_url_scheme( 'http://pixel.wp.com/g.gif' ) );
	...
	function admin_page ()
	{
		...
		<div class="wrap" id="jetpack-settings">
						...
						<div id="jp-disconnectors">
							<?php if ( current_user_can( 'jetpack_disconnect' ) ) : ?>
								<div id="jp-disconnect" class="jp-disconnect">
									<a href="<?php echo wp_nonce_url( Jetpack::admin_url( 'action=disconnect' ), 'jetpack-disconnect' ); ?>"
									   onclick="return confirm('<?php echo htmlspecialchars( __( 'Are you sure you want to disconnect from WordPress.com?', 'jetpack' ), ENT_QUOTES ); ?>');">
										<div
											class="deftext"><?php _e( 'Connected to WordPress.com', 'jetpack' ); ?></div>
										<div
											class="hovertext"><?php _e( 'Disconnect from WordPress.com', 'jetpack' ) ?></div>
									</a>
								</div>
							<?php endif; ?>
							<?php if ( $is_user_connected && ! $is_master_user ) : ?>
								<div id="jp-unlink" class="jp-disconnect">
									<a href="<?php echo wp_nonce_url( Jetpack::admin_url( 'action=unlink' ), 'jetpack-unlink' ); ?>">
										<div
											class="deftext"><?php _e( 'User linked to WordPress.com', 'jetpack' ); ?></div>
										<div
											class="hovertext"><?php _e( 'Unlink user from WordPress.com', 'jetpack' ) ?></div>
									</a>
								</div>
							<?php endif; ?>
						</div>
			...
			if ( ! $can_reconnect_jpms && ! $is_connected ) {
				?>
				<div id="message" class="updated jetpack-message jp-banner jp-multisite"
			...
			if ( ! Jetpack::is_development_mode() && $can_reconnect_jpms ) :
				...
				<div id="message" class="updated jetpack-message jp-banner" style="display:block !important;">
					...
					<div class="jp-banner__action-container">
						<a href="<?php echo $this->build_connect_url() ?>" class="jp-banner__button"
						   id="wpcom-connect"><?php _e( 'Connect', 'jetpack' ); ?></a>
					</div>
				</div>
			...
			<?php if ( Jetpack::is_active() && ! Jetpack::is_development_mode() && ! $is_user_connected ) : ?>
				...
				<div id="message" class="updated jetpack-message jp-banner" style="display:block !important;">
					...
					<div class="jp-banner__action-container is-full-width">
						<a href="<?php echo $this->build_connect_url() ?>" class="jp-banner__button"
						   id="wpcom-connect"><?php _e( 'Link account with WordPress.com', 'jetpack' ); ?></a>
					</div>
				</div>
			...
			<?php if ( $is_connected && $this->current_user_is_connection_owner() ) : ?>
				...
				<script type="text/javascript">
					jQuery(document).ready(function ($) {
						$.get(ajaxurl, {
							action: 'jetpack-check-news-subscription',
							rand: jQuery.now().toString() + Math.random().toString()
						}, function (data) {
							if ('subscribed' == data) {
							...
							$('.jp-news-link').click(function () {
								...
								$.get(ajaxurl, {
									action: 'jetpack-subscribe-to-news',
									rand: jQuery.now().toString() + Math.random().toString()
								}, function (data) {
									if ('subscribed' == data) {
			...
			<div id="jp-footer">
				...
				<p class="small">
					...
					<a href="http://automattic.com/privacy/"
					   target="_blank"><?php _e( 'Privacy Policy', 'jetpack' ); ?></a> |
					<a href="http://wordpress.com/tos/"
					   target="_blank"><?php _e( 'Terms of Service', 'jetpack' ); ?></a> |
			...
			<div id="jetpack-configuration" style="display:none;">
				<p><img width="16"
						src="<?php echo esc_url( plugins_url( 'images/wpspin_light-2x.gif', JETPACK__PLUGIN_FILE ) ); ?>"
						alt="Loading ..."/></p>
			</div>
		</div>
	<?php
	...
	function admin_screen_list_modules ()
	{
		...
		if ( ! Jetpack::is_active() ) $jetpack_connected = false;
				...
				<?php if ( $jetpack_connected || ( Jetpack::is_development_mode() && ! $module_data[ 'requires_connection' ] ) ) : ?>
					<?php if ( ! $activated && current_user_can( 'manage_options' ) && apply_filters( 'jetpack_can_activate_' . $module, true ) ) : ?>
						<a href="<?php echo esc_url( $toggle_url ); ?>"
						   class="<?php echo( 'inactive' == $css ? ' button-primary' : ' button-secondary' ); ?>"><?php echo $toggle; ?></a>&nbsp;
					<?php endif; ?>
					...
					?><?php if ( $activated && $module_data[ 'deactivate' ] && current_user_can( 'manage_options' ) ) : ?>
						<a style="display: none;" href="<?php echo esc_url( $toggle_url ); ?>"
						   class="jetpack-deactivate-button button-secondary"><?php echo $toggle; ?></a>&nbsp;<?php endif; ?>
	...
	<?php endif; ?>
		...
		<div id="jp-more-info-<?php echo esc_attr( $module ); ?>" style="display:none;">
			<?php
			if ( $jetpack_connected && has_action( 'jetpack_module_more_info_connected_' . $module ) ) do_action( 'jetpack_module_more_info_connected_' . $module ); else
	...
	{
		$secrets = array( wp_generate_password( 32, false ), // secret_1
	...
	public function validate_remote_register_response ( $response )
	{
		...
		if ( $entity ) $json = json_decode( $entity ); else
			$json = false;
	...
	public static function register ()
	{
		...
		if ( empty( $json->jetpack_secret ) || ! is_string( $json->jetpack_secret ) ) return new Jetpack_Error( 'jetpack_secret', '', $code );
		...
		Jetpack_Options::update_options( array( 'id' => (int) $json->jetpack_id, 'blog_token' => (string) $json->jetpack_secret, 'public' => $jetpack_public, 'jumpstart' => 'new_connection' ) );
	...
	function add_nonce ( $timestamp, $nonce )
	{
		...
		$old_nonce = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM `$wpdb->options` WHERE option_name = %s", "jetpack_nonce_{$timestamp}_{$nonce}" ) );

		if ( is_null( $old_nonce ) ) {
			$return = $wpdb->query( $wpdb->prepare( "INSERT INTO `$wpdb->options` (`option_name`, `option_value`, `autoload`) VALUES (%s, %s, %s)", "jetpack_nonce_{$timestamp}_{$nonce}", time(), 'no' ) );
		} else {
			$return = false;
		}
	...
	{
		$options[ 'jetpack_version' ] = array( 'desc' => __( 'Jetpack Plugin Version', 'jetpack' ), 'readonly' => true, 'value' => JETPACK__VERSION, );

		$options[ 'jetpack_client_id' ] = array( 'desc' => __( 'The Client ID/WP.com Blog ID of this site', 'jetpack' ), 'readonly' => true, 'value' => Jetpack_Options::get_option( 'id' ), );
		return $options;
	}
	...
	{
		return add_query_arg( urlencode_deep( array( 'jetpack-code' => get_user_meta( $user->ID, 'jetpack_json_api_' . $this->json_api_authorization_request[ 'client_id' ], true ), 'jetpack-user-id' => (int) $user->ID, 'jetpack-state' => $this->json_api_authorization_request[ 'state' ], ) ), $redirect_to );
	}
	...
	{
		return '<p class="message">' . sprintf( esc_html__( '%s wants to access your site&#8217;s data.  Log in to authorize that access.', 'jetpack' ), '<strong>' . esc_html( $this->json_api_authorization_request[ 'client_title' ] ) . '</strong>' ) . '<img src="' . esc_url( $this->json_api_authorization_request[ 'client_image' ] ) . '" /></p>';
	}
	...
	public static function get_avatar_url ( $id_or_email, $size = 96, $default = '', $force_display = false )
	{
		...
		if ( $force_display && ! $has_filter ) add_filter( 'pre_option_show_avatars', '__return_true' );

		$avatar = get_avatar( $id_or_email, $size, $default );

		if ( $force_display && ! $has_filter ) remove_filter( 'pre_option_show_avatars', '__return_true' );

		// If no data, fail out.
		if ( is_wp_error( $avatar ) || ! $avatar ) return array( null, null );

		// Pull out the URL.  If it's not there, fail out.
		if ( ! preg_match( '/src=["\']([^"\']+)["\']/', $avatar, $url_matches ) ) return array( null, null );
		$url = wp_specialchars_decode( $url_matches[ 1 ], ENT_QUOTES );

		// Pull out the class, but it's not a big deal if it's missing.
		$class = '';
		if ( preg_match( '/class=["\']([^"\']+)["\']/', $avatar, $class_matches ) ) $class = wp_specialchars_decode( $class_matches[ 1 ], ENT_QUOTES );
	...
	{
		if ( ! Jetpack::is_active() || Jetpack::is_development_mode() ) return false;
...
<?php
	...
	public function deprecated_hooks ()
	{
		 ...
		 */
		$deprecated_list = array( 'jetpack_bail_on_shortcode' => 'jetpack_shortcodes_to_include', );
	...
	public function wp_dashboard_setup ()
	{
		...
		if ( has_action( 'jetpack_dashboard_widget' ) ) {
			wp_add_dashboard_widget( 'jetpack_summary_widget', __( 'Jetpack', 'jetpack' ), array( __CLASS__, 'dashboard_widget' ) );
			...
			<div class="protect">
				...
				<?php elseif ( current_user_can( 'jetpack_activate_modules' ) && ! self::is_development_mode() ) : ?>
					<a href="<?php echo esc_url( wp_nonce_url( Jetpack::admin_url( array( 'action' => 'activate', 'module' => 'protect' ) ), 'jetpack_activate-protect' ) ); ?>"
					   ...
					   title="<?php esc_attr_e( 'Jetpack Protect helps to keep you secure from brute-force login attacks.', 'jetpack' ); ?>">
	...
	public function dashboard_widget_connect_to_wpcom ()
	{
		...
		<small><a
				href="<?php echo esc_url( wp_nonce_url( Jetpack::admin_url( 'jetpack-notice=dismiss' ), 'jetpack-deactivate' ) ); ?>">
