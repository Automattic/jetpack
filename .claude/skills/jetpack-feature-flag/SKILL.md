...
compatibility: "Jetpack monorepo. Requires the `automattic/jetpack-feature-flags` package (`projects/packages/feature-flags`, PHP 7.2+). The Jurassic Ninja workflow needs the Companion plugin's `wp companion feature-flag` command."
---
...
Flags ship off, so a JN site shows the old behaviour until you force the flag. The Companion plugin on JN provides a WP-CLI command for exactly this:
...
For the full command surface, the Settings-page equivalent, the two code filters, and the Automattician-only screen on WordPress.com Simple and Atomic, read [references/controlling-flags.md](references/controlling-flags.md).
