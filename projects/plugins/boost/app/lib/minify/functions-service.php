...
function jetpack_boost_page_optimize_service_request() {
	...
	if ( $use_cache && ! is_dir( $cache_dir ) && ! $utils->mkdir( $cache_dir, 0775, true ) ) {
		$use_cache = false;
	...
	if ( $use_cache && ( ! is_dir( $cache_dir ) || ! $utils->is_writable( $cache_dir ) || ! is_executable( $cache_dir ) ) ) {
		$use_cache = false;
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log(
	...
	if ( $use_cache ) {
		...
		if ( file_exists( $cache_file ) ) {
			...
			if ( file_exists( $cache_file_meta ) ) {
				$meta = json_decode( $utils->get_contents( $cache_file_meta ) );
			...
			$etag = '"' . md5( $utils->get_contents( $cache_file ) ) . '"';

			header( 'X-Page-Optimize: cached' );
			header( 'Cache-Control: max-age=' . 31536000 );
			header( 'ETag: ' . $etag );

			echo $utils->get_contents( $cache_file ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- We need to trust this unfortunately.
			die();
		}
	}
	...
	if ( $use_cache ) {
		$utils->put_contents( $cache_file, $content );
		$utils->put_contents( $cache_file_meta, $utils->json_encode( array( 'headers' => $headers ) ) );
	}
...
function jetpack_boost_page_optimize_build_output() {
	...
	foreach ( $args as $uri ) {
		...
		$buf = $utils->get_contents( $fullpath );
		if ( false === $buf ) {
			jetpack_boost_page_optimize_status_exit( 500 );
		}
