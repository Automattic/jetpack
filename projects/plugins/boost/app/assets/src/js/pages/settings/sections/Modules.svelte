<script lang="ts" context="module">
	...
	const [ generateCriticalCss, allowGenerateCriticalCss ] = once( state => {
		if ( state.status === 'not_generated' ) {
			regenerateCriticalCss();
		}

		continueGeneratingLocalCriticalCss( state );
	} );
</script>
...
<script lang="ts">
	...
	function handleCriticalCssToggled( e ) {
		moduleToggle( e );

		if ( e.detail.active ) {
			generateCriticalCss( $criticalCssState );
		} else {
			allowGenerateCriticalCss();
		}
	}

	function handleCloudCssToggled( e ) {
		moduleToggle( e );
...
<div class="jb-container--narrow">
	<Module
		...
		on:enabled={handleCriticalCssToggled}
		on:disabled={handleCriticalCssToggled}
		on:mountEnabled={() => generateCriticalCss( $criticalCssState )}
	>
		<h3 slot="title">
	...
	<Module
		...
		on:enabled={handleCloudCssToggled}
		on:disabled={handleCloudCssToggled}
	...
	<Module
		...
		on:enabled={moduleToggle}
		on:disabled={moduleToggle}
	>
		<h3 slot="title">
