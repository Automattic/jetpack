...
export function useDismissibleAlertState( alertId: 'performance_history_fresh_start' ) {
	...
	const { mutate } = useMutation( {
		mutationFn: async () => {
			const dismissed =
				data ?? dismissedAlertsSchema.parse( await requestDataSync( 'dismissed_alerts' ) );
			return dismissedAlertsSchema.parse(
				await requestDataSync( 'dismissed_alerts', { ...dismissed, [ alertId ]: true } )
			);
		},
		onSuccess: dismissed => queryClient.setQueryData( dismissedAlertsQueryKey, dismissed ),
	} );
