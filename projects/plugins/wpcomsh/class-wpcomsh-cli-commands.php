...
if ( class_exists( 'WP_CLI_Command' ) ) {
	...
	class WPCOMSH_CLI_Commands extends WP_CLI_Command {
		...
		/**
		 * Import a playground .sqlite file.
		 *
		 * ## OPTIONS
		 *
		 * [--collation]
		 * : The collation to use for the import.
		 *
		 * [--db-file]
		 * : The Playground SQLite database file to import. (required)
		 *
		 * [--exclude-tables]
		 * : A comma-separated list of tables to exclude from the import.
		 *
		 * [--home]
		 * : The home to use for the import.
		 *
		 * [--output-file]
		 * : The output file to use for the import. A random file will be used if not specified.
		 *
		 * [--siteurl]
		 * : The siteurl to use for the import.
		 *
		 * [--stdout]
		 * : Output to STDOUT if specified. Defaults to false.
		 *
		 * [--temp-tables]
		 * : Generate temporary tables instead of TRANSACTION. Defaults to false.
		 *
		 * @subcommand playground-db-import
		 */
		public function playground_import( $args, $assoc_args ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundInExtendedClassBeforeLastUsed
			$collation      = WP_CLI\Utils\get_flag_value( $assoc_args, 'collation', null );
			$db_file        = WP_CLI\Utils\get_flag_value( $assoc_args, 'db-file' );
			$exclude_tables = WP_CLI\Utils\get_flag_value( $assoc_args, 'exclude-tables', array() );
			$home           = WP_CLI\Utils\get_flag_value( $assoc_args, 'home' );
			$output_file    = WP_CLI\Utils\get_flag_value( $assoc_args, 'output-file' );
			$siteurl        = WP_CLI\Utils\get_flag_value( $assoc_args, 'siteurl' );
			$stdout         = WP_CLI\Utils\get_flag_value( $assoc_args, 'stdout', false );
			$temp_tables    = WP_CLI\Utils\get_flag_value( $assoc_args, 'temp-tables', false );

			if ( empty( $db_file ) ) {
				WP_CLI::error( 'Missing file path passed to --db-file' );
			}

			$options = array(
				'collation'      => $collation,
				'exclude_tables' => $exclude_tables ? explode( ',', $exclude_tables ) : array(),
				'home'           => $home,
				'output_mode'    => $stdout ? Imports\SQL_Generator::OUTPUT_TYPE_STDOUT : Imports\SQL_Generator::OUTPUT_TYPE_FILE,
				'siteurl'        => $siteurl,
				'tmp_tables'     => $temp_tables,
			);
			...
			$ret = Imports\Playground_DB_Importer::get_instance()->generate_sql( $db_file, $options );
