...
class Gravatar_Widget extends WP_Widget {
	function __construct() {
		$widget_ops  = array( 'classname' => 'widget_gravatar', 'description' => __( 'Insert a Gravatar image' ) );

		parent::__construct( 'gravatar', __( 'Gravatar' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( $instance['email'] ) {
			...
			if ( current_user_can('edit_theme_options') ) {
				echo '<p>' . sprintf( __( 'You need to pick a user or enter an email address in your <a href="%s">Gravatar Widget</a> settings.' ), admin_url( 'widgets.php' ) ) . '</p>';
			}
		}
	...
	function form( $instance ) {
		...
		$sizes  = array( 64 => __( 'Small (64 pixels)' ), 96 => __( 'Medium (96 pixels)' ), 128 => __( 'Large (128 pixels)' ), 256 => __( 'Extra Large (256 pixels)' ) );
		$aligns = array( 'none' => __( 'None' ), 'left' => __( 'Left' ), 'right' => __( 'Right' ), 'center' => __( 'Center' ) );
		?>
<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:' ); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></label></p>

<p><?php esc_html_e( 'Select a user or pick "custom" and enter a custom email address.' ); ?></p>
<p><?php wp_dropdown_users( array( 'show_option_none' => 'Custom', 'selected' => $email_user, 'name' => $this->get_field_name( 'email_user' ) ) );?></p>

<p id="gravatar_email_user"><label for="<?php echo $this->get_field_id( 'email' ); ?>"><?php esc_html_e( 'Custom Email Address:' ); ?> <input class="widefat" id="<?php echo $this->get_field_id('email'); ?>" name="<?php echo $this->get_field_name( 'email' ); ?>" type="text" value="<?php echo esc_attr( $email ); ?>" /></label></p>

<p>
	<label for="<?php echo $this->get_field_id( 'gravatar_size' ); ?>"><?php esc_html_e( 'Size:' ); ?>
...
<p>
	<label for="<?php echo $this->get_field_id( 'gravatar_align' ); ?>"><?php esc_html_e( 'Gravatar alignment:' ); ?>
...
<p><label for="<?php echo $this->get_field_id( 'gravatar_url' ); ?>"><?php esc_html_e( 'Gravatar link. This is an optional URL that will be used when anyone clicks on your Gravatar:' ); ?> <input  class="widefat" id="<?php echo $this->get_field_id('gravatar_url'); ?>" name="<?php echo $this->get_field_name( 'gravatar_url' ); ?>" type="text" value="<?php echo esc_attr( $gravatar_url ); ?>" /></label></p>
<p><label for="<?php echo $this->get_field_id( 'gravatar_text' ); ?>"><?php _e( 'Text displayed after Gravatar. This is optional and can be used to describe yourself or what your blog is about.' ); ?><br/> <textarea class="widefat" style="font-size: 0.9em" id="<?php echo $this->get_field_id('gravatar_text'); ?>" name="<?php echo $this->get_field_name( 'gravatar_text' ); ?>" rows="5"><?php echo htmlspecialchars( $gravatar_text ); ?></textarea></label></p>
<p><?php _e( 'You can modify your Gravatar from your <a href="/wp-admin/profile.php">profile page</a>.' )?></p>
		<?php
	}
