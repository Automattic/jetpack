/* jshint maxerr: 10000 */
( function( wp, $, _, undef ) { // Open closure
	...
	api.ColorsTool = api.Control.extend(
		{
			...
			ready: function() {
				...
				$( '.revert' ).on(
					...
					function() {
						...
						// Stat: 'revert-default'
						new Image().src = document.location.protocol + '//pixel.wp.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=revert-default&baba=' + Math.random();
					}
				);
				...
				if ( window.location.hash === '#colors' ) {
					...
					new Image().src = document.location.protocol + '//pixel.wp.com/g.gif?v=wpcom-no-pv&x_theme-customizer-colors=1&baba=' + Math.random();
				}
			},
			...
			breakSectionTitle: function() {
				...
				this.grid.find( '.bg' ).after( '<li class="text-placeholder clrs"><div><span class="customize-control-title">' + this.opts.colorsTitle + '</span></div></li>' );
			},
			...
			overrideCoreBg: function() {
				...
				ct.bgPrompt.find( '.choose-color' ).css( 'background-color', ct.getColor( ct.grid.find( '.bg' ) ) );
			},

			initPatterns: function() {
				var ct = this;
				if ( ! ct.opts.themeSupport.customBackground || ct.grid.find( '.bg' ).hasClass( 'unavailable' ) ) {
				...
				ct.patternPicker.on(
					...
					function ( e ) {
						...
						// Stat: 'patterns'
						new Image().src = document.location.protocol + '//pixel.wp.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=patterns&baba=' + Math.random();
			...
			handleMorePatternsClick: function() {
				...
				// Stat: 'more-patterns'
				new Image().src = document.location.protocol + '//pixel.wp.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=more-patterns&baba=' + Math.random();

				if ( ct.patternIndex < ct.patterns.length ) {
					ct.showPatterns( ct.patternIndex );
				}

				if ( ! ct.fetchingPatterns && 0 == ct.patterns.length || ct.patternIndex == ct.patterns.length - 5 ) {
			...
			initPalettes: function() {
				...
				var morePaletteClickHandler = function () {
					...
					// Stat: 'more-palettes'
					new Image().src = document.location.protocol + '//pixel.wp.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=more-palettes&baba=' + Math.random();

					if ( ! ct.fetchingPalettes && ct.paletteIndex >= ct.palettes.length - ( ct.palettesAtATime * 2 ) ) {
						...
						$.get(
							...
							function ( data ) {
								...
								if ( 0 == ct.paletteIndex || ct.paletteIndex >= ct.palettes.length - data.palettes.length - ct.palettesAtATime ) {
			...
			generatePaletteFromHeader: function() {
				...
				generatePalette.on(
					...
					function() {
						...
						new Image().src = document.location.protocol + '//pixel.wp.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=generate-palette&baba=' + Math.random();
					}
				);
			},
			...
			irisPicker: function() {
				...
				$( '#pick-your-nose' ).click(
					function( e ){
						e.preventDefault();
						$( '#iris-container' ).show();

						// Stat: 'color-picker'
						new Image().src = document.location.protocol + '//pixel.wp.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=color-picker&baba=' + Math.random();
					}
				);
			...
			colorPicker: function() {
				...
				other_colors, selected_color,
				label, bubble, width;

				// Bind to click event on each color li
				ct.grid.on(
					'click',
					'li:not(.text-placeholder)',
					function() {
				...
				ct.picker.find( '.color-suggestions' ).on(
					'click',
					'li',
					function() {
						...
						// Stat: 'suggestions'
						new Image().src = document.location.protocol + '//pixel.wp.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=suggestions&baba=' + Math.random();
					}
				);
			},
				...
				$( '#colourlovers-palettes' ).on(
					...
					function() {
						...
						if ( $( this ).hasClass( 'featured' ) ) {
							...
							new Image().src = document.location.protocol + '//pixel.wp.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=featured-palettes&baba=' + Math.random();
						} else {
							// Stat: 'palettes'
							new Image().src = document.location.protocol + '//pixel.wp.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=palettes&baba=' + Math.random();
						}
					}
				);
			},
				...
				ct.grid.on(
					...
					function( e, role ) {
						...
						if ( ! role || 'bg' === role ) {
							...
							if ( backgroundImage && ( backgroundImage.indexOf( 'colourlovers' ) !== -1 || backgroundImage === ct.opts.defaultImage || backgroundImage === ct.origBackground ) ) {
								ct.coreBgImage( '' );
							}
			...
			showPatterns: function( patternIndex ) {
				...
				for ( var i = patternIndex, _len = Math.min( patternIndex + pageSize, this.patterns.length ); i < _len; i++ ) {
					var pattern       = $( '<li/> ' ).addClass( 'pattern' );
					var pattern_link  = $( '<a/>' ).addClass( 'thumbnail' );
					var pattern_image = $( '<img/>' ).attr( 'src', this.patterns[i].preview_image_url ).addClass( 'pattern' );
	...
	api.ColorsTool.BackgroundChangeView = wp.Backbone.View.extend(
		{
			...
			updateImage: function(control) {
				...
				this.currentBgColor = settings.background_color
				|| this.controller.opts.defaultColors.bg;
			...
			updateBgSize: function(img) {
				...
				rW        = this.rectangle.width(), rH = this.rectangle.height(),
			...
			render: function() {
				...
				this.rectangle   = this.$el.find( '.background-rectangle' )
				.add( '.color-grid.main li.bg' );
	...
	api.ColorsTool.BackgroundOptionsView = wp.Backbone.View.extend(
		{
			...
			render: function() {
				...
				_.each(
					this.states(),
					function(value, key) {
						this.$el.find( 'input[name=' + key + '][value=' + value + ']' )
						.prop( 'checked', true );
					},
					this
				);
	...
	Backbone.on(
		...
		function(bucket, stat) {
			...
			'//pixel.wp.com/g.gif?v=wpcom-no-pv&x_' + bucket + '=' +
			stat + '&baba=' + Math.random();
			new Image().src = url;
		}
	);
