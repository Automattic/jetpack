...
( function( wp, $, _, undef ) { // Open closure
	...
	api.ColorsTool = api.Control.extend(
		{
			...
			ready: function() {
				...
				$( '.revert' ).on(
					...
					function() {
						...
						var colors = $( this ).hasClass( 'revert-default' ) ? ct.opts.defaultColors : ct.opts.colors;
						...
						if ( $( this ).hasClass( 'revert-default' ) ) {
							...
							// Stat: 'revert-default'
							new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=revert-default&baba='  +  Math.random();
						} else {
							api( 'background_image' ).set( ct.origBackground );
							// Stat: 'revert-undo'
							new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=revert-undo&baba='  +  Math.random();
						}
				...
				if ( window.location.hash === '#colors' ) {
					...
					new Image().src = document.location.protocol + '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_theme-customizer-colors=1&baba=' + Math.random();
				}
			},
			...
			breakSectionTitle: function() {
				...
				this.grid.find( '.bg' ).after( '<li class="text-placeholder"><div><span class="customize-control-title">' + this.opts.colorsTitle + '</span></div></li>' );
			},
			...
			initPatterns: function() {
				...
				ct.patternPicker.on(
					...
					function ( e ) {
						...
						// Stat: 'patterns'
						new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=patterns&baba='  +  Math.random();
					...
					function ( e ) {
						...
						// Stat: 'more-patterns'
						new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=more-patterns&baba='  +  Math.random();
			...
			initPalettes: function() {
				...
				$( '#more-palettes' ).on(
					...
					function () {
						...
						// Stat: 'more-palettes'
						new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=more-palettes&baba='  +  Math.random();
			...
			generatePaletteFromHeader: function() {
				...
				generatePalette.on(
					...
					function() {
						...
						new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=generate-palette&baba='  +  Math.random();
					}
				);
			},
			...
			irisPicker: function() {
				...
				$( '#pick-your-nose' ).click(
					function( e ){
						...
						// Stat: 'color-picker'
						new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=color-picker&baba='  +  Math.random();
					}
				);
			...
			colorPicker: function() {
				...
				ct.picker.find( '.color-suggestions' ).on(
					...
					function() {
						...
						// Stat: 'suggestions'
						new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=suggestions&baba='  +  Math.random();
					}
				);
			},
				...
				$( '#colourlovers-palettes' ).on(
					...
					function() {
						...
						if ( $( this ).hasClass( 'featured' ) ) {
							...
							new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=featured-palettes&baba='  +  Math.random();
						} else {
							// Stat: 'palettes'
							new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=palettes&baba='  +  Math.random();
						}
					}
				);
			},
				...
				ct.color.draggable(
					{
						...
						stop: function( event, ui ) {
							if ( ct.color.hasClass( 'color-swap' ) ) {
								...
								// Stat: 'drag-color'
								new Image().src  =  document.location.protocol  +  '//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_customizer-colors-actions=drag-color&baba='  +  Math.random();
							}
						}
					}
				).droppable(
			...
			showPalettes: function( paletteIndex ) {
				...
				for ( var i = paletteIndex, _len = paletteIndex + 6; i < _len; i++ ) {
					var new_palette = $( '<ul/>' ).addClass( 'color-grid colour-lovers' ).attr( 'title', ct.opts.dragHelp );
			...
			buyNotice: function() {
				...
				if ( $( '.buy-custom-design' ).length ) {
					...
					$( '.customizer-controls-slider .accordion-section-colors_manager_tool' ).append( ct.opts.buyMessage );

					// Close notice
					$( '.buy-custom-design .back-to-colors' ).on(
						'click',
						function() {
	...
	Backbone.on(
		...
		function(bucket, stat) {
			...
			'//stats.wordpress.com/g.gif?v=wpcom-no-pv&x_' + bucket + '=' +
