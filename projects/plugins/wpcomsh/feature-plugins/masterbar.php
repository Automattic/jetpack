...
function wpcomsh_activate_masterbar_module() {
	if ( ! defined( 'JETPACK__VERSION' ) || uses_wp_admin_interface() ) {
		return;
	}
...
function atomic_masterbar_filter_jetpack_modules( $modules ) {
	if ( isset( $modules['masterbar'] ) && uses_wp_admin_interface() ) {
...
function wpcomsh_set_connected_user_data_as_user_options( $transient, $value ) {
	...
	if ( isset( $value['color_scheme'] ) && ! uses_wp_admin_interface() ) {
...
function wpcomsh_update_plugin_add_filter() {
	...
	if ( ! class_exists( 'Automattic\Jetpack\Status' ) || uses_wp_admin_interface() ) {
		return;
	}
