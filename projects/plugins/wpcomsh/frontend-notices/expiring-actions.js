( function () {
	document.addEventListener( 'DOMContentLoaded', function () {
		...
		document.getElementById( 'wpcomsh-expiring-nudge' ).onclick = function () {
			bumpStat( 'clicked' );
			window.location.href = wpcomsh_expiring_data.checkout_link;
		};
	} );

	function expiringNotice() {
		var notice = document.createElement( 'div' );
		notice.setAttribute( 'id', 'wpcomsh-expiring-notice' );
		notice.classList.add( 'wpcomsh-expiring-nudge' );
		document.documentElement.classList.add( 'has-wpcomsh-expiring' );
		notice.innerHTML =
			'<div class="wpcomsh-expiring-nudge-inner">' +
			...
			'<img alt="🎁" src="https://s.w.org/images/core/emoji/14.0.0/svg/1f381.svg"/>' +
			'<span class="break-mobile subtitle"></span>' +
			'</div>' +
			'<button id="wpcomsh-expiring-nudge" class="wpcomsh-expiring-button"></button>' +
			'</div>';

		const title = document.createTextNode(wpcomsh_expiring_data.i18n.title);
		const subtitle = document.createTextNode(wpcomsh_expiring_data.i18n.subtitle);
		const button_text = document.createTextNode(wpcomsh_expiring_data.i18n.button_text);

		notice.getElementsByClassName('title')[0].appendChild(title);
		notice.getElementsByClassName('subtitle')[0].appendChild(subtitle);
		notice.getElementsByClassName('wpcomsh-expiring-button')[0].appendChild(button_text);
