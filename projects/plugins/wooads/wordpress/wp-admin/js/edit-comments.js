...
( function ( $ ) {
	...
	updateHtmlTitle = function ( diff ) {
		...
		titleRegEx =
			titleRegEx ||
			new RegExp(
				__( 'Comments (%s)' ).replace( '%s', '\\([0-9' + thousandsSeparator + ']+\\)' ) + '?'
			);
	...
	window.setCommentsList = function () {
		...
		delAfter = function ( r, settings ) {
				...
				if ( 'object' === typeof r ) {
					if (
						...
						if ( total_items_i18n ) {
							...
							$( '.total-pages' ).text(
								response.supplemental.total_pages_i18n.replace(
									'&nbsp;',
			...
			theList
				.get( 0 )
				.wpList.add( theExtraList.children( ':eq(0):not(.no-items)' ).remove().clone() );
