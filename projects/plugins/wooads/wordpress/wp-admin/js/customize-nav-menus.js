		...
		/** @lends wp.customize.Menus.MenuSection.prototype */ {
			...
			updateAssignedLocationsInSectionTitle: function ( themeLocationSlugs ) {
				...
				section.container.toggleClass(
					'assigned-to-menu-location',
					0 !== themeLocationSlugs.length
				);
			},
	...
	api.Menus.MenuItemControl = api.Control.extend(
		/** @lends wp.customize.Menus.MenuItemControl.prototype */ {
			...
			_setupUpdateUI: function () {
				...
				control.elements.title = new api.Element(
					control.container.find( '.edit-menu-item-title' )
				);
				control.elements.attr_title = new api.Element(
					control.container.find( '.edit-menu-item-attr-title' )
				);
				control.elements.target = new api.Element(
					control.container.find( '.edit-menu-item-target' )
				);
			...
			renderContent: function () {
				...
				if ( settingValue._invalid ) {
					...
					control.params.title = api.Menus.data.pendingTitleTpl.replace(
						'%s',
						control.params.title
					);
				}
				...
				menuList.on( 'sortstop', function () {
					setTimeout( function () {
						...
						_.each( menuItemContainerIds, function ( menuItemContainerId ) {
							var menuItemId, menuItemControl, matches;
							matches = menuItemContainerId.match(
								/^customize-control-nav_menu_item-(-?\d+)$/,
								''
							);
							if ( ! matches ) {
								return;
			...
			reflowMenuItems: function () {
				...
				reflowRecursively = function ( context ) {
					...
					_.each( currentMenuItemControls, function ( menuItemControl ) {
						...
						// Update depth.
						if (
							! menuItemControl.container.hasClass(
								'menu-item-depth-' + String( context.currentDepth )
							)
						) {
							_.each(
							...
							menuItemControl.container.addClass(
								'menu-item-depth-' + String( context.currentDepth )
							);
						}
		...
		_( data.nav_menu_item_updates ).each( function ( update ) {
			...
			if ( 'inserted' === update.status ) {
				...
				if ( settingValue.menu_item_parent < 0 ) {
					...
					settingValue.menu_item_parent =
						insertedMenuItemIdMapping[ settingValue.menu_item_parent ];
				}
