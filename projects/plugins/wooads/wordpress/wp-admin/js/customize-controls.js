		...
		/** @lends wp.customize~Container.prototype */ {
			...
			_animateChangeExpanded: function ( completeCallback ) {
				...
				if ( ! construct.panel || '' === construct.panel() ) {
					...
					api.panel( construct.panel() ).contentContainer.hasClass( 'skip-transition' )
				) {
					transitionParentPane = true;
				} else {
			...
			getContent: function () {
				...
				return content.detach().attr( {
					id: contentId,
					class:
						'customize-pane-child ' + content.attr( 'class' ) + ' ' + container.attr( 'class' ),
				} );
			},
		}
	);
		...
		/** @lends wp.customize.DateTimeControl.prototype */ {
			...
			parseDateTime: function parseDateTime( datetime ) {
				...
				if ( control.params.includeTime && control.params.twelveHourFormat ) {
					...
					date.hour =
						date.hour % midDayHour ? String( date.hour % midDayHour ) : String( midDayHour );
	...
	api.reflowPaneContents = _.bind( function () {
		...
		api.section.each( function ( section ) {
			...
			appendContainer = section.contentContainer.is( 'ul' )
				...
				: section.contentContainer.find( 'ul:first' );
			if (
				! api.utils.areElementListsEqual( controlContainers, appendContainer.children( '[id]' ) )
			) {
				_( controls ).each( function ( control ) {
