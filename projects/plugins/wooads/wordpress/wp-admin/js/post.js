		...
		$( '#' + taxonomy + 'checklist, #' + taxonomy + 'checklist-pop' ).on(
			...
			function () {
				...
				if ( id && t.parents( '#taxonomy-' + taxonomy ).length )
					$( '#in-' + taxonomy + '-' + id + ', #in-popular-' + taxonomy + '-' + id ).prop(
						'checked',
						c
					);
			}
	...
	if ( $( '#submitdiv' ).length ) {
		...
		updateText = function () {
			...
			if ( $postVisibilitySelect.find( 'input:radio:checked' ).val() == 'private' ) {
				...
				if ( 0 === optPublish.length ) {
					postStatus.append(
						'<option value="publish">' + __( 'Privately Published' ) + '</option>'
					);
				} else {
		...
		$postVisibilitySelect.find( '.save-post-visibility' ).on( 'click', function ( event ) {
			...
			switch ( selectedVisibility ) {
				case 'public':
					visibilityLabel = $( '#sticky' ).prop( 'checked' )
						? __( 'Public, Sticky' )
						: __( 'Public' );
					break;
				case 'private':
