...
( function ( $ ) {
	...
	themes.view.Theme = wp.Backbone.View.extend( {
		...
		expand: function ( event ) {
			...
			if (
				$( event.target ).is( '.theme-actions a, .update-message, .button-link, .notice-dismiss' )
			) {
				return;
			}
		...
		setNavButtonsState: function () {
			...
			if (
				_.isUndefined( this.model.collection.at( this.model.collection.indexOf( current ) + 1 ) )
			) {
