...
this[ 'wp' ][ 'richText' ] = /******/ ( function ( modules ) {
	...
	/******/ {
		...
		/***/ yyEc: /***/ function ( module, __webpack_exports__, __webpack_require__ ) {
			'use strict';
			// ESM COMPAT FLAG
			...
			__webpack_require__.d( __webpack_exports__, 'useAnchorRef', function () {
			...
			__webpack_require__.d( __webpack_exports__, '__unstableUseRichText', function () {
			...
			const getFormatTypes = Object( rememo[ 'a' /* default */ ] )(
			...
			function toFormat( _ref ) {
				let { type, attributes } = _ref;
				let formatType;

				if ( attributes && attributes.class ) {
					formatType = Object( external_wp_data_[ 'select' ] )( store ).getFormatTypeForClassName(
						attributes.class
					);
				...
				if ( ! formatType ) {
					formatType = Object( external_wp_data_[ 'select' ] )( store ).getFormatTypeForBareElement(
						type
					);
				}
			...
			function registerFormatType( name, settings ) {
				...
				if ( settings.className === null ) {
					const formatTypeForBareElement = Object( external_wp_data_[ 'select' ] )(
						store
					...
					const formatTypeForClassName = Object( external_wp_data_[ 'select' ] )(
						store
			...
			function insertLineSeparator( value ) {
				let startIndex =
					arguments.length > 1 && arguments[ 1 ] !== undefined ? arguments[ 1 ] : value.start;
				let endIndex =
					arguments.length > 2 && arguments[ 2 ] !== undefined ? arguments[ 2 ] : value.end;
			...
			function toTree( _ref2 ) {
				...
				for ( let i = 0; i < formatsLength; i++ ) {
					...
					if ( character === OBJECT_REPLACEMENT_CHARACTER ) {
						...
						if (
							! isEditableTree &&
							( ( _replacements$i = replacements[ i ] ) === null || _replacements$i === void 0
								? void 0
			...
			function createElementHTML( _ref6 ) {
				...
				for ( const key in attributes ) {
					...
					attributeString += ` ${ key }="${ Object( external_wp_escapeHtml_[ 'escapeAttribute' ] )(
						attributes[ key ]
					) }"`;
				}
				...
				return children
					.map( child => {
						...
						return child.text === undefined
							...
							: Object( external_wp_escapeHtml_[ 'escapeEditableHTML' ] )( child.text );
					} )
					.join( '' );
			}
			...
			function toggleFormat( value, format ) {
				...
				if ( format.title ) {
					...
					Object( external_wp_a11y_[ 'speak' ] )(
						Object( external_wp_i18n_[ 'sprintf' ] )(
							Object( external_wp_i18n_[ '__' ] )( '%s applied.' ),
							format.title
						),
						'assertive'
					);
			...
			function useFormatBoundaries( props ) {
				...
				return Object( external_wp_compose_[ 'useRefEffect' ] )( element => {
					function onKeyDown( event ) {
						...
						const reverseKey =
							direction === 'rtl'
								? external_wp_keycodes_[ 'RIGHT' ]
								: external_wp_keycodes_[ 'LEFT' ];
			...
			function useIndentListItemOnSpace( props ) {
				...
				return Object( external_wp_compose_[ 'useRefEffect' ] )( element => {
					function onKeyDown( event ) {
						const { keyCode, shiftKey, altKey, metaKey, ctrlKey } = event;
						const { multilineTag, createRecord, handleChange } = propsRef.current;
						...
						if (
							...
							metaKey ||
							ctrlKey ||
							keyCode !== external_wp_keycodes_[ 'SPACE' ] ||
							multilineTag !== 'li'
						) {
							return;
						}
			...
			function useInputAndSelection( props ) {
				...
				return Object( external_wp_compose_[ 'useRefEffect' ] )( element => {
					...
					function onInput( event ) {
						...
						const { record, applyRecord, createRecord, handleChange } = propsRef.current; // The browser formatted something or tried to insert HTML.
					...
					function onFocus() {
						const { record, isSelected, onSelectionChange, applyRecord } = propsRef.current;
			...
			function useDelete( props ) {
				...
				return Object( external_wp_compose_[ 'useRefEffect' ] )( element => {
					function onKeyDown( event ) {
						...
						if (
							keyCode !== external_wp_keycodes_[ 'DELETE' ] &&
							keyCode !== external_wp_keycodes_[ 'BACKSPACE' ]
						) {
							return;
						}
			...
			function useSpace() {
				return Object( external_wp_compose_[ 'useRefEffect' ] )( element => {
					function onKeyDown( event ) {
						...
						const { keyCode, altKey, metaKey, ctrlKey, key } = event; // Only consider the space key without modifiers pressed.
			...
			function useRichText( _ref ) {
				...
				if ( ! record.current ) {
					...
					const hasRelevantInitFormat =
						( ( _record$current = record.current ) === null || _record$current === void 0
							? void 0
					...
					record.current = { ...record.current, start: selectionStart, end: selectionEnd };
				}
				/**
