...
wp.customize.selectiveRefresh = ( function ( $, api ) {
	...
	Partial = self.Partial = api.Class.extend(
		/** @lends wp.customize.SelectiveRefresh.Partial.prototype */ {
			id: null,
			...
			createEditShortcutForPlacement: function ( placement ) {
				...
				illegalContainerSelector =
					'area, audio, base, bdi, bdo, br, button, canvas, col, colgroup, command, datalist, embed, head, hr, html, iframe, img, input, keygen, label, link, map, math, menu, meta, noscript, object, optgroup, option, param, progress, rp, rt, ruby, script, select, source, style, svg, table, tbody, textarea, tfoot, thead, title, tr, track, video, wbr';
				if (
					! $placementContainer.length ||
					$placementContainer.is( illegalContainerSelector ) ||
					$placementContainer.closest( illegalAncestorSelector ).length
				) {
					return;
				}
			...
			addEditShortcutToPlacement: function ( placement, $editShortcut ) {
				...
				if (
					! $placementContainer.is( ':visible' ) ||
					'none' === $placementContainer.css( 'display' )
				) {
			...
			createEditShortcut: function () {
				...
				$image = $(
					'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M13.89 3.39l2.71 2.72c.46.46.42 1.24.03 1.64l-8.01 8.02-5.56 1.16 1.16-5.58s7.6-7.63 7.99-8.03c.39-.39 1.22-.39 1.68.07zm-2.73 2.79l-5.59 5.61 1.11 1.11 5.54-5.65zm-2.97 8.23l5.58-5.6-1.07-1.08-5.59 5.6z"/></svg>'
				);
				$button.append( $image );
			...
			renderContent: function ( placement ) {
				...
				try {
					...
					if (
						wp.emoji &&
						wp.emoji.parse &&
						! $.contains( document.head, placement.container[ 0 ] )
					) {
	...
	self.Placement = Placement = api.Class.extend(
		/** @lends wp.customize.selectiveRefresh.prototype */ {
			/**
		...
		self._debouncedTimeoutId = setTimeout( function () {
			...
			_.each( self._pendingPartialRequests, function ( pending, partialId ) {
				...
				if ( ! self.partial.has( partialId ) ) {
					pending.deferred.rejectWith( pending.partial, [
						new Error( 'partial_removed' ),
						partialsPlacements[ partialId ],
					] );
				} else {
					/*
					 ...
					 */
					partialPlacementContexts[ partialId ] = _.map(
						partialsPlacements[ partialId ],
						function ( placement ) {
			...
			request.done( function ( data ) {
				...
				_.each( self._pendingPartialRequests, function ( pending, partialId ) {
					...
					if ( ! _.isArray( data.contents[ partialId ] ) ) {
						pending.deferred.rejectWith( pending.partial, [
							new Error( 'unrecognized_partial' ),
							partialsPlacements[ partialId ],
						] );
					} else {
	...
	self.addPartials = function ( rootElement, options ) {
		...
		containerElements.each( function () {
			...
			if ( ! partial ) {
				...
				partialOptions.constructingContainerContext =
					containerElement.data( 'customize-partial-placement-context' ) || {};
				Constructor =
					self.partialConstructor[ containerElement.data( 'customize-partial-type' ) ] ||
					self.Partial;
			 ...
			 */
			if (
				options.triggerRendered &&
				! containerElement.data( 'customize-partial-content-rendered' )
			) {
	...
	api.bind( 'preview-ready', function () {
		...
		api.selectiveRefresh.bind(
			...
			function handleSettingValiditiesResponse( data ) {
		...
		api.selectiveRefresh.editShortcutVisibility.bind( function ( visibility ) {
			...
			shouldAnimateHide =
				'hidden' === visibility &&
				body.hasClass( 'customize-partial-edit-shortcuts-shown' ) &&
				! body.hasClass( 'customize-partial-edit-shortcuts-hidden' );
