...
( function () {
	...
	var qt = ( window.QTags = function ( settings ) {
		...
		if (
			id === 'content' &&
			typeof adminpage === 'string' &&
			( adminpage === 'post-new-php' || adminpage === 'post-php' )
		) {
			// Back compat hack :-(
		...
		onclick = function ( e ) {
			...
			var target = e.target || e.srcElement,
				visible = target.clientWidth || target.offsetWidth,
				i;
	...
	qt.insertContent = function ( content ) {
		...
		if ( document.selection ) {
			...
			canvas.value =
				text.substring( 0, startPos ) + content + text.substring( endPos, text.length );
	...
	qt.Button.prototype.html = function ( idPrefix ) {
		...
		if ( this.id === 'fullscreen' ) {
			return (
				'<button type="button"' +
				id +
				' class="ed_button qt-dfw qt-fullscreen"' +
				title +
				ariaLabel +
				'></button>'
			);
	...
	qt.ImgButton.prototype.callback = function ( e, c, ed, defaultValue ) {
		...
		var src = prompt( quicktagsL10n.enterImageURL, defaultValue ),
			alt;
		if ( src ) {
	...
	qt.TextDirectionButton = function () {
		qt.Button.call(
			this,
			'textdirection',
			quicktagsL10n.textdirection,
			'',
			quicktagsL10n.toggleTextdirection
		);
	};
	...
	edButtons[ 50 ] = new qt.TagButton(
		...
		'',
		'',
		{ ariaLabel: quicktagsL10n.del, ariaLabelClose: quicktagsL10n.delClose }
	);
	edButtons[ 60 ] = new qt.TagButton(
		'ins',
		'ins',
		...
		'',
		'',
		{ ariaLabel: quicktagsL10n.ins, ariaLabelClose: quicktagsL10n.insClose }
	);
