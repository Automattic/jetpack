...
<div class="upsell_banner">
	...
	public function add_woocommerce_installation_menu() {
		...
		$woo_plugin_is_active = is_plugin_active( 'woocommerce/woocommerce.php' );

		// If this is not Atomic and not a Simple site, or if Woo plugin is already active, return and do nothing.
		if ( ( ! $is_woa_site && ! $is_simple_site ) || $woo_plugin_is_active ) {
			return;
		}

		// By default, assume we won't add Woocommerce upsell/installation menu item.
		$woop_upsell = false;

		/*
		 * 'woop-upsell' is a "feature" defined in store-product-list.php on WPCOM that is available on plans we want to
		 * market Woop to. In this case, if this feature is "active", we display the "WooCommerce" marketing/installation
		 * menu item. For CFT purposes is_automattician() can be checked in store-product-list.php.
		 */
		$feature = 'woop-upsell';
		if ( $is_simple_site ) {
			// If this is a Simple site, check the local method.
			require_once WP_CONTENT_DIR . '/admin-plugins/wpcom-billing/store-product-list.php';
			$woop_upsell = \Store_Product_List::has_active_feature( $feature );
		} else {
			// If this is an Atomic site, check the "features" API.
			$response = Client::wpcom_json_api_request_as_user(
				"/sites/$this->domain/features",
				'1.1',
				array(
					'method'  => 'GET',
					'headers' => array(
			...
			// TODO: log error, but fail silently. Don't break site just to show Woop upsell.
			if ( ! is_wp_error( $response ) ) {
				$body = json_decode( wp_remote_retrieve_body( $response ) );

				if ( in_array( $feature, $body->active, true ) ) {
					$woop_upsell = true;
				}
			}
		}
		...
		if ( $woop_upsell ) {
			...
			add_menu_page( $title, $title, 'activate_plugins', $menu_url, null, $icon_url, 55.5 );
		}
	}
