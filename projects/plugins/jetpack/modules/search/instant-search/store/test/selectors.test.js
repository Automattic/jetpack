...
describe( 'hasActiveQuery', () => {
	test( 'returns false if reducers are at their initial values', () => {
		expect( hasActiveQuery( { searchQuery: null, filters: {}, staticFilters: {}, sort: null } ) ).toEqual( false );
	} );
	test( 'returns true if there is a defined search query', () => {
		expect( hasActiveQuery( { searchQuery: '', filters: {}, staticFilters: {}, sort: null } ) ).toEqual( true );
		expect( hasActiveQuery( { searchQuery: 'hello', filters: {}, staticFilters: {}, sort: null } ) ).toEqual( true );
		expect( hasActiveQuery( { searchQuery: null, filters: {}, staticFilters: {}, sort: null } ) ).toEqual( false );
	} );
	...
	test( 'returns true if there is a defined sort value', () => {
		expect( hasActiveQuery( { searchQuery: null, filters: {}, staticFilters: {}, sort: 'relevance' } ) ).toEqual(
			true
		);
		expect( hasActiveQuery( { searchQuery: null, filters: {}, staticFilters: {}, sort: null } ) ).toEqual( false );
	} );
