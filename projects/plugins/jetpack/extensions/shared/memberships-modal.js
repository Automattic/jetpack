...
const tb_pathToImage = 'images/loadingAnimation.gif';
	...
	try {
		if ( typeof document.body.style.maxHeight === 'undefined' ) {
			...
			if ( document.getElementById( 'TB_HideSelect' ) === null ) {
				...
				document.getElementById( 'TB_overlay' ).addEventListener( 'click', tb_remove, false );
			}
		} else {
			//all others
		...
		document
			.querySelector( 'body' )
			.append( "<div id='TB_load'><img src='" + imgLoader.src + "' /></div>" ); //add loader to the page
		...
		if (
			...
			TB_FoundURL = false;
			if ( imageGroup ) {
				TB_TempArray = document.querySelector( 'a[@rel=' + imageGroup + ']' );
				for (
					TB_Counter = 0;
			...
			imgPreloader.onload = function () {
				...
				if ( ! ( TB_PrevHTML === '' ) ) {
					function goPrev() {
						...
						document.querySelector( 'body' ).append( "<div id='TB_window'></div>" );
				...
				document.getElementById( 'TB_ImageOff' ).addEventListener( 'click', tb_remove, false );
			...
			if ( url.indexOf( 'TB_iframe' ) !== -1 ) {
				...
				if ( params.modal !== 'true' ) {
					...
					//iframe modal
					document.getElementById( 'TB_overlay' ).unbind();
					document
						.getElementById( 'TB_window' )
						.append(
...
function tb_remove() {
	document.getElementById( 'TB_imageOff' ).removeEventListener( 'click', tb_remove );
	document.getElementById( 'TB_closeWindowButton' ).removeEventListener( 'click', tb_remove );
	document.getElementById( 'TB_window' ).remove();
