...
function GooglePhotosPickerAuth( props ) {
	...
	const onAuthorize = useCallback( () => {
		...
		apiFetch( {
			path: getApiUrl( 'connection', SOURCE_GOOGLE_PHOTOS_PICKER ),
		} )
			.then( service => {
				if ( service.error ) {
	...
	return (
		<div className="jetpack-external-media-auth">
			...
			<Button variant="primary" disabled={ isAuthing } onClick={ onAuthorize }>
				{ __( 'Connect to Google Photos Picker', 'jetpack' ) }
			</Button>
		</div>
	);
}

export default GooglePhotosPickerAuth;
