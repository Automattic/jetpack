...
// AI image disclosure: render a visible "Generated by AI" disclosure on the
// front end for images whose attachment carries the provenance meta set
// server-side at generation. Shown for AI images by default; a site can disable
// it everywhere via the `jetpack_ai_image_disclosure` filter. Lives here rather
// than its own extension module so it loads with Image Studio on the front end.

const AI_DISCLOSURE_GENERATED_META = '_wpcom_image_studio_ai_generated';
const AI_DISCLOSURE_FILTER         = 'jetpack_ai_image_disclosure';
const AI_DISCLOSURE_STYLE_HANDLE   = 'jetpack-ai-image-disclosure';

/**
 * Whether the disclosure should be rendered for a given image block.
 *
 * @param array $block The parsed block, including its attributes.
 * @return bool
 */
function ai_image_disclosure_should_show( $block ) {
	...
	if ( ! get_post_meta( $attachment_id, AI_DISCLOSURE_GENERATED_META, true ) ) {
		return false;
	}

	/**
	 * Filter whether the "Generated by AI" disclosure is shown site-wide.
	 *
	 * @since 16.0
	 *
	 * @param bool $enabled Whether the disclosure is enabled. Default true.
	 */
	return (bool) apply_filters( AI_DISCLOSURE_FILTER, true );
}

/**
 * Inject the disclosure badge and caption line into a rendered image block.
 *
 * @param string $block_content The block's rendered HTML.
 * @param array  $block         The parsed block, including its attributes.
 * @return string The filtered HTML.
 */
function ai_image_disclosure_render( $block_content, $block ) {
	if ( ! ai_image_disclosure_should_show( $block ) ) {
		return $block_content;
	}

	$label = __( 'Generated by AI', 'jetpack' );

	$marked = ai_image_disclosure_add_marker_class( $block_content );
	if ( null === $marked ) {
		return $block_content;
	}

	return ai_image_disclosure_add_markup( $marked, $label );
}
add_filter( 'render_block_core/image', __NAMESPACE__ . '\ai_image_disclosure_render', 10, 2 );

/**
 * Add the marker class to the block's figure using the HTML API.
 *
 * @param string $block_content The block's rendered HTML.
 * @return string|null The updated HTML, or null when no figure is present.
 */
function ai_image_disclosure_add_marker_class( $block_content ) {
	...
	$tags->add_class( 'jetpack-ai-image-disclosure' );

	return $tags->get_updated_html();
}

/**
 * Add the corner badge and caption disclosure within the figure.
 *
 * The badge sits inside the figure as a positioned overlay. The caption
 * disclosure is appended to an existing figcaption when present so a user's
 * caption is never overwritten, or a new figcaption is added when there is none.
 * Both are inserted at the closing figure boundary, the only point the HTML API
 * cannot express as an attribute edit.
 *
 * @param string $block_content The block HTML with the marker class added.
 * @param string $label         Translated disclosure label.
 * @return string
 */
function ai_image_disclosure_add_markup( $block_content, $label ) {
	$badge = sprintf(
		'<span class="jetpack-ai-image-disclosure__badge" aria-hidden="true">%1$s</span><span class="screen-reader-text">%2$s</span>',
		esc_html( $label ),
		esc_html( $label )
	);

	$disclosure = sprintf(
		'<span class="jetpack-ai-image-disclosure__caption">%s</span>',
		esc_html( $label )
	);

	// A core/image block renders one <figure> wrapping one optional <figcaption>,
	// so targeting the first closing tag is sufficient and avoids matching any
	// nested figure/figcaption a theme might add.
	if ( false !== stripos( $block_content, '</figcaption>' ) ) {
		$with_caption = preg_replace(
			'/<\/figcaption>/i',
		...
		$with_caption = preg_replace(
			'/<\/figure>/i',
			'<figcaption class="wp-element-caption jetpack-ai-image-disclosure__caption-wrap">' . $disclosure . '</figcaption></figure>',
			$block_content,
			1
		);
	}

	return preg_replace( '/<\/figure>/i', $badge . '</figure>', $with_caption, 1 );
}

/**
 * Register and enqueue the front-end disclosure styles.
 *
 * @return void
 */
function ai_image_disclosure_enqueue_styles() {
	wp_register_style( AI_DISCLOSURE_STYLE_HANDLE, false, array(), \JETPACK__VERSION );
	wp_enqueue_style( AI_DISCLOSURE_STYLE_HANDLE );

	$css = <<<'CSS'
.jetpack-ai-image-disclosure{position:relative;}
.jetpack-ai-image-disclosure__badge{position:absolute;bottom:.5em;right:.5em;padding:.15em .5em;font-size:.75em;line-height:1.4;color:#fff;background:rgba(0,0,0,.6);border-radius:2px;pointer-events:none;}
.jetpack-ai-image-disclosure__caption{display:block;font-size:.8em;opacity:.8;}
CSS;

	wp_add_inline_style( AI_DISCLOSURE_STYLE_HANDLE, $css );
}
add_action( 'wp_enqueue_scripts', __NAMESPACE__ . '\ai_image_disclosure_enqueue_styles' );
