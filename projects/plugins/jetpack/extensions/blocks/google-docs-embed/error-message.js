const embeds = document.querySelectorAll( '.wp-block-p2-embed' );
...
const showError = () => {
	...
	embeds.forEach( embed => {
		...
		const privateErrorMsg =
			'<p class="wp-block-p2-embed__error-msg">This document is private. To view the document, login to a Google account that the document has been shared with and then refresh this page.</p>';
		...
		if (
			embedUrlComponents === null ||
			typeof embedUrlComponents[ 1 ] === 'undefined' ||
			typeof embedUrlComponents[ 2 ] === 'undefined' ||
			typeof embedUrlComponents[ 3 ] === 'undefined'
		) {
