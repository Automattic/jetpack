...
function load_assets( $attributes ) {
	...
	foreach ( $data as $item ) {
		...
		$output .= '<span><a class="jetpack-top-posts-title" href="' . esc_url( $item['href'] ) . '">' . esc_html( $item['title'] ) . '</a></span>';
		...
		if ( $attributes['displayContext'] && ! empty( $item['context'] ) && is_array( $item['context'] ) ) {
			...
			$output .= '<span><a class="jetpack-top-posts-context has-small-font-size" href="' . esc_url( get_category_link( $context->term_id ) ) . '">' . esc_html( $context->name ) . '</a></span>';
		}
