...
class Jetpack {
	...
	private function __construct() {
		...
		/**
		 * Move me somewhere that isn't a total hack :)
		 */
		add_action(
			'admin_head',
			function () {
				?>
				<script>
					window.askJetpack = ( question ) => {
						const apiNonce = JP_CONNECTION_INITIAL_STATE.apiNonce;

						async function requestToken() {
							const request = await fetch( '/wp-json/jetpack/hack/get-openai-jwt?_cacheBuster=' + Date.now(), {
						...
						async function askQuestion() {
							...
							const url = new URL( 'https://public-api.wordpress.com/wpcom/v2/sites/' + blogId + '/jetpack-openai-query' );
						...
						askQuestion().catch( err => console.error( err ) );
					};
				</script>
				<?php
			}
