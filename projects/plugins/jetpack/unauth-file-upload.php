 ...
 *
 * Uses the Jetpack blog token, which this feature already depends on (the file content is
 * fetched from WP.com as the blog). There is nothing extra to store, and reconnecting Jetpack
 * rotates the token and so invalidates any outstanding links.
 *
 * @return string The signing key, or an empty string if the site has no blog token.
 */
function get_download_signing_key() {
	...
	if ( $blog_token && ! is_wp_error( $blog_token ) && ! empty( $blog_token->secret ) ) {
		return (string) $blog_token->secret;
	}
...
function handle_file_download() {
	...
	if ( '' !== $token ) {
		...
		if ( ! verify_download_token( $file_id, $expires, $token, $token_type ) ) {
			wp_die( esc_html__( 'Invalid download link.', 'jetpack' ) );
		}
