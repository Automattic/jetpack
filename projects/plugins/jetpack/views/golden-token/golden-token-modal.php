 ...
 * Golden token Modal.
 *
 * Will try to show on any wp-admin page if jetpack-golden-token param is in the URL.
 *
...
// @TODO
$has_redeemable_token = apply_filters( 'jetpack_golden_token_redeemable', false );
$show_modal           = isset( $_GET['jetpack-golden-token'] ) || $has_redeemable_token;

if ( ! $show_modal ) {
	return;
}

$show_redeedmed = isset( $_GET['jetpack-golden-token'] ) && 'redeemed' === $_GET['jetpack-golden-token'];

wp_enqueue_style( 'jetpack-golden-token', plugins_url( 'css/golden-token.css', JETPACK__PLUGIN_FILE ) );
wp_enqueue_script( 'jetpack-golden-token', plugins_url( 'views/golden-token/js/index.js', JETPACK__PLUGIN_FILE ), array(), JETPACK__VERSION, true );
?>

<div id="jpgt-modal-container" onclick="jpgtCloseModal()"></div>
<div id="jpgt-modal" class="jpgt-modal <?php echo $show_redeedmed ? 'animating' : 'animating1'; ?>">
	<svg
		id="jetpack-logo"
	...
	<div id="jpgt-video-wrap" class="jpgt-video-wrap" onclick="jpgtPlayVideo()">
		<video
			id="jpgt-video"
	...
	<div class="jpgt-content-wrap">
		<div class="jpgt-content-wrap-text">
			<p class="hi-user">Hey, <?php echo esc_html( wp_get_current_user()->display_name ); ?></p>
			<h2>
				<?php esc_html_e( 'Your exclusive Jetpack Experience awaits', 'jetpack' ); ?>
			</h2>
			<p>
				<?php esc_html_e( 'You have been gifted a Jetpack Gold Token. This unlocks a lifetime of Jetpack powers for your website.', 'jetpack' ); ?>
			</p>
		</div>
		<button onclick="jpgtPlayVideo()">
			<?php esc_html_e( 'Redeem your token', 'jetpack' ); ?>
		</button>
	</div>

	<div class="powers-wrap jpgt-content-wrap">
		<div class="jpgt-content-wrap-text">
			<h2>
				<?php esc_html_e( 'Super powers are ready!', 'jetpack' ); ?>
			</h2>
			<p>
				<?php esc_html_e( 'Your Jetpack Gold Token provides a lifetime license for this website and includes the following products:', 'jetpack' ); ?>
			</p>
		</div>

		<div class="jpgt-products">
			<div>
				<svg
					width="24"
					height="24"
				...
				<h3>
					<?php esc_html_e( 'VaultPress Backup', 'jetpack' ); ?>
				</h3>
				<p>
					<?php esc_html_e( 'Save every change and get back online quickly with one‑click restores.', 'jetpack' ); ?>
				</p>
			</div>
			<div>
				<svg
				...
				<h3>
					<?php esc_html_e( 'Scan', 'jetpack' ); ?>
				</h3>
				<p>
					<?php esc_html_e( 'Automated scanning and one‑click fixes to keep your site ahead of security threats.', 'jetpack' ); ?>
				</p>
			</div>
		</div>

		<a href="<?php echo esc_url( admin_url( 'admin.php?page=my-jetpack' ) ); ?>">
			<button>
				<?php esc_html_e( 'Explore your new powers', 'jetpack' ); ?>
			</button>
		</a>
	</div>
</div>
