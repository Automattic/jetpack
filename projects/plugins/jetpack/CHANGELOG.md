...
- Admin-menu REST endpoint now surfaces notification counts from the central menu-badges registry, so Calypso/Simple sidebars get authoritative Jetpack menu counts. [#50190]
- Expose the AI Launchpad state options (enabled, dismissed, completed) in the /sites endpoint options, so Calypso clients can gate My Home and launchpad surfaces on them. [#50455]
...
- Podcast: make the module active-state the single load gate. The module no longer requires a Jetpack connection (its core feed/settings/block are local), and it loads from late_initialization whenever active, so the podcast feed keeps serving even while disconnected. [#50242]
- Updated package dependencies. [#50248]
- Update package dependencies. [#49272]
- Update package dependencies. [#49864]
- Update package dependencies. [#50407]
- Update package dependencies. [#50436]
