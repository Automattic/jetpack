...
class Jetpack_Sync_Checksum_Test extends WP_UnitTestCase {
	...
	/**
	 * Test that get_range_edges uses MIN/MAX approximation for unfiltered single-key tables.
	 */
	public function test_get_range_edges_uses_min_max_approximation() {
		$user_id = self::factory()->user->create();
		...
		// Posts table has filter_values (disallowed post types), so it uses the
		// regular COUNT path. Use a custom table config without filter_values to
		// exercise the MIN/MAX approximation path.
		...
		// item_count should be MAX - MIN + 1 (approximate, >= actual count).
		$expected_approx = $last_id - $first_id + 1;
		$this->assertSame( $expected_approx, (int) $range['item_count'] );
		$this->assertSame( $first_id, (int) $range['min_range'] );
		$this->assertSame( $last_id, (int) $range['max_range'] );

		remove_filter( 'jetpack_sync_checksum_allowed_tables', array( $this, 'set_allowed_tables' ) );
	}

	/**
	 * Test that get_range_edges MIN/MAX approximation is not used for filtered tables.
	 */
	public function test_get_range_edges_min_max_skipped_for_filtered_tables() {
		$user_id = self::factory()->user->create();

		// Create 5 allowed posts + 1 disallowed.
		for ( $i = 0; $i < 5; $i++ ) {
		...
		// Posts table has filter_values, so it should use the exact COUNT path
		// and return the filtered count (5, not 6).
		$tc    = new Table_Checksum( 'posts' );
		$range = $tc->get_range_edges();

		$this->assertSame( 5, (int) $range['item_count'] );
	}

	/**
	 * Test that get_range_edges MIN/MAX approximation is not used for meta tables.
	 */
	public function test_get_range_edges_min_max_skipped_for_meta_tables() {
		$user_id = self::factory()->user->create();
		...
		// Postmeta has a parent_table and compound keys (DISTINCT), so it should
		// use the parent count path, not the MIN/MAX approximation.
		$tc    = new Table_Checksum( 'postmeta' );
		$range = $tc->get_range_edges();
