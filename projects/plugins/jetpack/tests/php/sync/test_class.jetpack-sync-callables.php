...
class WP_Test_Jetpack_Sync_Functions extends WP_Test_Jetpack_Sync_Base {
	...
	public function test_sync_callable_whitelist() {
		...
		$callables = array(
			...
			'main_network_site'                => Utils::main_network_site_url(),
	...
	function test_get_protocol_normalized_url_works_with_no_history() {
		...
		$option_key    = Utils::HTTPS_CHECK_OPTION_PREFIX . $callable_type;
		delete_option( $option_key );

		$this->assertStringStartsWith(
			'http://',
			Utils::get_protocol_normalized_url( $callable_type, $this->return_example_com() )
		);
		...
		$this->assertStringStartsWith(
			'https://',
			Utils::get_protocol_normalized_url( $callable_type, $this->return_https_example_com() )
		);
	...
	function test_get_protocol_normalized_url_stores_max_history() {
		...
		$this->assertCount( Utils::HTTPS_CHECK_HISTORY, get_option( $option_key ) );
		delete_option( $option_key );
	}
	...
	function test_get_protocol_normalized_url_returns_http_when_https_falls_off() {
		...
		for ( $i = 1; $i < Utils::HTTPS_CHECK_HISTORY; $i++ ) {
			$this->assertStringStartsWith(
				'https://',
				Utils::get_protocol_normalized_url( $callable_type, $this->return_example_com() )
			);
	...
	function test_get_protocol_normalized_url_returns_new_value_cannot_parse() {
		...
		$this->assertEquals(
			$test_url,
			Utils::get_protocol_normalized_url( 'home_url', $test_url )
		);
	}

	function test_get_protocol_normalized_url_cleared_on_reset_data() {
		Utils::get_protocol_normalized_url( 'home_url', get_home_url() );
		Utils::get_protocol_normalized_url( 'site_url', get_site_url() );
		Utils::get_protocol_normalized_url( 'main_network_site_url', network_site_url() );

		$url_callables = array( 'home_url', 'site_url', 'main_network_site_url' );
		foreach( $url_callables as $callable ) {
			$this->assertInternalType( 'array', get_option( Utils::HTTPS_CHECK_OPTION_PREFIX . $callable ) );
		}

		Sender::get_instance()->uninstall();

		foreach( $url_callables as $callable ) {
			$this->assertFalse( get_option( Utils::HTTPS_CHECK_OPTION_PREFIX . $callable ) );
		}
	}
	...
	function test_get_raw_url_by_option_bypasses_filters() {
		...
		$this->assertTrue( 'http://filteredurl.com' !== Utils::get_raw_url( 'home' ) );
	...
	function test_get_raw_url_by_constant_bypasses_filters() {
		...
		if ( is_multisite() ) {
			$this->assertTrue( $this->return_filtered_url() !== Utils::get_raw_url( 'home' ) );
			$this->assertTrue( $this->return_filtered_url() !== Utils::get_raw_url( 'siteurl' ) );
		} else {
			$this->assertEquals( 'http://constanturl.com', Utils::get_raw_url( 'home' ) );
			$this->assertEquals( 'http://constanturl.com', Utils::get_raw_url( 'siteurl' ) );
		}
	...
	function test_get_raw_url_returns_with_http_if_is_ssl() {
		...
		$this->assertEquals( $home_option, Utils::get_raw_url( 'home' ) );

		// Now, with https
		$_SERVER['HTTPS'] = 'on';
		$this->assertEquals(
			set_url_scheme( $home_option, 'http' ),
			Utils::get_raw_url( 'home' )
		);
		unset( $_SERVER['HTTPS'] );
	}
	...
	function test_user_can_stop_raw_urls() {
		...
		$this->assertTrue( 'http://filteredurl.com' !== Utils::home_url() );

		// Now, without, which should return the filtered URL
		Constants::set_constant( 'JETPACK_SYNC_USE_RAW_URL', false );
		$this->assertEquals( $this->return_filtered_url(), Utils::home_url() );
		Constants::clear_constants();
