...
class Image_Studio_Test extends \WP_UnitTestCase {
	...
	// -------------------------------------------------------------------------
	// AI image disclosure render tests.
	// -------------------------------------------------------------------------

	/**
	 * Build a core/image block and its rendered markup for a given attachment.
	 *
	 * @param int    $attachment_id Attachment ID for the block, or 0 for none.
	 * @param string $caption       Optional user caption to include.
	 * @param bool   $linked        Whether to wrap the image in a link.
	 * @return array{0: string, 1: array} The block content and parsed block.
	 */
	private function make_disclosure_image_block( $attachment_id, $caption = '', $linked = false ) {
		$figcaption = '' === $caption
			? ''
			: '<figcaption class="wp-element-caption">' . $caption . '</figcaption>';

		$img   = '<img src="https://example.com/image.png" alt="" class="wp-image-' . (int) $attachment_id . '"/>';
		$inner = $linked ? '<a href="https://example.com/page">' . $img . '</a>' : $img;

		$content = '<figure class="wp-block-image size-large">' . $inner . $figcaption . '</figure>';

		$attrs = $attachment_id ? array( 'id' => (int) $attachment_id ) : array();
	...
	/**
	 * Create an attachment flagged with the server-side AI provenance meta.
	 *
	 * @return int Attachment ID.
	 */
	private function create_ai_generated_attachment() {
		...
		update_post_meta( $attachment_id, ImageStudio\AI_DISCLOSURE_GENERATED_META, true );
		return $attachment_id;
	}

	/**
	 * Badge and caption disclosure are injected when the attachment carries the
	 * AI-generated provenance meta and the gate is on (default).
	 */
	public function test_disclosure_renders_when_ai_generated() {
		$id                      = $this->create_ai_generated_attachment();
		list( $content, $block ) = $this->make_disclosure_image_block( $id );

		$output = ImageStudio\ai_image_disclosure_render( $content, $block );

		$this->assertStringContainsString( 'jetpack-ai-image-disclosure', $output );
		$this->assertStringContainsString( 'jetpack-ai-image-disclosure__badge', $output );
		$this->assertStringContainsString( 'jetpack-ai-image-disclosure__caption', $output );
		$this->assertStringContainsString( 'Generated by AI', $output );
		$this->assertStringContainsString( 'screen-reader-text', $output );

		remove_all_filters( ImageStudio\AI_DISCLOSURE_FILTER );
	}

	/**
	 * Nothing is injected when the attachment is not flagged AI-generated.
	 */
	public function test_disclosure_renders_nothing_when_not_ai_generated() {
		...
		$this->assertSame( $content, ImageStudio\ai_image_disclosure_render( $content, $block ) );
	}

	/**
	 * Nothing is injected when the block has no image id.
	 */
	public function test_disclosure_renders_nothing_when_no_id() {
		list( $content, $block ) = $this->make_disclosure_image_block( 0 );

		$this->assertSame( $content, ImageStudio\ai_image_disclosure_render( $content, $block ) );
	}

	/**
	 * Returning false from the filter suppresses the disclosure site-wide.
	 */
	public function test_disclosure_filter_can_disable() {
		$id                      = $this->create_ai_generated_attachment();
		list( $content, $block ) = $this->make_disclosure_image_block( $id );

		add_filter( ImageStudio\AI_DISCLOSURE_FILTER, '__return_false' );
		$output = ImageStudio\ai_image_disclosure_render( $content, $block );
		remove_all_filters( ImageStudio\AI_DISCLOSURE_FILTER );
	...
	/**
	 * An existing user caption is preserved and the disclosure appended to it.
	 */
	public function test_disclosure_does_not_overwrite_existing_caption() {
		$id                      = $this->create_ai_generated_attachment();
		list( $content, $block ) = $this->make_disclosure_image_block( $id, 'My own caption' );

		$output = ImageStudio\ai_image_disclosure_render( $content, $block );
		remove_all_filters( ImageStudio\AI_DISCLOSURE_FILTER );

		$this->assertStringContainsString( 'My own caption', $output );
		$this->assertStringContainsString( 'jetpack-ai-image-disclosure__caption', $output );
		$this->assertSame( 1, substr_count( $output, '<figcaption' ) );
	}

	/**
	 * Content without a figure is returned unchanged (null-guard path).
	 */
	public function test_disclosure_renders_unchanged_when_no_figure() {
		$id      = $this->create_ai_generated_attachment();
		$content = '<img src="https://example.com/image.png" alt="" class="wp-image-' . $id . '"/>';
		$block   = array(
			'blockName' => 'core/image',
			'attrs'     => array( 'id' => $id ),
		);
	...
	/**
	 * A linked image still has the badge and caption placed within the figure.
	 */
	public function test_disclosure_renders_for_linked_image() {
		$id                      = $this->create_ai_generated_attachment();
		list( $content, $block ) = $this->make_disclosure_image_block( $id, '', true );
		...
		$this->assertStringContainsString( 'jetpack-ai-image-disclosure__badge', $output );
		$this->assertStringContainsString( 'jetpack-ai-image-disclosure__caption', $output );
		$this->assertStringContainsString( '<a href="https://example.com/page">', $output );
		$this->assertLessThan(
			strpos( $output, '</figure>' ),
			strpos( $output, 'jetpack-ai-image-disclosure__badge' )
		);
	}
}
