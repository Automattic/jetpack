...
class REST_Controller {
	...
	public static function register_rest_routes() {
		// Install a Helper Script to assist Jetpack Protect fetch data.
		register_rest_route(
			'jetpack/v4',
			'/protect-helper-script',
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => __CLASS__ . '::install_protect_helper_script',
				'permission_callback' => __CLASS__ . '::protect_permissions_callback',
				'args'                => array(
					'helper' => array(
						'description' => __( 'base64 encoded Protect Helper Script body.', 'jetpack-protect' ),
						'type'        => 'string',
						'required'    => true,
					),
				),
		...
		// Delete a Protect Helper Script.
		register_rest_route(
			'jetpack/v4',
			'/protect-helper-script',
			array(
				'methods'             => WP_REST_Server::DELETABLE,
				'callback'            => __CLASS__ . '::delete_protect_helper_script',
				'permission_callback' => __CLASS__ . '::protect_permissions_callback',
				'args'                => array(
					'path' => array(
						'description' => __( 'Path to Protect Helper Script', 'jetpack-protect' ),
						'type'        => 'string',
						'required'    => true,
					),
				),
	...
	/**
	 * The Protect endpoints should only be available via site-level authentication.
	 ...
	 */
	public static function protect_permissions_callback() {
	...
	/**
	 * Install the Protect Helper Script.
	 *
	 * @access public
	 * @static
	 *
	 ...
	 */
	public static function install_protect_helper_script( $request ) {
		$helper_script = $request->get_param( 'helper' );

		// phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode
		$helper_script = base64_decode( $helper_script );
		if ( ! $helper_script ) {
			return new WP_Error( 'invalid_args', __( 'Helper script body must be base64 encoded', 'jetpack-protect' ), 400 );
		}
	 ...
	 */
	public static function delete_protect_helper_script( $request ) {
