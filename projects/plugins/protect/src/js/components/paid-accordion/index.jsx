...
import { Icon, check, chevronDown, chevronUp, warning } from '@wordpress/icons';
			...
			<button className={ styles[ 'accordion-header' ] } onClick={ handleClick }>
				<div>
					<Text className={ styles[ 'accordion-header-label' ] } mb={ 1 }>
						<Icon icon={ icon || warning } className={ styles[ 'accordion-header-label-icon' ] } />
						{ label }
					</Text>
					<Text
						...
						<>
							{ firstDetected && (
								<Text className={ styles[ 'accordion-header-status' ] }>
									...
									<FixDetails date={ fixedOn || firstDetected } isFixed={ !! fixedOn } />
								</Text>
							) }
