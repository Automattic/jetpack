...
const filename = ( { oldFileName, newFileName } ) => {
	...
	if ( prev === next ) {
		...
		return (
			<Fragment>
				{ base && <span className={ styles[ 'diff-viewer__path-prefix' ] }>{ base }</span> }
				<span className={ styles[ 'diff-viewer__path' ] }>{ name }</span>
			</Fragment>
		);
	}
	...
	for ( let i = 0, slash = 0; i < length; i++ ) {
		...
		if ( prev[ i ] !== next[ i ] ) {
			return (
				<Fragment>
					{ slash !== 0 && (
						<span className={ styles[ 'diff-viewer__path-prefix' ] }>
							{ prev.slice( 0, slash ) }
						</span>
					) }
					<span className={ styles[ 'diff-viewer__path' ] }>{ prev.slice( slash ) }</span>
					{ ' → ' }
					{ slash !== 0 && (
						<span className={ styles[ 'diff-viewer__path-prefix' ] }>
							{ next.slice( 0, slash ) }
					...
					<span className={ styles[ 'diff-viewer__path' ] }>{ next.slice( slash ) }</span>
				</Fragment>
			);
		}
	}
	...
	return (
		<Fragment>
			{ prevBase && <span className={ styles[ 'diff-viewer__path-prefix' ] }>{ prevBase }</span> }
			<span className={ styles[ 'diff-viewer__path' ] }>{ prevName }</span>
			{ ' → ' }
			{ nextBase && <span className={ styles[ 'diff-viewer__path-prefix' ] }>{ nextBase }</span> }
			<span className={ styles[ 'diff-viewer__path' ] }>{ nextName }</span>
		</Fragment>
	);
};
