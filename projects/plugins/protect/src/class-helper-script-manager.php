...
class Helper_Script_Manager {
	...
	const HELPER_HEADER  = "<?php /* Jetpack Protect Helper Script */\n";
	const EXPIRY_TIME    = 8 * 3600; // 8 hours
	const MAX_FILESIZE   = 1024 * 1024; // 1 MiB

	const README_LINES = array(
		'These files have been put on your server by Jetpack Protect. They are cleaned up automatically when we no longer need them.',
		'If you no longer have Jetpack Protect connected to your site, you can delete them manually.',
	...
	public static function install_helper_script( $script_body ) {
		...
		for ( $attempt = 0; $attempt < $max_attempts; $attempt++ ) {
			...
			if ( ! file_exists( $file_path ) ) {
				...
				\wp_schedule_single_event( time() + self::EXPIRY_TIME + 60, 'jetpack_protect_cleanup_helper_scripts' );
