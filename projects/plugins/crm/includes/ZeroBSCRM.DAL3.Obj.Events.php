...
class zbsDAL_events extends zbsDAL_ObjectLayer {
	...
	protected $objectModel           = array(
		...
		'complete'       => array(
			'fieldname' => 'zbse_complete',
			'format'    => 'bool',
		),
		'show_on_portal' => array(
			'fieldname' => 'zbse_show_on_portal',
			'format'    => 'bool',
		),
		'show_on_cal'    => array(
			'fieldname' => 'zbse_show_on_cal',
			'format'    => 'bool',
		),
		'created'        => array(
			'fieldname' => 'zbse_created',
			'format'    => 'uts',
		),
		'lastupdated'    => array(
			'fieldname' => 'zbse_lastupdated',
			'format'    => 'uts',
		),
		...
		if (
			...
			if ( isset( $potentialRes ) && isset( $potentialRes->ID ) ) {
				...
				if ( $withReminders ) {
					...
					$res['reminders'] = $this->DAL()->eventreminders->getEventreminders(
						array(
							'eventID'     => $potentialRes->ID,
							'perPage'     => 1000,
	...
	public function getEvents( $args = array() ) {
		...
		#} Count override
		if ( $count ) {
			$query = 'SELECT COUNT(event.ID) FROM ' . $ZBSCRM_t['events'] . ' as event' . $joinQ;
		}
		...
		if ( ! empty( $externalSource ) ) {
			...
			// completed status
		if ( $isComplete ) {
			$wheres['status'] = array( 'zbse_complete', '=', '1' );
		}
		if ( $isIncomplete ) {
			$wheres['status'] = array( 'zbse_complete', '<>', '1' );
		}
		...
		if ( isset( $potentialRes ) && is_array( $potentialRes ) && count( $potentialRes ) > 0 ) {
			...
			foreach ( $potentialRes as $resDataLine ) {
				...
				if ( $withReminders ) {
					...
					// add all event reminder lines
					$resArr['reminders'] = $this->DAL()->eventreminders->getEventreminders(
						array(
							'eventID'     => $resDataLine->ID,
							'perPage'     => 1000,
							'ignoreowner' => zeroBSCRM_DAL2_ignoreOwnership( ZBS_TYPE_TASK ),
						)
					);
	...
	public function addUpdateEvent( $args = array() ) {
		...
		if ( is_array( $limitedFields ) ) {
			...
			// add update time
			if ( ! isset( $dataArr['zbse_lastupdated'] ) ) {
				$dataArr['zbse_lastupdated'] = time();
				$typeArr[]                   = '%d'; }
		} else {
				...
				// FULL UPDATE/INSERT
					...
					if ( ! empty( $id ) && $id > 0 ) {
						...
						if ( isset( $data['created'] ) && ! empty( $data['created'] ) && $data['created'] !== -1 ) {
							$dataArr['zbse_created'] = $data['created'];
							$typeArr[]               = '%d';
						} else {
							$dataArr['zbse_created'] = time();
							$typeArr[]               = '%d';
						}
					}
		}
							...
							if ( isset( $data['reminders'] ) && is_array( $data['reminders'] ) ) {
								...
								if ( count( $data['reminders'] ) > 0 ) {
									...
									foreach ( $data['reminders'] as $reminder ) {
											...
											$this->DAL()->eventreminders->addUpdateEventreminder(
												array(
													'id'   => $reminderID,
													'data' => $reminder,
												)
	...
	private function tidy_event( $obj = false, $withCustomFields = false ) {
		...
		if ( isset( $obj->ID ) ) {
			...
			*/
			...
			if ( $withCustomFields ) {
				$res = $this->tidyAddCustomFields( ZBS_TYPE_TASK, $obj, $res, false );
			}
		}
