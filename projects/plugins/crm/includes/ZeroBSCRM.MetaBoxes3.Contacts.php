...
function zeroBSCRM_CustomersMetaboxSetup() {
	...
	if ( zeroBSCRM_is_customer_edit_page() ) {
		...
		if ( zeroBSCRM_isExtensionInstalled( 'portal' ) ) {
			$zeroBS__Metabox_ContactPortal = new zeroBS__Metabox_ContactPortal( __FILE__ );
		}
		...
		#} Social
		if ( zeroBSCRM_getSetting( 'usesocial' ) == '1' ) {
			$zeroBS__Metabox_ContactSocial = new zeroBS__Metabox_ContactSocial( __FILE__ );
		}

		#} AKA
		if ( zeroBSCRM_getSetting( 'useaka' ) == '1' ) {
			$zeroBS__Metabox_ContactAKA = new zeroBS__Metabox_ContactAKA( __FILE__ );
		}

		#} Ownership
		if ( zeroBSCRM_getSetting( 'perusercustomers' ) == '1' ) {
			$zeroBS__Metabox_Ownership = new zeroBS__Metabox_Ownership( __FILE__, ZBS_TYPE_CONTACT );
		}

		#} B2B mode (assign to co)
		if ( zeroBSCRM_getSetting( 'companylevelcustomers' ) == '1' ) {
			$zeroBS__Metabox_ContactCompany = new zeroBS__Metabox_ContactCompany( __FILE__ );
		}
	}
...
<script type="text/javascript">var zbscrmjs_secToken = '<?php echo esc_js( wp_create_nonce( 'zbscrmjs-ajax-nonce' ) ); ?>';</script>
	...
	public function save_data( $contact_id, $contact ) {
		...
		if ( ! defined( 'ZBS_C_SAVED' ) ) {
			...
			// Company assignment?
			if ( isset( $_POST['zbs_company'] ) ) {
				$dataArr['companies'] = array( (int) sanitize_text_field( $_POST['zbs_company'] ) );
			}
			...
			if ( $contact_id == -1 && ! empty( $addUpdateReturn ) && $addUpdateReturn != -1 ) {
				...
				$zbsJustInsertedMetaboxID = $contact_id;
	...
	public function html( $contact, $metabox ) {
		...
		<div class="action-wrap">
			<div class="ui dropdown jpcrm-button white-bg jpcrm-dropdown"><?php esc_html_e( 'Contact Actions', 'zero-bs-crm' ); ?><i class="fa fa-angle-down"></i>
				...
				<?php
				foreach ( $this->actions as $actKey => $action ) {
...
class zeroBS__Metabox_ContactCustomFiles extends zeroBS__Metabox {
	...
	public function __construct( $plugin_file, $idOverride = '', $titleOverride = '' ) {
		...
		if ( ! empty( $idOverride ) ) {
			$this->metaboxID = $idOverride;
		}
		if ( ! empty( $titleOverride ) ) {
			$this->metaboxTitle = __( $titleOverride, 'zero-bs-crm' );
		}
			...
			$thisFileSlotName = $args['title'];
				...
				?>
							...
							if ( is_array( $zbsFiles ) && count( $zbsFiles ) > 0 ) {
								?>
									<tr class="wh-large zbsFileSlotWrap"><th class="zbsFileSlotTitle"><label><?php echo '<span>' . esc_html( count( $zbsFiles ) ) . '</span> ' . esc_html__( 'File(s)', 'zero-bs-crm' ) . ':'; ?></label></th>
											<td class="">
											<?php
												$fileLineIndx = 1; foreach ( $zbsFiles as $zbsFile ) {
							...
							?>
								...
								<?php
								#adapted from http://code.tutsplus.com/articles/attaching-files-to-your-posts-using-wordpress-custom-meta-boxes-part-1--wp-22291
	...
	public function save_data( $contact_id, $contact ) {
		...
		if ( zeroBSCRM_is_customer_edit_page() && ! defined( 'ZBS_CUSTOMFILES_SAVED' ) ) {
			...
			$settings = $zbs->settings->get( 'customfields' );
			$cfbInd   = 1;
			...
			if ( isset( $settings['customersfiles'] ) && is_array( $settings['customersfiles'] ) && count( $settings['customersfiles'] ) > 0 ) {
				foreach ( $settings['customersfiles'] as $cfb ) {
					...
					if ( ! empty( $thisFileSlotName ) && ! empty( $filePerma ) ) {
						$cfbsubs[ $filePerma ] = $thisFileSlotName;
					}
				}
			}

			if ( count( $cfbsubs ) > 0 ) {
				foreach ( $cfbsubs as $cfSubKey => $cfSubName ) {

								/* --- security verification --- */
					if ( isset( $_POST['zbsc_file_attachment_nonce'] ) ) {
...
class zeroBS__Metabox_ContactPortal extends zeroBS__Metabox {
	...
	public function html( $contact, $metabox ) {
		...
		if ( $user_object ) {
			...
			if ( zeroBSCRM_isCustomerPortalDisabled( $contact['id'] ) ) {
		...
		} elseif ( is_array( $zbsCustomer ) && isset( $zbsCustomer['email'] ) && ! empty( $zbsCustomer['email'] ) ) {
...
class zeroBS__Metabox_ContactSocial extends zeroBS__Metabox {
	...
	public function save_data( $contact_id, $contact ) {
		...
		foreach ( $zbsSocialAccountTypes as $socialKey => $socialAccType ) {
			...
			if ( isset( $_POST[ 'zbs-social-' . $socialAccType['slug'] ] ) && ! empty( $_POST[ 'zbs-social-' . $socialAccType['slug'] ] ) ) {
				$zbsSocials[ $socialKey ] = sanitize_text_field( $_POST[ 'zbs-social-' . $socialAccType['slug'] ] );
			}
		}
...
class zeroBS__Metabox_ContactAKA extends zeroBS__Metabox {
	...
	public function html( $contact, $metabox ) {
		...
		if ( gettype( $zbsCustomer ) != 'array' ) {
			...
			?>
			<div class="ui message"><?php esc_html_e( 'You will not be able to add an alias until you\'ve saved this contact', 'zero-bs-crm' ); ?></div>
			<?php
			...
			?>
			<div id="zbs-aka-alias-wrap">
			<?php

			// each alias: ID,aka_alias,aka_create,aka_lastupdated
			if ( is_array( $customerAliases ) && count( $customerAliases ) > 0 ) {
...
class zeroBS__Metabox_ContactCompany extends zeroBS__Metabox {
	...
	public function html( $contact, $metabox ) {
		...
		if ( 'contact' == $this->objType ) {
			$coID = -1;
			if ( is_array( $contact ) && isset( $contact['id'] ) ) {
				$coID = zeroBS_getCustomerCompanyID( $contact['id'] );
			}
			if ( ! empty( $coID ) ) {
				...
				// < 3.0
				if ( isset( $co ) && isset( $co['meta'] ) && isset( $co['meta']['coname'] ) ) {
					$coName = $co['meta']['coname'];
				}

				if ( empty( $coName ) && isset( $co['coname'] ) ) {
					$coName = $co['coname'];
				}
			}
		}
...
class zeroBS__Metabox_Contact_Activity extends zeroBS__Metabox {
	...
	public function html( $obj, $metabox ) {
		...
		if ( is_array( $obj ) && isset( $obj['id'] ) ) {
			...
			$pinned_logs = $zbs->DAL->logs->getLogsForObj(
				array(
