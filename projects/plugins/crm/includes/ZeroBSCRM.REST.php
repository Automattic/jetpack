...
function zeroBSCRM_rest_getCompanies( WP_REST_Request $request ) {
	...
	$searchQuery = '';
	if ( isset( $request['s'] ) ) {
		$searchQuery = sanitize_text_field( $request['s'] );
	}
	$potentialID = -1;
	if ( isset( $request['id'] ) ) {
	...
	if ( $potentialID > 0 ) {
		return zeroBS_getCompany( $potentialID );
	}
...
function zeroBSCRM_rest_getContacts( WP_REST_Request $request ) {
	...
	// if id, pass back obj singular
	if ( $potentialID > 0 ) {
		return zeroBS_getCustomer( $potentialID );
	}
	...
	if DAL3+ */
		...
		$retA = $zbs->DAL->contacts->getContacts(
			array(
				'simplified'   => true,
...
function zeroBSCRM_rest_getConCom( WP_REST_Request $request ) {
	...
	if ( $potentialID > 0 && $objectType == ZBS_TYPE_CONTACT ) {
		return zeroBS_getCustomer( $potentialID );
	}
	if ( $potentialID > 0 && $objectType == ZBS_TYPE_COMPANY ) {
		return zeroBS_getCompany( $potentialID );
	}
	...
	if ( is_array( $retA ) && count( $retA ) > 0 ) {
		...
		$retB = array();
		$retB = $zbs->DAL->companies->getCompanies(
			array(
				'simplified'   => true,
				'searchPhrase' => $searchQuery,
				'sortByField'  => 'ID',
