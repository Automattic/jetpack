...
/* ======================================================
   ...
   ====================================================== */
    ...
    function zeroBSCRM_pages_logout() {
      ...
      ?><script type="text/javascript">window.location='<?php echo esc_html( wp_logout_url() ); ?>';</script><h1 style="text-align:center">Logging you out!</h1><?php
...
function zeroBSCRM_pages_home() {
  ...
  if (!current_user_can('admin_zerobs_manage_options'))  { wp_die( esc_html__('You do not have sufficient permissions to access this page.',"zero-bs-crm") ); }
                        ...
                        if(isset($_GET['sendtest']) && !empty($_GET['sendtest'])){
                          ...
                          */
                          ...
                          if (in_array($emailtab, array(ZBSEMAIL_EVENTNOTIFICATION))){
                              ...
                              ?><div class="ui blue label right floated"><i class="circle info icon link"></i> <?php esc_html_e("Note: This email requires cron.","zero-bs-crm"); ?> <a href="<?php echo esc_url($zbs->urls['kbcronlimitations']); ?>"><?php esc_html_e('Read about WordPress cron','zero-bs-crm'); ?></a></div><?php
                          ...
                          echo '<div class="field">';
                            ...
                            ?><div class="ui teal label right floated"><i class="circle info icon link"></i> <?php esc_html_e("You can set up different delivery methods in your ","zero-bs-crm"); ?> <a href="<?php echo jpcrm_esc_link( $zbs->slugs['settings'] ) . '&tab=maildelivery'; ?>"><?php esc_html_e('Delivery Methods Settings','zero-bs-crm'); ?></a></div><?php
                            echo '</div></div>';
  ...
  <div id="zbs-welcome">
    <div class="container">
      ...
      <div id="action-buttons" class='block'>
        <h6><?php esc_html_e("Jetpack CRM makes it easy for you to manage your customers using WordPress. To get started, ","zero-bs-crm"); echo '<a href="https://jetpackcrm.com/learn/" target="_blank">'; esc_html_e("watch the video tutorial","zero-bs-crm"); echo '</a> '; esc_html_e("or read our guide on how create your first customer","zero-bs-crm"); ?>:</h6>
            ...
            <strong><?php esc_html_e("Data Tool Extensions Installed","zero-bs-crm");?>:</strong><br /><br />
              <?php 
                ...
                if (zeroBSCRM_isExtensionInstalled('csvpro') && isset($zeroBSCRM_CSVImporterslugs)){
                  ...
                  ?><button type="button" class="ui button primary" onclick="javascript:window.location='?page=<?php echo esc_attr( $zeroBSCRM_CSVImporterslugs['app'] ); ?>';" class="ui button primary" style="padding: 7px 16px;font-size: 16px;height: 46px;margin-bottom:8px;"><?php esc_html_e('CSV Importer',"zero-bs-crm"); ?></button><br /><?php
                  // tagger post v1.1 
                  if (isset($zeroBSCRM_CSVImporterslugs['tagger'])) { 
                    ?><button type="button" class="ui button primary" onclick="javascript:window.location='?page=<?php echo esc_attr( $zeroBSCRM_CSVImporterslugs['tagger'] ); ?>';" class="ui button primary" style="padding: 7px 16px;font-size: 16px;height: 46px;margin-bottom:8px;"><?php esc_html_e('CSV Tagger',"zero-bs-crm"); ?></button><br /><?php
                  }
                ...
                if ($zbsDataToolsInstalled == 0){
                  ##WLREMOVE
                  ?><?php esc_html_e("You do not have any Pro Data Tools installed as of yet","zero-bs-crm");?>! <a href="<?php echo esc_url( $zbs->urls['productsdatatools'] ); ?>" target="_blank"><?php esc_html_e("Get some now","zero-bs-crm");?></a><?php
                  ##/WLREMOVE
                }
                      ...
                      if($showLinkButton){
                        ...
                        if ($extkey == 'apiconnector'){
                            ...
                            if (!$installed){
                              ...
                              if (isset($extension->docs) && !empty($extension->docs)) echo "<a href='". esc_url($extension->docs) ."' target='_blank'><span class='ui button blue mini'>" . esc_html__('Docs', 'zero-bs-crm') . "</span></a>";
                          }
                        echo '</div>';
                      }
