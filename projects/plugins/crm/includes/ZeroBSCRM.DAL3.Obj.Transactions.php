...
class zbsDAL_transactions extends zbsDAL_ObjectLayer {
    ...
    protected $objectModel           = array(
        ...
        'created'        => array(
			'fieldname' => 'zbst_created',
			'format'    => 'uts',
		),
        'lastupdated'    => array(
			'fieldname' => 'zbst_lastupdated',
			'format'    => 'uts',
		),
				...
				if ( isset( $potentialRes ) && isset( $potentialRes->ID ) ) {
					...
					if ( $withLineItems ) {
						...
						$res['lineitems'] = $this->DAL()->lineitems->getLineitems(
                            array(
								'associatedObjType' => ZBS_TYPE_TRANSACTION,
					...
					if ( $withAssigned ) {
						...
						.. but we use 1:1, at least now: */
							...
							$res['invoice_id'] = $this->DAL()->getFirstIDLinkedToObj(
                                array(
		...
		public function getTransactions( $args = array() ) {
            ...
            // This catches listview and other exception sort cases
					...
					if ( $external_source_uid && $sortByField === 'external_source' ) {
						$sortByField = array( 'external_source_uids' => $sortOrder );
					}
					...
					if ( ! empty( $ownQ )) $additionalWhere = $this->spaceAnd( $additionalWhere ) . $ownQ; // adds str to query
					#} / Ownership
		...
		public function addUpdateTransaction( $args = array() ) {
			...
			if ( is_array( $limitedFields ) ) {
				...
				// add update time
				if ( ! isset( $dataArr['zbst_lastupdated'] ) ) {
					$dataArr['zbst_lastupdated'] = time();
					$typeArr[]                   = '%d'; }
			} else {
                ...
                if ( ! empty( $id ) && $id > 0 ) {
                    ...
                    if ( isset( $data['created'] ) && ! empty( $data['created'] ) && $data['created'] !== -1 ) {
                        $dataArr['zbst_created'] = $data['created'];
						$typeArr[]               = '%d';
                    } else {
                        $dataArr['zbst_created'] = time();
						$typeArr[]               = '%d';
                    }
						...
						if ( isset( $data['lineitems'] ) && is_array( $data['lineitems'] ) ) {
							...
							if ( count( $data['lineitems'] ) > 0 ) {
								...
								foreach ( $data['lineitems'] as $lineitem ) {
									...
									$lineItemID                               = false;
									if (isset( $lineitem['ID'] )) $lineItemID = $lineitem['ID'];
		...
		public function setTransactionStatus( $id = -1, $status = -1 ) {
			...
			if ( $id > 0 && ! empty( $status ) && $status !== -1 ) {
				...
				return $this->addUpdateTransaction(
                    array(
						'id'            => $id,
						'limitedFields' => array(
							array(
		...
		private function tidy_transaction( $obj = false, $withCustomFields = false ) {
            ...
            if ( isset( $obj->ID ) ) {
				...
				*/
				...
				$res['title']  = $obj->zbst_title === null ? '' : $this->stripSlashes( $obj->zbst_title );
				...
				$res['date_paid']           = $obj->zbst_date_paid === null ? null : (int) $obj->zbst_date_paid;
				$res['date_paid_date']      = ( isset( $obj->zbst_date_paid ) && $obj->zbst_date_paid > 0 ) ? zeroBSCRM_locale_utsToDatetime( $obj->zbst_date_paid ) : false;
				$res['date_completed']      = $obj->zbst_date_completed === null ? null : (int) $obj->zbst_date_completed;
		...
		public function get_transaction_contacts( $transaction_id ) {
			...
			return $this->DAL()->contacts->getContacts(
                array(
					'hasObjTypeLinkedTo' => ZBS_TYPE_TRANSACTION,
		...
		public function get_transaction_invoice_id( $transaction_id ) {
			...
			return $this->DAL()->getFirstIDLinkedToObj(
                array(
