...
class zbsDAL_contacts extends zbsDAL_ObjectLayer {
	...
	*/
		...
		if ( ( ! empty( $id ) && $id > 0 )
			...
			if ( isset( $potentialRes ) && isset( $potentialRes->ID ) ) {
				...
				if ( $withLastLog ) {
					...
					$res['lastlog'] = $this->DAL()->logs->getLogsForObj(
						array(
				...
				if ( $withInvoices ) {
					...
					$res['invoices'] = $zbs->DAL->invoices->getInvoices(
						array(
				...
				if ( $withQuotes ) {
					...
					// DAL3 ver, more perf, gets all
					$res['quotes'] = $zbs->DAL->quotes->getQuotes(
						array(
				...
				if ( $withTransactions ) {
					...
					// DAL3 ver, more perf, gets all
					$res['transactions'] = $zbs->DAL->transactions->getTransactions(
						array(
				...
				if ( $withCompanies ) {
					...
					$res['companies'] = $this->DAL()->getObjsLinkedToObj(
						array(
				...
				if ( $withTasks ) {
					...
					// DAL3 ver, more perf, gets all
					$res['tasks'] = $zbs->DAL->events->getEvents(
						array(
				...
				if ( $withExternalSources ) {
					...
					$res['external_sources'] = $zbs->DAL->contacts->getExternalSourcesForContact(
						array(
				...
				if ( $withExternalSourcesGrouped ) {
					...
					$res['external_sources'] = $zbs->DAL->getExternalSources(
						-1,
						array(
	...
	public function getContacts( $args = array() ) {
		...
		if ( ! $simplified && ! $count && ! $onlyColumns && ! $onlyObjTotals ) {
			...
			if ( is_array( $addrCustomFields ) && count( $addrCustomFields ) > 0 ) {
				...
				foreach ( $addrCustomFields as $cK => $cF ) {
					...
					if ( 'zbsc_' . $cfKey == $sortByField ) {
						$sortByField = $cKey;
					}
					if ( 'zbsc_' . $cfKey2 == $sortByField ) {
						$sortByField = $cKey2;
					}
		...
		#} Count override
		if ( $count ) {
			$query = 'SELECT COUNT(contact.ID) FROM ' . $ZBSCRM_t['contacts'] . ' as contact' . $joinQ;
		}

		#} simplified override
		if ( $simplified ) {
			$query = 'SELECT contact.ID as id,CONCAT(contact.zbsc_fname," ",contact.zbsc_lname) as name,contact.zbsc_created as created, contact.zbsc_email as email FROM ' . $ZBSCRM_t['contacts'] . ' as contact' . $joinQ;
		}
		...
		if ( ! empty( $searchPhrase ) ) {
			...
			if ( $b2bMode == 1 ) {
				$searchWheres['incompanywithname'] = array( 'ID', 'IN', '(SELECT DISTINCT zbsol_objid_from FROM ' . $ZBSCRM_t['objlinks'] . ' WHERE zbsol_objtype_from = ' . ZBS_TYPE_CONTACT . ' AND zbsol_objtype_to = ' . ZBS_TYPE_COMPANY . ' AND zbsol_objid_to IN (SELECT ID FROM ' . $ZBSCRM_t['companies'] . ' WHERE zbsco_name LIKE %s))', '%' . $searchPhrase . '%' );
			}
		...
		if ( ! empty( $olderThan ) && $olderThan > 0 ) {
			$wheres['olderThan'] = array( 'zbsc_created', '<=', '%d', $olderThan );
		}
		#} newerThan
		if ( ! empty( $newerThan ) && $newerThan > 0 ) {
			$wheres['newerThan'] = array( 'zbsc_created', '>=', '%d', $newerThan );
		}

		// status
		if ( ! empty( $hasStatus ) ) {
			$wheres['hasStatus'] = array( 'zbsc_status', '=', '%s', $hasStatus );
		}
		if ( ! empty( $otherStatus ) ) {
			$wheres['otherStatus'] = array( 'zbsc_status', '<>', '%s', $otherStatus );
		}

		#} contactedBefore
		if ( ! empty( $contactedBefore ) && $contactedBefore > 0 ) {
			$wheres['contactedBefore'] = array( 'zbsc_lastcontacted', '<=', '%d', $contactedBefore );
		}
		#} contactedAfter
		if ( ! empty( $contactedAfter ) && $contactedAfter > 0 ) {
			$wheres['contactedAfter'] = array( 'zbsc_lastcontacted', '>=', '%d', $contactedAfter );
		}
		...
		if ( array_key_exists( $sortByField, $sort_map ) ) {
			...
			$sortByField = array(
				'zbsc_lname' => $sortOrder,
				'zbsc_fname' => $sortOrder,
			);
		...
		if ( isset( $potentialRes ) && is_array( $potentialRes ) && count( $potentialRes ) > 0 ) {
			...
			foreach ( $potentialRes as $resDataLine ) {
				...
				} elseif ( $onlyColumns && is_array( $onlyColumnsFieldArr ) && count( $onlyColumnsFieldArr ) > 0 ) {
				...
				if ( $withDND ) {
					...
					$dnd          = -1;
					$potentialDND = $this->stripSlashes( $this->decodeIfJSON( $resDataLine->dnd ) );
					if ( $potentialDND == '1' ) {
						$dnd = 1;
					}
				...
				if ( $withLastLog ) {
					...
					$potentialLogs = $this->DAL()->logs->getLogsForObj(
						array(
					...
					if ( is_array( $potentialLogs ) && count( $potentialLogs ) > 0 ) {
						$resArr['lastlog'] = $potentialLogs[0];
					}
					...
					if ( is_array( $potentialLogs ) && count( $potentialLogs ) > 0 ) {
						$resArr['lastcontactlog'] = $potentialLogs[0];
					}
				}
				...
				if ( $withAssigned ) {
					...
					// add all assigned companies
					$resArr['company'] = $zbs->DAL->companies->getCompanies(
						array(
				...
				if ( $withInvoices ) {
					...
					#} only gets first 100?
					// DAL3 ver, more perf, gets all
					$resArr['invoices'] = $zbs->DAL->invoices->getInvoices(
						array(
				...
				if ( $withQuotes ) {
					...
					// DAL3 ver, more perf, gets all
					$resArr['quotes'] = $zbs->DAL->quotes->getQuotes(
						array(
				...
				if ( $withTransactions ) {
					...
					// DAL3 ver, more perf, gets all
					$resArr['transactions'] = $zbs->DAL->transactions->getTransactions(
						array(
				...
				if ( $withTasks ) {
					...
					// DAL3 ver, more perf, gets all
					$resArr['tasks'] = $zbs->DAL->events->getEvents(
						array(
				...
				if ( $withExternalSources ) {
					...
					$resArr['external_sources'] = $zbs->DAL->contacts->getExternalSourcesForContact(
						array(
				...
				if ( $withExternalSourcesGrouped ) {
					...
					$resArr['external_sources'] = $zbs->DAL->getExternalSources(
						-1,
						array(
	...
	public function addUpdateContact( $args = array() ) {
		...
		if ( is_array( $limitedFields ) ) {
			...
			if ( ! isset( $data['email'] ) || empty( $data['email'] ) ) {
				...
				if ( ! empty( $id ) && $id > 0 ) {
					...
					$potentialUSERID = (int) $this->getContact(
						-1,
						array(
							'email'       => $data['email'],
							'ignoreOwner' => 1,
					...
					if ( isset( $potentialUSERID ) && ! empty( $potentialUSERID ) && $potentialUSERID > 0 ) {
						$id = $potentialUSERID;
					}
				}
			}
		...
		if ( empty( $data['status'] ) ) {
			...
			if ( empty( $data['status'] ) ) {
				$data['status'] = zeroBSCRM_getSetting( 'defaultstatus' );
			}
		}
		...
		if ( $update ) {
			...
			if ( $wpdb->update(
					...
					if ( isset( $data['aliases'] ) && is_array( $data['aliases'] ) ) {
						...
						foreach ( $data['aliases'] as $alias ) {
							...
							if ( zeroBS_canUseCustomerAlias( $alias ) ) {
								...
								if ( in_array( $alias, $existingAliasesSimple ) ) {
									continue;
								}
					...
					foreach ( $customFields as $cK => $cF ) {
						...
						if ( isset( $data[ $cK ] ) ) {
							...
							if ( $do_not_update_blanks && empty( $data[ $cK ] ) ) {
								...
								// add update
								$cfID = $this->addUpdateCustomField(
									array(
										'data' => array(
											'objtype' => ZBS_TYPE_CONTACT,
											'objid'   => $id,
				...
				if ( ! $silentInsert ) {
					...
					if ( is_array( $fallBackLog ) && ! empty( $fallBackLog['type'] ) && ! empty( $fallBackLog['shortdesc'] ) // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UndefinedVariable,WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
					) {
						...
						$zbsNoteLongDesc = '';
						if ( isset( $fallBackLog['longdesc'] ) && ! empty( $fallBackLog['longdesc'] ) ) {
							$zbsNoteLongDesc = $fallBackLog['longdesc'];
						}

						#} Only raw checked... but proceed.
						$newOrUpdatedLogID = zeroBS_addUpdateContactLog(
							$id,
							-1,
							-1,
							array(
			...
			if ( $wpdb->insert(
				...
				*/
				...
				if ( ! $silentInsert ) {
					...
					$dataArr['ID'] = $newID;
					$this->events_manager->contact()->created( $dataArr );
	...
	public function addUpdateContactCompanies( $args = array() ) {
		...
		return $this->DAL()->addUpdateObjLinks(
			array(
				'objtypefrom' => ZBS_TYPE_CONTACT,
				'objtypeto'   => ZBS_TYPE_COMPANY,
				'objfromid'   => $id,
	...
	public function tidy_contact( $obj = false, $withCustomFields = false ) { // phpcs:ignore
		...
		if ( isset( $obj->ID ) ) {
			...
			*/
			...
			if ( $zbs->db1CompatabilitySupport ) {
				$res['meta'] = $res;
			}
			...
			// latest logs
			if ( isset( $obj->lastlog ) ) {
				$res['lastlog'] = $obj->lastlog;
			}
			if ( isset( $obj->lastcontactlog ) ) {
			...
			if ( isset( $obj->quotes_total ) ) {
				$res['quotes_total'] = $obj->quotes_total;
			}
			...
			if ( isset( $obj->transactions_paid_total ) ) {
				$res['transactions_paid_total'] = $obj->transactions_paid_total;
			}
			...
			if ( $withCustomFields ) {
				$res = $this->tidyAddCustomFields( ZBS_TYPE_CONTACT, $obj, $res, true );
			}
	...
	public function getContactEmails( $id = -1 ) {
		...
		if ( $id > 0 ) {
			...
			// main record
			$mainEmail = $this->DAL()->getFieldByID(
				array(
					'id'          => $id,
					'objtype'     => ZBS_TYPE_CONTACT,
					'colname'     => 'zbsc_email',
			...
			if ( zeroBSCRM_validateEmail( $mainEmail ) ) {
				$emails[] = $mainEmail;
			}
			...
			if ( is_array( $aliases ) ) {
				foreach ( $aliases as $alias ) {
					if ( ! in_array( $alias['aka_alias'], $emails ) ) {
						$emails[] = $alias['aka_alias'];
					}
				}
			}
		}
	...
	public function getContactFullName( $id = -1, $contactArr = false ) {
		...
		if ( $id > 0 ) {
			...
			$contact = $this->getContact(
				...
				array(
					'withCustomFields' => false,
					'fields'           => array( 'zbsc_prefix', 'zbsc_fname', 'zbsc_lname' ),
					'ignoreowner'      => true,
				)
			);
	...
	public function getContactTags( $id = -1 ) {
		...
		if ( $id > 0 ) {
			...
			return $this->DAL()->getTagsForObjID(
				array(
					'objtypeid' => ZBS_TYPE_CONTACT,
					'objid'     => $id,
				)
		...
		if ( $id > 0 ) {
			...
			return $this->addUpdateContact(
				array(
					...
					'limitedFields' => array(
						array(
							'key'  => 'zbsc_lastcontacted',
							'val'  => $lastContactedUTS,
							'type' => '%d',
						),
					),
				)
			...
			switch ( $avatarMode ) {
				...
				case 1: // gravitar
					...
					if ( ! empty( $potentialEmail ) ) {
						return zeroBSCRM_getGravatarURLfromEmail( $potentialEmail );
					}
	...
	public function getContactCompanies( $id = -1 ) {
		...
		if ( ! empty( $id ) ) {
			...
			if ( is_array( $contact ) && isset( $contact['companies'] ) ) {
				return $contact['companies'];
			}
			*/

			// cleaner:
			return $this->DAL()->getObjsLinkedToObj(
				array(
					'objtypefrom' => ZBS_TYPE_CONTACT, // contact
					'objtypeto'   => ZBS_TYPE_COMPANY, // company
					'objfromid'   => $id,
	...
	public function listViewObj( $contact = false, $columnsRequired = array() ) {
		...
		if ( is_array( $contact ) && isset( $contact['id'] ) ) {
			...
			if ( in_array( 'totalvalue', $columnsRequired ) ) {
				...
				$resArr['totalvalue'] = zeroBSCRM_formatCurrency( 0 );
				if ( isset( $contact['total_value'] ) ) {
					$resArr['totalvalue'] = zeroBSCRM_formatCurrency( $contact['total_value'] );
				}
			}
			...
			if ( in_array( 'transactiontotal', $columnsRequired ) ) {
				...
				if ( isset( $contact['transactions_value'] ) ) {
					$resArr['transactionstotal'] = zeroBSCRM_formatCurrency( $contact['transactions_value'] );
				}
			}
			// v3.0
			...
			if ( in_array( 'company', $columnsRequired ) ) {
				...
				if ( ! empty( $coID ) ) {
					...
					if ( ! isset( $companyNameCache[ $coID ] ) ) {
						...
						if ( isset( $co ) && isset( $co['name'] ) ) {
							$coName = $co['name'];
						}
						if ( empty( $coName ) ) {
							$coName = jpcrm_label_company() . ' #' . $co['id'];
						}
	...
	public function format_fullname( $contactArr = array() ) {
		...
		$str = '';
		if ( $usePrefix ) {
			if ( isset( $contactArr['prefix'] ) ) {
				$str .= $contactArr['prefix'];
			}
		}
	...
	public function format_name_etc( $contactArr = array(), $args = array() ) {
		...
		// First line of addr?
		if ( $incFirstLineAddr ) {
			if ( isset( $contactArr['addr1'] ) && ! empty( $contactArr['addr1'] ) ) {
				$str .= ' (' . $contactArr['addr1'] . ')';
			}
		}
