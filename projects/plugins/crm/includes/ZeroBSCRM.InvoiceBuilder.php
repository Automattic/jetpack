...
function zeroBSCRM_invoicing_generateInvoiceHTML( $invoice_id = -1, $template = 'pdf', $html = '' ) {
	...
	if ( $invsettings['invtax'] != 0 || $invsettings['invpandp'] != 0 || $invsettings['invdis'] != 0 ) {
		...
		$totals_table .= isset( $invoice['net'] ) && ! empty( $invoice['net'] ) ? esc_html( zeroBSCRM_formatCurrency( $invoice['net'] ) ) : esc_html( zeroBSCRM_formatCurrency( 0 ) );
		$totals_table .= '</span></td></tr>';
	}

	if ( isset( $invoice['discount'] ) && ! empty( $invoice['discount'] ) && $invsettings['invdis'] == 1 && isset( $invoice['totals'] ) && is_array( $invoice['totals'] ) && isset( $invoice['totals']['discount'] ) ) {
		$invoice_percent = $invoice['discount_type'] == '%' && $invoice['discount'] != 0 ? (float) $invoice['discount'] . '% ' : '';
		$totals_table   .= '<tr class="discount">';
		$totals_table   .= '<td class="bord bord-l" style="text-align:right; text-transform: uppercase;">' . esc_html( $invoice_percent . __( 'Discount', 'zero-bs-crm' ) ) . '</td>';
		$totals_table   .= '<td class="bord row-amount" id="zbs_discount_combi" style="text-align:right"><span class="zbs-totals">-' . esc_html( zeroBSCRM_formatCurrency( $invoice['totals']['discount'] ) ) . '</span></td></tr>';
	}

	if ( $invsettings['invpandp'] == 1 ) {
		$totals_table .= '<tr class="postage_and_pack">';
		$totals_table .= '<td class="bord bord-l" style="text-align:right; text-transform: uppercase;">' . esc_html__( 'Postage and packaging', 'zero-bs-crm' ) . '</td>';
		$totals_table .= '<td class="bord row-amount" id="pandptotal" style="text-align:right;"><span class="zbs-totals">';
		$totals_table .= isset( $invoice['shipping'] ) && ! empty( $invoice['shipping'] ) ? esc_html( zeroBSCRM_formatCurrency( $invoice['shipping'] ) ) : esc_html( zeroBSCRM_formatCurrency( 0 ) );
		$totals_table .= '</span></td></tr>';
	}

	if ( $invsettings['invtax'] == 1 ) {
		$tax_lines = isset( $invoice['totals'] ) && is_array( $invoice['totals'] ) && isset( $invoice['totals']['taxes'] ) ? $invoice['totals']['taxes'] : false;

		if ( is_array( $tax_lines ) && ! empty( $tax_lines ) ) {
			foreach ( $tax_lines as $tax ) {
				$tax_name      = isset( $tax['name'] ) ? $tax['name'] : __( 'Tax', 'zero-bs-crm' );
				$totals_table .= '<tr class="ttclass">';
				$totals_table .= '<td class="bord bord-l" style="text-align:right">' . esc_html( $tax_name ) . '</td>';
				$totals_table .= '<td class="bord bord-l row-amount zbs-tax-total-span" style="text-align:right"><span class="zbs-totals">';
				$totals_table .= isset( $tax['value'] ) && ! empty( $tax['value'] ) ? esc_html( zeroBSCRM_formatCurrency( $tax['value'] ) ) : esc_html( zeroBSCRM_formatCurrency( 0 ) );
				$totals_table .= '</span></td></tr>';
			}
		} else {
			$totals_table .= '<tr class="ttclass">';
			$totals_table .= '<td class="bord bord-l" style="text-align:right">' . esc_html__( 'Tax', 'zero-bs-crm' ) . '</td>';
			$totals_table .= '<td class="bord bord-l row-amount zbs-tax-total-span" style="text-align:right"><span class="zbs-totals">';
			$totals_table .= isset( $invoice['tax'] ) && ! empty( $invoice['tax'] ) ? esc_html( zeroBSCRM_formatCurrency( $invoice['tax'] ) ) : esc_html( zeroBSCRM_formatCurrency( 0 ) );
			$totals_table .= '</span></td></tr>';
		}
	}
	...
	if ( $invoice['total'] == 0 ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
		...
		$partials_table .= '<br/><table id="partials" class="table table-totals striped">';
	}
	...
	if ( is_array( $partials ) && count( $partials ) > 0 ) {
		...
		foreach ( $partials as $partial ) {
			...
			if ( isset( $partial ) && $partial['status_bool'] == 1 ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
				...
				$partials_table .= '<td class="bord row-amount" style="text-align:right;"><span class="zbs-partial-value">';
...
function zeroBSCRM_invoicing_generateInvPart_tableHeaders( $zbs_invoice_hours_or_quantity = 1 ) {
	...
	if ( $zbs_invoice_hours_or_quantity == 1 ) {
		$table_headers .= '<th class="jpcrm-invoice-lineitem-quantity" style="text-align:right;">' . esc_html__( 'Quantity', 'zero-bs-crm' ) . '</th>';
		$table_headers .= '<th class="jpcrm-invoice-lineitem-price" style="text-align:right;">' . esc_html__( 'Price', 'zero-bs-crm' ) . '</th>';
	} else {
		$table_headers .= '<th class="jpcrm-invoice-lineitem-quantity" style="text-align:right;">' . esc_html__( 'Hours', 'zero-bs-crm' ) . '</th>';
		$table_headers .= '<th class="jpcrm-invoice-lineitem-price" style="text-align:right;">' . esc_html__( 'Rate', 'zero-bs-crm' ) . '</th>';
	}
	$table_headers .= '<th class="jpcrm-invoice-lineitem-amount" style="text-align:right;">' . esc_html__( 'Amount', 'zero-bs-crm' ) . '</th>';
