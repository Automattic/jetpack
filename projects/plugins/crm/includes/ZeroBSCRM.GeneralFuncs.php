 ...
 */
function jpcrm_settings_get_second_address_visibility( $settings ) {
	if ( ! isset( $settings['secondaddress'] ) || $settings['secondaddress'] !== 1 ) {
		return SETTING_SECOND_ADDRESS_ALWAYS_HIDE;
	}

	if ( isset( $settings['secondaddresstoggle'] ) && $settings['secondaddresstoggle'] === 1 ) {
		return SETTING_SECOND_ADDRESS_ALLOW_CHANGE_VISIBILITY;
	}

	return SETTING_SECOND_ADDRESS_ALWAYS_SHOW;
}
