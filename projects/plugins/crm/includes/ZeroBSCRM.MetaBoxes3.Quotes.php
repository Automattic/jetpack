...
class zeroBS__Metabox_Quote extends zeroBS__Metabox {
	...
	public function html( $quote, $metabox ) {
		...
		?>                
				...
				$quoteContactID = -1;
				if ( is_array( $quote ) && isset( $quote['contact'] ) && is_array( $quote['contact'] ) && count( $quote['contact'] ) > 0 ) {
					$quoteContactID = $quote['contact'][0]['id']; // get_post_meta($post->ID, 'zbs_customer_quote_customer', true);
				}
				$templateUsed = -1;
				if ( is_array( $quote ) && isset( $quote['template'] ) ) {
					$templateUsed = $quote['template']; // get_post_meta($post->ID, 'zbs_quote_template_id', true);
				}
				...
				$quote['templated'] = false;
				if ( $templateUsed !== -1 && ! empty( $templateUsed ) ) {
					$quote['templated'] = true;
				}

				#} if customer id is empty, but prefill isn't, use prefill
				if ( $quoteContactID == -1 && isset( $_GET['zbsprefillcust'] ) ) {
					$quoteContactID = (int) $_GET['zbsprefillcust'];
				}

				#} pass to other metaboxes (cache?)
				global $zbsCurrentEditQuote;
				$zbsCurrentEditQuote = $quote;
				...
				#} New quote?
				if ( ! isset( $quote['id'] ) ) {
					echo '<input type="hidden" name="zbscrm_newquote" value="1" />';
				}

				#} pass this if already templated:
				if ( $useQuoteBuilder == 1 && isset( $quote['template'] ) ) {
					echo '<input type="hidden" name="zbscrm_templated" id="zbscrm_templated" value="1" />';
				}
				...
				?>
					...
					#} 27/09/16 - switched select for typeahead
						...
						?>
							<input type="hidden" name="zbscq_customer" id="zbscq_customer" value="<?php echo esc_attr( $quoteContactID ); ?>" />
							<?php
	...
	public function save_data( $quoteID, $quote ) {
		...
		if ( ! defined( 'ZBS_OBJ_SAVED' ) ) {
			...
			if ( $quoteID > 0 ) {
				$quote['template'] = (int) $zbs->DAL->quotes->getQuoteTemplateID( $quoteID );
			}
			...
			if ( isset( $_POST['zbs_quote_content'] ) ) {
				...
				if ( isset( $_POST['zbs_quote_template_id'] ) ) {
					$quote['template'] = (int) sanitize_text_field( $_POST['zbs_quote_template_id'] );
				}
			}
				...
				$quoteZBSID  = (int) $quoteID + $quoteOffset;
			if ( isset( $_POST['zbsquoteid'] ) && ! empty( $_POST['zbsquoteid'] ) ) {
				$quoteZBSID = (int) $_POST['zbsquoteid'];
			}
				// update_post_meta($post_id,"zbsid",$quoteID);
				$extraMeta['zbsid'] = $quoteZBSID;
				#} and increment this
			if ( ! empty( $quoteZBSID ) ) {
				zeroBSCRM_setMaxQuoteID( $quoteZBSID );
			}

				// assignments
				$zbsQuoteContact = -1;
			if ( isset( $_POST['zbscq_customer'] ) ) {
				$zbsQuoteContact = (int) sanitize_text_field( $_POST['zbscq_customer'] );
			}
				$quote['contacts'] = ( $zbsQuoteContact > 0 ) ? array( $zbsQuoteContact ) : array();
				$zbsQuoteCompany   = -1;
			if ( isset( $_POST['zbscq_company'] ) ) {
				$zbsQuoteCompany = (int) sanitize_text_field( $_POST['zbscq_company'] );
			}
			...
			if ( $quoteID > 0 && isset( $_POST['quote_status'] ) ) {
				...
				// add/update
				$addUpdateReturn = $zbs->DAL->quotes->addUpdateQuote(
					array(
			...
			if ( $quoteID == -1 && ! empty( $addUpdateReturn ) && $addUpdateReturn != -1 ) {
				...
				$zbsJustInsertedMetaboxID = $quoteID;
		...
		if ( $useQuoteBuilder == '1' ) {
			...
			if ( gettype( $quote ) != 'array' || ! isset( $quote['templated'] ) ) {
				...
				if ( ! zeroBSCRM_isExtensionInstalled( 'portal' ) ) {
					...
					} elseif ( isset( $quoteContactID ) && $quoteContactID > 0 ) {
		...
		if ( $useQuoteBuilder == '1' && ( is_array( $quote ) && isset( $quote['templated'] ) && $quote['templated'] ) ) {
			...
			if ( isset( $quote ) && is_array( $quote ) && isset( $quote['accepted'] ) && $quote['accepted'] > 0 ) {
				...
				?>
					...
					if ( ! empty( $acceptedBy ) ) {
						?>
						<tr><td style="text-align:center"><?php esc_html_e( 'By: ', 'zero-bs-crm' ); ?> <a href="mailto:<?php echo esc_attr( $acceptedBy ); ?>" target="_blank"
						<?php
						if ( ! empty( $acceptedIP ) ) {
												echo ' title="IP address:' . esc_attr( $acceptedIP ) . '"'; }
						?>
><?php echo esc_html( $acceptedBy ); ?></a></td></tr><?php } ?>                            
						</table>   
			...
			#} Gross hide :/
			?>
				<style type="text/css">#wpzbscquote_status {display:none;}</style>
				<?php
