...
class zeroBSCRM_Plugin_Updater {
	...
	public function check_update( $transient ) {
		...
		if ( $installedExts > 0 || $this->get_license_key() !== false ) {
			...
			if ( ! is_wp_error( $response ) ) {
				...
				} elseif ( $response['license_key_valid'] == 'brand.template' ) {
						...
						foreach ( $official_ver as $extUpdateInfo ) {
							...
							if ( isset( $extUpdateInfo['zbskey'] ) && $extUpdateInfo['zbskey'] == $pluginDetails['key'] ) {
								...
								if ( version_compare( $pluginDetails['ver'], $extUpdateInfo['version'], '<' ) ) {
									...
									$parameters = sprintf( __( 'Please update %1$s from version %2$s to version %3$s.', 'zero-bs-crm' ), $pluginDetails['name'], $pluginDetails['ver'], $extUpdateInfo['version'] );
				...
				if ( zeroBSCRM_isWL() ) {
					...
					if ( isset( $core ) && is_array( $core ) && isset( $core['ver'] ) ) {
						...
						if ( $coreUpdateInfo && version_compare( $core['ver'], $coreUpdateInfo['version'], '<' ) ) {
							...
							// generate nag
							// then we are adding the notification to "please update Jetpack CRM extension "X")
							$parameters = sprintf( __( 'Please update %1$s from version %2$s to version %3$s.', 'zero-bs-crm' ), $core['name'], $core['ver'], $coreUpdateInfo['version'] );
		...
		if ( $action === 'plugin_information' && isset( $arg->slug ) ) {
			...
			if ( is_array( $possibleExtension ) ) {
				...
				if ( ! is_wp_error( $res ) ) {
					...
					} else {                          // default to latest in core
					...
					if ( isset( $res['banners'] ) ) {
						$obj->banners = $res['banners'];
					}
					if ( isset( $res['downloaded'] ) ) {
						$obj->downloaded = $res['downloaded'];
					}
					if ( isset( $res['last_updated'] ) ) {
						$obj->last_updated = $res['last_updated'];
					}
				} // / end if ! wp error
	...
	public function isLicenseValid() {
		...
		if ( isset( $licenseKeyArr['validity'] ) ) {
			$licenseValid = ( $licenseKeyArr['validity'] === 'true' );
		}
	...
	public function isCRMExtension( $slug = '' ) {
		...
		if ( ! empty( $slug ) ) {
			...
			foreach ( $this->installedExts as $extName => $extDeets ) {
				...
				// is slug in this arr?
				if ( is_array( $extDeets ) && isset( $extDeets['slug'] ) && $extDeets['slug'] == $slug ) {
					return true;
				}
			}
		}
	...
	public function api_request( $action, $data ) {
		...
		if ( $action !== 'ext_info' && isset( $zbsExtUpdateCache ) && is_array( $zbsExtUpdateCache ) ) {
			...
			$sslIgnore = false;
			if ( $licensingAttempts > 0 && ! $this->isLicenseValid() && $hasHitError ) {
				$sslIgnore = true;
			}
			if ( $sslIgnore ) {
			...
			if ( ! is_wp_error( $responseFull ) && ! in_array( $httpCode, $unacceptableHTTPCodes ) ) {
				...
				if ( is_wp_error( $responseFull ) ) {
					...
					// log the err
					$zbs->DAL->updateSetting(
						'licensingerror',
						array(
							'time' => time(),
							'err'  => $responseFull->get_error_message(),
						)
					);
				...
				if ( in_array( $httpCode, $unacceptableHTTPCodes ) ) {
					...
					// log the err
					$msg = '';
					if ( is_array( $responseFull ) && isset( $responseFull['message'] ) ) {
						$msg = $responseFull['message'];
					}
					$zbs->DAL->updateSetting(
						'licensingerror',
						array(
							'time' => time(),
							'err'  => 'Error:' . $httpCode . ' ' . $msg,
						)
					);
	...
	public function maybe_adjust_source_dir( $source, $remote_source, $upgrader, $extraArgs ) {
			...
			$ours   = false;
		$pluginPath = false;
		if ( isset( $extraArgs ) && isset( $extraArgs['plugin'] ) && ! empty( $extraArgs['plugin'] ) ) {
			$pluginPath = $extraArgs['plugin'];
		}
		...
		if ( $pluginPath !== false ) {
			...
			if ( is_array( $ourExts ) ) {
				foreach ( $ourExts as $x => $ext ) {
					if ( $ext['path'] == $pluginPath ) {
										$ours = true;
					}
				}
			}
...
function zeroBSCRM_updates_pluginHasUpdate( $name = '', $textDomain = '' ) {
	...
	foreach ( $list as $i => $item ) {
		...
		if ( ( ! empty( $name ) && strtolower( $name ) == strtolower( $item->Name ) )
			||
