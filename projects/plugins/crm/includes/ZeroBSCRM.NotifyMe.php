...
function zeroBSCRM_notifyme_echo_type( $type = '', $title = '', $sender = -999, $content = '' ) {
	...
	switch ( $type ) {
		...
		case 'extension.update':
			##WLREMOVE
			echo wp_kses( sprintf( __( '🔔 [URGENT] Your extension(s) need updating <a href="%1$s" target="_blank">Click here to retrieve the new versions</a>. (If you don\'t know your login, please <a href="%2$s" target="_blank">Email Support</a>.)', 'zero-bs-crm' ), esc_url( $zbs->urls['account'] ), esc_url( $zbs->urls['support'] ) ), $zbs->acceptable_restricted_html );
			##/WLREMOVE
			break;
		...
		case 'db2.update.253.errors':
				...
				$errStr             = '';
			if ( isset( $zbsMigrationErrors ) && is_array( $zbsMigrationErrors ) ) {
				foreach ( $zbsMigrationErrors as $zme ) {
					$errStr .= '<br />' . $zme;
				}
			}

			printf( __( 'Jetpack CRM has tried to update your core CRM to 2.70 but hit errors. Please send the following error report to Support:<hr /><strong>Migration Error Report:</strong>%1$s</hr> <a href="%2$s" target="_blank">Contact Support</a>', 'zero-bs-crm' ), $errStr, esc_url( $zbs->urls['support'] ) );
			break;
		...
		case 'smtp.3017.needtocheck':
			printf( __( 'Important: 🔔 Jetpack CRM has improved the encryption of SMTP passwords.<br />Please check each of your Delivery Methods still works by loading <a href="%s">Mail Delivery Methods</a> and clicking \'send test\', validating that it still sends', 'zero-bs-crm' ), jpcrm_esc_link( $zbs->slugs['settings'] . '&tab=maildelivery' ) );
			break;
		...
		case 'curl.timeout.error':
			printf( __( 'Failed to retrieve key file, your server may not be able to connect to the CRM CDN: %s. If this message persists, please contact support', 'zero-bs-crm' ), $content );
...
function zeroBSCRM_notifyme_activity() {
	...
	if ( count( $notifes ) == 0 ) {
		...
		foreach ( $notifes as $n ) {
			...
			$sender_url = '';
			if ( isset( $n ) && isset( $n->sender_id ) ) {
				$sender_url = get_author_posts_url( $n->sender_id );
			}
