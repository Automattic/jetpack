...
class zeroBSCRM_Delete {
	...
	function __construct( $args = array() ) {
		...
		if ( ! is_array( $this->obj ) || ! isset( $this->obj['id'] ) ) {
			$this->canDelete = false;
		}
		...
		if ( isset( $_POST['zbs-delete-form-master'] ) && $_POST['zbs-delete-form-master'] == $this->objTypeID ) {
			...
			if ( wp_verify_nonce( $_POST['zbs-delete-nonce'], 'delete-nonce' ) ) {
				...
				if ( isset( $_POST['zbs-delete-kill-children'] ) ) {
					if ( $_POST['zbs-delete-kill-children'] == 'no' ) {
						$this->killChildren = false;
					}
					if ( $_POST['zbs-delete-kill-children'] == 'yes' ) {
				...
				if ( isset( $_POST['zbs-delete-id'] ) && $_POST['zbs-delete-id'] == $this->objID ) {
					...
					switch ( $this->objTypeID ) {
						...
						case ZBS_TYPE_CONTACT:
							// delete
							$deleted = $zbs->DAL->contacts->deleteContact(
								array(
									'id'          => $this->objID,
						...
						case ZBS_TYPE_COMPANY:
							// delete
							$deleted = $zbs->DAL->companies->deleteCompany(
								array(
									'id'          => $this->objID,
									'saveOrphans' => $saveOrphans,
								)
						...
						case ZBS_TYPE_QUOTE:
							// delete
							$deleted = $zbs->DAL->quotes->deleteQuote(
								array(
									'id'          => $this->objID,
									'saveOrphans' => $saveOrphans,
								)
						...
						case ZBS_TYPE_INVOICE:
							// delete
							$deleted = $zbs->DAL->invoices->deleteInvoice(
								array(
									'id'          => $this->objID,
									'saveOrphans' => $saveOrphans,
								)
						...
						case ZBS_TYPE_TRANSACTION:
							// delete
							$deleted = $zbs->DAL->transactions->deleteTransaction(
								array(
									'id'          => $this->objID,
									'saveOrphans' => $saveOrphans,
								)
						...
						case ZBS_TYPE_FORM:
							// delete
							$deleted = $zbs->DAL->forms->deleteForm(
								array(
									'id'          => $this->objID,
									'saveOrphans' => $saveOrphans,
								)
						...
						case ZBS_TYPE_TASK:
							...
							// delete
							$deleted = $zbs->DAL->events->deleteEvent(
								array(
									'id'          => $this->objID,
									'saveOrphans' => $saveOrphans,
								)
						...
						case ZBS_TYPE_QUOTETEMPLATE:
							// delete
							$deleted = $zbs->DAL->quotetemplates->deleteQuotetemplate(
								array(
									'id'          => $this->objID,
									'saveOrphans' => $saveOrphans,
								)
