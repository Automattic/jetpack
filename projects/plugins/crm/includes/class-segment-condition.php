	...
	public function init( $constructionArgs = array() ) {
		...
		if ( $this->addFilters && $this->key !== false && is_array( $this->condition ) ) {
			...
			if ( isset( $this->condition['name'] ) ) {
				$this->condition['name'] = __( $this->condition['name'], 'zero-bs-crm' );
			}
			if ( isset( $this->condition['category'] ) ) {
				$this->condition['category'] = __( $this->condition['category'], 'zero-bs-crm' );
			}
			if ( isset( $this->condition['description'] ) ) {
				$this->condition['description'] = __( $this->condition['description'], 'zero-bs-crm' );
			}
