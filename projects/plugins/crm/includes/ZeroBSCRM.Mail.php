...
function zeroBSCRM_mailDelivery_accountDDL( $selectedOption = -1, $id = 'zbs-mail-delivery-acc', $showProtocol = true, $withSemanticWrap = true ) {
	...
	if ( count( $zbsSMTPAccs ) <= 0 ) {
		...
		$defaultDetails = ''; if ( is_array( $defaultFromDeets ) ) {
			...
			if ( isset( $defaultFromDeets['name'] ) ) {
				$defaultDetails = $defaultFromDeets['name'];
			}
		...
		// none
		?><select class="fluid" id="<?php echo esc_attr( $id ); ?>" name="<?php echo esc_attr( $id ); ?>"><option value="-1"><?php echo esc_html( $defaultDetails ); ?></option></select>
		<?php
		...
		?>
		<select class="fluid" id="<?php echo esc_attr( $id ); ?>" name="<?php echo esc_attr( $id ); ?>">
		<?php
		foreach ( $zbsSMTPAccs as $key => $acc ) {
			...
			if ( $showProtocol ) {
				$accStrExtra = ' (wp_mail)';
				if ( isset( $acc['mode'] ) && $acc['mode'] == 'smtp' ) {
					$accStrExtra = ' (SMTP)';
				}
				$accStr .= $accStrExtra;
			}
	...
	if ( $withSemanticWrap ) {
		...
		?>
		<script type="text/javascript">jQuery(function(){ jQuery('#<?php echo esc_html( $id ); ?>').dropdown();});</script>
		<?php
...
function zeroBSCRM_mailDelivery_sendMessage( $mailDeliveryAccKey = '', $mail = -1 ) {
	...
	if ( $mailDeliveryAccKey == -1 || ! is_array( $mailSettings ) ) {
		...
		if ( ! is_array( $mailSettings ) ) {
			$mailSettings = zeroBSCRM_mailDelivery_retrieveACCWPDefault();
		}
	}
	...
	if ( isset( $mailSettings ) && is_array( $mailSettings ) ) {
		...
		if ( ! isset( $mailSettings ) || ! is_array( $mailSettings ) || ! isset( $mailSettings['fromemail'] ) ) {
			...
			return array(
				false,
				'errorMsg' => __( '<span>#289</span>Could not load settings for this mail delivery option', 'zero-bs-crm' ),
			);
		...
		if ( ! isset( $mail ) || ! is_array( $mail ) || ! isset( $mail['toEmail'] ) ) {
			...
			return array(
				false,
				'errorMsg' => __( '<span>#290</span>Could not load message for this mail delivery call', 'zero-bs-crm' ),
			);
		...
		*/
		...
		$options     = array(
			...
			// send to
		if ( isset( $mail['toEmail'] ) && ! empty( $mail['toEmail'] ) && zeroBSCRM_validateEmail( $mail['toEmail'] ) ) {
		...
		if ( isset( $mail['textbody'] ) && ! empty( $mail['textbody'] ) ) {
		...
		if ( isset( $mail['overrideSendName'] ) && ! empty( $mail['overrideSendName'] ) ) {
		...
		if ( empty( $body ) || empty( $sendToEmail ) || ! zeroBSCRM_validateEmail( $sendToEmail ) ) {
			...
			return array(
				false,
				'errorMsg' => __( '<span>#291</span>Could not process message details for this mail delivery call', 'zero-bs-crm' ),
			);
		...
		if ( isset( $mail['tracking'] ) && is_array( $mail['tracking'] ) && count( $mail['tracking'] ) > 0 ) {
			...
			$emailTypeID = -1;
			if ( isset( $mail['tracking']['emailTypeID'] ) ) {
				$emailTypeID = $mail['tracking']['emailTypeID'];
			}
			$targetObjID = -1;
			if ( isset( $mail['tracking']['targetObjID'] ) ) {
				$targetObjID = $mail['tracking']['targetObjID'];
			}
			$senderWPID = 0;
			if ( isset( $mail['tracking']['senderWPID'] ) ) {
				$senderWPID = $mail['tracking']['senderWPID'];
			}
			$senderEmailAddress = $mailSettings['fromemail'];
			$associatedObjID    = -1;
			if ( isset( $mail['tracking']['associatedObjID'] ) ) {
				$associatedObjID = $mail['tracking']['associatedObjID'];
			}
			$emailSubject = '';
			if ( isset( $mail['subject'] ) ) {
		...
		switch ( $mailSettings['mode'] ) {
			...
			default:
				return array(
					false,
					'errorMsg' => __( '<span>#289</span>Mail Delivery mode not found', 'zero-bs-crm' ),
				);
				break;
	...
	return array(
		false,
		'errorMsg' => __( '<span>#297</span>No settings found for this mail delivery option', 'zero-bs-crm' ),
	);
}

// [DREAMWORLD]: WH $arg'itizes this
function zeroBSCRM_mailDelivery_sendViaSMTP( $smtpHost = '', $smtpPort = '', $smtpUser = '', $smtpPass = '', $smtpSecurity = 'tls', $sendFrom = '', $sendFromName = '', $toEmail = '', $toName = '', $subject = '', $msgText = '', $msgHTML = '', $debugMail = false, $returnDebug = false, $attachments = false ) {
	#,$ccEmails='',$bccEmails='',$replyToAddr='',$returnPath=''

	global $retDebugStr;

	if ( $debugMail ) {
		$retDebugStr = '<h2>Debugging SMTP</h2>';
	}
	...
	! empty( $smtpPort )
		...
		global $zbsDebug;
		$zbsDebug['return'] = $returnDebug;
		$zbsDebug['debug']  = $debugMail;
		...
		if ( $debugMail ) {
			$retDebugStr .= 'SMTP Details Present<br />';
		}
		...
		if ( $debugMail ) {
			if ( ! $returnDebug ) {
				echo '<pre>'; #} This is for the mail func debugger
			}
		...
		if ( $debugMail ) {
			$retDebugStr .= 'Mailer settings overriden<br />';
		}
		...
		if ( $debugMail ) {
			$retDebugStr .= 'Mail body prepped<br />';
		}
		...
		try {
			...
			if ( $debugMail ) {
				...
				if ( isset( $toEmail ) ) {
					$mail_error_data['to'] = $toEmail;
				}
				if ( isset( $subject ) ) {
					$mail_error_data['subject'] = $subject;
				}
				// likely huge & not necessary below:
				// if (isset($message))      $mail_error_data['message'] = $message;
				if ( isset( $attachments ) ) {
					$mail_error_data['attachments'] = $attachments;
				}
	...
	' . __( 'This error suggests that your Port & Security settings are not correct, or that you have the wrong value for SMTP Host.', 'zero-bs-crm' );
					...
					if ( ! $emailWasSent ) {
						...
						if ( ! empty( $replacementHost ) || ! empty( $replacementPort ) || ! empty( $replacementSecurityMode ) ) {
							...
							#} any? both?
							if ( ! empty( $replacementHost ) ) {
								$smtpSettings['host'] = $replacementHost;
							}
							if ( ! empty( $replacementPort ) ) {
								$smtpSettings['port'] = $replacementPort;
							}
							if ( ! empty( $replacementSecurityMode ) ) {
								$smtpSettings['security'] = $replacementSecurityMode; # tls, ssl, none
							}
...
function jpcrm_mail_delivery_send_via_gmail_oauth( $args ) {
		...
		foreach ( $default_args as $argK => $argV ) {
			$$argK = $argV;
...
function zeroBSCRM_mailDelivery_retrieveACCByKey( $key = -1 ) {
	...
	if ( $key !== -1 ) {
		...
		if ( ! is_array( $existingZBSSMTPAccs ) ) {
			return false;
		}
		if ( isset( $existingZBSSMTPAccs[ $key ] ) ) {
			return $existingZBSSMTPAccs[ $key ];
		}
	}
	return false;
}
...
function zeroBSCRM_mailDelivery_makeKey( $accDeets = array() ) {
	...
	if ( is_array( $accDeets ) ) {
		...
		if ( isset( $accDeets['fromemail'] ) ) {
			$str = $accDeets['fromemail'];
		}
		if ( empty( $str ) && isset( $accDeets['fromname'] ) ) {
			$str .= ':' . $accDeets['fromname'];
		}
