...
use Automattic\JetpackCRM\Learn_Menu;
...
function zeroBSCRM_admin_menu_settings() { 
	...
	if(zeroBSCRM_isAdminPage()){
		...
		?><script>var zbscrmjs_custom_admin = '<?php echo esc_html( $zbs_custom_admin ); ?>';</script><?php 
		...
		if ($hiding_wp){ ?><script type="text/javascript">jQuery(function(){ jQuery('body').addClass('zbs-fullscreen'); jQuery('#wpcontent').addClass('zbs-menu-open'); });</script><?php }
...
function zeroBSCRM_admin_top_menu($branding = 'zero-bs-crm', $page = 'dash'){
		...
		if(zeroBSCRM_isAdminPage()){
			...
			?><script type="text/javascript">var zbscrmjs_topMenuSecToken = '<?php echo esc_js( wp_create_nonce( 'zbscrmjs-ajax-nonce-topmenu' ) ); ?>';</script><?php
	    ...
	    <div class="ui mobile tablet only" id="zbs-mobile-nav">
			...
			if (zeroBSCRM_isLocal()){
				...
				?><div class="item" style="float: right;color: #FFF;margin-top: -2.5em;"><?php esc_html_e('Developer Mode','zero-bs-crm'); ?></div><?php
		...
		<div class="right menu ui inverted zbs-admin-bg-menu mobile hidden" style="z-index:5">
			...
			<div class="ui simple dropdown item select<?php esc_attr( zeroBS_menu_active_type('contact') ); ?>" id="zbs-contacts-topmenu" style="min-width:114px;z-index:2">
				...
				<div class="menu ui inverted zbs-admin-bg-menu zbs-dropdown">
					...
					echo jpcrm_esc_link('tags',-1,'zerobs_customer',false,'contact' ); ?>"><i class="icon tags"></i> <?php esc_html_e("Tags","zero-bs-crm"); ?></a>
					<?php } ?>
		  ...
		  if (zeroBSCRM_permsViewQuotes() && zeroBSCRM_getSetting('feat_quotes') > 0){ ?>
			<div class="ui simple dropdown item select<?php zeroBS_menu_active_type('quote'); ?>" id="zbs-quotes-topmenu" style="z-index:5">
				...
				<div class="menu ui inverted zbs-admin-bg-menu zbs-dropdown">
					...
					<a class="item" href="<?php 
					echo jpcrm_esc_link( 'tags',-1,ZBS_TYPE_QUOTE,false,'quote' ); ?>"><i class="icon tags"></i> <?php esc_html_e("Tags","zero-bs-crm"); ?></a>
					<?php } ?>
		...
		<div class="ui simple dropdown item select" id="zbs-extensions-topmenu" style="z-index:5">
		  ...
		  <div class="ui simple dropdown item" id="userbutt" style="z-index:5">
		    <span class="text"><?php 
		    $uid = get_current_user_id();
		    echo jpcrm_get_avatar( $uid, 30 ); ?></span>
		    <i class="dropdown icon"></i>
		    <div class="menu ui inverted zbs-admin-bg-menu zbs-dropdown">
			    ...
			    <div class="column">
			      ...
			      <div class="ui link list">
					...
					<?php // slack for admins :)
				  	if (zeroBSCRM_isZBSAdminOrAdmin()){ 
				  		?><a href="<?php echo esc_url( $zbs->urls['community'] ); ?>" target="_blank" class="item"><i class="slack icon"></i> <?php esc_html_e("Join our Slack","zero-bs-crm");?></a><?php 
				  	} ?>
					<?php ##/WLREMOVE ?>
					...
					<?php // welcome tour and crm resources page for admins :)
				  	if (zeroBSCRM_isZBSAdminOrAdmin()){
					  	##WLREMOVE ?>
