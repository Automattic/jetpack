...
function zeroBSCRM_FormsMetaboxSetup() {
	...
	if ( ! zeroBSCRM_hasPaidExtensionActivated() ) {
		$zeroBS__Metabox_FormMoreFields = new zeroBS__Metabox_FormMoreFields( __FILE__ );
	}
	##/WLREMOVE
...
class zeroBS__Metabox_FormLanguage extends zeroBS__Metabox {
	...
	public function html( $form, $metabox ) {
		...
		?>
				...
				<?php
				foreach ( $zbsFormFields as $fieldK => $fieldV ) {
		...
		if ( ! defined( 'ZBS_OBJ_SAVED' ) ) {
			...
			*/
			...
			if ( $form_id == -1 && ! empty( $addUpdateReturn ) && $addUpdateReturn != -1 ) {
				...
				$zbsJustInsertedMetaboxID = $form_id;
...
class zeroBS__Metabox_FormSettings extends zeroBS__Metabox {
	...
	public function html( $form, $metabox ) {
		...
		if ( is_array( $form ) && isset( $form['id'] ) ) {
			...
			// DAL3+ saved in obj
			$zbsfs = 'simple';
		if ( is_array( $form ) && isset( $form['style'] ) ) {
			$zbsfs = $form['style'];
		}
		...
		?>
			...
			if ( $formID > 0 ) {
				...
				?>
				[jetpackcrm_form id="<?php echo esc_attr( $formID ); ?>" style="<?php echo esc_attr( $zbsfs ); ?>"]
				<?php
			...
			<div id="form-chooser">
				...
				<div class="third" id="naked-form">
					<div class="naked choice 
					<?php
					if ( $zbsfs == 'naked' ) {
						echo 'selected';}
					?>
					" data-pid="<?php echo esc_attr( $formID ); ?>" data-style="naked">
						...
						<div class="content">
							<div class="form-wrapper">
								<div class="input">
								<?php
								if ( ! empty( $zbsForm['fname'] ) ) {
									echo esc_html( $zbsForm['fname'] );
								} else {
									echo 'First Name'; }
								?>
								</div><div class="input">
		<?php
		if ( ! empty( $zbsForm['email'] ) ) {
			echo esc_html( $zbsForm['email'] );
		} else {
			esc_html_e( 'Email', 'zero-bs-crm' ); }
		?>
</div><div class="send">
		<?php
		if ( ! empty( $zbsForm['submit'] ) ) {
			echo esc_html( $zbsForm['submit'] );
		} else {
			echo 'Submit'; }
		?>
</div>
				...
				<div class="third" id="cgrab-form">
					<div class="cgrab choice 
					<?php
					if ( $zbsfs == 'cgrab' ) {
						echo 'selected';}
					?>
					" data-pid="<?php echo esc_attr( $formID ); ?>" data-style="cgrab">
						<div class="blobby">
							<p>Lorem Ipsum Text here</p>
						</div>
						<div class="content">
							<h1>
							<?php
							if ( ! empty( $zbsForm['header'] ) ) {
								echo esc_html( $zbsForm['header'] );
							} else {
								echo 'Want to find out more?'; }
							?>
							</h1>
							<h3>
							<?php
							if ( ! empty( $zbsForm['subheader'] ) ) {
								echo esc_html( $zbsForm['subheader'] );
							} else {
								echo 'Drop us a line. We follow up on all contacts'; }
							?>
								</h3>
							<div class="form-wrapper">
								<div class="input">
								...
								<div class="input">
								<?php
								if ( ! empty( $zbsForm['lname'] ) ) {
									echo esc_html( $zbsForm['lname'] );
								} else {
									echo 'Last Name'; }
								?>
													</div>
								...
								<div class="textarea">
								<?php
								if ( ! empty( $zbsForm['notes'] ) ) {
									echo esc_html( $zbsForm['notes'] );
								} else {
									echo 'Your Message'; }
								?>
														</div>
							...
							<div class="trailer">
							<?php
							if ( ! empty( $zbsForm['spam'] ) ) {
								echo esc_html( $zbsForm['spam'] );
							} else {
								echo 'We will not send you spam. Our team will be in touch within 24 to 48 hours Mon-Fri (but often much quicker)'; }
							?>
												</div>
						</div>
						<div class="clear"></div>
				...
				<div class="third" id="simple-form">
					...
					if ( $zbsfs == 'simple' ) {
						echo 'selected';}
					?>
					" data-pid="<?php echo esc_attr( $formID ); ?>" data-style="simple">
						<div class="blobby">
							<p>Lorem Ipsum Text here</p>
							<p>Lorem Ipsum <span class="br">s</span> Text <span class="br">s</span> here</p>
							<p>Lorem Ipsum Text here</p>
