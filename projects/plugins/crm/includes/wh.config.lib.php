...
class WHWPConfigLib {
	...
	function __construct( $config = array() ) {
		...
		if ( is_array( $config ) ) {
			...
			if ( isset( $config['conf_key'] ) ) {
				$this->settingsKey = $config['conf_key'];
			}
			if ( isset( $config['conf_ver'] ) ) {
				$this->settingsVer = $config['conf_ver'];
			}
			if ( isset( $config['conf_defaults'] ) ) {
				$this->settingsDefault = $config['conf_defaults'];
			}
			if ( isset( $config['conf_plugin'] ) ) {
				$this->settingsPlugin = $config['conf_plugin'];
			}
			if ( isset( $config['conf_pluginver'] ) ) {
				$this->settingsPluginVer = $config['conf_pluginver'];
			}
			if ( isset( $config['conf_plugindbver'] ) ) {
				$this->settingsPluginDBVer = $config['conf_plugindbver'];
			}
			if ( isset( $config['conf_dmzkey'] ) ) {
				$this->settingsDMZKey = $config['conf_dmzkey'];
			}
			if ( isset( $config['conf_protected'] ) ) {
				$this->settingsProtected = $config['conf_protected'];
			}
		} else {
			exit( 'WHConfigLib initiated incorrectly.' );
		}
	...
	function resetToDefaults() {
		...
		if ( isset( $this->settingsProtected ) && is_array( $this->settingsProtected ) ) {
			foreach ( $this->settingsProtected as $protectedKey ) {
	...
	function dmzGet( $dmzKey, $confKey ) {
		...
		if ( empty( $dmzKey ) === true || empty( $confKey ) === true ) {
			return false;
		}
	...
	function dmzDelete( $dmzKey, $confKey ) {
		...
		$newSettings      = array();
		if ( isset( $existingSettings ) && is_array( $existingSettings ) ) {
			foreach ( $existingSettings as $k => $v ) {
				if ( $k != $confKey ) {
					$newSettings[ $k ] = $v;
				}
			}
	...
	function dmzUpdateConfig( $dmzKey, $config ) {
		...
		if ( empty( $dmzKey ) === true || empty( $config ) === true ) {
			return false;
		}
	...
	function loadDMZFromDB() {
		...
		if ( ! is_array( $this->settingsDMZRegister ) ) {
			$this->settingsDMZRegister = array();
		}

			#} Load anything logged in register
		if ( is_array( $this->settingsDMZRegister ) && count( $this->settingsDMZRegister ) > 0 ) {
			foreach ( $this->settingsDMZRegister as $regEntry ) {
	...
	function saveToDB() {
		...
		if ( count( $this->settings ) > 0 ) {
			foreach ( $this->settings as $settingKey => $settingVal ) {
		...
		if ( isset( $this->settingsDMZRegister ) && is_array( $this->settingsDMZRegister ) ) {
			foreach ( $this->settingsDMZRegister as $dmzKey ) { # => $dmzVal
	...
	function loadFromDB() {
		...
		global $zbs;
			...
			$this->settings = $zbs->DAL->getSettings(
				array(
					'autoloadOnly' => true,
					'fullDetails'  => false,
				)
	...
	function createBackup( $backupLabel = '' ) {
		...
		$existingBK = get_option( $this->settingsKey . '_bk' );
		if ( ! is_array( $existingBK ) ) {
			$existingBK = array();
		}
		...
		if ( ! empty( $backupLabel ) ) {
			$existingBK[ time() ]['backupLabel'] = sanitize_text_field( $backupLabel ); #} For named settings bk
		}
	...
	function reloadFromBK( $bkkey ) {
		...
		if ( isset( $backups[ $bkkey ] ) ) {
			if ( is_array( $backups[ $bkkey ] ) ) {
