...
function zeroBSCRM_html_linkedContactTags( $contactID = -1, $tags = false, $classStr = '', $echo = true, $trim = false, $limit = false ) {
	...
	if ( $contactID > 0 && $tags == false ) {
		$tags = zeroBSCRM_getCustomerTagsByID( $contactID );
	}
		...
		foreach ( array_slice( $logs, 1 ) as $l ) {
			if ( $l['type'] == 'created' ) {
				...
				if ( count( $logsToShow ) < 9 ) {
					if ( in_array( $l['type'], $logTypesToPrioritise ) ) {
						$logsToShow[] = $l;
					}
				} else {
					break;
...
function zeroBSCRM_html_linkedCompanyTags( $contactID = -1, $tags = false, $classStr = '' ) {
	...
	if ( count( $tags ) > 0 ) {
		foreach ( $tags as $tag ) {
			...
			?>
		<a class="<?php echo esc_attr( $classStr ); ?>" href="<?php echo jpcrm_esc_link( $zbs->slugs['managecompanies'] ) . '&zbs_tag=' . $tagID; ?>"><?php echo esc_html( $tagName ); ?></a>
			<?php
		}
	}
}
...
function zeroBSCRM_html_companyTimeline( $companyID = -1, $logs = false, $companyObj = false ) {
	...
	<ul class="zbs-timeline">
				...
				$i        = 0; foreach ( $logsToShow as $log ) {
					...
					$ico    = '';
					$logKey = strtolower( str_replace( ' ', '_', str_replace( ':', '_', $log['type'] ) ) );
					if ( isset( $zeroBSCRM_logTypes['zerobs_company'][ $logKey ] ) ) {
						$ico = $zeroBSCRM_logTypes['zerobs_company'][ $logKey ]['ico'];
					}
					// these are FA ico's at this point
					...
					?>
				<li class="zbs-timeline-item
					<?php
					if ( $sameDate && $notLast ) {
						echo '-contd';
					}
					if ( empty( $logTitle ) ) {
						echo ' zbs-timeline-item-notitle';
					}
					if ( ! $notLast ) {
						echo ' zbs-last-item'; // last item (stop rolling padding)
					}
					?>
				zbs-single-log" 
					<?php
					if ( isset( $log['id'] ) && $log['id'] !== -1 ) {
						echo 'id="zbs-company-log-' . esc_attr( $log['id'] ) . '"';
					}
					?>
				>
					<?php
					...
					?>
						...
						if ( isset( $zeroBSCRM_logTypes['zerobs_company'][ $logKey ] ) ) {
							echo esc_html( $zeroBSCRM_logTypes['zerobs_company'][ $logKey ]['label'] );
						}
						?>
						</h3>
						<p>
...
function zeroBSCRM_html_invoiceStatusLabel( $inv = array() ) {
	...
	// <3.0
	if ( isset( $inv['meta'] ) && isset( $inv['meta']['status'] ) ) {
...
function zeroBSCRM_html_editFields( $objArr = false, $fields = false, $postPrefix = 'zbs_', $skipFields = array() ) {
	...
	if ( is_array( $fields ) ) {
		...
		foreach ( $fields as $fieldK => $fieldV ) {
			...
			if ( ! in_array( $fieldK, $skipFields ) ) {
				zeroBSCRM_html_editField( $objArr, $fieldK, $fieldV, $postPrefix );
			}
		}
	}
}
