...
class zeroBS__Metabox {
	...
	public function initMetabox() {
		...
		if ( $this->metaboxScreen == 'zerobs_edit_contact' ) {
			$this->metaboxScreen = 'zbs-add-edit-contact-edit';
		}
	...
	public function save_meta_box( $objID, $obj ) {
		...
		if ( isset( $_POST[ $this->metaboxID . '_nonce' ] ) && wp_verify_nonce( $_POST[ $this->metaboxID . '_nonce' ], 'save_' . $this->metaboxID ) ) {
			...
			if ( $objID == -1 && isset( $zbsJustInsertedMetaboxID ) && ! empty( $zbsJustInsertedMetaboxID ) && $zbsJustInsertedMetaboxID > 0 ) {
				$objID = $zbsJustInsertedMetaboxID;
			}
	...
	public function dalOutputMessage( $messages = array(), $type = 'error', $fallback = '' ) {
		...
		switch ( $type ) {
			...
			case 'notice':
				...
				if ( empty( $fallback ) ) {
					$fallback = __( 'General Notice', 'zero-bs-crm' );
				}
			...
			default:
				...
				if ( empty( $fallback ) ) {
					$fallback = __( 'General Error', 'zero-bs-crm' );
				}
...
function zeroBSCRM_do_meta_boxes( $screen, $context, $object ) {
	...
	if ( empty( $screen ) ) {
		...
		global $zbsPage;
		$isNewEdit = false;
	if ( isset( $zbsPage['new_edit'] ) ) {
		$isNewEdit = $zbsPage['new_edit']; // echo 'pre:<pre>'.print_r($zbsPage,1).'</pre>';
	}
	...
	if ( is_array( $screenOpts ) && isset( $screenOpts['mb_hidden'] ) && is_array( $screenOpts['mb_hidden'] ) ) {
		...
		$minimised = array();
	if ( is_array( $screenOpts ) && isset( $screenOpts['mb_mini'] ) && is_array( $screenOpts['mb_mini'] ) ) {
		$minimised = $screenOpts['mb_mini'];
	}
...
function zeroBSCRM_applyScreenOptions( $screenOpts = false, $page = '', $context = '' ) {
	...
	$screenOptionsList = array();
	if ( is_array( $screenOpts ) && isset( $screenOpts[ 'mb_' . $context ] ) && is_array( $screenOpts[ 'mb_' . $context ] ) ) {
		$screenOptionsList = $screenOpts[ 'mb_' . $context ];
	}
	...
	if ( isset( $zbs->metaboxes[ $page ][ $context ] ) && isset( $screenOptionsList ) && is_array( $screenOptionsList ) && count( $screenOptionsList ) > 0 ) {
			...
			foreach ( $mbIDsToAdd as $mbIDToAdd ) {
				...
				if ( $existingMB !== false ) {
					...
					if ( $tabgroup ) {
						...
						// set if not set
						if ( ! isset( $soMetaboxes[ $metaboxID ] ) ) {
							$soMetaboxes[ $metaboxID ] = array();
						}
						if ( ! isset( $soMetaboxes[ $metaboxID ]['boxes'] ) ) {
							$soMetaboxes[ $metaboxID ]['boxes'] = array();
						}
		...
		if ( count( $soMetaboxes ) > 0 ) {
			...
			if ( isset( $metaboxes['high'] ) && is_array( $metaboxes['high'] ) && count( $metaboxes['high'] ) > 0 ) {
				foreach ( $metaboxes['high'] as $mbID => $mb ) {
					if ( ! in_array( $mbID, array_keys( $soMetaboxes ) ) ) {
						$newHighPriority[ $mbID ] = $mb;
					}
				}
			}
			...
			foreach ( array( 'sorted', 'core', 'default', 'low' ) as $priority ) {
				if ( isset( $metaboxes[ $priority ] ) && count( $metaboxes[ $priority ] ) > 0 ) {
					foreach ( $metaboxes[ $priority ] as $mbID => $mb ) {
						if ( ! in_array( $mbID, array_keys( $soMetaboxes ) ) ) {
							$newHighPriority[ $mbID ] = $mb;
						}
					}
...
function zeroBSCRM_do_meta_box_html( $box, $page, $hidden, $object, $minimised, $isTabPane = false, $isActiveTab = false ) {
		...
		*/
		...
		// headless?
		$headless = false;
	if ( isset( $box['headless'] ) && $box['headless'] ) {
		$headless = true;
	}

		// extra classes
		$extraClasses = '';
	if ( isset( $box['extraclasses'] ) && $box['extraclasses'] ) {
		$extraClasses = $box['extraclasses'];
	}
	...
	if ( isset( $box['capabilities'] ) && is_array( $box['capabilities'] ) ) {
		...
		if ( isset( $box['capabilities']['can_minimise'] ) && $box['capabilities']['can_minimise'] ) {
			$canMinimise = true;
		}
		if ( isset( $box['capabilities']['can_hide'] ) && $box['capabilities']['can_hide'] ) {
			$canHide = true;
		}
		if ( isset( $box['capabilities']['can_move'] ) && $box['capabilities']['can_move'] ) {
			$canMove = true;
		}
...
function zeroBSCRM_getCurrentMetaboxes() {
	...
	foreach ( array( 'normal', 'side' ) as $context ) {
		...
		if ( is_array( $mb ) && count( $mb ) > 0 ) {
			$metaboxes[ $context ] = $mb;
		}
	}
...
function zeroBSCRM_getMetaboxes( $page = '', $context = '' ) {
	...
	if ( isset( $zbs->metaboxes[ $page ] ) && isset( $zbs->metaboxes[ $page ][ $context ] ) ) {
		return $zbs->metaboxes[ $page ][ $context ];
	}
