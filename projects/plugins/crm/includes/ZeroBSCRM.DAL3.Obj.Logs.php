...
class zbsDAL_logs extends zbsDAL_ObjectLayer {
	...
	protected $objectModel = array(
		...
		// other fields
		'objtype'     => array(
			'fieldname' => 'zbsl_objtype',
			'format'    => 'int',
		),
		'objid'       => array(
			'fieldname' => 'zbsl_objid',
			'format'    => 'int',
		),
		'type'        => array(
			'fieldname' => 'zbsl_type',
		...
		'shortdesc'   => array(
			'fieldname' => 'zbsl_shortdesc',
			'format'    => 'str',
		),
		'longdesc'    => array(
			'fieldname' => 'zbsl_longdesc',
			'format'    => 'str',
		),
		'pinned'      => array(
			'fieldname' => 'zbsl_pinned',
			'format'    => 'int',
		),
		'created'     => array(
			'fieldname' => 'zbsl_created',
			'format'    => 'uts',
		),
		'lastupdated' => array(
			'fieldname' => 'zbsl_lastupdated',
			'format'    => 'uts',
		),
	...
	public function getLog( $args = array() ) {
		...
		if ( $id ) {
			...
			if ( isset( $potentialRes ) && isset( $potentialRes->ID ) ) {
				...
				if ( $incMeta ) {
					$potentialRes->meta = $this->DAL()->getMeta(
						array(
	...
	public function getLogsForObj( $args = array() ) {
		...
		if ( $objid < 0 ) {
			...
			$objtype = (int) $objtype;
		if ( ! isset( $objtype ) || $objtype == -1 || $this->DAL()->objTypeKey( $objtype ) === -1 ) {
			return false;
		}
		...
		#} ============= WHERE ================
			...
			#} objtype
		if ( ! empty( $objtype ) && $objtype !== -1 ) {
			$wheres['zbsl_objtype'] = array( 'zbsl_objtype', '=', '%d', $objtype );
		}

			#} notetype
		if ( ! empty( $notetype ) && $notetype !== -1 ) {
			$wheres['zbsl_type'] = array( 'zbsl_type', '=', '%s', $notetype );
		}
		...
		if ( isset( $potentialRes ) && is_array( $potentialRes ) && count( $potentialRes ) > 0 ) {
			...
			foreach ( $potentialRes as $resDataLine ) {
				...
				if ( $incMeta ) {
					$resDataLine->meta = $this->DAL()->getMeta(
						array(
	...
	public function getLogCount( $args = array() ) {
		...
		if ( $withType !== false && ! empty( $withType ) ) {
			$whereArr['status'] = array( 'zbsl_type', '=', '%s', $withType );
		}
	...
	public function addUpdateLog( $args = array() ) {
		...
		if ( ! isset( $data['objtype'] ) || $data['objtype'] == -1 || $this->DAL()->objTypeKey( $data['objtype'] ) === -1 ) {
			return false;
		}

			// check id present + legit
			$data['objid'] = (int) $data['objid'];
		if ( empty( $data['objid'] ) || $data['objid'] <= 0 ) {
			return false;
		}
		...
		if ( $ignore_if_existing_desc_type ) {
			...
			$existing_logs = $this->getLogsForObj(
				array(
		...
		if ( $ignore_if_existing_desc_type
			...
			// either or
		} elseif ( $ignore_if_existing_desc_type
				||
				is_array( $ignore_if_meta_matching ) && isset( $ignore_if_meta_matching['key'] ) && isset( $ignore_if_meta_matching['value'] ) ) {
		...
		if ( isset( $data['created'] ) && ! empty( $data['created'] ) && $data['created'] !== -1 ) {
			$dataArr['zbsl_created'] = $data['created'];
			$dataTypes[]             = '%d';
		}
	...
	public function deleteLog( $args = array() ) {
		...
		$id = (int) $id;
		if ( ! empty( $id ) && $id > 0 ) {
			return zeroBSCRM_db2_deleteGeneric( $id, 'logs' );
		}
