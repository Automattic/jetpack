...
class jpcrm_templating_placeholders {
	...
	public function build_placeholders( $include_custom_fields = true ) {
		...
		if ( isset( $placeholders['quote']['title'] ) ) {
			$placeholders['quote']['title']['aliases'] = array( '##QUOTE-TITLE##', '##QUOTETITLE##' );
		}
		if ( isset( $placeholders['quote']['value'] ) ) {
			$placeholders['quote']['value']['aliases'] = array( '##QUOTEVALUE##' );
		}
		if ( isset( $placeholders['quote']['date'] ) ) {
			$placeholders['quote']['date']['aliases'] = array( '##QUOTEDATE##' );
		}
	...
	public function get_placeholders_for_tooling( $tooling = array( 'global' ), $hydrate_aliases = false, $split_by_category = false ) {
		...
		if ( is_string( $tooling ) ) {
			$tooling = array( $tooling );
		}
		...
		if ( is_array( $placeholders ) ) {
			...
			foreach ( $placeholders as $placeholder_group_key => $placeholder_group ) {
				...
				foreach ( $placeholder_group as $placeholder_key => $placeholder ) {
					...
					if ( in_array( $placeholder_group_key, $tooling )
						||
