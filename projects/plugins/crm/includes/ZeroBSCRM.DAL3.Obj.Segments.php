...
class zbsDAL_segments extends zbsDAL_ObjectLayer {
	...
	protected $objectModel = array(
		...
		// other fields
		'name'         => array(
			'fieldname' => 'zbsseg_name',
			'format'    => 'str',
		),
		'slug'         => array(
			'fieldname' => 'zbsseg_slug',
			'format'    => 'str',
		),
		'matchtype'    => array(
			'fieldname' => 'zbsseg_matchtype',
			'format'    => 'str',
		),
		'created'      => array(
			'fieldname' => 'zbsseg_created',
			'format'    => 'uts',
		),
		'lastupdated'  => array(
			'fieldname' => 'zbsseg_lastupdated',
			'format'    => 'uts',
		),
		'compilecount' => array(
			'fieldname' => 'zbsseg_compilecount',
			'format'    => 'int',
		),
		'lastcompiled' => array(
			'fieldname' => 'zbsseg_lastcompiled',
			'format'    => 'uts',
		),
	...
	public function getSegments( $ownerID = -1, $perPage = 10, $page = 0, $withConditions = false, $searchPhrase = '', $inArr = '', $sortByField = '', $sortOrder = 'DESC' ) {
		...
		if ( $ownerID != -99 ) {
			...
			if ( ! empty( $ownerID ) ) {
				$wheres['zbs_owner'] = array( '=', $ownerID, '%d' );
			}
		}
		...
		if ( isset( $potentialSegments ) && is_array( $potentialSegments ) ) {
		...
		if ( count( $segments ) > 0 ) {
			foreach ( $segments as $segment ) {
				...
				if ( $withConditions ) {
					$resArr['conditions'] = $this->getSegmentConditions( $segment->ID );
				}
	...
	public function getSegmentsCountIncParams( $ownerID = -1, $perPage = 10, $page = 0, $withConditions = false, $searchPhrase = '', $inArr = '', $sortByField = '', $sortOrder = 'DESC' ) {
		...
		if ( isset( $potentialSegmentCount ) && isset( $potentialSegmentCount->segcount ) ) {
			$segmentCount = $potentialSegmentCount->segcount;
		}
	...
	public function segmentConditionsToArgs( $conditions = array(), $matchType = 'all' ) {
		...
		if ( is_array( $conditions ) && count( $conditions ) > 0 ) {
			...
			foreach ( $conditions as $condition ) {
				...
				$newArgs         = $this->segmentConditionArgs( $condition, $conditionIndx );
	...
	public function isContactInSegment( $contactID = -1, $segmentID = -1, $segmentObj = false ) {
		...
		if ( $segmentID > 0 && $contactID > 0 ) {
			...
			if ( ! isset( $contactGetArgs['additionalWhereArr'] ) ) {
				$contactGetArgs['additionalWhereArr'] = array();
			}
		...
		if ( is_array( $quote ) ) {
			...
			if ( count( $segments ) > 0 ) {
				foreach ( $segments as $segment ) {
					...
					if ( $this->segmentHasConditionType(
						$segment,
						array(
							'quotecount',
						)
	...
	public function updateSegmentCompiled( $segmentID = -1, $segmentCount = 0, $compiledUTS = -1 ) {
		...
		if ( $segmentID > 0 ) {
					...
					$count = 0;
			if ( $segmentCount > 0 ) {
				$count = (int) $segmentCount;
			}
					$compiled = time();
			if ( $compiledUTS > 0 ) {
				$compiled = (int) $compiledUTS;
			}
	...
	public function addUpdateSegment( $segmentID = -1, $segmentOwner = -1, $segmentName = '', $segmentConditions = array(), $segmentMatchType = 'all', $forceCompile = false ) {
		...
		if ( $segmentID > 0 ) {
			...
			if ( empty( $segmentName ) ) {
				$segmentName = __( 'Untitled Segment', 'zero-bs-crm' );
			}
	...
	public function addUpdateSegmentCondition( $conditionID = -1, $segmentID = -1, $conditionDetails = array() ) {
		...
		if ( isset( $conditionDetails['type'] ) ) {
			$condition['type'] = $conditionDetails['type'];
		}
		if ( isset( $conditionDetails['value'] ) ) {
			$condition['val'] = $conditionDetails['value'];
		}
		if ( isset( $conditionDetails['operator'] ) && $conditionDetails['operator'] !== -1 ) {
			$condition['operator'] = $conditionDetails['operator'];
		}
		if ( isset( $conditionDetails['value2'] ) ) {
			$condition['valsecondary'] = $conditionDetails['value2'];
		}
	...
	public function segmentHasConditionType( $segment = array(), $condition_types = array() ) {
		...
		if ( is_array( $segment ) && isset( $segment['conditions'] ) && is_array( $segment['conditions'] ) && is_array( $condition_types ) ) {
			...
			foreach ( $segment['conditions'] as $condition ) {
				...
				if ( in_array( $condition['type'], $condition_types ) ) {
					return true;
				}
			}
		}
