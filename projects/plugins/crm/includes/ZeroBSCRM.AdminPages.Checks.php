...
/* ======================================================
  ...
  function zeroBS_isPage($pageArr=array(),$postTypes=false,$adminPages=false,$includeFrontEnd=false){
      ...
      if (!$includeFrontEnd) if (!is_admin()) return false;
      ...
      if (in_array( $pagenow, $pageArr )){
        ...
        if (is_array($postTypes)){
          ...
          // catches get
          if (isset($_GET['post'])) $postID = (int)sanitize_text_field($_GET['post']);

          // catches post saving / update
          if (isset($_POST['post_ID'])) $postID = (int)sanitize_text_field($_POST['post_ID']);

          // check post type?
          if ($postID !== -1 && in_array(get_post_type($postID),$postTypes)) return true;

          // WH added: needed for post-new.php etc.
          if (isset($_GET['post_type']) && in_array(sanitize_text_field($_GET['post_type']),$postTypes)) return true;

        } elseif (is_array($adminPages)){

          // check page slug
          if (isset($_GET['page']) && in_array(sanitize_text_field($_GET['page']),$adminPages)) return true;
  ...
  function zeroBS_hasGETParams($pageArr=array(),$params=array()){
      ...
      if (in_array( $pagenow, $pageArr )){
        ...
        if (is_array($params) && count($params) > 0){
          ...
          foreach ($params as $p) if (!isset($_GET[$p])) return false;
  ...
  function zeroBS_hasGETParamsWithValues($pageArr=array(),$params=array(),$noneOfTheseParams=array()){
      ...
      if (in_array( $pagenow, $pageArr )){
        ...
        if (is_array($params) && count($params) > 0){
          ...
          foreach ($params as $p => $v) if (!isset($_GET[$p]) || $_GET[$p] != $v) return false;

          // check for $noneOfTheseParams
          foreach ($noneOfTheseParams as $p) if (isset($_GET[$p])) return false;
  ...
  function zeroBSCRM_isAPIRequest(){
   ...
   //below is more reliable as QUERY_STRING will always be set for API requests.
    ...
    if (isset($_SERVER['QUERY_STRING']) && strpos('#'.$_SERVER['QUERY_STRING'], 'api_key=zbscrm_') > 0) return true;
    ...
    if (zeroBSCRM_isRestUrl()) return true;
  ...
  function zeroBSCRM_isAdminPage(){
    ...
    // basic slug check
    if (isset($_GET['page'])) if (in_array($_GET['page'],$zbs->slugs)) return true;

    // custom post type pages
    if (zeroBSCRM_is_ZBS_custom_post_page()) return true;

    // odd pages?
    if ((isset($_GET['page'])) && ($_GET['page'] === 'zbs-noaccess' || $_GET['page'] === 'manage-customers' || $_GET['page'] === 'manage-invoices-crm')) return true;

    // custom slug checks?
    if (isset($_GET['zbsslug'])) if (in_array(sanitize_text_field($_GET['zbsslug']),$zbs->slugs)) return true;  

    // lastly... grasping at straws... check for defined global (defined in global admin script)
    if (defined('ZBS_PAGE')) $isOurPage = true;
  ...
  function zeroBSCRM_is_zbs_edit_page($specificType=false,$isNew=false){
      ...
      if ($specificType !== false){
        ...
        if (
            zeroBS_hasGETParamsWithValues(
              array( 'admin.php' ),
              array('page'=>'zbs-add-edit','action'=>'edit','zbstype'=>$specificType)
            )
          ){

          // looking to see if is new page?
          if ($isNew){
            if (!isset($_GET['zbsid'])) return  true;
          } else
            return true;
      ...
      } else if (zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>'zbs-add-edit','action'=>'edit'))) return true;
  ...
  function zeroBSCRM_is_delete_page($new_edit = null){
      ...
      // then if we have these, it's an edit page
      if (zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>'zbs-add-edit','action'=>'delete'))) return true;
  ...
  function zeroBSCRM_isAnyContactPage(){
    ...
    if (zeroBSCRM_is_contact_list_page()) return true;
    if (zeroBSCRM_is_customer_edit_page()) return true;
    if (zeroBSCRM_is_customer_new_page()) return true;
    if (zeroBSCRM_is_customertags_page()) return true;

    // for now we count co's as here:
    if (zeroBSCRM_is_company_list_page()) return true;
    if (zeroBSCRM_is_company_new_page()) return true;
    if (zeroBSCRM_is_company_edit_page()) return true;
    if (zeroBSCRM_is_companytags_page()) return true;
  ...
  function zeroBSCRM_is_customer_edit_page(){
    ...
    if ($isPage) 
      return true;
        ...
        if (
            (zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>'zbs-add-edit','action'=>'edit','zbstype'=>'contact')))
            ||
            (zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>'zbs-add-edit','action'=>'edit'),array('zbstype')))
            ) return true;
    }
    return false;
  ...
  function zeroBSCRM_is_customer_view_page(){
    ...
    // either has zbstype = contact, or no zbstype (as is default)
    if (
        (zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>'zbs-add-edit','action'=>'view','zbstype'=>'contact')))
        ||
        (zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>'zbs-add-edit','action'=>'view'),array('zbstype')))
        ) return true;
    ...
    if ($isPage) 
      return true;
        ...
        if (
          ...
          if (!isset($_GET['zbsid']) || $_GET['zbsid'] == -1) return  true;
  ...
  function zeroBSCRM_is_customertags_page(){
    ...
    if ($isPage) 
      ...
      global $zbs;
      return zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>$zbs->slugs['tagmanager'],'tagtype'=>'contact'));
    }
  ...
  function zeroBSCRM_is_company_view_page(){
    ...
    if (
        zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>'zbs-add-edit','action'=>'view','zbstype'=>'company'))
      ) return true;
  ...
  function zeroBSCRM_is_companytags_page(){
    ...
    if ($isPage) 
      ...
      global $zbs;
      return zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>$zbs->slugs['tagmanager'],'tagtype'=>'company'));
    }
  ...
  function zeroBSCRM_isAnyTransactionPage(){
    ...
    if (zeroBSCRM_is_transaction_list_page()) return true;
    if (zeroBSCRM_is_transaction_new_page()) return true;
    if (zeroBSCRM_is_transaction_edit_page()) return true;
    if (zeroBSCRM_is_transactiontags_page()) return true;
  ...
  function zeroBSCRM_is_transactiontags_page(){
    ...
    if ($isPage) 
      ...
      global $zbs;
      return zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>$zbs->slugs['tagmanager'],'tagtype'=>'transaction'));
    }
  ...
  function zeroBSCRM_isAnyInvoicePage(){
    ...
    if (zeroBSCRM_is_invoice_list_page()) return true;
    if (zeroBSCRM_is_invoice_new_page()) return true;
    if (zeroBSCRM_is_invoice_edit_page()) return true;
  ...
  function zeroBSCRM_is_invoicetags_page(){
    ...
    //v3.0+ only
    global $zbs;
    if ($zbs->isDAL3()) return zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>$zbs->slugs['tagmanager'],'tagtype'=>'invoice'));
  ...
  function zeroBSCRM_isAnyQuotePage(){
    ...
    if (zeroBSCRM_is_quote_list_page()) return true;
    if (zeroBSCRM_is_quo_new_page()) return true;
    if (zeroBSCRM_is_quo_edit_page()) return true;
    if (zeroBSCRM_is_quotetemplate_new_page()) return true;
    if (zeroBSCRM_is_quotetemplate_edit_page()) return true;
  ...
  function zeroBSCRM_is_quotetags_page(){
    ...
    //v3.0+ only
    global $zbs;
    if ($zbs->isDAL3()) return zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>$zbs->slugs['tagmanager'],'tagtype'=>'quote'));
  ...
  function zeroBSCRM_isAnyEventPage(){
    ...
    if (zeroBSCRM_is_event_list_page()) return true;
    if (zeroBSCRM_is_task_new_page()) return true;
    if (zeroBSCRM_is_task_edit_page()) return true;
    if (zeroBSCRM_is_task_calendar_page()) return true;
    if (zeroBSCRM_is_task_list_page()) return true;
    if (zeroBSCRM_is_eventtags_page()) return true;
  ...
  function zeroBSCRM_is_eventtags_page(){
    ...
    //v3.0+ only
    global $zbs;
    if ($zbs->isDAL3()) return zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>$zbs->slugs['tagmanager'],'tagtype'=>'event'));
  ...
  function zeroBSCRM_isAnyFormPage(){
    ...
    if (zeroBSCRM_is_form_new_page()) return true;
    if (zeroBSCRM_is_form_edit_page()) return true;
    if (zeroBSCRM_is_form_list_page()) return true;
    if (zeroBSCRM_is_formtags_page()) return true;
    if (zeroBSCRM_is_task_list_page()) return true;
    if (zeroBSCRM_is_eventtags_page()) return true;
  ...
  function zeroBSCRM_is_formtags_page(){
    ...
    //v3.0+ only
    global $zbs;
    if ($zbs->isDAL3()) return zeroBS_hasGETParamsWithValues(array( 'admin.php' ),array('page'=>$zbs->slugs['tagmanager'],'tagtype'=>'form'));
