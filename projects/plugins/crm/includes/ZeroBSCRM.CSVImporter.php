...
function zeroBSCRM_CSVImporterLitepages_header($subpage=''){
	...
	if (!current_user_can('admin_zerobs_customers'))  { wp_die( esc_html__('You do not have sufficient permissions to access this page.',"zero-bs-crm") ); }
...
?>
    ...
    <div id="zbs-admin-top-bar">
      <div id="zbs-list-top-bar">
          <div class="zbs-white"><span class="add-new-button"><?php esc_html_e('CSV Importer Lite',"zero-bs-crm"); if (!empty($subpage)) echo esc_html( ': '.$subpage ); ?></span>
...
function jpcrm_csvimporter_lite_preflight_checks( $stage ) {
	...
	if ($ignore_first_line) $num_lines--;
						...
						foreach ( $file_details['field_map'] as $fieldID => $fieldTarget ) {
							...
							?>
								...
								<div class="zbscrm-impcsv-map">
									<div class="zbscrm-impcsv-from"><?php if (!empty($fromStr)) echo '"'. esc_html( $fromStr ) .'"'; else echo esc_html( sprintf( __( 'Field #%s', 'zero-bs-crm' ), $fieldID ) ); ?></div>
									<div class="zbscrm-impcsv-arrow"><?php if ($fieldTarget != "ignorezbs") echo '<i class="fa fa-long-arrow-right"></i>'; else echo '-'; ?></div>
									<div class="zbscrm-impcsv-to"><?php if ($fieldTarget != "ignorezbs") echo '"'. esc_html( $fieldTargetName ) .'"'; else esc_html_e( 'Ignore', 'zero-bs-crm' ); ?></div>
								</div>
							<?php
	   								...
	   								if (count($customerFields) > 0){
	   									...
	   									if (!empty($newCustID) && empty($potentialCustomerID)) {
											...
											echo '<div class="zbscrm-import-log-line">'.
																										sprintf(
																											__( 'Successfully added contact #<a href="%s" target="_blank">%d</a>... <i class="fa fa-user"></i><span>+1</span>', 'zero-bs-crm' ),
