...
class zbsDAL_invoices extends zbsDAL_ObjectLayer {
	...
	protected $objectModel           = array(
		...
		*/
		...
		'pdf_template'        => array(
			'fieldname' => 'zbsi_pdf_template',
			'format'    => 'str',
		),
		'portal_template'     => array(
			'fieldname' => 'zbsi_portal_template',
			'format'    => 'str',
		),
		'email_template'      => array(
			'fieldname' => 'zbsi_email_template',
			'format'    => 'str',
		),
		'invoice_frequency'   => array(
			'fieldname' => 'zbsi_invoice_frequency',
			'format'    => 'int',
		),
		'currency'            => array(
			'fieldname' => 'zbsi_currency',
			'format'    => 'curr',
		),
		'pay_via'             => array(
			'fieldname' => 'zbsi_pay_via',
			'format'    => 'int',
		),
		/*
		...
		'address_to_objtype'  => array(
			'fieldname' => 'zbsi_address_to_objtype',
			'format'    => 'int',
		),
		...
		'allow_partial'       => array(
			'fieldname' => 'zbsi_allow_partial',
			'format'    => 'bool',
		),
		'allow_tip'           => array(
			'fieldname' => 'zbsi_allow_tip',
			'format'    => 'bool',
		),
		'send_attachments'    => array(
			'fieldname' => 'zbsi_send_attachments',
			'format'    => 'bool',
		...
		'hours_or_quantity'   => array(
			'fieldname' => 'zbsi_hours_or_quantity',
			'format'    => 'bool',
		),
		'date'                => array(
			'fieldname' => 'zbsi_date',
		...
		'due_date'            => array(
			'fieldname' => 'zbsi_due_date',
			'format'    => 'uts',
		),
		'paid_date'           => array(
			'fieldname' => 'zbsi_paid_date',
			'format'    => 'uts',
		),
		'hash_viewed'         => array(
			'fieldname' => 'zbsi_hash_viewed',
			'format'    => 'uts',
		),
		'hash_viewed_count'   => array(
			'fieldname' => 'zbsi_hash_viewed_count',
			'format'    => 'int',
		),
		'portal_viewed'       => array(
			'fieldname' => 'zbsi_portal_viewed',
			'format'    => 'uts',
		),
		'portal_viewed_count' => array(
			'fieldname' => 'zbsi_portal_viewed_count',
			'format'    => 'int',
		),
		'net'                 => array(
			'fieldname' => 'zbsi_net',
		...
		'discount'            => array(
			'fieldname' => 'zbsi_discount',
			'format'    => 'decimal',
		),
		'discount_type'       => array(
			'fieldname' => 'zbsi_discount_type',
			'format'    => 'str',
		),
		'shipping'            => array(
			'fieldname' => 'zbsi_shipping',
			'format'    => 'decimal',
		),
		'shipping_taxes'      => array(
			'fieldname' => 'zbsi_shipping_taxes',
			'format'    => 'str',
		),
		'shipping_tax'        => array(
			'fieldname' => 'zbsi_shipping_tax',
			'format'    => 'decimal',
		),
		'taxes'               => array(
			'fieldname' => 'zbsi_taxes',
			'format'    => 'str',
		),
		'tax'                 => array(
			'fieldname' => 'zbsi_tax',
		...
		'total'               => array(
			'fieldname' => 'zbsi_total',
			'format'    => 'decimal',
		),
		'created'             => array(
			'fieldname' => 'zbsi_created',
			'format'    => 'uts',
		),
		'lastupdated'         => array(
			'fieldname' => 'zbsi_lastupdated',
			'format'    => 'uts',
		),
			...
			( ! empty( $externalSource ) && ! empty( $externalSourceUID ) )
				...
				if ( isset( $potentialRes ) && isset( $potentialRes->ID ) ) {
					...
					if ( $withTransactions ) {
						...
						$res['transactions'] = $this->DAL()->transactions->getTransactions(
							array(
								'assignedInvoice' => $potentialRes->ID,
								'perPage'         => 1000,
		...
		public function getInvoices( $args = array() ) {
			...
			#} Count override
			if ( $count ) {
				$query = 'SELECT COUNT(invoice.ID) FROM ' . $ZBSCRM_t['invoices'] . ' as invoice' . $joinQ;
			}
			...
			if ( isset( $potentialRes ) && is_array( $potentialRes ) && count( $potentialRes ) > 0 ) {
				...
				foreach ( $potentialRes as $resDataLine ) {
					...
					if ( $withTransactions ) {
						...
						// add all line item lines
						$resArr['transactions'] = $this->DAL()->transactions->getTransactions(
							array(
								'assignedInvoice' => $resDataLine->ID,
								'perPage'         => 1000,
								'ignoreowner'     => zeroBSCRM_DAL2_ignoreOwnership( ZBS_TYPE_TRANSACTION ),
							)
						);
		...
		public function getInvoiceCount( $args = array() ) {
			...
			if ( $withStatus !== false && ! empty( $withStatus ) ) {
				$whereArr['status'] = array( 'zbsi_status', '=', '%s', $withStatus );
			}
		...
		public function addUpdateInvoice( $args = array() ) {
			...
			if ( $data['status'] === null || ! isset( $data['status'] ) || empty( $data['status'] ) ) {
			...
			if ( is_array( $limitedFields ) ) {
				...
				// add update time
				if ( ! isset( $dataArr['zbsi_lastupdated'] ) ) {
					$dataArr['zbsi_lastupdated'] = time();
					$typeArr[]                   = '%d'; }
			} else {
				...
				if ( ! empty( $id ) && $id > 0 ) {
					...
					if ( isset( $data['created'] ) && ! empty( $data['created'] ) && $data['created'] !== -1 ) {
						$dataArr['zbsi_created'] = $data['created'];
						$typeArr[]               = '%d';
					} else {
						$dataArr['zbsi_created'] = time();
						$typeArr[]               = '%d';
					}
				}

				// if a blank hash is passed, generate a new one
				if ( isset( $dataArr['zbsi_hash'] ) && $dataArr['zbsi_hash'] == '' ) {
					$dataArr['zbsi_hash'] = zeroBSCRM_generateHash( 20 );
				}
			}
		...
		private function tidy_invoice( $obj = false, $withCustomFields = false ) {
			...
			if ( isset( $obj->ID ) ) {
				...
				*/
				...
				if ( $withCustomFields ) {
					$res = $this->tidyAddCustomFields( ZBS_TYPE_INVOICE, $obj, $res, false );
				}
			}
				...
				if ( $invsettings['invdis'] == 1 ) {
					...
					if ( $discountType == 'value' ) {
						...
						if ( isset( $invoice['discount'] ) && ! empty( $invoice['discount'] ) && isset( $invoice['net'] ) ) {
							...
							if ( $invDiscount > 0 ) {
								$discountAmount = ( $invDiscount / 100 ) * $invoice['net'];
							}
			...
			if ( $invsettings['invtax'] == 1 ) {
				...
				if ( $invsettings['invpandp'] == 1 ) {
					...
					if ( $shippingV > 0 ) {
						...
						// tax on shipping - recalc.
						if ( isset( $invoice['shipping_taxes'] ) ) {
		...
		public function getInvoiceContact( $id = -1 ) {
			...
			if ( $id > 0 ) {
				...
				$contacts = $this->DAL()->contacts->getContacts(
					array(
				...
				if ( is_array( $contacts ) && isset( $contacts[0] ) ) {
					return $contacts[0];
				}
			}
		...
		public function getInvoiceCompany( $id = -1 ) {
			...
			if ( $id > 0 ) {
				...
				$companies = $this->DAL()->companies->getCompanies(
					array(
				...
				if ( is_array( $companies ) && isset( $companies[0] ) ) {
					return $companies[0];
				}
			}
		...
		public function listViewObj( $invoice = false, $columnsRequired = array() ) {
			...
			if ( is_array( $invoice ) && isset( $invoice['id'] ) ) {
				...
				if ( isset( $invoice['id_override'] ) && empty( $resArr['title'] ) ) {
					$resArr['title'] = $invoice['id_override'];
				}
				...
				$resArr['total'] = zeroBSCRM_formatCurrency( 0 );
				if ( isset( $invoice['total'] ) ) {
					$resArr['total'] = zeroBSCRM_formatCurrency( $invoice['total'] );
				}
