...
function zeroBSCRM_mailTemplate_emailPreview( $templateID = -1 ) {
	...
	if ( $templateID > 0 ) {
		...
		if ( isset( $message_content->zbsmail_body ) ) {
	...
	if ( isset( $_GET['zbsmail-template-preview'] ) && $_GET['zbsmail-template-preview'] == 1 ) {
		...
		if ( current_user_can( 'admin_zerobs_manage_options' ) ) {
			...
			if ( isset( $_GET['template_id'] ) && ! empty( $_GET['template_id'] ) ) {
				...
				// replacements
				echo $placeholder_templating->replace_placeholders(
					array( 'global', 'invoice' ),
					$html,
					array(
	...
	if ( ! empty( $quoteID ) ) {
		...
		if ( ! empty( $templatedHTML ) ) {
			...
			if ( isset( $quote ) && is_array( $quote ) ) {
				...
				$bodyHTML = $placeholder_templating->replace_placeholders(
					array( 'global', 'quote', 'contact' ),
					$bodyHTML,
					$replacements,
					array(
						ZBS_TYPE_QUOTE   => $quote,
						ZBS_TYPE_CONTACT => $quote_contact,
					)
				);
...
function zeroBSCRM_mailTemplate_getHeaders( $templateID = -1 ) {
	...
	if ( $templateID > 0 ) {
		...
		// but we use this :)
		if ( isset( $mailTemplate->zbsmail_bccto ) && ! empty( $mailTemplate->zbsmail_bccto ) ) {
			$headers[] = 'Bcc: ' . sanitize_email( $mailTemplate->zbsmail_bccto );
		}
	} else {
		$headers = array( 'Content-Type: text/html; charset=UTF-8' );
	}
