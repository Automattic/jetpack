...
( function ( global, factory ) {
	...
	let Hopscotch;
	let HopscotchBubble;
	let HopscotchCalloutManager;
	let HopscotchI18N;
	let customI18N;
	let customRenderer;
	let customEscape;
	let templateToUse = 'bubble_default';
	const Sizzle = window.Sizzle || null;
	let utils;
	let callbacks;
	let helpers;
	let winLoadHandler;
	...
	const hasJquery =
		( typeof jQuery === 'undefined' ? 'undefined' : _typeof( jQuery ) ) !== undefinedStr;
	...
	utils = {
		...
		addClass: function addClass( domEl, classToAdd ) {
			let domClasses, classToAddArr, setClass, i, len;
		...
		removeClass: function removeClass( domEl, classToRemove ) {
			let domClasses, classToRemoveArr, currClass, i, len;
		...
		getPixelValue: function getPixelValue( val ) {
			const valType = typeof val === 'undefined' ? 'undefined' : _typeof( val );
			if ( valType === 'number' ) {
				return val;
			}
			if ( valType === 'string' ) {
		...
		valOrDefault: function valOrDefault( val, valDefault ) {
			return ( typeof val === 'undefined' ? 'undefined' : _typeof( val ) ) !== undefinedStr
				? val
				: valDefault;
		},
		...
		flipPlacement: function flipPlacement( step ) {
			if ( step.isRtl && ! step._isFlipped ) {
				let props = [ 'orientation', 'placement' ],
					prop,
					i;
				if ( step.xOffset ) {
	...
	HopscotchBubble.prototype = {
		...
		_getStepI18nNum: function _getStepI18nNum( idx ) {
			const stepNumI18N = utils.getI18NString( 'stepNums' );
		...
		_handleBubbleClick: function _handleBubbleClick( evt ) {
			...
			const targetElement = evt.target || evt.srcElement;
			...
			if ( action === 'cta' ) {
				...
				if ( this.opt.isTourBubble ) {
					const currStepNum = winHopscotch.getCurrStepNum(),
						currTour = winHopscotch.getCurrTour(),
		...
		init: function init( initOpt ) {
			...
			initOpt =
				( typeof initOpt === 'undefined' ? 'undefined' : _typeof( initOpt ) ) === undefinedStr
					? {}
					: initOpt;
	...
	Hopscotch = function Hopscotch( initOptions ) {
		var self = this,
			...
			adjustWindowScroll = function adjustWindowScroll( cb ) {
				...
				// Smooth scroll to scroll target
					...
					if (
						( typeof YAHOO === 'undefined' ? 'undefined' : _typeof( YAHOO ) ) !== undefinedStr &&
					...
					// Use my crummy setInterval scroll solution if we're using plain, vanilla Javascript.
						...
						scrollIncr =
							Math.abs( windowTop - scrollToVal ) / ( getOption( 'scrollDuration' ) / 10 );
			...
			showStepHelper = function showStepHelper( stepNum ) {
				const step = currTour.steps[ stepNum ],
					bubble = getBubble(),
					targetEl = utils.getStepTarget( step );
			...
			setStateHelper = function setStateHelper() {
				let cookieVal = currTour.id + ':' + currStepNum,
		...
		this.getCalloutManager = function () {
			if (
				( typeof calloutMgr === 'undefined' ? 'undefined' : _typeof( calloutMgr ) ) === undefinedStr
			) {
		...
		this.startTour = function ( tour, stepNum ) {
			...
			if (
				( typeof stepNum === 'undefined' ? 'undefined' : _typeof( stepNum ) ) !== undefinedStr
			) {
			...
			if (
				...
				( typeof cookieTourStep === 'undefined' ? 'undefined' : _typeof( cookieTourStep ) ) !==
					undefinedStr
			) {
				currStepNum = cookieTourStep;
				if ( cookieSkippedSteps.length > 0 ) {
					for ( let i = 0, len = cookieSkippedSteps.length; i < len; i++ ) {
			...
			findStartingStep( currStepNum, skippedSteps, function ( stepNum ) {
				const target = stepNum !== -1 && utils.getStepTarget( currTour.steps[ stepNum ] );
		 ...
		 * getCurrTour
		 *
		 * @returns {Object} The currently loaded tour.
		 */
		 ...
		 * getCurrTarget
		 *
		 * @returns {Object} The currently visible target.
		 */
		 ...
		 * getCurrStepNum
		 *
		 * @returns {number} The current zero-based step number.
		 */
		 ...
		 *
		 * @returns {Array} Array of skipped step indexes
		 */
		this.getSkippedStepsIndexes = function () {
			let skippedStepsIdxArray = [],
				stepIds;
		...
		this.unlisten = function ( evtType, cb ) {
			let evtCallbacks = callbacks[ evtType ],
				i,
				len;
		...
		this.setRenderer = function ( render ) {
			const typeOfRender = typeof render === 'undefined' ? 'undefined' : _typeof( render );
	...
	const winHopscotch$1 = winHopscotch;
