...
class Task_Trigger_Test extends JPCRM_Base_Test_Case {
	...
	public function test_task_created_trigger() {
		...
		// Fake task data.
		$task_data = $this->automation_faker->task_data();

		// We expect the workflow to be executed on task_created task with the task data.
	...
	public function test_task_deleted_trigger() {
		...
		// We expect the workflow to be executed on task_deleted task with the task data.
		$workflow->expects( $this->once() )
		->method( 'execute' )
		->with(
			$this->equalTo( $trigger ),
		...
		do_action( 'jpcrm_task_delete', $task_data );
	}
	...
	public function test_task_updated_trigger() {
		...
		// We expect the workflow to be executed on task_updated task with the task data.
		$workflow->expects( $this->once() )
		->method( 'execute' )
		->with(
			$this->equalTo( $trigger ),
