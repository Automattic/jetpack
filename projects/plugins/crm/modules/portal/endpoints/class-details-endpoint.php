...
class Details_Endpoint extends Client_Portal_Endpoint {
	...
	function get_value( $fieldK, $zbsCustomer ) {
		...
		switch ( $fieldK ) {
			...
			case 'secaddr1':
				if ( isset( $zbsCustomer['secaddr_addr1'] ) ) {
					$value = $zbsCustomer['secaddr_addr1'];
				}
				break;

			case 'secaddr2':
				if ( isset( $zbsCustomer['secaddr_addr2'] ) ) {
					$value = $zbsCustomer['secaddr_addr2'];
				}
				break;

			case 'seccity':
				if ( isset( $zbsCustomer['secaddr_city'] ) ) {
					$value = $zbsCustomer['secaddr_city'];
				}
				break;

			case 'seccounty':
				if ( isset( $zbsCustomer['secaddr_county'] ) ) {
					$value = $zbsCustomer['secaddr_county'];
				}
				break;

			case 'seccountry':
				if ( isset( $zbsCustomer['secaddr_country'] ) ) {
					$value = $zbsCustomer['secaddr_country'];
				}
				break;

			case 'secpostcode':
				if ( isset( $zbsCustomer['secaddr_postcode'] ) ) {
					$value = $zbsCustomer['secaddr_postcode'];
				}
				break;
		}
			...
			<select <?php echo esc_attr( $extra_attributes ); ?> name="zbsc_<?php echo esc_attr( $fieldK ); ?>" id="<?php echo esc_attr( $fieldK ); ?>" class="form-control zbs-watch-input" autocomplete="<?php echo esc_attr( jpcrm_disable_browser_autocomplete() ); /* phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase */ ?>">
				...
				if ( isset( $options ) && count( $options ) > 0 ) {
					...
					foreach ( $options as $opt ) {
						...
						if ( isset( $value ) && strtolower( $value ) == strtolower( $opt ) ) {
							echo ' selected="selected"';
						}
