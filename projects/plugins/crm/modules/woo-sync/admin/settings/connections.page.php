...
function jpcrm_settings_page_html_woosync_connections() {
	...
	// catch pause request
	elseif ( isset( $_GET['pause'] ) && zeroBSCRM_isZBSAdminOrAdmin() ) {
	...
	elseif ( isset( $_GET['resume'] ) && zeroBSCRM_isZBSAdminOrAdmin() ) {
		...
		?>
			<div class="ui icon big warning message">
				...
				<div class="content">
					...
					<p><?php printf( __( 'Are you sure you want to disconnect the connection to the store at <br><code>%s</code>?', 'zero-bs-crm' ), $sync_sites[ $site_key_to_disconnect ]['domain'] ); ?></p>
			...
			<tfoot>
				...
				<tr>
					<td colspan="2" class="wmid" style="padding-top:1.5em">
						<button class="ui blue button" id="jpcrm-woosync-connect-to-store"><i class="plug icon"></i> 
						<?php
						if ( count( $sync_sites ) == 0 ) {
		...
		<div class="ui grid">
			<div class="twelve wide column">
				...
				if ( $site_info['last_sync_fired'] != -1 || isset( $site_info['total_order_count'] ) ) {
					...
					?>
					<div class="jpcrm-woocommerce-connection-details">
					<?php
