...
function jpcrm_settings_page_html_woosync_connection_edit() {
	...
	if ( ! isset( $sync_site ) || ! is_array( $sync_site ) ) {
		...
		<h3 style="text-align: center;" class="ui blue header"><?php printf( esc_html__( 'Edit %s WooCommerce Connection', 'zero-bs-crm' ), esc_html( $site_title ) ); ?></h3>
		
		<?php
		if ( isset( $connection_updated ) ) {
