...
function jpcrm_render_company_view_page( $id = -1 ){
  ...
  if (!empty($id) && $id > 0){
      ...
      global $zbs;
          ...
          #} PREP
          $companyEmail = ''; if (isset($company['email'])) $companyEmail = $company['email'];
          ...
          $companyTotalValue = 0; if (isset($company['total_value'])) $companyTotalValue = $company['total_value'];
          $companyQuotesValue = 0; if (isset($company['quotes_total'])) $companyQuotesValue = $company['quotes_total'];
          $companyInvoicesValue = 0; if (isset($company['invoices_total'])) $companyInvoicesValue = $company['invoices_total'];
          $companyTransactionsValue = 0; if (isset($company['transactions_total'])) $companyTransactionsValue = $company['transactions_total'];
          ...
          <div class="ui divided grid" style="margin-top:-1em;">
            ...
            <div class="ten wide column" id="zbs-company-panel">
              ...
              <?php
                ...
                // prep
                $statusStr = ''; if (isset($company) && isset($company['status']) && !empty($company['status'])) $statusStr = $company['status'];
              
                // compiled addr str
                $addrStr = ''; if (isset($company)) $addrStr = zeroBS_companyAddr($company['id'],$company,'full','<br />');
                $addr2Str = ''; if (isset($company)) $addr2Str = zeroBS_companySecondAddr($company['id'],$company,'full','<br />');

                // tels?
                $tels = array(); 
                if (isset($company) && isset($company['maintel']) && !empty($company['maintel'])) $tels['maintel'] = $company['maintel'];
                if (isset($company) && isset($company['sectel']) && !empty($company['sectel'])) $tels['sectel'] = $company['sectel'];
                ...
                <div class="ui top attached tabular menu">
                  ...
                  if (is_array($companyVitalTabs) && count($companyVitalTabs) > 0){
                    ...
                    foreach ($companyVitalTabs as $tab){
                      ...
                      ?><div data-tab="<?php echo esc_attr( $tabID ); ?>" class="item"><?php echo esc_html( $tabName ); ?></div><?php
                        ...
                        <tr>
                          ...
                          <td>
                            <?php 
                                  ...
                                  } else esc_html_e('No phone number on File',"zero-bs-crm"); ?></td>
                        </tr>
                        <tr class="wraplines">
                ...
                <!-- / company vitals -->
                  ...
                  <div id="zbs-doc-menu">
                    <div class="ui top attached tabular menu">
                      ...
                      <?php if ($useInvoices == "1"){ ?><div data-tab="invoices" class="<?php if (!isset($activeTab)) { echo 'active '; $activeTab = 'invoices'; } ?>item"><?php esc_html_e('Invoices',"zero-bs-crm"); ?></div><?php } ?>                      
                      <?php if ($useTrans == "1"){ ?><div data-tab="transactions" class="<?php if (!isset($activeTab)) { echo 'active '; $activeTab = 'transactions'; } ?>item"><?php esc_html_e('Transactions',"zero-bs-crm"); ?></div><?php } ?>
                      <div data-tab="files" class="<?php if (!isset($activeTab)) { echo 'active '; $activeTab = 'files'; } ?>item"><?php esc_html_e('Files','zero-bs-crm'); ?></div>                    
                      <?php if ($useTasks == "1"){ ?><div data-tab="tasks" class="<?php if (!isset($activeTab)) { echo 'active '; $activeTab = 'tasks'; } ?>item"><?php esc_html_e('Tasks',"zero-bs-crm"); ?></div><?php } ?>
                    </div>
                    ...
                    <div class="ui bottom attached <?php if ($activeTab == 'invoices') echo 'active '; ?>tab segment" data-tab="invoices">
                        <table class="ui celled table unstackable">
                              <thead>
                                <tr>
                                  <th><?= esc_html( $zbs->settings->get('reflabel') ) ?></th>
                                          ...
                                          <div class="ui info icon message" id="zbsNoInvoiceResults">
                                            <div class="content">
                                              ...
                                              <p>
                                                ...
                                                echo wp_kses( sprintf(__('This %s does not have any invoices yet. Do you want to <a href="%s">create one</a>?',"zero-bs-crm"),jpcrm_label_company(), esc_url( $new_invoice_url ) ), $zbs->acceptable_restricted_html );
                                                ?>
                                              </p>
                                            </div>
                                          </div>
                    ...
                    <div class="ui bottom attached <?php if ($activeTab == 'transactions') echo 'active '; ?>tab segment" data-tab="transactions">
                        ...
                        <table class="ui celled table unstackable">
                              <thead>
                                ...
                                if (in_array('id', $activeTransactionColumns)) echo '<th>'.esc_html( zeroBS_objDraw_transactionColumnHeader('id') ).'</th>';

                                foreach ($activeTransactionColumns as $col){ 

                                  // id pulled out above
                                  if ($col != 'id') echo '<th>'.esc_html( zeroBS_objDraw_transactionColumnHeader($col) ).'</th>'; 
                                  ...
                                  foreach($company['transactions'] as $zbsTransaction){
                                    ...
                                    echo "<tr>";
                                      ...
                                      // for now quickly generated in php for this freelance.
                                        ...
                                        // for now, pick out id so always on left
                                        if (in_array('id', $activeTransactionColumns)) echo '<td>'. wp_kses( zeroBS_objDraw_transactionColumnTD('id',$zbsTransaction), $zbs->acceptable_restricted_html ) .'</td>';
                                          ...
                                          <div class="ui info icon message" id="zbsNoTransactionResults">
                                            <div class="content">
                                              ...
                                              <p>
                                                <?php
                                                //prefill doesn't yet exist for companies
                                                echo wp_kses( sprintf(__('This %s does not have any transactions yet. Do you want to <a href="%s">create one</a>?',"zero-bs-crm"),jpcrm_label_company(), esc_url( $new_transaction_url ) ), $zbs->acceptable_restricted_html );
                                                ?>
                                              </p>
                                            </div>
                                          </div>
                                  ...
                                  <td colspan="4">
                                      <div class="ui info icon message" id="zbsNoFileResults">
                                        <div class="content">
                                          ...
                                          <p>
                                            <?php
                                            echo wp_kses( sprintf(__('This %s does not have any files yet. Do you want to <a href="%s">upload one</a>?',"zero-bs-crm"),jpcrm_label_company(), esc_url( $new_file_url ) ), $zbs->acceptable_restricted_html );
                                            ?>
                                          </p>
                                        </div>
                                      </div>
                                            ...
                                            <div class="ui info icon message" id="zbsNoTaskResults">
                                              <div class="content">
                                                ...
                                                <p>
                                                  <?php
                                                  //prefill doesn't yet exist for companies
                                                  echo wp_kses( sprintf(__('This %s does not have any tasks yet. Do you want to <a href="%s">create one</a>?',"zero-bs-crm"),jpcrm_label_company(), esc_url( $new_task_url )), $zbs->acceptable_restricted_html );
                                                  ?>
                                                </p>
                                              </div>
                                            </div>
