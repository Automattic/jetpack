...
<div class="ui vertical fluid menu" id="zbs-settings-menu">
    ...
    <div class="branding item" id="zbs-settings-head-tour">
        <?= esc_html__('CRM Settings', 'zero-bs-crm' ) ?>
    </div>
    ...
    <?php foreach( $sortedTabs as $tab => $tabArr ): ?>
        ...
        <?php if ( isset( $tabArr['submenu'] ) && count( $tabArr['submenu'] ) > 0 ){ ?>
            <div class="item">
                <a class='item zbs-settings-head <?= ( $tab == $current ) ? ' active' : '' ?>>' href='?page=<?= esc_attr( $zbs->slugs['settings'] ) ?>&tab=<?= esc_attr( $tab ) ?>'><?= esc_html( $ico.$name ) ?></a>
                <div class="menu">
                <?php foreach ( $tabArr['submenu'] as $tab2 => $tabArr2 ){ ?>
                    ...
                    <?php
                        ...
                        $name2 = $tabArr2['name'];
                        $new = '';
                        if (in_array($tab2, $tabsNew)) $new = '<span class="ui label green tiny">New</span>';
                    ...
                    <a class='item <?= esc_attr( $class ) ?>' href='<?= esc_url( $url ) ?>'><?= esc_html( $new.$ico2.$name2 ) ?></a>
        ...
        <?php } else { ?>
            ...
            <a class='item <?= ( $tab == $current ) ? ' active' : '' ?>' href='?page=<?= esc_attr( $zbs->slugs['settings'] ) ?>&tab=<?= esc_attr( $tab ) ?>'><?= esc_html( $ico.$name ) ?></a>

        <?php } ?>

    <?php endforeach ?>
    <?php ##WLREMOVE ?>
    <a class="item" href="<?= jpcrm_esc_link( $zbs->slugs['extensions'] ); ?>"><i class="ui orange puzzle piece icon"></i> <?= esc_html__( 'Extensions','zero-bs-crm' ) ?></a>
    <?php ##/WLREMOVE ?>
</div>