			...
			// checks out, send a test :)
				...
				if ($sent){
					...
					$existing_mail_delivery_methods = zeroBSCRM_getSetting('smtpaccs');
						...
						if (count($existing_mail_delivery_methods) == 1) $zbs->settings->update('smtpaccsdef',$settingsKey);
...
function jpcrm_ajax_mail_delivery_validate_api_oauth(){
	...
	// Retrieve parameters
	$send_from_name = '';  if ( isset( $_POST['send_from_name'] ) ) $send_from_name = sanitize_text_field( $_POST['send_from_name'] );
	$send_from_email = ''; if ( isset( $_POST['send_from_email'] ) ) $send_from_email = sanitize_text_field( $_POST['send_from_email'] );
	$oauth_provider = '';  if ( isset( $_POST['oauth_provider'] ) ) $oauth_provider = sanitize_text_field( $_POST['oauth_provider'] );
	...
	if ( $zbs->oauth->legitimate_provider( $oauth_provider ) ){
		...
		if ( !empty( $send_from_name ) ){
			...
			if ( !empty( $send_from_email ) && zeroBSCRM_validateEmail( $send_from_email ) ){
			    ...
			    if ( jpcrm_mail_delivery_send_via_gmail_oauth( array(
