...
$zeroBSCRM_killDenied = true; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
					...
					<div class="col-xs-12">
						<div class="col-md-12">
							...
							<div class="wizopt">
								<label>
									<?php
									esc_html_e( jpcrm_label_company() . ' Name / CRM Name:', 'zero-bs-crm' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
									?>
								</label>
							...
							<div class="wizopt">
								...
								<select class='form-control' id='zbs_crm_curr' name='zbs_crm_curr'>
									...
									if ( isset( $settings['currency'] ) && isset( $settings['currency']['strval'] ) && $settings['currency']['strval'] ) {
										$current_setting = $settings['currency']['strval'];
									}

									if ( empty( $current_setting ) ) {

										$locale = get_locale(); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- legacy usage
									...
									foreach ( $whwpCurrencyList as $currency_obj ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
										?>
										<option value="<?php echo esc_attr( $currency_obj[1] ); ?>"
											<?php
											if ( $current_setting === $currency_obj[1] ) {
												echo ' selected="selected"';}
											?>
										><?php echo esc_html( $currency_obj[0] . ' (' . $currency_obj[1] . ')' ); ?></option>
										<?php
									}
									?>
								</select>
							...
							<div class="wizopt" style="display:none">
								...
								<p>
									<?php
									esc_html_e( "Jetpack CRM an run in 'Business to business' (B2B) Mode, which allows you to manage 'Contacts' under '" . jpcrm_label_company( true ) . "', instead of just 'Contacts'. For most small businesses and freelancers, this isn't necessary.", 'zero-bs-crm' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
									?>
								</p>
