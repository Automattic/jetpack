...
async function insertPayPalBlock( page ) {
	...
	await page.locator( 'input[placeholder="Search"]' ).fill( 'PayPal' );
	await page.locator( 'button.editor-block-list-item-jetpack-paypal-payment-buttons' ).click();
...
async function publishPost( page ) {
	await page.locator( 'button.editor-post-publish-button__button' ).click();
...
test.describe( 'PayPal Payment Buttons Block', () => {
	...
	test.describe( 'Legacy Block Compatibility', () => {
		...
		test( 'legacy paste-code block shows read-only indicator in editor', async ( { page } ) => {
			...
			await page.locator( 'button[role="menuitemradio"]:has-text("Code editor")' ).click();
			...
			await page.locator( 'button[aria-label="Options"]' ).click();
			await page.locator( 'button[role="menuitemradio"]:has-text("Visual editor")' ).click();
	...
	test.describe( 'Format Switcher', () => {
		test( 'format switcher shows 3 options in creation form', async ( { page } ) => {
			...
			await expect(
				block.locator( '.jetpack-paypal-payment-buttons__format-switcher option[value="BUTTON"]' )
			).toBeVisible();
			await expect(
				block.locator( '.jetpack-paypal-payment-buttons__format-switcher option[value="LINK"]' )
			).toBeVisible();
			await expect(
				block.locator( '.jetpack-paypal-payment-buttons__format-switcher option[value="QR"]' )
			).toBeVisible();
		} );
		...
		test( 'default format is Button', async ( { page } ) => {
			...
			await expect(
				block.locator( '.jetpack-paypal-payment-buttons__format-switcher select' )
			).toHaveValue( 'BUTTON' );
		...
		test( 'selecting Link format selects the Link option', async ( { page } ) => {
			await setupPayPalMocks( page );
			await goToNewPost( page );
			const canvas = await insertPayPalBlock( page );

			const block = canvas.locator( '.wp-block-jetpack-paypal-payment-buttons' );
			const switcher = block.locator( '.jetpack-paypal-payment-buttons__format-switcher select' );
		...
		test( 'selecting QR code format selects the QR option', async ( { page } ) => {
			await setupPayPalMocks( page );
			await goToNewPost( page );
			const canvas = await insertPayPalBlock( page );
		...
		test( 'format switcher appears in InspectorControls after creation', async ( { page } ) => {
			...
			const formatPanel = page.locator( 'button[aria-label="Styles"]' );
		...
		test( 'frontend renders Link format as anchor tag', async ( { page } ) => {
			...
			// Frontend: should render a plain anchor link, not a full button card.
			const paypalLink = page.locator( '.jetpack-paypal-button__paypal-link' );
			await expect( paypalLink ).toBeVisible();
			const href = await paypalLink.getAttribute( 'href' );
			expect( href ).toContain( 'paypal.com' );

			// Should NOT render the full button card.
			await expect( page.locator( '.jetpack-paypal-button__checkout-link' ) ).toBeHidden();
		} );

		test( 'frontend renders QR Code format with standalone canvas', async ( { page } ) => {
			await setupPayPalMocks( page );
			await goToNewPost( page );
			const canvas = await insertPayPalBlock( page );
		...
		test( 'changing format does not require recreating the PayPal product', async ( { page } ) => {
			...
			if ( await formatPanel.isVisible( { timeout: 2000 } ).catch( () => false ) ) {
				...
				const switcher = page.locator( '.jetpack-paypal-payment-buttons__format-switcher select' );
