				...
				if ( isDown || isUp ) {
					if ( isDown ) {
						if ( isSearchFocused ) {
							...
							this.selected &&
							this.selected.nextAll( '.widget-tpl:visible' ).length !== 0
						) {
						...
						if ( isSearchFocused ) {
							...
							this.selected &&
							this.selected.prevAll( '.widget-tpl:visible' ).length !== 0
						) {
	...
	api.Widgets.WidgetControl = api.Control.extend(
		/** @lends wp.customize.Widgets.WidgetControl.prototype */ {
			...
			_setupReorderUI: function () {
				 ...
				 */
				this.container
					.find( '.widget-title-action' )
					.after( $( api.Widgets.data.tpl.widgetReorderNav ) );
				 ...
				 */
				this.container
					.find( '.widget-area-select' )
					.on( 'click keypress', 'li', function ( event ) {
						if ( event.type === 'keypress' && event.which !== 13 && event.which !== 32 ) {
							return;
						}
						event.preventDefault();
					...
					_( removedWidgetIds ).each( function ( removedWidgetId ) {
						...
						setTimeout( function () {
							...
							if ( api.Widgets.savedWidgetIds[ removedWidgetId ] ) {
								...
								api.value( 'sidebars_widgets[wp_inactive_widgets]' )(
									_( inactiveWidgets ).unique()
								);
							}
