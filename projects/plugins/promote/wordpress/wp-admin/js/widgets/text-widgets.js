...
wp.textWidgets = ( function ( $ ) {
	...
	component.TextWidgetControl = Backbone.View.extend(
		/** @lends wp.textWidgets.TextWidgetControl.prototype */ {
			/**
			 * View events.
			 *
			 * @type {Object}
			...
			initialize: function initialize( options ) {
				...
				control.customHtmlWidgetPointer = control.$el.find(
			...
			updateFields: function updateFields() {
				...
				if ( control.fields.text.is( ':visible' ) ) {
					...
					control.editor &&
					! control.editorFocused &&
					syncInput.val() !== control.fields.text.val()
				) {
			...
			initializeEditor: function initializeEditor() {
				...
				triggerChangeIfDirty = function () {
					...
					if ( control.editor.isDirty() ) {
						...
						if ( wp.customize && wp.customize.state ) {
							wp.customize
								.state( 'processing' )
								.set( wp.customize.state( 'processing' ).get() + 1 );
							_.delay( function () {
								wp.customize
									.state( 'processing' )
									.set( wp.customize.state( 'processing' ).get() - 1 );
				...
				control.syncContainer
					.closest( '.widget' )
					.find( '[name=savewidget]:first' )
					.on( 'click', function onClickSaveButton() {
						triggerChangeIfDirty();
					} );
				...
				function buildEditor() {
					...
					onInit = function () {
						...
						editor.on( 'pastepreprocess', function ( event ) {
							...
							if (
								-1 !== component.dismissedPointers.indexOf( 'text_widget_paste_html' ) ||
								! content ||
								! /&lt;\w+.*?&gt;/.test( content )
							) {
								return;
