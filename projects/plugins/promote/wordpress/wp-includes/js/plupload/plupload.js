...
( function ( window, o, undef ) {
	...
	*/
		...
		xmlEncode: function ( str ) {
			...
			return str
				? ( '' + str ).replace( xmlEncodeRegExp, function ( chr ) {
	 ...
	 */
		...
		function calc() {
			...
			if ( total.size === undef ) {
				...
				total.bytesPerSec = Math.ceil(
					total.loaded / ( ( +new Date() - startTime || 1 ) / 1000.0 )
				);
		...
		function onUploadFile( up, file ) {
			...
			if (
				up.settings.resize.enabled &&
				runtimeCan( blob, 'send_binary_string' ) &&
				!! ~o.inArray( blob.type, [ 'image/jpeg', 'image/png' ] )
			) {
				// Resize if required
		...
		plupload.extend( this, {
			...
			splice: function ( start, length ) {
				...
				var removed = files.splice(
					start === undef ? 0 : start,
					length === undef ? files.length : length
				);
