( function ( tinymce ) {
		...
		tinymce.ui.Control.extend( {
			renderHtml: function () {
				return (
					...
					tinymce.translate( 'Paste URL or type to search' ) +
					'" />' +
	...
	tinymce.PluginManager.add( 'wplink', function ( editor ) {
		...
		var speak =
			typeof window.wp !== 'undefined' && window.wp.a11y && window.wp.a11y.speak
		...
		editor.on( 'preinit', function () {
			if ( editor.wp && editor.wp._createToolbar ) {
				...
				var editButtons = [ 'wp_link_input', 'wp_link_apply' ];
		...
		editor.on( 'keydown', function ( event ) {
			...
			if (
				event.altKey ||
				( tinymce.Env.mac && ( ! event.metaKey || event.ctrlKey ) ) ||
				...
				if ( $ && $.ui && $.ui.autocomplete ) {
					...
					$input
						...
						.autocomplete( {
							...
							messages: {
								noResults:
									typeof window.uiAutocompleteL10n !== 'undefined'
						...
						var fallbackTitle =
								typeof window.wpLinkL10n !== 'undefined' ? window.wpLinkL10n.noTitle : '',
