...
this[ 'wp' ][ 'blockDirectory' ] = /******/ ( function ( modules ) {
	...
	/******/ {
		...
		/***/ '7f3f': /***/ function ( module, __webpack_exports__, __webpack_require__ ) {
			'use strict';
			// ESM COMPAT FLAG
			...
			__webpack_require__.d( resolvers_namespaceObject, 'getDownloadableBlocks', function () {
			...
			const blockManagement = function () {
				...
				switch ( action.type ) {
					case 'ADD_INSTALLED_BLOCK_TYPE':
						return { ...state, installedBlockTypes: [ ...state.installedBlockTypes, action.item ] };
					...
					case 'SET_INSTALLING_BLOCK':
						return {
							...state,
							isInstalling: { ...state.isInstalling, [ action.blockId ]: action.isInstalling },
						};
				}
			...
			function selectors_isRequestingDownloadableBlocks( state, filterValue ) {
				...
				return ( _state$downloadableBl =
					( _state$downloadableBl2 = state.downloadableBlocks[ filterValue ] ) === null ||
					_state$downloadableBl2 === void 0
						? void 0
						: _state$downloadableBl2.isRequesting ) !== null && _state$downloadableBl !== void 0
			...
			function selectors_getDownloadableBlocks( state, filterValue ) {
				...
				return ( _state$downloadableBl3 =
					( _state$downloadableBl4 = state.downloadableBlocks[ filterValue ] ) === null ||
					_state$downloadableBl4 === void 0
						? void 0
						: _state$downloadableBl4.results ) !== null && _state$downloadableBl3 !== void 0
			...
			const getNewBlockTypes = Object( external_wp_data_[ 'createRegistrySelector' ] )(
				select => state => {
			...
			const getUnusedBlockTypes = Object( external_wp_data_[ 'createRegistrySelector' ] )(
				select => state => {
					const usedBlockTree = select( external_wp_blockEditor_[ 'store' ] ).getBlocks();
					const installedBlockTypes = getInstalledBlockTypes( state );
			...
			const actions_installBlockType = block => async _ref => {
				...
				try {
					...
					dispatch.addInstalledBlockType( { ...block, links: { ...block.links, ...links } } );
					await loadAssets();
					...
					registry.dispatch( external_wp_notices_[ 'store' ] ).createInfoNotice(
					...
					let message =
						error.message || Object( external_wp_i18n_[ '__' ] )( 'An error occurred.' ); // Errors we throw are fatal
			...
			const actions_uninstallBlockType = block => async _ref2 => {
				...
				try {
					...
					registry
						...
						.createErrorNotice(
							error.message || Object( external_wp_i18n_[ '__' ] )( 'An error occurred.' )
						);
				}
			};
			/**
				...
				try {
					...
					const blocks = results.map( result =>
						Object( external_lodash_[ 'mapKeys' ] )( result, ( value, key ) =>
							Object( external_lodash_[ 'camelCase' ] )( key )
						)
					);
			...
			function AutoBlockUninstaller() {
				const { uninstallBlockType } = Object( external_wp_data_[ 'useDispatch' ] )( store );
				const shouldRemoveBlockTypes = Object( external_wp_data_[ 'useSelect' ] )( select => {
					const { isAutosavingPost, isSavingPost } = select( external_wp_editor_[ 'store' ] );
						...
						hasNotice
							...
							: Object( external_wp_element_[ 'createElement' ] )(
									...
									Object( external_wp_element_[ 'createElement' ] )(
										...
										!! statusText
											? statusText
											: Object( external_wp_htmlEntities_[ 'decodeEntities' ] )( description )
									),
									isInstallable &&
			...
			function DownloadableBlocksList( _ref ) {
				let { items, onHover = external_lodash_[ 'noop' ], onSelect } = _ref;
				const composite = Object( external_wp_components_[ '__unstableUseCompositeState' ] )();
				const { installBlockType } = Object( external_wp_data_[ 'useDispatch' ] )( store );
			...
			/* harmony default export */ var downloadable_blocks_panel = Object(
				...
				Object( external_wp_data_[ 'withSelect' ] )( ( select, _ref2 ) => {
					...
					const { getDownloadableBlocks, isRequestingDownloadableBlocks } = select( store );
					const { canInsertBlockType } = select( external_wp_blockEditor_[ 'store' ] );
					const hasPermission = select( external_wp_coreData_[ 'store' ] ).canUser(
						'read',
			...
			Object( external_wp_plugins_[ 'registerPlugin' ] )( 'block-directory', {
				render() {
					return Object( external_wp_element_[ 'createElement' ] )(
						external_wp_element_[ 'Fragment' ],
						null,
						Object( external_wp_element_[ 'createElement' ] )( AutoBlockUninstaller, null ),
						Object( external_wp_element_[ 'createElement' ] )(
