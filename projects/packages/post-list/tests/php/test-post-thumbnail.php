...
/**
 * Unit tests for the Post_Thumbnail class.
 *
 * @package automattic/jetpack-post-list
 */
class Test_Post_Thumbnail extends TestCase {
	...
	public function test_return_null_when_not_in_class() {
		...
		$attachment_id = Post_Thumbnail::get_first_image_id_from_post_content( $post_content );
	...
	/**
	 * Returns ID when it is silly long.
	 */
	public function test_silly_long_id() {
		$post_content = '<img class="wp-image-99999999999999" />';

		$attachment_id = Post_Thumbnail::get_first_image_id_from_post_content( $post_content );
		$this->assertSame( 99999999999999, $attachment_id );
	}
	...
	public function test_find_id_in_the_middle() {
		$post_content = '<img class="first wp-image-2 end" />';

		$attachment_id = Post_Thumbnail::get_first_image_id_from_post_content( $post_content );
		$this->assertSame( 2, $attachment_id );
	}
	...
	public function test_find_id_on_the_end() {
		$post_content = '<img class="first wp-image-240" />';

		$attachment_id = Post_Thumbnail::get_first_image_id_from_post_content( $post_content );
		$this->assertSame( 240, $attachment_id );
	}
	...
	public function test_find_id_on_the_front() {
		$post_content = '<img class="wp-image-0 end" />';
...
<!-- /wp:gallery -->';
		...
		$attachment_id = Post_Thumbnail::get_first_image_id_from_post_content( $post_content_gallery_image );
...
<!-- /wp:jetpack/slideshow -->';
		...
		$attachment_id = Post_Thumbnail::get_first_image_id_from_post_content( $post_content_slideshow_image );
...
<!-- /wp:image -->';
		...
		$attachment_id = Post_Thumbnail::get_first_image_id_from_post_content( $post_content_image_block );
		$this->assertSame( 24, $attachment_id );
	}
}
