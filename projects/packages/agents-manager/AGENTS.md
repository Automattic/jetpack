...
- **Node `meta` is a client contract**: `menu_title` is the label, because `title` is wp-admin markup — keep it unescaped, since core escapes it again for the group `aria-label`. `icon` is a glyph *name*, the same key `get_icon()` uses; consumers supply anything `@wordpress/icons` lacks (`help`, `ask-ai`, any custom glyph). `route` is the in-app destination for panel items with no `href`. `html` is wp-admin only.
