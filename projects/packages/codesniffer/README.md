...
This ruleset is intended to be used with PHPCompatibiity's `testVersion` set to `7.4-`.
...
Note this isn't a perfect replacement for raising `testVersion`, as it cannot _add_ PHPCompatibility rules that do not trigger when `testVersion` indicates support for PHP 7.4.
For example, `Jetpack-Compat-83` disables the `PHPCompatibility.Classes.NewTypedConstants.Found` rule ("Typed constants are not supported in PHP 8.2 or earlier") but cannot enable more specific rules like `PHPCompatibility.Classes.NewTypedConstants.InvalidType` ("%s is not supported as a type declaration for constants") that would be triggered if `testVersion` were set to `8.3-`.
