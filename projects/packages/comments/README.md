...
A signed-in reader's choices are saved as they are made, and the tray shows what they already have. The browser has no WordPress.com session to ask with, so it asks the site, through the `jetpack_comments_subscriptions` admin-ajax action, and the site asks WordPress.com over its blog connection: `wpcom/v2/sites/{id}/comments/subscriptions`, served in process on Simple. The site names the reader by email. A popup sign-in carries a signature WordPress.com issued over that email at the exchange, kept in the passport, and a Simple site's login is the WordPress.com user, so either is acted on at once. A site login elsewhere has no such proof, so WordPress.com confirms it by email first, and the tray shows the request as on until then.
