 ...
 * Jetpack Comments.
 *
 * Loads the comment experience on WordPress.com and Jetpack sites.
 *
...
use Automattic\Jetpack\Comments\Blocks\Editor;
use Automattic\Jetpack\Comments\Blocks\Utils;
use Automattic\Jetpack\Status\Host;
use WP_Error;

/**
 * Renders the comment form and handles the comments submitted through it.
 *
...
class Comment_Form {
	...
	public function __construct() {
		...
		// Add the comment form app.
		add_action( 'comment_form_must_log_in_after', array( $this, 'render_element' ) );
		add_filter( 'comment_form_submit_field', array( $this, 'render_element' ) );
		...
		add_action( 'comment_post', array( $this, 'add_comment_meta_data' ) );
		...
		// Filter to ensure JetpackScriptData.site.host and is_wpcom_platform is set, to ensure Jetpack blocks work as expected via the comment form.
	...
	/**
	 * Load the div where the app is rendered.
	 */
	public function render_element() {
		...
		$element = '<div class="comment-form__verbum ' . esc_attr( $color_scheme ) . '"></div>' . $this->hidden_fields();

		// If the blog requires login, the app needs to be wrapped in a <form> to work.
		// The app is given `mustLogIn` to handle the login flow.
		if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
			printf(
				'<form action="%s" method="post" id="commentform" class="comment-form">%s</form>',
				esc_url( $comment_url ),
				$element // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Markup built above from escaped values and hidden_fields().
			);
		} else {
			return $element;
		}
	...
	public function enqueue_assets() {
		if ( ! Utils::should_show_comments() && ! $this->should_enqueue_assets ) {
			return;
		}
		...
		$hovercard_i18n = array(
			'Edit your profile →'      => __( 'Edit your profile →', 'jetpack-comments' ),
			'View profile →'           => __( 'View profile →', 'jetpack-comments' ),
			'Contact'                  => __( 'Contact', 'jetpack-comments' ),
			'Send money'               => __( 'Send money', 'jetpack-comments' ),
			'Gravatar not found.'      => __( 'Gravatar not found.', 'jetpack-comments' ),
			'This profile is private.' => __( 'This profile is private.', 'jetpack-comments' ),
			'Too Many Requests.'       => __( 'Too Many Requests.', 'jetpack-comments' ),
			'Internal Server Error.'   => __( 'Internal Server Error.', 'jetpack-comments' ),
			'Sorry, we are unable to load this Gravatar profile.' => __( 'Sorry, we are unable to load this Gravatar profile.', 'jetpack-comments' ),
		);
		...
		wp_add_inline_script(
			...
			'window.VerbumComments = ' . wp_json_encode(
				array(
					'Log in or provide your name and email to leave a reply.' => __( 'Log in or provide your name and email to leave a reply.', 'jetpack-comments' ),
					'Log in or provide your name and email to leave a comment.' => __( 'Log in or provide your name and email to leave a comment.', 'jetpack-comments' ),
					'Receive web and mobile notifications for posts on this site.' => __( 'Receive web and mobile notifications for posts on this site.', 'jetpack-comments' ),
					'Name'                               => __( 'Name', 'jetpack-comments' ),
					'Email (address never made public)'  => __( 'Email (address never made public)', 'jetpack-comments' ),
					'Website (optional)'                 => __( 'Website (optional)', 'jetpack-comments' ),
					'Leave a reply. (log in optional)'   => __( 'Leave a reply. (log in optional)', 'jetpack-comments' ),
					'Leave a comment. (log in optional)' => __( 'Leave a comment. (log in optional)', 'jetpack-comments' ),
					'Log in to leave a reply.'           => __( 'Log in to leave a reply.', 'jetpack-comments' ),
					'Log in to leave a comment.'         => __( 'Log in to leave a comment.', 'jetpack-comments' ),
					/* translators: %s is the name of the provider (WordPress, Facebook, Twitter) */
					'Logged in via %s'                   => __( 'Logged in via %s', 'jetpack-comments' ),
					'Log out'                            => __( 'Log out', 'jetpack-comments' ),
					'Your browser is blocking the cookies needed to log in and comment here. Allow cookies in your privacy settings, then reload the page.' => __( 'Your browser is blocking the cookies needed to log in and comment here. Allow cookies in your privacy settings, then reload the page.', 'jetpack-comments' ),
					'Email'                              => __( 'Email', 'jetpack-comments' ),
					'(Address never made public)'        => __( '(Address never made public)', 'jetpack-comments'), // phpcs:ignore PEAR.Functions.FunctionCallSignature.SpaceBeforeCloseBracket
					'Instantly'                          => __( 'Instantly', 'jetpack-comments' ),
					'Daily'                              => __( 'Daily', 'jetpack-comments' ),
					'Reply'                              => __( 'Reply', 'jetpack-comments' ),
					'Comment'                            => __( 'Comment', 'jetpack-comments' ),
					'WordPress'                          => __( 'WordPress', 'jetpack-comments' ),
					'Weekly'                             => __( 'Weekly', 'jetpack-comments' ),
					'Notify me of new posts'             => __( 'Notify me of new posts', 'jetpack-comments' ),
					'Email me new posts'                 => __( 'Email me new posts', 'jetpack-comments' ),
					'Email me new comments'              => __( 'Email me new comments', 'jetpack-comments' ),
					'Cancel'                             => __( 'Cancel', 'jetpack-comments' ),
					'Write a comment...'                 => __( 'Write a comment...', 'jetpack-comments' ),
					'Write a reply...'                   => __( 'Write a reply...', 'jetpack-comments' ),
					...
					'We\'ll keep you in the loop!'       => __( 'We\'ll keep you in the loop!', 'jetpack-comments' ),
					'Loading your comment...'            => __( 'Loading your comment...', 'jetpack-comments' ),
					/* translators: %s is the name of the site */
					'Discover more from'                 => sprintf( __( 'Discover more from %s', 'jetpack-comments' ), html_entity_decode( get_bloginfo( 'name' ), ENT_QUOTES ) ),
					'Subscribe now to keep reading and get access to the full archive.' => __( 'Subscribe now to keep reading and get access to the full archive.', 'jetpack-comments' ),
					'Continue reading'                   => __( 'Continue reading', 'jetpack-comments' ),
					'Never miss a beat!'                 => __( 'Never miss a beat!', 'jetpack-comments' ),
					'Interested in getting blog post updates? Simply click the button below to stay in the loop!' => __( 'Interested in getting blog post updates? Simply click the button below to stay in the loop!', 'jetpack-comments' ),
					'Enter your email address'           => __( 'Enter your email address', 'jetpack-comments' ),
					'Subscribe'                          => __( 'Subscribe', 'jetpack-comments' ),
					'Comment sent successfully'          => __( 'Comment sent successfully', 'jetpack-comments' ),
					'Save my name, email, and website in this browser for the next time I comment.' => __( 'Save my name, email, and website in this browser for the next time I comment.', 'jetpack-comments' ),
					...
					'allowedBlocks'                      => Utils::get_allowed_blocks(),
	...
	public function comment_form_defaults( $args ) {
		$title_reply_default = __( 'Leave a comment', 'jetpack-comments' );
		...
		return array_merge(
			...
			array(
				...
				'title_reply_to'       => __( 'Leave a reply to %s', 'jetpack-comments' ),
				'cancel_reply_link'    => __( 'Cancel reply', 'jetpack-comments' ),
...
HTML;
	...
	public static function verify_facebook_identity() {
		...
		if ( empty( $data['access_token'] ) ) {
			return new WP_Error( 'facebook', __( 'Error: your Facebook login has expired.', 'jetpack-comments' ) );
		}
	...
	public function check_comment_allowed( int $comment_id ) {
		...
		wp_die( esc_html__( 'Sorry, this comment could not be posted.', 'jetpack-comments' ) );
	}
	 ...
	 * @param int $comment_id The comment ID.
	 */
	public function add_comment_meta_data( $comment_id ) {
		$comment_meta = array();
		...
		$loaded_editor                       = isset( $_POST['verbum_loaded_editor'] ) ? sanitize_text_field( wp_unslash( $_POST['verbum_loaded_editor'] ) ) : '';
		$subscription_modal_show             = isset( $_POST['verbum_show_subscription_modal'] ) && in_array( $_POST['verbum_show_subscription_modal'], $allowed_subscription_modal_statuses, true ) ? sanitize_text_field( wp_unslash( $_POST['verbum_show_subscription_modal'] ) ) : '';
		...
		if ( in_array( $loaded_editor, $allowed_comments_sources, true ) ) {
			bump_stats_extras( 'verbum-comment-editor', $loaded_editor );
		}
		if ( $subscription_modal_show ) {
			bump_stats_extras( 'verbum-subscription-modal', $subscription_modal_show );
		}
		switch ( $service ) {
			case 'facebook':
