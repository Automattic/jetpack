<?php
/**
 * Comment settings on the Discussion screen.
 *
...
/**
 * Settings is responsible for additional admin settings for comments.
 *
...
class Settings {
	...
	protected function setup_globals() {
		...
		$this->default_commenting = true;

		// Default allow blocks in comments
		$this->default_allow_blocks = true;

		// Default option values.
		$this->default_greeting = __( 'Leave a Reply', 'jetpack-comments' );
		...
		$this->color_schemes = array(
			'light'       => __( 'Light', 'jetpack-comments' ),
			'dark'        => __( 'Dark', 'jetpack-comments' ),
			'transparent' => __( 'Transparent', 'jetpack-comments' ),
		);
	}
	...
	public function add_settings() {
		...
		add_settings_field(
			...
			__( 'Jetpack Comments', 'jetpack-comments' ),
			array( $this, 'commenting_field' ),
			'discussion'
		);
		...
		register_setting(
			'discussion',
			'enable_verbum_commenting',
			array( $this, 'enable_commenting_sanitize' )
		);

		add_settings_field(
			'enable_blocks_comments',
			__( 'Allow Blocks', 'jetpack-comments' ),
		...
		add_settings_field(
			...
			__( 'Greeting Text', 'jetpack-comments' ),
			array( $this, 'comment_form_greeting_setting' ),
			'discussion',
			'jetpack_comment_form',
			array( 'label_for' => 'jetpack-comment-form-greeting' )
		);
		...
		add_settings_field(
			...
			__( 'Color Scheme', 'jetpack-comments' ),
	...
	public function comment_form_settings_section() {
		...
		<p id="jetpack-comments-settings"><?php esc_html_e( 'Adjust your Comments form with a clever greeting and color-scheme.', 'jetpack-comments' ); ?></p>

		<?php
	}

	/**
	 * Prints HTML for the commenting setting
	 */
	public function commenting_field() {
		?>
			<label><input name="enable_verbum_commenting" type="checkbox" <?php checked( $this->is_commenting_enabled(), true, true ); ?> value="1" />
				<?php esc_html_e( 'Let visitors use a WordPress.com or Facebook account to comment', 'jetpack-comments' ); ?>
			</label>
		<?php
	}

	/**
	 * Prints HTML for the allow-blocks setting
	 */
	...
	public function is_commenting_enabled() {
		return (bool) get_option( 'enable_verbum_commenting', $this->default_commenting );
	}

	/**
	 * Are blocks in comments enabled?
	 *
	 * @return boolean
	 */
	...
	public function comment_form_greeting_setting() {
		...
		<p class="description"><?php esc_html_e( 'A few catchy words to motivate your readers to comment', 'jetpack-comments' ); ?></p>
	...
	public function comment_form_color_scheme_setting() {
		...
		<fieldset>
			<legend class="screen-reader-text"><?php esc_html_e( 'Color Scheme', 'jetpack-comments' ); ?></legend>

			<?php foreach ( $this->color_schemes as $key => $label ) : ?>

				<label>
					<input type="radio" name="jetpack_comment_form_color_scheme" value="<?php echo esc_attr( $key ); ?>" <?php checked( $scheme, $key ); ?>>
	...
	/**
	 * Sanitize the commenting setting
	 *
	 * @param string $val The commenting string.
	 * @return string
	 */
	public function enable_commenting_sanitize( $val ) {
		return $val ? '1' : '0';
	}
}
