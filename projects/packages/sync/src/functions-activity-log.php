 ...
 *     @type string $severity    Optional. 'info', 'warning', 'error', or 'critical'. Defaults to 'info'.
...
function jetpack_activity_log_event( array $args ) {
	...
	$content = _jetpack_activity_log_sanitize_string( $args['content'] ?? '', 5000 );

	if ( '' === trim( $source ) || '' === trim( $title ) || '' === trim( $content ) ) {
		return false;
	}

	$valid_severity = array( 'info', 'warning', 'error', 'critical' );
	$severity       = in_array( $args['severity'] ?? '', $valid_severity, true ) ? $args['severity'] : 'info';
	...
	if ( ! empty( $args['external_id'] ) ) {
		$payload['external_id'] = mb_substr( (string) $args['external_id'], 0, 100 );
	}

	if ( ! empty( $args['link'] ) ) {
		$link = esc_url_raw( (string) $args['link'] );
		if ( '' !== $link ) {
			$payload['link'] = mb_substr( $link, 0, 2000 );
		}
	}
 ...
 * @return string
 */
function _jetpack_activity_log_sanitize_string( $value, $max ) {
	return mb_substr( wp_strip_all_tags( (string) $value ), 0, $max );
}
...
/**
 * Belt-and-braces: never auto-share Activity Log entries via Jetpack Social,
...
/**
 * Belt-and-braces: never include Activity Log entries in Jetpack sitemaps,
