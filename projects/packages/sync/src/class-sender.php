...
class Sender {
	...
	public function do_sync() {
		...
		$is_dedicated_sync_request = $this->is_dedicated_sync_request();
		...
		if ( $is_dedicated_sync_request ) {
			...
			return $this->spawn_dedicated_sync( $this->sync_queue );
		}
	}
	 ...
	 */
	private function is_dedicated_sync_request() {
		...
		if ( $is_dedicated_sync_request ) {
			$is_valid = isset( $_POST['nonce'] ) &&
				wp_verify_nonce( $_POST['nonce'], 'jetpack_sync_dedicated_request_' . $this->sync_queue->id );
			if ( ! $is_valid ) {
	 ...
	 */
	public function spawn_dedicated_sync( $queue ) {
		if ( $queue->is_locked() ) {
		...
		$args = array(
			...
			'blocking'  => false, // Inspired by cron spawning.
			'timeout'   => 0.01,
