...
class Dedicated_Sender {
	...
	public static function is_dedicated_sync_request() {
		...
		$check_url = self::prepare_url_for_dedicated_request_check( esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ) );
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$check_url = self::prepare_url_for_dedicated_request_check( esc_url_raw( wp_unslash( $_GET['rest_route'] ) ) );
		if ( strpos( $check_url, 'jetpack/v4/sync/spawn-sync' ) !== false ) {
			return true;
		}
