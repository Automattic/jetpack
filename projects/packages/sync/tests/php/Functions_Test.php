...
class Functions_Test extends BaseTestCase {
	 ...
	 * @param int    $expected The expected callable value.
	 */
	#[DataProvider( 'provide_stored_blog_public_values' )]
	public function test_get_site_visibility_returns_stored_value( $stored, $expected ) {
		update_option( 'blog_public', $stored );

		$this->assertSame( $expected, Functions::get_site_visibility() );
	}

	/**
	 * Data provider for test_get_site_visibility_returns_stored_value.
	 *
	 * @return array[]
	 */
	 ...
	 */
	public function test_get_site_visibility_defaults_to_public() {
		delete_option( 'blog_public' );

		$this->assertSame( 1, Functions::get_site_visibility() );
	}
	 ...
	 */
	#[DataProvider( 'provide_non_private_blog_public_values' )]
	public function test_get_site_visibility_is_private_when_filtered( $stored ) {
		update_option( 'blog_public', $stored );
	...
	/**
	 * Data provider for test_get_site_visibility_is_private_when_filtered.
	 *
	 * @return array[]
	 */
	 ...
	 */
	public function test_site_visibility_is_a_default_callable() {
		$this->assertSame(
			array( 'Automattic\\Jetpack\\Sync\\Functions', 'get_site_visibility' ),
			Defaults::get_callable_whitelist()['jetpack_site_visibility']
		);
	}
}
