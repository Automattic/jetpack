...
class Activity_Log_Event_Test extends BaseTestCase {
	...
	/**
	 * Tests that the `extra` payload accepts flat scalar values and persists them.
	 */
	public function test_activity_log_event_persists_extra_scalars() {
		$post_id = Activity_Log_Event::create(
			array(
				'title'   => 'Podcast show launched',
				'content' => 'First episode published.',
		...
		$this->assertSame( 123, $payload['extra']['post_id'] );
		$this->assertSame( 'https://example.com/?p=123', $payload['extra']['post_url'] );
		$this->assertSame( 'Hello world', $payload['extra']['post_title'] );
		$this->assertTrue( $payload['extra']['flagged'] );
	}

	/**
	 * Tests that non-scalar `extra` values are dropped, while valid neighbours stay.
	 */
	public function test_activity_log_event_drops_non_scalar_extra_values() {
		$post_id = Activity_Log_Event::create(
			array(
				'title'   => 'Cache flushed',
				'content' => 'Plain text note.',
				'extra'   => array(
					'post_id'  => 7,
					'nested'   => array( 'bad' => true ),
					'obj'      => new \stdClass(),
					42         => 'numeric-key',
					'sanitize' => " <b>Title</b>\x00",
				),
			)
		);
		...
		$this->assertArrayHasKey( 'extra', $payload );
		$this->assertSame( 7, $payload['extra']['post_id'] );
		$this->assertSame( 'Title', $payload['extra']['sanitize'] );
		$this->assertArrayNotHasKey( 'nested', $payload['extra'] );
		$this->assertArrayNotHasKey( 'obj', $payload['extra'] );
		$this->assertArrayNotHasKey( '42', $payload['extra'] );
	}

	/**
	 * Tests that `extra` is omitted from the payload when empty or non-array.
	 */
	public function test_activity_log_event_omits_empty_extra() {
		$post_id = Activity_Log_Event::create(
			array(
				'title'   => 'Cache flushed',
				'content' => 'Plain text note.',
	...
	/**
	 * Tests that the `extra` payload caps at MAX_EXTRA_ENTRIES.
	 */
	public function test_activity_log_event_caps_extra_entries() {
		$extra = array();
		for ( $i = 0; $i < Activity_Log_Event::MAX_EXTRA_ENTRIES + 5; $i++ ) {
			$extra[ 'k' . $i ] = $i;
		}
		...
		$this->assertCount( Activity_Log_Event::MAX_EXTRA_ENTRIES, $payload['extra'] );
	}
