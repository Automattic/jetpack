...
class Main_Features {
	 ...
	 * it, `delivery.standalone` names a separate plugin that also does, and `delivery.free`
	 * says it can be used without paying. `paid_highlights` lists what paying adds,
	...
	public static function get_feature_definitions() {
		return array(
			...
			'jetpack-ai'    => array(
				...
				'description'      => __( 'Draft, edit and translate your posts without leaving the editor.', 'jetpack-my-jetpack' ),
				'long_description' => __( 'Turn a prompt into a draft, table or form, then change its tone, translate it or summarize it in the editor. You can also generate images, titles and excerpts for your posts.', 'jetpack-my-jetpack' ),
				'icon'             => 'star',
				'product'          => 'jetpack-ai',
				...
				'plans'            => array( 'complete' ),
				'paid_product'     => __( 'Jetpack AI', 'jetpack-my-jetpack' ),
			),
			'jetpack-forms' => array(
				...
				'description'      => __( 'Get messages, sign-ups and feedback from visitors, with no extra plugins.', 'jetpack-my-jetpack' ),
				'long_description' => __( 'Add contact, registration and feedback forms to any page, with no limit on responses. You get each response by email and can review or export it from your dashboard.', 'jetpack-my-jetpack' ),
				'icon'             => 'list-bullets',
				'product'          => 'jetpack-forms',
				'essential'        => true,
				'paid_highlights'  => array(
					__( 'Let visitors attach files to their submissions', 'jetpack-my-jetpack' ),
					__( 'Up to 10 files per upload field, 20 MB each', 'jetpack-my-jetpack' ),
				),
				'delivery'         => array(
					'in_jetpack' => true,
					'free'       => true,
	...
	public static function get_features() {
		...
		foreach ( self::get_feature_definitions() as $slug => $definition ) {
			$standalone_plugin_url = self::get_standalone_plugin_url( $definition );
	...
	/**
	 * The WordPress.org page of the plugin that ships a feature on its own, outside Jetpack.
	 *
	 * @param array $definition A single entry from the feature catalog.
	 * @return string Plugin directory URL, or an empty string when only Jetpack ships the feature.
	 */
	private static function get_standalone_plugin_url( array $definition ) {
		if ( empty( $definition['product'] ) ) {
			return '';
		}
		...
		if ( ! $product_class || empty( $product_class::$plugin_slug ) || Product::JETPACK_PLUGIN_SLUG === $product_class::$plugin_slug ) {
			return '';
		}

		return 'https://wordpress.org/plugins/' . $product_class::$plugin_slug . '/';
	}
