...
final class WafRuntimeTest extends PHPUnit\Framework\TestCase {
	...
	/**
	 * Blocking an IP from the block list must not fatal when WordPress is absent.
	 *
	...
	public function testBlockOnIpBlockListDoesNotRequireWordPress() {
		...
		$this->assertFalse( defined( 'ABSPATH' ), 'This test is only meaningful without WordPress loaded.' );
		...
		$this->assertStringContainsString( 'Jetpack WAF Blocked Request', $log, 'The request should still be blocked and logged.' );
	}
}
