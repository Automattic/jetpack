...
class Dashboard_REST_Controller {
	...
	public function register_rest_routes() {
		...
		register_rest_route(
			static::$namespace,
			sprintf( '/sites/%1$d/wordads/dsp/api/v1/wpcom/media/openverse(\?.*)?', $site_id ),
			array(
				'methods'             => WP_REST_Server::READABLE,
	...
	public function get_dsp_media( $req ) {
		...
		// We don't use sub_path in the blaze media, only query strings
		if ( isset( $params['sub_path'] ) ) {
			unset( $req->get_params()['sub_path'] );
		}
	...
	public function get_dsp_openverse( $req ) {
		...
		return $this->get_dsp_generic( 'v1/wpcom/media/openverse', $req );
	}
