 ...
 *
 * @package automattic/jetpack-ai-helper
 */
...
class AI_Helper {
	...
	public static function get_status_permission_check( $request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter, VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			return new WP_Error(
				'rest_forbidden',
				__( 'Sorry, you are not allowed to access Jetpack AI help on this site.', 'jetpack-ai-helper' ),
	...
	public static function is_enabled() {
		...
		if ( defined( 'IS_WPCOM' ) && IS_WPCOM ) {
			$default = true;
		} elseif ( ( new Status\Host() )->is_woa_site() ) {
			$default = true;
		}
	...
	public static function get_gpt_completion( $content, $post_id ) {
		...
		if ( ( new Status() )->is_offline_mode() ) {
			return new WP_Error(
				'dev_mode',
				__( 'Jetpack AI is not available in offline mode.', 'jetpack-ai-helper' )
			);
		}
