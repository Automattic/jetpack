...
Consumers may replace the default Jetpack Connection authentication and transport by implementing `Wpcom_Request_Client` and using the `jetpack_help_center_wpcom_request_client` filter before `Help_Center::init()` runs.
...
- PHP changes here require a package release and updates to consuming plugins such as `jetpack-mu-plugin`.
