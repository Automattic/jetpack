...
The flat dashboard layout remains exposed through the existing `jetpack_premium_analytics_dashboard_default_layout` filter and default-layout REST route. The layout default is also added to the shared preference-defaults filter so first-load seeding and reset-to-default behavior can continue to share the same flat layout source.
