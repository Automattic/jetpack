...
- **Flat search, filter, sort, and pagination.** Rows pass through DataViews' `filterSortAndPaginate` untouched: child rows count as pagination items, a matching child whose parent is filtered out renders indented but orphaned, and sorting by any field re-orders the rows flat — visually breaking the hierarchy grouping. Leave `sort` out of the initial view to preserve hierarchy order.
