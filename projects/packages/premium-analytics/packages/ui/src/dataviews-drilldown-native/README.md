...
`collapsible` puts a chevron on every row that has children and folds that row's whole subtree away when it is clicked. Folded rows leave the table entirely, so they stop counting towards pagination too.
