...
type RequestReportParamsMap = {
	...
	'visitors-by-location': Parameters<
		typeof reportVisitorsByLocationQuery
	>[ 0 ];
...
export async function prefetchReport< T extends keyof RequestReportParamsMap >(
	...
	switch ( reportType ) {
		...
		case 'coupons-by-date':
			return queryClient.ensureQueryData(
				reportCouponsByDateQuery(
					params as RequestReportParamsMap[ 'coupons-by-date' ]
				)
			);
		...
		case 'customers-by-date':
			return queryClient.ensureQueryData(
				reportCustomersByDateQuery(
					params as RequestReportParamsMap[ 'customers-by-date' ]
				)
			);
		...
		case 'visitors-by-location':
			return queryClient.ensureQueryData(
				reportVisitorsByLocationQuery(
					params as RequestReportParamsMap[ 'visitors-by-location' ]
				)
			);
		...
		case 'sessions-by-device':
			return queryClient.ensureQueryData(
				reportSessionsByDeviceQuery(
					params as RequestReportParamsMap[ 'sessions-by-device' ]
				)
			);
		...
		case 'products':
			return queryClient.ensureQueryData(
				reportProductsQuery(
					params as RequestReportParamsMap[ 'products' ]
				)
			);
		...
		case 'conversion-rate':
			return queryClient.ensureQueryData(
				reportConversionRateQuery(
					params as RequestReportParamsMap[ 'conversion-rate' ]
				)
			);
