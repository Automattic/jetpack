...
function parseTimeSeriesRows( payload: unknown ) {
	const response = coerceStatsRecord( payload );
	const timeline = coerceStatsRecord( response.timeline );
	const matrixRows = parseMatrixRows( Object.keys( timeline ).length ? timeline : response );
...
export function sanitizeStatsTimeSeriesResponse(
	...
	const response = coerceStatsRecord( payload );
	const timeline = coerceStatsRecord( response.timeline );
	const unit = String( timeline.unit ?? response.unit ?? query?.period ?? 'day' );
