...
class GrossSalesOverTimeController extends AbstractCSVReportController {
	...
	public function get_report_label(): string {
		return __( 'Gross Sales Over Time', 'jetpack-premium-analytics' );
	}
	...
	public function get_column_headers( ?string $interval = null ): array {
		return array(
			...
			'orders_value_gross' => __( 'Gross sales', 'jetpack-premium-analytics' ),
		);
	}
	...
	public function format_row_for_csv( array $item ): array {
		...
		return array(
			...
			'orders_value_gross' => self::format_amount( $item['orders_value_gross'] ?? $defaults['orders_value_gross'] ),
		);
	}
}
