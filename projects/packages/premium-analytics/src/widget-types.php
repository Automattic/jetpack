...
/**
 * Hydrates a widget type registry from the build manifest.
 *
 * The package's own registrant on the registration action, safe to call directly: a widget
 * type already registered is skipped. Each manifest widget is copied into the registry, gated
 * by REGISTRABLE_WIDGET_TYPES_FILTER so a consumer can drop a candidate first.
 *
 ...
 *
 * The plugin form of the contract: call it from a callback on
 * Widget_Type_Registry::REGISTER_ACTION, where the API is loaded and the
 * registry is hydrating. The metadata strings arrive translated, as a
 * section's label does.
 *
 * @since $$next-version$$
 *
