...
describe( 'TopPostsWidget', () => {
	...
	it( 'renders the fetched top posts as links', async () => {
		render( <TopPostsWidget attributes={ { period: 'day', date: '2026-06-10', num: 10 } } /> );
	...
	it( 'filters rows by post type when the name attribute is set', async () => {
		render(
			<TopPostsWidget attributes={ { period: 'day', date: '2026-06-10', num: 10, name: 'page' } } />
		);
	...
	it( 'renders the empty state when there are no views', async () => {
		...
		render( <TopPostsWidget attributes={ { period: 'day', date: '2026-06-10' } } /> );
