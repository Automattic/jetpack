...
class Social_Admin_Page_Test extends BaseTestCase {
	...
	/**
	 * The Social menu is not registered when the Publicize module is inactive and
	 * the standalone Jetpack Social plugin is not active - even for an administrator.
	 */
	public function test_menu_hidden_when_module_inactive_and_no_social_plugin() {
		// `Admin_Menu::add_menu()` adds this load hook synchronously, so its
		// presence reflects whether the menu passed the visibility gates.
		remove_all_actions( 'load-jetpack_page_jetpack-social' );

		Social_Admin_Page::init()->add_menu();

		$this->assertFalse(
			has_action( 'load-jetpack_page_jetpack-social' ),
			'Social menu should not be registered when the module and plugin are both inactive.'
		);
	}

	/**
	 * The Social menu is registered when the Publicize module is active.
	 */
	public function test_menu_registered_when_module_active() {
		remove_all_actions( 'load-jetpack_page_jetpack-social' );
		...
		$this->assertNotFalse(
			has_action( 'load-jetpack_page_jetpack-social' ),
			'Social menu should be registered when the Publicize module is active.'
		);
	...
	/**
	 * Mock a site + user connection so the menu's connection gate passes.
	 *
