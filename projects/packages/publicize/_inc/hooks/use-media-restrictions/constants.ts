...
export function getErrorLabels() {
	return {
		[ FILE_SIZE_ERROR ]: __( 'File too big', 'jetpack-publicize-pkg' ),
		[ VIDEO_LENGTH_TOO_LONG_ERROR ]: __( 'Video too long', 'jetpack-publicize-pkg' ),
		[ VIDEO_LENGTH_TOO_SHORT_ERROR ]: __( 'Video too short', 'jetpack-publicize-pkg' ),
		[ DIMENSION_ERROR ]: __( 'Invalid dimensions', 'jetpack-publicize-pkg' ),
	};
}
...
export function getErrorLabel( errorType: string ) {
	return getErrorLabels()[ errorType ] || __( 'Invalid media', 'jetpack-publicize-pkg' );
}
