...
export function ShareStatusLabel( { status, message } ) {
	...
	const icon = isSuccessful ? (
		...
		<IconTooltip
			shift={ true }
			inline={ false }
			title={ __( 'Sharing failed with the following message:', 'jetpack-publicize-pkg' ) }
	...
	return (
		...
		>
			...
			<div className={ styles[ 'share-status-label' ] }>
				{ isSuccessful
					? _x( 'Shared', 'The sharing is successful', 'jetpack-publicize-pkg' )
					: __( 'Failed', 'jetpack-publicize-pkg' ) }
			</div>
		</div>
	);
}
