...
const ADD_MEDIA_LABEL = __( 'Choose Media', 'jetpack-publicize-pkg' );
	...
	const renderHeaderSection = () => {
		...
		return disabledNoticeMessage ? (
			...
			<p className={ styles.subtitle }>
				{ __( 'Choose a visual to accompany your post.', 'jetpack-publicize-pkg' ) }
			</p>
		);
	};
		...
		<ThemeProvider>
			<BaseControl __nextHasNoMarginBottom={ true } className={ styles.wrapper }>
				<BaseControl.VisualLabel>
					{ __( 'Attached Media', 'jetpack-publicize-pkg' ) }
				</BaseControl.VisualLabel>
				{ renderHeaderSection() }
				<MediaWrapper { ...mediaWrapperProps }>
					<MediaPicker
						buttonLabel={ ADD_MEDIA_LABEL }
						subTitle={ __( 'Add an image or video', 'jetpack-publicize-pkg' ) }
