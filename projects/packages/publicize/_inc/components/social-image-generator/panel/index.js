...
const SocialImageGeneratorPanel = () => {
	...
	const { save: saveToMediaLibrary, isSaving } = useSaveImageToLibrary( {
		...
		onSuccess: () => {
			createSuccessNotice( __( 'Image saved to media library.', 'jetpack-publicize-pkg' ) );
		},
	} );
			...
			<ThemeProvider>
				<BaseControl __nextHasNoMarginBottom={ true }>
					<BaseControl.VisualLabel>
						{ __( 'Link preview image', 'jetpack-publicize-pkg' ) }
					</BaseControl.VisualLabel>
					<ToggleControl
						label={ __( 'Enable Social Image', 'jetpack-publicize-pkg' ) }
						help={
							! isEnabled
								? __( 'Social Image is disabled for this post.', 'jetpack-publicize-pkg' )
								: ''
						}
						checked={ isEnabled }
