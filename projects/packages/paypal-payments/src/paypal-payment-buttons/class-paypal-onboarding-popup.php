...
<body>
		...
		<script>
			try {
				var errorChannel = new BroadcastChannel( <?php echo $channel; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- wp_json_encode with JSON_HEX_TAG. ?> );
				errorChannel.postMessage( {
					type: 'paypal-onboarding-incomplete',
					message: <?php echo wp_json_encode( $error, JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- wp_json_encode with JSON_HEX_TAG. ?>,
				} );
				errorChannel.close();
			} catch ( e ) {}
		</script>
