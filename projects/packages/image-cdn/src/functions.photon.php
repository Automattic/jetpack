 ...
 */
function jetpack_image_cdn_url( $image_url, $args = array(), $scheme = null ) {
	...
	if ( jetpack_image_cdn_endswith( strtolower( $image_url_parts['host'] ), '.files.wordpress.com' ) ) {
		$is_wpcom_image = true;
		if ( isset( $args['ssl'] ) ) {
	...
	if (
		...
		return jetpack_image_cdn_url_scheme( $photon_url, $scheme );
	}
 ...
 */
add_filter( 'jetpack_photon_url', 'jetpack_image_cdn_url', 10, 3 );
 ...
 */
add_filter( 'jetpack_photon_pre_args', 'jetpack_image_cdn_parse_wpcom_query_args', 10, 2 );
 ...
 */
function jetpack_image_cdn_parse_wpcom_query_args( $args, $image_url ) {
 ...
 * @return string URL.
 */
function jetpack_image_cdn_url_scheme( $url, $scheme ) {
...
add_filter( 'jetpack_photon_skip_for_url', 'jetpack_image_cdn_banned_domains', 9, 2 );
 ...
 */
function jetpack_image_cdn_banned_domains( $skip, $image_url ) {
 ...
 * @return bool
 */
function jetpack_image_cdn_endswith( $haystack, $needle ) {
 ...
 */
function get_jetpack_image_cdn_content_width() {
