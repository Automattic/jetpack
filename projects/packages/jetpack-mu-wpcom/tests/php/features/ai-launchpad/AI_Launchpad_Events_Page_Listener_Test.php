<?php
/**
 * Test class for AI_Launchpad_Events_Page_Listener.
 *
...
/**
 * Tests the events-page marker listener.
 *
...
class AI_Launchpad_Events_Page_Listener_Test extends \WorDBless\BaseTestCase {
	...
	/**
	 * Publishing a page completes add_events_page only when the page carries the marker meta and the site is
	...
	public function test_publishing_page_completes_task_only_when_marked_and_eligible( $marked, $eligible, $expected ) {
		...
		if ( $marked ) {
			update_post_meta( $page_id, AI_Launchpad_Events_Page_Listener::META_KEY, true );
		}
		...
		$this->assertSame( $expected, ! empty( $statuses['add_events_page'] ) );
	}
	...
	public function test_a_later_edit_of_a_published_page_does_not_complete_again() {
		...
		AI_Launchpad_Events_Page_Listener::maybe_complete( 'publish', 'publish', get_post( $page_id ) );

		$statuses = (array) get_option( 'launchpad_checklist_tasks_statuses', array() );
		$this->assertArrayNotHasKey( 'add_events_page', $statuses );
	}
	...
	public function test_registers_marker_meta() {
		...
		$this->assertTrue(
			registered_meta_key_exists( 'post', AI_Launchpad_Events_Page_Listener::META_KEY, 'page' )
		);
	}

	/**
	 * The in-progress draft lookup finds a page by this listener's own marker, and nothing else: the gallery,
	 * About and contact pages carry their own markers, and an events card must never reopen one of those drafts.
	 */
	public function test_get_draft_id_queries_its_own_marker_meta() {
		...
		$this->assertSame( 8181, AI_Launchpad_Events_Page_Listener::get_draft_id() );
		$this->assertSame( array( AI_Launchpad_Events_Page_Listener::META_KEY ), $queried );
	}
}
