...
class Launchpad_Task_Definitions_Test extends \WorDBless\BaseTestCase {
	...
	/**
	 * Test that {@see wpcom_launchpad_add_subscribe_block_check()} completes the task for any published
	 * post type containing the Subscribe block — not just templates and template parts.
	 */
	public function test_add_subscribe_block_check_completes_for_published_post() {
		remove_all_filters( 'wpcom_launchpad_extended_task_definitions' );

		$post_id = wp_insert_post(
			array(
				'post_title'   => 'Post with a Subscribe block',
				'post_content' => '<!-- wp:jetpack/subscriptions /-->',
				'post_status'  => 'publish',
				'post_type'    => 'post',
			)
		);

		wpcom_launchpad_add_subscribe_block_check( $post_id, get_post( $post_id ) );
	...
	/**
	 * Test that {@see wpcom_launchpad_add_subscribe_block_check()} still ignores unpublished content,
	 * published content without the Subscribe block, and non-viewable post types (e.g. synced
	 * patterns, which hold the block without rendering a subscribe form on the front end).
	 */
	public function test_add_subscribe_block_check_ignores_drafts_blockless_and_nonviewable_content() {
		remove_all_filters( 'wpcom_launchpad_extended_task_definitions' );

		$draft_id = wp_insert_post(
			array(
				'post_title'   => 'Draft with a Subscribe block',
				'post_content' => '<!-- wp:jetpack/subscriptions /-->',
				'post_status'  => 'draft',
				'post_type'    => 'post',
			)
		);
		wpcom_launchpad_add_subscribe_block_check( $draft_id, get_post( $draft_id ) );

		$blockless_id = wp_insert_post(
			array(
				'post_title'   => 'Published without the block',
				'post_content' => '<!-- wp:paragraph --><p>Hello</p><!-- /wp:paragraph -->',
				'post_status'  => 'publish',
				'post_type'    => 'page',
			)
		);
		wpcom_launchpad_add_subscribe_block_check( $blockless_id, get_post( $blockless_id ) );

		$pattern_id = wp_insert_post(
			array(
				'post_title'   => 'Synced pattern with a Subscribe block',
				'post_content' => '<!-- wp:jetpack/subscriptions /-->',
				'post_status'  => 'publish',
				'post_type'    => 'wp_block',
			)
		);
		wpcom_launchpad_add_subscribe_block_check( $pattern_id, get_post( $pattern_id ) );

		$statuses = get_option( 'launchpad_checklist_tasks_statuses', array() );
		$this->assertArrayNotHasKey( 'add_subscribe_block', $statuses );
	}

	/**
	 * Test that {@see wpcom_launchpad_add_subscribe_block_widget_check()} completes the task when a
	 * block widget contains the Subscribe block (widget saves never fire save_post), and ignores
	 * widget payloads without it.
	 */
	public function test_add_subscribe_block_widget_check() {
		remove_all_filters( 'wpcom_launchpad_extended_task_definitions' );

		wpcom_launchpad_add_subscribe_block_widget_check(
			null,
			array(
