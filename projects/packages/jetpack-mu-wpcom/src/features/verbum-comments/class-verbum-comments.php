...
class Verbum_Comments {
	 ...
	 */
	private function filter_blocks_recursive( $blocks ) {
		...
		foreach ( $blocks as $block ) {
			if ( in_array( $block['blockName'], $allowed_blocks, true ) ) {
				...
				if ( isset( $block['innerBlocks'] ) && is_array( $block['innerBlocks'] ) ) {
					$filtered_block['innerBlocks'] = $this->filter_blocks_recursive( $block['innerBlocks'] );
				}
		...
		wp_add_inline_script(
			...
			'window.VerbumComments = ' . wp_json_encode(
				array(
					...
					'allowedBlocks'                     => self::get_allowed_blocks(),
