...
function wpcom_launchpad_get_task_definitions() {
	$task_definitions = array(
		...
		'professional_email_mailbox_created' => array(
			...
			'is_complete_callback' => 'wpcom_launchpad_is_legacy_task_option_completed',
			'is_visible_callback'  => 'wpcom_launchpad_is_professional_email_mailbox_created_visible',
			'get_calypso_path'     => function ( $task, $default, $data ) {
...
/**
 * Checks if the old checklist task option is marked as complete.
 *
 * @param array $task The task data.
 * @return bool True if the option for the task is marked as complete, false otherwise.
 */
...
/**
 * Checks if the Professional Email task should be visible.
 *
 * @return bool True if the task should be visible, false otherwise.
 */
function wpcom_launchpad_is_professional_email_mailbox_created_visible() {
	if ( ! function_exists( '\Onboarding_Checklist\get_professional_email_task' ) ) {
		return false;
	}

	return null !== \Onboarding_Checklist\get_professional_email_task();
}

/**
 * Checks if the blogname set task should be visible.
 *
 * @return bool True if the task should be visible, false otherwise.
 */
function wpcom_launchpad_is_blogname_set_visible() {
	...
	return strtotime( $blog_details->registered ) > strtotime( '2020-03-25' );
}
