...
add_filter(
	...
	function ( $settings ) {
		if ( ! empty( $_GET['wp_theme_preview'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			$settings['__experimentalDashboardLink'] = 'https://wordpress.com/themes/' . wpcom_get_site_slug();
		} else {
			// Point the `<` link to wpcom home.
			$settings['__experimentalDashboardLink'] = 'https://wordpress.com/home/' . wpcom_get_site_slug();
		}
		return $settings;
	}
);
