<?php
/**
 * AI Launchpad video-page completion listener.
 *
...
/**
 * Completes the `add_video_page` task from wp-admin when the AI-created video page is published.
 *
 ...
 *
 * The marker is also what puts the task "in progress": a video page saved but not yet published reopens
 * through get_draft_id() instead of the card creating a second one. Each page task queries its own marker for
 * that, so a gallery, contact or events draft never puts the video card in progress.
 */
class AI_Launchpad_Video_Page_Listener {

	/**
	 * Marker meta set by createVideoPage on the AI-created video page.
	 */
	const META_KEY = '_wpcom_ai_launchpad_video_page';
	...
	/**
	 * Completes add_video_page when a page carrying the marker is first published.
	 *
	...
	public static function maybe_complete( $new_status, $old_status, $post ) {
		...
		AI_Launchpad_Task_Registry::mark_complete( 'add_video_page' );
	}

	/**
	 * Returns the ID of the newest unpublished AI-created video page (a `draft` page carrying the marker), or null.
	 *
	 * @return int|null
	 */
...
AI_Launchpad_Video_Page_Listener::register();
