<?php
/**
 * An in-wp-admin Backup page for WordPress.com Simple and WoA sites.
 *
 * @package automattic/jetpack-mu-wpcom
 */

use Automattic\Jetpack\Constants;
use Automattic\Jetpack\WP_Build_Polyfills\WP_Build_Polyfills;

/**
 * Admin page slug. Matches Jetpack_Backup::JETPACK_BACKUP_SLUG so the URL
 * survives a transfer to WoA. Nothing else claims it on either platform today:
 * the standalone Backup plugin owns it elsewhere, but wpcomsh force-deactivates
 * that plugin on WoA. This page yields the slug once backups are actually live,
 * so the Jetpack plugin can take it over there.
 */
const WPCOM_BACKUP_MENU_SLUG = 'jetpack-backup';

/**
 * Page name for wp-build, as declared in routes/wpcom-backup/package.json.
 *
 * Deliberately not the menu slug: wp-build derives its own page slug and render
 * function from this name.
 */
const WPCOM_BACKUP_WP_BUILD_PAGE = 'wpcom-backup';

/**
 * Render callback generated by wp-build in build/pages/wpcom-backup/page-wp-admin.php.
 */
const WPCOM_BACKUP_RENDER_CALLBACK = 'jetpack_mu_wpcom_wpcom_backup_wp_admin_render_page';

/**
 * Calypso flow that initiates the transfer, reports progress and returns here.
 */
const WPCOM_BACKUP_TRANSFER_FLOW_URL = 'https://wordpress.com/setup/transferring-hosted-site';

/**
 * Site feature the transfer flow waits for before handing the reader back.
 */
const WPCOM_BACKUP_TRANSFER_FEATURE = 'backups-self-serve';

/**
 * Tracks context recorded for the WoA transfer
 */
const WPCOM_BACKUP_TRANSFER_CONTEXT = 'jetpack_product_activation';

/**
 * Site has no backup-capable plan. Offer an upgrade.
 */
const WPCOM_BACKUP_STATE_UPGRADE = 'upgrade';

/**
 * A transfer to WoA is already running or queued. Offer nothing; report status.
 */
const WPCOM_BACKUP_STATE_IN_PROGRESS = 'in_progress';

/**
 * Site has the plan and can transfer. Offer activation.
 */
const WPCOM_BACKUP_STATE_ACTIVATE = 'activate';

/**
 * Boot the feature, loading the wp-build assets only on the Backup page itself.
 *
 * @return void
 */
function wpcom_backup_init() {
	if ( ! wpcom_backup_should_register() ) {
		return;
	}

	add_action( 'admin_menu', 'wpcom_backup_register_page', 1000000 );

	if ( ! wpcom_backup_is_backup_admin_request() ) {
		return;
	}

	wpcom_backup_load_wp_build();

	add_action( 'current_screen', 'wpcom_backup_alias_screen_id' );
	add_action( 'admin_enqueue_scripts', 'wpcom_backup_enqueue_initial_state', 20 );
}
add_action( 'init', 'wpcom_backup_init' );

/**
 * Whether the site runs on WordPress.com's Atomic infrastructure.
 *
 * Read through Constants rather than `defined()` so tests can exercise both
 * platforms in one process.
 *
 * @return bool
 */
function wpcom_backup_is_atomic() {
	return Constants::is_true( 'IS_ATOMIC' );
}

/**
 * Whether this page should claim the Backup slug at all.
 *
 * The dashboard's own gate, restated: a plan that includes backups only makes
 * them live on Atomic infrastructure, so on WoA an entitled site has working
 * backups and this page steps aside for the one that manages them. A Simple
 * site is never Atomic, so the page always applies there — it offers the
 * transfer that makes the plan's backups real.
 *
 * @return bool
 */
function wpcom_backup_should_register() {
	if ( ! wpcom_backup_is_atomic() ) {
		return true;
	}

	return ! wpcom_backup_has_backup_feature();
}

/**
 * Whether the current request targets the Backup admin page.
 *
 * `$_GET['page']` is populated by wp-admin/admin.php before any of our hooks
 * run, so this is reliable from plugins_loaded onwards.
 *
 * @return bool
 */
function wpcom_backup_is_backup_admin_request() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only routing check; no form data is processed.
	if ( ! is_admin() || ! isset( $_GET['page'] ) ) {
		return false;
	}

	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only routing check; no form data is processed.
	return WPCOM_BACKUP_MENU_SLUG === sanitize_text_field( wp_unslash( $_GET['page'] ) );
}

/**
 * Register the Backup page, then take it back out of the Jetpack menu.
 * Removed from the menu until the mirroring Calypso page is deprecated.
 *
 * Removal is deferred on this page's own request because `get_admin_page_parent()`
 * finds the page's parent by searching `$submenu`
 *
 * If we drop the entry before `admin.php` resolves the hookname
 * the page stops rendering entirely.
 *
 * @return void
 */
function wpcom_backup_register_page() {
	// Registering a second entry under a slug someone else owns would leave two,
	// and hide_menu_entry() would then drop theirs rather than ours.
	if ( wpcom_backup_slug_is_claimed() ) {
		return;
	}

	wpcom_backup_owns_page( true );

	// build/build.php defines the render callback but only loads on this page's
	// own request, so elsewhere fall back rather than fataling.
	$callback = function_exists( WPCOM_BACKUP_RENDER_CALLBACK )
		? WPCOM_BACKUP_RENDER_CALLBACK
		: '__return_empty_string';

	add_submenu_page(
		'jetpack',
		// "VaultPress Backup" is a product name, do not translate.
		'Jetpack VaultPress Backup',
		'VaultPress Backup',
		'manage_options',
		WPCOM_BACKUP_MENU_SLUG,
		$callback
	);

	if ( wpcom_backup_is_backup_admin_request() ) {
		// admin_head runs after routing and before menu-header.php prints.
		add_action( 'admin_head', 'wpcom_backup_hide_menu_entry', 0 );
		return;
	}

	wpcom_backup_hide_menu_entry();
}

/**
 * Whether this page registered the Backup slug on this request.
 *
 * `register_page()` cannot run before the hooks that depend on it are added, so
 * they read the answer here rather than being wired conditionally.
 *
 * @param bool|null $owns Set the answer; omit to read it.
 * @return bool
 */
function wpcom_backup_owns_page( $owns = null ) {
	static $value = false;

	if ( null !== $owns ) {
		$value = (bool) $owns;
	}

	return $value;
}

/**
 * Whether something has already registered the Backup page.
 *
 * The Jetpack plugin serves this slug through `Admin_Menu`, which registers at
 * `admin_menu` priority 1000 — before this page's own hook — so by the time it
 * runs the entry is there to find. Stepping aside is the same hand-off
 * should_register() makes, for the case where the other page is on this site
 * rather than implied by the plan.
 *
 * Index 2 is matched exactly as `remove_submenu_page()` matches it, because
 * that call is what this guards: a looser test would drift from it.
 *
 * @return bool
 */
function wpcom_backup_slug_is_claimed() {
	global $submenu;

	if ( empty( $submenu['jetpack'] ) || ! is_array( $submenu['jetpack'] ) ) {
		return false;
	}

	foreach ( $submenu['jetpack'] as $item ) {
		if ( is_array( $item ) && isset( $item[2] ) && WPCOM_BACKUP_MENU_SLUG === $item[2] ) {
			return true;
		}
	}

	return false;
}

/**
 * Drop the Backup entry from the Jetpack submenu, leaving it routable by URL.
 *
 * Deleting this call is all that is needed to surface the page in the sidebar.
 *
 * @return void
 */
function wpcom_backup_hide_menu_entry() {
	remove_submenu_page( 'jetpack', WPCOM_BACKUP_MENU_SLUG );
}

/**
 * Load the wp-build generated asset registrations and the boot polyfills.
 *
 * `build/` is produced at release time, so an unbuilt checkout has none; bail
 * quietly, since register_page() already falls back to an empty render.
 *
 * @return void
 */
function wpcom_backup_load_wp_build() {
	$wp_build_index = dirname( __DIR__, 3 ) . '/build/build.php';

	if ( ! file_exists( $wp_build_index ) ) {
		return;
	}

	require_once $wp_build_index;

	// Register polyfills for WP < 7.0 (must run before enqueue).
	WP_Build_Polyfills::register(
		'jetpack-mu-wpcom',
		array_merge(
			WP_Build_Polyfills::SCRIPT_HANDLES,
			WP_Build_Polyfills::MODULE_IDS
		)
	);
}

/**
 * Make wp-build's enqueue check pass without changing the page URL.
 *
 * Its enqueue callback needs a screen ID of `<page>`, which our `jetpack-backup`
 * slug does not produce. The body class is untouched and still reads
 * `jetpack_page_jetpack-backup`, which the route stylesheet scopes to.
 *
 * @param object|null $screen The current screen, passed by WP as a \WP_Screen.
 * @return void
 */
function wpcom_backup_alias_screen_id( $screen ) {
	if ( ! is_object( $screen ) || ! wpcom_backup_owns_page() ) {
		return;
	}

	$screen->id = WPCOM_BACKUP_WP_BUILD_PAGE;
}

/**
 * Hand the resolved state to the page's boot script.
 *
 * @return void
 */
function wpcom_backup_enqueue_initial_state() {
	$handle = WPCOM_BACKUP_WP_BUILD_PAGE . '-wp-admin-prerequisites';

	if ( ! wpcom_backup_owns_page() || ! wp_script_is( $handle, 'registered' ) ) {
		return;
	}

	// Every consumer of this ID is reachable on Simple only, where WordPress and
	// WordPress.com agree on it. The feature check, which does run on WoA where
	// they disagree, takes no ID at all.
	$blog_id = get_current_blog_id();
	$user_id = get_current_user_id();
	$domain  = wp_parse_url( home_url(), PHP_URL_HOST );
	$state   = wpcom_backup_get_state( $blog_id, $user_id );

	// Only the activation prompt reads eligibility, and both lists come from the
	// same result, so resolve it once and only for that state.
	$eligibility = WPCOM_BACKUP_STATE_ACTIVATE === $state
		? wpcom_backup_get_eligibility( $blog_id, $user_id )
		: null;

	wp_localize_script(
		$handle,
		'wpcomBackupInitialState',
		array(
			'state'       => $state,
			'domain'      => $domain,
			// A null result means the library was unavailable, not that the site
			// failed a check; let the transfer flow reject it instead.
			'isEligible'  => null === $eligibility || ! empty( $eligibility['is_eligible'] ),
			'errors'      => wpcom_backup_get_transfer_errors( $eligibility ),
			'warnings'    => wpcom_backup_get_transfer_warnings( $eligibility ),
			'upgradeUrl'  => wpcom_backup_get_upgrade_url( $domain ),
			'activateUrl' => wpcom_backup_get_activate_url(),
			'supportUrl'  => 'https://wordpress.com/support/backups/',
		)
	);
}

/**
 * Whether the site's plan includes self-serve backups.
 *
 * @return bool
 */
function wpcom_backup_has_backup_feature() {
	if ( ! function_exists( 'wpcom_site_has_feature' ) || ! defined( '\WPCOM_Features::BACKUPS_SELF_SERVE' ) ) {
		return false;
	}

	// No blog ID: WordPress numbers an Atomic single site 1, which is not its
	// WordPress.com blog ID, and the lookup throws rather than correcting it.
	// Passing none lets wpcom resolve the current site on either platform.
	return (bool) wpcom_site_has_feature( \WPCOM_Features::BACKUPS_SELF_SERVE );
}

/**
 * Whether a transfer to WoA is already underway.
 *
 * @param int $blog_id Blog ID.
 * @return bool
 */
function wpcom_backup_is_transfer_in_progress( $blog_id ) {
	if ( ! function_exists( 'require_lib' ) ) {
		return false;
	}

	require_lib( 'atomic' );

	if ( function_exists( '\A8C\Atomic\has_site_pending_automated_transfer' ) ) {
		// @phan-suppress-next-line PhanUndeclaredFunction -- wpcom-only; pending addition to stub-defs.php.
		if ( \A8C\Atomic\has_site_pending_automated_transfer( $blog_id ) ) {
			return true;
		}
	}

	if ( function_exists( '\A8C\Atomic\is_wpcom_atomic' ) ) {
		// Second argument includes pending/active/provisioned transfers.
		// @phan-suppress-next-line PhanUndeclaredFunction -- wpcom-only; pending addition to stub-defs.php.
		return (bool) \A8C\Atomic\is_wpcom_atomic( $blog_id, true );
	}

	return false;
}

/**
 * Transfer eligibility for the site, or null when it cannot be determined.
 *
 * An in-progress transfer is reported as a `transfer_already_exists` error, so
 * that state is checked first and never reaches here as a blocker.
 *
 * @param int $blog_id Blog ID.
 * @param int $user_id User ID.
 * @return array|null { is_eligible: bool, errors: array, warnings: array }
 */
function wpcom_backup_get_eligibility( $blog_id, $user_id ) {
	if ( ! function_exists( 'require_lib' ) ) {
		return null;
	}

	require_lib( 'atomic' );

	if ( ! function_exists( '\A8C\Atomic\Eligibility\get_status_for_site' ) ) {
		return null;
	}

	// @phan-suppress-next-line PhanUndeclaredFunction -- wpcom-only; pending addition to stub-defs.php.
	return \A8C\Atomic\Eligibility\get_status_for_site( $blog_id, $user_id );
}

/**
 * Resolve which prompt the page should show.
 *
 * @param int $blog_id Blog ID.
 * @param int $user_id User ID.
 * @return string One of the WPCOM_BACKUP_STATE_* constants.
 */
function wpcom_backup_get_state( $blog_id, $user_id ) {
	// On WoA the plan is the only question: the site is already on the
	// infrastructure that runs backups, so there is nothing to transfer, and
	// should_register() has already stepped the page aside if it has the plan.
	if ( ! wpcom_backup_has_backup_feature() || wpcom_backup_is_atomic() ) {
		$state = WPCOM_BACKUP_STATE_UPGRADE;
	} elseif ( wpcom_backup_is_transfer_in_progress( $blog_id ) ) {
		$state = WPCOM_BACKUP_STATE_IN_PROGRESS;
	} else {
		// Eligibility failures are explained by the activation prompt's own
		// confirmation step, so they do not get a state of their own.
		$state = WPCOM_BACKUP_STATE_ACTIVATE;
	}

	/**
	 * Filter the state rendered by the Backup page.
	 *
	 * Lets each prompt be previewed on a sandbox without arranging real state.
	 *
	 * @param string $state   One of the WPCOM_BACKUP_STATE_* constants.
	 * @param int    $blog_id Blog ID.
	 * @param int    $user_id User ID.
	 */
	return apply_filters( 'wpcom_backup_state', $state, $blog_id, $user_id );
}

/**
 * Reasons a site cannot be transferred, in the shape the API returns them.
 *
 * The code is kept because the page maps it to its own copy; the API's message
 * is passed through as the fallback for codes it does not recognize.
 *
 * @param array|null $eligibility Result of wpcom_backup_get_eligibility().
 * @return array[] Errors as { code, message }.
 */
function wpcom_backup_get_transfer_errors( $eligibility ) {
	$errors = array();

	if ( ! empty( $eligibility['errors'] ) && is_array( $eligibility['errors'] ) ) {
		foreach ( $eligibility['errors'] as $error ) {
			if ( empty( $error['code'] ) ) {
				continue;
			}

			$errors[] = array(
				'code'    => (string) $error['code'],
				'message' => isset( $error['message'] ) ? (string) $error['message'] : '',
			);
		}
	}

	return $errors;
}

/**
 * Non-blocking warnings to confirm before a transfer starts.
 *
 * The API groups warnings by type; they are flattened here because the page
 * renders one list and a PHP map would localize as a JSON array once empty.
 *
 * @param array|null $eligibility Result of wpcom_backup_get_eligibility().
 * @return array[] Warnings in the API's own shape, minus the grouping.
 */
function wpcom_backup_get_transfer_warnings( $eligibility ) {
	$warnings = array();

	if ( ! empty( $eligibility['warnings'] ) && is_array( $eligibility['warnings'] ) ) {
		foreach ( $eligibility['warnings'] as $group ) {
			if ( ! is_array( $group ) ) {
				continue;
			}

			foreach ( $group as $warning ) {
				if ( empty( $warning['id'] ) ) {
					continue;
				}

				$warnings[] = array(
					'id'           => (string) $warning['id'],
					'description'  => isset( $warning['description'] ) ? (string) $warning['description'] : '',
					'domain_names' => ( ! empty( $warning['domain_names']['current'] ) && ! empty( $warning['domain_names']['new'] ) )
						? array(
							'current' => (string) $warning['domain_names']['current'],
							'new'     => (string) $warning['domain_names']['new'],
						)
						: null,
					'support_url'  => isset( $warning['support_url'] ) ? (string) $warning['support_url'] : '',
				);
			}
		}
	}

	return $warnings;
}

/**
 * Checkout, with the plan that includes backups already in the cart.
 *
 * Matches the dashboard's upsell CTA, which sells the plan rather than opening
 * a comparison the reader has to navigate.
 *
 * @param string $domain Site domain.
 * @return string
 */
function wpcom_backup_get_upgrade_url( $domain ) {
	return 'https://wordpress.com/checkout/' . $domain . '/business';
}

/**
 * URL of the Calypso flow that transfers the site and switches backups on.
 *
 * `redirect_to` brings the reader back here afterwards. It is resolved fresh by
 * the flow, so it carries the new address when the transfer changes one.
 *
 * @return string
 */
function wpcom_backup_get_activate_url() {
	return add_query_arg(
		array(
			'siteId'                    => get_current_blog_id(),
			'feature'                   => WPCOM_BACKUP_TRANSFER_FEATURE,
			'initiate_transfer_context' => WPCOM_BACKUP_TRANSFER_CONTEXT,
			'redirect_to'               => rawurlencode(
				admin_url( 'admin.php?page=' . WPCOM_BACKUP_MENU_SLUG )
			),
		),
		WPCOM_BACKUP_TRANSFER_FLOW_URL
	);
}
