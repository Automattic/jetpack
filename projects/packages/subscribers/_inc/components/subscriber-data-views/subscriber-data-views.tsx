...
import { isAtomicSite, isSimpleSite } from '@automattic/jetpack-script-data';
...
const SubscriberDataViews = ( {
	...
	//);
	const couponsAndGiftsEnabled = false;
	...
	const { data: subscribersTotals } = useSubscriberCountQuery();
	...
	const fields = useMemo(
		() => [
			{
				id: 'media',
				label: __( 'Media', 'jetpack-subscribers' ),
			...
			{
				id: 'name',
				label: __( 'Name', 'jetpack-subscribers' ),
			...
			{
				id: 'plan',
				label: __( 'Plan', 'jetpack-subscribers' ),
				...
				elements: [
					{ label: __( 'Paid', 'jetpack-subscribers' ), value: SubscribersFilterBy.Paid },
					{ label: __( 'Free', 'jetpack-subscribers' ), value: SubscribersFilterBy.Free },
				],
				filterBy: {
			...
			},
			{
				...
				label: __( 'Email subscriber', 'jetpack-subscribers' ),
				getValue: ( { item }: { item: Subscriber } ) => ( item.is_email_subscriber ? 'yes' : 'no' ),
				render: ( { item }: { item: Subscriber } ) => {
					const noTooltip =
						isEnglishLocale || hasTranslation( 'Reader only subscriber' ) ? (
							<Tooltip text={ __( 'Reader only subscriber', 'jetpack-subscribers' ) }>
								<span className="subscriber-data-views__tooltip-text">
									{ __( 'No', 'jetpack-subscribers' ) }
								</span>
							</Tooltip>
						) : (
							__( 'No', 'jetpack-subscribers' )
					...
					return (
						<div>{ item.is_email_subscriber ? __( 'Yes', 'jetpack-subscribers' ) : noTooltip }</div>
					);
				},
				elements: [
					{
						label: __( 'True', 'jetpack-subscribers' ),
						value: SubscribersFilterBy.EmailSubscriber,
					},
					{
						label: __( 'False', 'jetpack-subscribers' ),
			...
			{
				id: 'date_subscribed',
				label: __( 'Since', 'jetpack-subscribers' ),
	...
	const actions = useMemo< Action< Subscriber >[] >( () => {
		...
		const baseActions = [
			{
				id: 'view',
				label: __( 'View', 'jetpack-subscribers' ),
		...
		if ( couponsAndGiftsEnabled ) {
			baseActions.push( {
				id: 'gift',
				label: __( 'Gift a subscription', 'jetpack-subscribers' ),
				callback: ( items: Subscriber[] ) => {
			...
			<section className="subscriber-data-views__list">
				...
				{ shouldShowLaunchpad ? (
					...
					<DataViews< Subscriber >
						...
						getItemId={ getSubscriberId }
						defaultLayouts={ selectedSubscriber ? { list: {} } : { table: {} } }
						actions={ actions }
						search
						searchLabel={ __( 'Search by name, username or email…', 'jetpack-subscribers' ) }
					/>
				) }
			</section>
			{
