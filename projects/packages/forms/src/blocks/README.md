...
- Block's `save()` can likely simply return `null`, i.e. no content. Don't serialize internal HTML structure of the block in posts as it's not useful fallback when the block/plugin is disabled, or in RSS and emails.
