...
class Form_Field_Registry {
	 ...
	 *     @type string   $plugin_file          Path to the plugin's main file (__FILE__). When provided,
	 *                                          asset URLs are auto-resolved from conventional paths:
	 *                                          - editor_script: build/index.js
	 *                                          - editor_style: build/editor-style.css
	 *                                          - view_script: build/view.js
	 *                                          - view_style: build/view-style.css
	 *                                          - dashboard_script: src/dashboard.js
	 *                                          Dependencies and versions are loaded from build/index.asset.php.
	 *                                          Individual values can be overridden by setting them explicitly,
	...
	public static function register( $field_type, $args = array() ) {
		...
		// Auto-resolve asset paths if plugin_file is provided.
		if ( ! empty( $args['plugin_file'] ) && file_exists( $args['plugin_file'] ) ) {
			$args = self::resolve_plugin_assets( $args );
		}
	...
	/**
	 * Resolve plugin asset paths from conventional locations.
	 *
	 * When plugin_file is provided, this method auto-resolves asset URLs
	 * from conventional paths, and loads dependencies/versions from the
	 * asset file generated by @wordpress/scripts.
	 *
	 * Conventional structure:
	 * - build/index.js         (editor script)
	 * - build/index.asset.php  (dependencies and version)
	 * - build/editor-style.css (editor styles)
	 * - build/view.js          (frontend script)
	 * - build/view-style.css   (frontend styles)
	 * - src/dashboard.js       (dashboard script)
	 *
	 * @param array $args The field configuration arguments.
	 * @return array Modified arguments with resolved asset paths.
	 */
	private static function resolve_plugin_assets( $args ) {
		...
		// Define conventional asset paths.
		// Each entry: [ 'url_key', 'deps_key', 'ver_key', 'relative_path', 'use_asset_deps', 'use_asset_ver' ]
		$conventions = array(
			array( 'editor_script', 'editor_script_deps', 'editor_script_ver', 'build/index.js', true, true ),
			array( 'editor_style', 'editor_style_deps', 'editor_style_ver', 'build/editor-style.css', false, true ),
			array( 'view_script', 'view_script_deps', 'view_script_ver', 'build/view.js', false, true ),
			array( 'view_style', 'view_style_deps', 'view_style_ver', 'build/view-style.css', false, true ),
			array( 'dashboard_script', 'dashboard_script_deps', 'dashboard_script_ver', 'src/dashboard.js', false, true ),
			array( 'dashboard_style', 'dashboard_style_deps', 'dashboard_style_ver', 'build/dashboard-style.css', false, true ),
		);

		foreach ( $conventions as $convention ) {
			list( $url_key, $deps_key, $ver_key, $relative_path, $use_asset_deps, $use_asset_ver ) = $convention;

			// Skip if explicitly set to false (disabled).
			if ( isset( $args[ $url_key ] ) && $args[ $url_key ] === false ) {
				$args[ $url_key ] = '';
				continue;
			}

			// Skip if already set to a non-empty value (explicit override).
			if ( ! empty( $args[ $url_key ] ) ) {
				continue;
			}

			// Check if the file exists at the conventional path.
			$file_path = $plugin_path . $relative_path;
			if ( file_exists( $file_path ) ) {
				$args[ $url_key ] = $plugin_url . $relative_path;

				// Set dependencies from asset file if applicable and not explicitly set.
				if ( $use_asset_deps && empty( $args[ $deps_key ] ) && ! empty( $asset['dependencies'] ) ) {
					$args[ $deps_key ] = $asset['dependencies'];
				}

				// Set version from asset file if applicable and not explicitly set.
				if ( $use_asset_ver && $args[ $ver_key ] === '1.0.0' && ! empty( $asset['version'] ) ) {
					$args[ $ver_key ] = $asset['version'];
				}
			}
		}
