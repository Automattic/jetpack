...
class Contact_Form_Plugin {
		...
		if ( $block && ! empty( $block->parsed_block['innerBlocks'] ) ) {
			...
			foreach ( $block->parsed_block['innerBlocks'] as $inner_block ) {
				...
				if ( 'jetpack/options' === $block_name ) {
					...
					$atts['optionsdata'] = \wp_json_encode( $options_data, JSON_UNESCAPED_SLASHES );
	...
	protected static function get_style_variation_shortcode_attributes( $block_name, $attrs ) {
		...
		return array(
			'stylevariationattributes' => isset( $picked_attributes['style'] ) ? \wp_json_encode( $picked_attributes['style'], JSON_UNESCAPED_SLASHES ) : '',
