...
- [ ] Install dependencies and build assets (see [Building the Plugin](#building-the-plugin) below)
...
After creating all files from the templates above, install dependencies and build the assets:

```bash
cd jetpack-forms-{FIELD_TYPE}-field
npm install
npm run build
```

This produces the `build/` directory with all compiled assets:

| Output File | Source | Purpose |
|-------------|--------|---------|
| `build/index.js` | `src/index.js` | Compiled editor block script |
| `build/index.asset.php` | _(auto-generated)_ | Dependency and version metadata |
| `build/index.css` | `src/editor.scss` | Editor styles bundled with the block |
| `build/editor-style.css` | `src/editor.scss` | Editor styles loaded via `register_block_type()` (iframe support) |
| `build/view.js` | `src/view.js` | ES module for Interactivity API on the frontend |
| `build/view-style.css` | `src/view.scss` | Frontend styles |
| `build/*-rtl.css` | _(auto-generated)_ | RTL stylesheet variants |

For development with live rebuilds, use `npm run start` instead.

**Note:** The `@babel/runtime` package is a required dependency (not just devDependency) — `@wordpress/scripts` needs it at build time via `rtlcss-webpack-plugin`. Make sure it's listed under `dependencies` in your `package.json`.

After building, activate the plugin in WordPress and the field block will be available in the form editor.
...
- `tools/docker/wordpress/wp-content/plugins/jetpack-forms-emoji-rating-field/` - Emoji experience rating field
