...
if ( ! function_exists( 'register_jetpack_forms_responses_wp_admin_route' ) ) {
	/**
	 ...
	 */
	function register_jetpack_forms_responses_wp_admin_route( $path, $content_module = null, $route_module = null ) {
...
if ( ! function_exists( 'register_jetpack_forms_responses_wp_admin_menu_item' ) ) {
	/**
	 ...
	 */
	function register_jetpack_forms_responses_wp_admin_menu_item( $id, $label, $to, $parent_id = '' ) {
...
if ( ! function_exists( 'jetpack_forms_responses_wp_admin_enqueue_scripts' ) ) {
	/**
	 ...
	 */
	function jetpack_forms_responses_wp_admin_enqueue_scripts( $hook_suffix ) {
		// Only enqueue on our page
		if ( ! isset( $_GET['page'] ) || 'jetpack-forms-responses-wp-admin' !== $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return;
		}
		...
		jetpack_forms_responses_wp_admin_preload_data();

		// Get all registered routes
		$routes = get_jetpack_forms_responses_wp_admin_routes();
...
if ( ! function_exists( 'jetpack_forms_responses_wp_admin_render_page' ) ) {
	/**
	 * Render the jetpack-forms-responses-wp-admin page.
	 * Call this function from add_menu_page or add_submenu_page.
	 * This renders within the normal WordPress admin interface.
	 */
	function jetpack_forms_responses_wp_admin_render_page() {
		?>
...
add_action( 'admin_enqueue_scripts', 'jetpack_forms_responses_wp_admin_enqueue_scripts' );

if ( ! function_exists( 'jetpack_forms_responses_wp_admin_register_page' ) ) {
	/**
	 * Register the jetpack-forms-responses-wp-admin base page handler.
	 ...
	 *
	 * Example:
	 * $url = admin_url( 'admin.php?page=jetpack-forms-responses-wp-admin&p=' . urlencode( '/your/route' ) );
	 * add_menu_page( 'Title', 'Menu', 'capability', $url, '', 'icon', 10 );
	 */
	function jetpack_forms_responses_wp_admin_register_page() {
		add_submenu_page(
			'nothing-jetpack-forms-responses',                        // Hidden page (no parent)
			__( 'jetpack-forms-responses (WP Admin)', 'gutenberg' ), // Page title
			__( 'jetpack-forms-responses (WP Admin)', 'gutenberg' ), // Menu title (not visible)
			'read',                                          // Minimum capability
			'jetpack-forms-responses-wp-admin',                       // Menu slug for URL routing
			'jetpack_forms_responses_wp_admin_render_page' // Callback function
		);
	}
}

// Hook the registration to admin_menu
add_action( 'admin_menu', 'jetpack_forms_responses_wp_admin_register_page' );
