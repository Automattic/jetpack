...
class AJAX {
	...
	private function is_current_user_authed_for_video( $guid, $embedded_post_id ) {
		...
		switch ( $video_info->privacy_setting ) {
			...
			default:
				// check if site is private to use this info as the default value for the Jetpack option, in the case it's not defined
				$blog_is_private                = ( intval( get_option( 'blog_public', '' ) ) === -1 );
				$is_videopress_private_for_site = get_option( 'videopress_private_enabled_for_site', $blog_is_private );
