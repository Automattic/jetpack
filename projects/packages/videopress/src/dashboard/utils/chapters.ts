 ...
 *
 * Line recognition reuses the legacy `extractSingleChapter` directly (same
 * cross-tree import precedent as `../hooks/use-upload.ts`), so a line is a
 * chapter here exactly when the legacy pipeline would treat it as one.
 * Unlike the legacy `extractVideoChapters`, `parseDescription` keeps rows
 * in document order instead of sorting: `serializeDescription` maps
...
import { extractSingleChapter } from '../../client/utils/video-chapters/extract-video-chapters';
