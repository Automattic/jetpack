...
describe( 'useSettings', () => {
	it( 'fetches videopress/v1/settings', async () => {
		mockApiFetch( async ( { path } ) => {
			if ( path === '/videopress/v1/settings' ) {
				return {
					...
					videopress_auto_generated_captions_disabled: true,
		...
		expect( result.current.data?.videoPressAutoGeneratedCaptionsDisabled ).toBe( true );
...
describe( 'useUpdateSettings', () => {
	it( 'POSTs videopress_videos_private_for_site and optimistically updates the cache', async () => {
		...
		mockApiFetch( async ( { path, method, data } ) => {
			...
			return {
				...
				videopress_auto_generated_captions_disabled: false,
				site_is_private: false,
				site_type: 'jetpack',
			};
		} );
	...
	it( 'POSTs videopress_auto_generated_captions_disabled and optimistically updates the cache', async () => {
		const calls: { path?: string; method?: string; data?: unknown }[] = [];
		// Track server state so the refetch after onSettled returns the updated value.
		let serverValue = false;
		mockApiFetch( async ( { path, method, data } ) => {
			...
			if ( method === 'POST' ) {
				serverValue = ( data as Record< string, unknown > )
					?.videopress_auto_generated_captions_disabled as boolean;
				return { code: 'success', message: 'ok', data: 200 };
			}
			return {
				videopress_videos_private_for_site: false,
				videopress_auto_generated_captions_disabled: serverValue,
				site_is_private: false,
				site_type: 'jetpack',
			};
		} );
		...
		await act( async () => {
			await mutationResult.current.mutateAsync( {
				videoPressAutoGeneratedCaptionsDisabled: true,
			} );
		} );

		const postCall = calls.find( c => c.method === 'POST' );
		expect( postCall?.data ).toEqual( { videopress_auto_generated_captions_disabled: true } );
		await waitFor( () =>
			expect( settingsResult.current.data?.videoPressAutoGeneratedCaptionsDisabled ).toBe( true )
		);
	} );
