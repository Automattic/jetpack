...
/**
 * Create an isolated query client and mutation wrapper.
 *
...
describe( 'useRestoreOriginal', () => {
	it( 'DELETEs the guid-scoped edits endpoint and invalidates the edits query', async () => {
		...
		const { client, wrapper, invalidateSpy } = makeWrapper();
		client.setQueryData( [ EDITS_QUERY_KEY, 'abc123' ], {
			...acceptedResponse,
			job: { ...acceptedResponse.job, status: 'idle' },
		} );
		...
		expect( client.getQueryData( [ EDITS_QUERY_KEY, 'abc123' ] ) ).toEqual( acceptedResponse );
