...
import type { ApiPlaylistTerm, Playlist, PlaylistType } from '../types/playlist';
 ...
 * /wp/v2/videopress-playlists. The playlist type rides in the term meta
 * (`vps_playlist_type`); name and description are core term fields. The
		...
		mutationFn: async ( { name, description, type } ) => {
			const raw = await apiFetch< ApiPlaylistTerm >( {
				...
				data: {
					...
					...( type !== undefined ? { meta: { vps_playlist_type: type } } : {} ),
				},
			} );
