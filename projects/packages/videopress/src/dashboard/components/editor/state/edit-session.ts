...
/**
 * Sum of removed time: the union of the given ranges intersected with the window.
 *
...
/**
 * The minimum output the next action must preserve.
 *
...
/**
 * Largest integer x in [lo, hi] satisfying `pred`, assuming `pred` is monotone true→false as x
 * grows.
 *
...
/**
 * Smallest integer x in [lo, hi] satisfying `pred`, assuming `pred` is monotone false→true as x
 * grows.
 *
 * @param lo   - Lower bound (inclusive).
 * @param hi   - Upper bound (inclusive).
...
/**
 * Keep the selection only while its cut still exists.
 *
 * @param cuts       - Next cut list.
 * @param selectedId - Previously selected id.
 * @return The normalized selected id or null.
 */
function normalizeSelection( cuts: CutRange[], selectedId: string | null ): string | null {
...
/**
 * Structural equality of the persistable edit state of two sessions — everything except the
 * selection.
 *
 * @param a - One session.
...
/**
 * Full structural equality of two sessions (edits AND selection), used to return the previous
 * reference on no-op actions.
 *
 * @param a - One session.
 * @param b - Another session.
...
/**
 * Build a session from serialized operations.
 *
...
/**
 * Move the trim start, clamped so the output never drops below the required minimum.
 *
...
/**
 * Move the trim end, clamped so the output never drops below the required minimum.
 *
 * @param state - Current session.
...
/**
 * Move one or both edges of a cut.
 *
 * @param state   - Current session.
...
/**
 * Move a whole cut, preserving its width.
 *
 * @param state   - Current session.
