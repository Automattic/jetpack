...
describe( 'integration with the edit-session reducer', () => {
	...
	it( 'coalesces a trim drag into one undo entry', () => {
		...
		for ( const ms of [ 500, 1000, 1500, 2000 ] ) {
			h = editorReducer( h, { type: 'TRANSIENT', action: { type: 'SET_TRIM_START', ms } } );
		}
	...
	it( 'RESET clears editor history', () => {
		let h = createHistory( createEditSession( 10000 ) );
		h = editorReducer( h, { type: 'SET_TRIM_START', ms: 2000 } );
		h = editorReducer( h, { type: 'ADD_CUT', atMs: 5000, id: 'c1' } );
		expect( h.past ).toHaveLength( 2 );

		h = editorReducer( h, { type: 'RESET' } );
		expect( h.past ).toEqual( [] );
		expect( h.future ).toEqual( [] );
		expect( h.present ).toEqual( createEditSession( 10000 ) );
	} );
} );
