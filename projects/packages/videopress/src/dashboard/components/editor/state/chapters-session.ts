/**
 * Pure chapters-session state core for the Studio Chapters tool.
 *
 * Chapters are stored second-granular: the video description's `H:MM:SS`
 * lines are the single source of truth (`utils/chapters.ts` owns
 * parse/serialize/validate). The session works in integer MILLISECONDS so
 * marker drags can be smooth, and quantizes back to whole seconds when a
