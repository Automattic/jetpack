...
/**
 * A 10s session with one cut from 4000 to 6000ms (selected — ADD_CUT selects the cut it creates).
 *
 * @return The session.
 */
...
function transient( action: EditSessionAction ) {
	return action.type === 'SELECT_CUT'
		? { type: 'TRANSIENT', action }
		: { type: 'TRANSIENT', fromBase: true, action };
}

/**
 * Shorthand for a from-base TRANSIENT-wrapped session action (body moves replay against the
 * gesture's base state).
 *
 * @param action - The inner action.
 * @return The wrapped action.
 */
...
describe( 'StudioEditorTrimHandles', () => {
	...
	it( 'exposes slider semantics on the handles', () => {
		...
		expect( handle ).toHaveValue( 10000 );
...
/**
 * Exercise gestures through real session and history reducers.
 *
 * @param props         - Initial editor state.
 * @param props.session - Session before the gesture.
 * @return A stateful overlay and undo control.
 */
function StatefulOverlay( { session }: { session: EditSession } ) {
	const [ history, dispatch ] = useReducer(
		withHistory( editSessionReducer ),
...
it( 'restores cuts clipped by an earlier trim preview when the handle returns', () => {
	render( <StatefulOverlay session={ sessionWithCut() } /> );
	const handle = screen.getByRole( 'slider', { name: 'Trim start' } );
	fireEvent.pointerDown( handle, { button: 0, pointerId: 1, clientX: 0 } );
	fireEvent.pointerMove( handle, { pointerId: 1, clientX: 700 } );
	expect( screen.queryByTestId( 'studio-timeline-cut-cut-a' ) ).not.toBeInTheDocument();
	fireEvent.pointerMove( handle, { pointerId: 1, clientX: 0 } );
	fireEvent.pointerUp( handle, { pointerId: 1 } );
	expect( screen.getByRole( 'slider', { name: 'Cut start' } ) ).toHaveValue( 4000 );
	expect( screen.getByRole( 'slider', { name: 'Cut end' } ) ).toHaveValue( 6000 );
} );

it( 'restores a neighboring cut when a resizing edge moves back after merging', () => {
	render( <StatefulOverlay session={ sessionWithTwoCuts() } /> );
	const handle = screen.getByTestId( 'studio-timeline-cut-start-cut-b' );
	...
	fireEvent.pointerUp( handle, { pointerId: 1 } );
	expect( screen.getByTestId( 'studio-timeline-cut-cut-a' ) ).toBeInTheDocument();
	expect( screen.getByTestId( 'studio-timeline-cut-start-cut-b' ) ).toHaveValue( 4000 );
} );

it( 'keeps the endpoint trim hit areas inside the video bounds', () => {
	renderOverlay( createEditSession( 10000 ) );
...
it( 'selects a resized cut even when another cut was selected at grab time', () => {
	render( <StatefulOverlay session={ sessionWithTwoCuts() } /> );
	const handle = screen.getByTestId( 'studio-timeline-cut-start-cut-a' );
...
it( 'focuses a grabbed cut so the document shortcuts can act on it', () => {
	renderOverlay( sessionWithCut() );
	const cut = screen.getByRole( 'group', { name: 'Cut' } );
	fireEvent.pointerDown( cut, { button: 0, pointerId: 1, clientX: 500 } );
