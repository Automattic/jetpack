...
function getTicks(): HTMLElement[] {
	return screen.queryAllByTestId( 'studio-timeline-ruler-tick' );
}

describe( 'StudioEditorTimeRuler', () => {
	it( 'places a labelled tick per adaptive step across the duration', () => {
		// pxPerMs 0.1 → the smallest step with >= 80px spacing is 1000ms.
		render( <StudioEditorTimeRuler durationMs={ 10000 } pxPerMs={ 0.1 } /> );
	...
	it( 'widens the step when zoomed out', () => {
		...
		render( <StudioEditorTimeRuler durationMs={ 60000 } pxPerMs={ 0.01 } /> );
		const ticks = getTicks();
	...
	it( 'renders no ticks without a usable scale or duration', () => {
		const { rerender } = render( <StudioEditorTimeRuler durationMs={ 10000 } pxPerMs={ 0 } /> );
		expect( getTicks() ).toHaveLength( 0 );
		rerender( <StudioEditorTimeRuler durationMs={ 0 } pxPerMs={ 0.1 } /> );
		expect( getTicks() ).toHaveLength( 0 );
	} );
} );
