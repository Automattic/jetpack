...
function CaptionManagerModalInner( {
	...
	const startManualTrack = useCallback(
		async ( sourceTrack: VideoTextTrack | null = null ) => {
			...
			let nextManualTrack: ManualTrack;
			if ( ! sourceTrack ) {
				nextManualTrack = emptyManualTrack( defaultLanguageTag );
			} else if ( isAutoGeneratedTrack( sourceTrack ) ) {
				/*
				 * Editing an auto-generated track authors a fresh manual track for its
				 * language: seed it like a new track (default kind, language-derived
				 * label) rather than inheriting the "Auto-Generated" server label,
				 * which would make the published track read as auto-generated.
				 */
				nextManualTrack = emptyManualTrack(
					getManualLanguageTagFromTrackKey( sourceTrack.srcLang )
				);
			} else {
				nextManualTrack = {
