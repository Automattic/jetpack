 ...
 * track for the target language.
 *
 * An auto-generated track can't be overwritten in place — it lives under a
 * generated `auto_*` key while the published track uses the real language tag —
 * but publishing under its own underlying language supersedes it, so it's
 * returned for removal instead of lingering as a duplicate. Publishing under a
 * different language leaves the auto-generated track untouched.
 *
...
export const resolveManagedTrackToUpdate = (
	...
	const sourceTrackMatch =
		sourceTrack &&
		( ! isAutoGeneratedTrack( sourceTrack ) ||
			getManualLanguageTagFromTrackKey( sourceTrack.srcLang ) === srcLang )
