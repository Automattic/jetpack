...
describe( 'CaptionManagerModal', () => {
	...
	it( 'replaces an auto-generated track when publishing manual edits under its language', async () => {
		const user = userEvent.setup();
		const onTracksChange = jest.fn();
		const autoTrack = {
			kind: 'captions' as const,
		...
		await waitFor( () =>
			expect( fetchTrackContentForGuid ).toHaveBeenCalledWith( autoTrack, 'abc123', false )
		);
		expect( screen.getByLabelText( 'Language' ) ).toHaveValue( 'en' );
		await user.click( screen.getByText( 'Publish' ) );

		await waitFor( () => expect( uploadTrackForGuid ).toHaveBeenCalled() );
		// The published track carries the language, not the source's "Auto-Generated" label.
		expect( uploadTrackForGuid ).toHaveBeenCalledWith(
			expect.objectContaining( { srcLang: 'en', label: 'English' } ),
			'abc123'
		);
		await waitFor( () => expect( saveCaptionTrack ).toHaveBeenCalled() );
		expect( saveCaptionTrack ).toHaveBeenCalledWith(
		...
		// The superseded auto-generated track is removed, not left behind as a duplicate.
		await waitFor( () =>
			expect( deleteTrackForGuid ).toHaveBeenCalledWith(
				{ kind: 'captions', srcLang: 'auto_en' },
				'abc123'
			)
		);
		// A single English track remains; it must not read as auto-generated.
		expect( onTracksChange ).toHaveBeenCalledWith( [
			expect.objectContaining( {
				srcLang: 'en',
	...
	it( 'removes the auto-generated source track when republishing it under its language', async () => {
		const user = userEvent.setup();
		render(
			<CaptionManagerModal
				{ ...defaultProps }
		...
		// The superseded auto-generated track is the one removed, not the co-existing manual track.
		await waitFor( () =>
			expect( deleteTrackForGuid ).toHaveBeenCalledWith(
				{ kind: 'captions', srcLang: 'auto_en' },
				'abc123'
		...
		expect( deleteTrackForGuid ).not.toHaveBeenCalledWith(
			{ kind: 'captions', srcLang: 'en' },
			'abc123'
		);
	} );
