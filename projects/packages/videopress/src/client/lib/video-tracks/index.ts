...
/**
 * Whether this is the modernized VideoPress dashboard on a WordPress.com Simple
 * site (wp-admin). It emits `JPVIDEOPRESS_INITIAL_STATE`; Calypso does not. There
 * the proxied apiFetch/global path carries no upload authorization (403), and
 * `getMediaToken('upload')` can't mint a one-time token — its admin-ajax handler
 * reaches the classic `media/token` endpoint in-process, which isn't dispatchable
 * on Simple. So we mint the token via the site-scoped proxy and take the same
 * direct public-api fetch self-hosted Jetpack uses.
 *
 ...
 * public-api fetch: self-hosted Jetpack and Atomic, plus the Simple dashboard.
 * Only Calypso keeps the proxied apiFetch/global path.
 *
 * @return {boolean} Whether to use the upload-token transport.
 */
const shouldUseUploadTokenFetch = () => shouldUseJetpackVideoFetch() || isSimpleDashboard();

/**
 * Obtain a one-time upload token for a track request. On the Simple dashboard the
 * only reachable minter is the site-scoped `media/token` proxy endpoint (the
 * user's auth reaches it, and it returns a blog-scoped token the native tracks
 * endpoint accepts); elsewhere the admin-ajax token works.
 *
...
export const uploadTrackForGuid = ( track: UploadTrackDataProps, guid: string ) => {
	...
	if ( shouldUseUploadTokenFetch() ) {
		return new Promise( function ( resolve, reject ) {
			getUploadTokenData( tmpFile.name )
				.then( ( { token, blogId } ) => {
