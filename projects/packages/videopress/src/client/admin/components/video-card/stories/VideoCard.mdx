import { Meta, Story, Canvas } from '@storybook/addon-docs';
import VideoCard from '../index';

<Meta
  title="Packages/VideoPress/Video Details Card"
  component={VideoCard}
/>

# VideoCard

React component that shows a card with details about the given video. It is possible to change it via the edit button.
It is composed of two simpler components: VideoThumbnail and VideoDetails components.

<Story id="packages-videopress-video-card--default" />

-------

# VideoThumbnailDropdown

<Story id="packages-videopress-video-card--video-dropdown" />

# VideoThumbnail

<Story id="packages-videopress-video-card--video-thumbnail" />

# VideoDetails

<Story id="packages-videopress-video-card--video-details" />
