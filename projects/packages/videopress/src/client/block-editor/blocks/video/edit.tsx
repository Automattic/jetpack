...
import useAutoGeneratedChapter from '../../hooks/use-auto-generated-chpater';
import useMediaToken from '../../hooks/use-media-token';
import { usePreview, UsePreviewResult } from '../../hooks/use-preview';
 ...
 * Types
 */
import type { VideoBlockAttributes, TrackProps, VideoPreview } from './types';
...
export default function VideoPressEdit( {
	...
	// Get media token for private videos.
	const token = useMediaToken( { guid, isPrivate, tokenType: 'playback' } );

	// Store if the chapters file is auto-generated in a local state.
	const chapter = tracks?.find( ( track: TrackProps ) => track.kind === 'chapters' )?.[ 0 ];
	const isAutoGeneratedChapter = useAutoGeneratedChapter( { guid, chapter, token, isPrivate } );
	...
	const { preview, isRequestingEmbedPreview }: UsePreviewResult = usePreview( videoPressUrl );
	const {
		html: previewHtml,
		scripts,
		width: previewWidth,
