...
export default function VideoPressEdit( { attributes, setAttributes, isSelected, clientId } ) {
	...
	const [ isAutoGeneratedChapter, setIsAutoGeneratedChapter ] = useState( false );
	...
	const chapterUrl = chapter
		? `https://videos.files.wordpress.com/${ guid }/${ chapter.src }`
		: null;
	...
	useEffect( () => {
		...
		fetch( chapterUrl ).then( function ( response ) {
