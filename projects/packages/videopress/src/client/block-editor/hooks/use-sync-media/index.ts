		...
		updateMediaHandler( dataToUpdate )
			.then( () => {
				...
				if (
					...
					/*
					 * Real duration (ms) for the final cue's end time — matching the
					 * chapters `syncChapters` output — with the generator's own
					 * 999:59:59 sentinel as fallback when the duration is unknown.
					 */
					const videoDurationMs =
						typeof videoData?.duration === 'number' && videoData.duration > 0
							? videoData.duration
							: undefined;
					const track: UploadTrackDataProps = {
						...
						srcLang: 'en',
						kind: 'chapters',
						tmpFile: generateChaptersFile( dataToUpdate.description, videoDurationMs ),
					};

					debug( 'Autogenerated track: %o', track );

					uploadTrackForGuid( track, attributes.guid )
						.then( response => {
							const uploadedTrack = normalizeVideoTextTrackResponse( response, track );

							/*
							 * A response with no stored track file is an upload failure
							 * (e.g. an error envelope); surface it instead of persisting
							 * a src-less track and refreshing the preview.
							 */
							if ( ! uploadedTrack?.src ) {
								debug( 'auto-generated chapter track upload failed: %o', response );
								const errorBody = response as { message?: string; error?: string } | null;
								setError(
									new Error(
										errorBody?.message ||
											errorBody?.error ||
											__( 'Failed to upload chapters track.', 'jetpack-videopress-pkg' )
									)
								);
								return;
							}
						...
						.catch( ( uploadError: Error ) => {
							debug(
								'%o Error uploading auto-generated chapters track: %o',
								attributes?.guid,
								uploadError
							);
