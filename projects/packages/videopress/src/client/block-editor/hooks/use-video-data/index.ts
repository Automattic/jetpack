...
export default function useVideoData( {
	...
	const [ isAutogeneratedChapter, setIsAutogeneratedChapterFile ] = useState( false );
		...
		async function fetchVideoItem( token: string | null = null ) {
			try {
				...
				if ( enChapterFilename ) {
					...
					setIsAutogeneratedChapterFile( isAutogenerated );
				}
			} catch ( errorData ) {
				/*
	...
	return { videoData, isRequestingVideoData, isAutogeneratedChapter };
}
