...
class Admin_UI {
	...
	/**
	 * Filter name that gates the Studio expansion of the wp-build dashboard.
	 *
	 * When this filter returns true, the dashboard exposes the additional
	 * Studio routes (playlists, per-video analytics, video editor). Unlike
	 * the modernization filter, this defaults to false.
	 */
	const STUDIO_FILTER = 'jetpack_videopress_studio';

	/**
	 * Route paths that belong to the Studio expansion.
	 *
	...
	/**
	 * Remove the Studio routes from the generated route registry when the
	 * Studio filter is off.
	 *
	...
	public static function maybe_strip_studio_routes() {
		...
		// Studio routes would leak, but nothing fatals.
		if ( empty( $GLOBALS[ $global_name ] ) || ! is_array( $GLOBALS[ $global_name ] ) ) {
			return;
		}

		$GLOBALS[ $global_name ] = self::strip_studio_routes( $GLOBALS[ $global_name ] );
	}

	/**
	 * Filter the Studio route entries out of a wp-build route registry array.
	 *
	 * Pure helper extracted from `maybe_strip_studio_routes()` so the
	 * filtering logic is unit-testable without the generated global.
	 *
	 * @internal For use by `maybe_strip_studio_routes()` and tests only.
	 *
	 * @param array $routes Route entries as generated by `build/routes.php`, each an array with a `path` key.
	 * @return array Route entries with the Studio paths removed.
	 */
	public static function strip_studio_routes( $routes ) {
		return array_values(
			array_filter(
				$routes,
				static function ( $route ) {
					return ! isset( $route['path'] ) || ! in_array( $route['path'], self::STUDIO_ROUTE_PATHS, true );
				}
			)
		);
	}
	...
	/**
	 * Returns true when the Studio feature filter is enabled.
	 *
	 * Note the default is false: the Studio routes and UI stay hidden unless
	 * a site explicitly opts in via the filter.
	 *
	 * @return bool
	 */
	public static function is_studio_enabled() {
		return (bool) apply_filters( self::STUDIO_FILTER, false );
	}
