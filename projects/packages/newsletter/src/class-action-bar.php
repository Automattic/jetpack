...
/**
 * The floating Action Bar on the front end of WordPress.com Simple sites. Ported from wpcom `wp-content/mu-plugins/actionbar.php`.
 */
class Action_Bar {
	/**
	...
	public static function load() {
		...
		add_action( 'wp_ajax_unfold_actionbar', array( __CLASS__, 'unfold' ) );
		add_action( 'wp_ajax_nopriv_unfold_actionbar', array( __CLASS__, 'unfold' ) );
		add_action( 'wp_ajax_actionbar_stats', array( __CLASS__, 'ajax_stats' ) );
		add_action( 'wp_ajax_nopriv_actionbar_stats', array( __CLASS__, 'ajax_stats' ) );
	}
	...
	public static function enqueue_scripts() {
				...
				$status_message .= '<div>' .
					wp_kses(
						sprintf(
							...
							esc_url( self::localized_url( 'https://wordpress.com/support/contact/' ) )
						),
						array(
							'a' => array(
								'href' => array(),
		...
		if ( ! is_user_logged_in() && self::logged_out_follow_disabled() ) {
			self::restore_locale();
			return;
		}
	...
	public static function footer() {
		...
		$asset_path = dirname( __DIR__ ) . '/build/action-bar.asset.php';
	 ...
	 * @return bool
	 */
	private static function is_vip() {
	 ...
	 * @return string
	 */
	private static function localized_url( $url ) {
	 ...
	 * @return bool
	 */
	private static function is_folded( $user_id ) {
	 ...
	 */
	private static function set_folded( $user_id, $folded ) {
	 ...
	 * @return bool
	 */
	private static function logged_out_follow_disabled() {
		$settings = get_option( 'subscription_options' );
		$disabled = self::is_vip() && ( ! isset( $settings['loggedoutfollow'] ) || 'off' === $settings['loggedoutfollow'] );
	 ...
	 */
	private static function switch_to_user_locale() {
	 ...
	 */
	private static function email_default( $current_user ) {
	 ...
	 */
	private static function icon_markup( $name, $size = 24 ) {
		$fill_icons = array(
	...
	private static function icon( $name, $size = 24 ) {
		echo self::icon_markup( $name, $size ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Static SVG markup, name escaped in the builder.
	}
	 ...
	 * @return string
	 */
	private static function menu_item( $args ) {
		$args = wp_parse_args(
			$args,
			array(
				'href'   => '',
		...
		return sprintf(
			...
			esc_html( $args['label'] ),
			$args['icon'] ? '<span class="actnbr-menu__icon">' . self::icon_markup( $args['icon'], 18 ) . '</span>' : ''
		);
	}
	 ...
	 */
	private static function menu_group( array $items ) {
	 ...
	 * @return string
	 */
	private static function blavatar() {
	 ...
	 */
	private static function site_title( $site_url, $site_name, $blavatar ) {
		$item = self::menu_item(
			array(
				'href'   => $site_url,
				'label'  => $site_name,
	 ...
	 */
	private static function follow_links( $is_following ) {
		?>
	 ...
	 */
	private static function html( $is_rtl ) {
		$current_user = wp_get_current_user();
		$site_id      = get_current_blog_id();
		...
		if (
			...
			! self::logged_out_follow_disabled() &&
		...
		if ( is_singular() ) {
			...
			if ( $should_use_calypso_links && ! self::is_vip() && ( ! is_super_admin() || $is_member ) ) {
				$path_prefix = null;
			...
			<ul>
					...
					?>
						<li class="actnbr-btn">
							<a class="actnbr-action actnbr-actn-reblog" href="" role="button">
								<?php self::icon( 'reusable-block', 20 ); ?><span><?php esc_html_e( 'Reblog', 'jetpack-newsletter' ); ?></span>
							</a>
						</li>
					<?php
				}
				...
				if ( $show_follow ) {
					?>
						<li class="actnbr-btn actnbr-hidden">
							<?php self::follow_links( $is_following ); ?>
							<div class="actnbr-popover actnbr-panel actnbr-notice" id="follow-bubble" role="dialog" aria-label="<?php echo esc_attr( sprintf( /* translators: %s: site name */ __( 'Subscribe to %s', 'jetpack-newsletter' ), $site_name ) ); ?>">
								...
								<?php self::site_title( $site_url, $site_name, $blavatar ); ?>
								<?php
								if ( $is_logged_in ) {
									?>
				...
				<li class="actnbr-ellipsis actnbr-hidden">
					<button type="button" class="actnbr-more-toggle" aria-haspopup="true" aria-expanded="false" aria-label="<?php esc_attr_e( 'More options', 'jetpack-newsletter' ); ?>">
						<?php self::icon( 'more-horizontal', 24 ); ?>
					</button>
					<div class="actnbr-popover actnbr-menu" role="menu" aria-label="<?php esc_attr_e( 'Site options', 'jetpack-newsletter' ); ?>">
						...
						if ( $is_singular ) {
							$items[] = sprintf(
								...
								self::icon_markup( 'copy', 18 ),
								self::icon_markup( 'check', 18 )
							);
						}
	 ...
	 * @return bool
	 */
	private static function can_reblog( $site_id, $post_id ) {
	 ...
	 * @return bool
	 */
	private static function gdpr_applies() {
	 ...
	 */
	private static function bump_stat( $stat_value ) {
		$whitelist = array(
			'clicked_login_link',
			'clicked_login_nudge',
	...
	public static function fold() {
		self::bump_stat( 'folded' );

		if ( is_user_logged_in() ) {
			self::set_folded( get_current_user_id(), true );
		}
	...
	public static function unfold() {
		self::bump_stat( 'expanded' );

		if ( is_user_logged_in() ) {
			self::set_folded( get_current_user_id(), false );
		}
	...
	public static function settings_field() {
		add_settings_field( 'wpcom_hide_action_bar', __( 'Action Bar visibility', 'jetpack-newsletter' ), array( __CLASS__, 'settings_field_display' ), 'general', 'default', array( 'label_for' => 'wpcom_hide_action_bar' ) );
	 ...
	 */
	public static function settings_field_display() {
		?>
		...
		<p class="description"><a href="<?php echo esc_url( self::localized_url( 'https://wordpress.com/support/action-bar/' ) ); ?>" data-target="wpcom-help-center"><?php esc_html_e( 'Learn more about the Action Bar', 'jetpack-newsletter' ); ?></a>.</p>
		<?php
	}
}
