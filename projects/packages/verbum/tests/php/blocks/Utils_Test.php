<?php
/**
 * Tests for the Verbum block utilities.
 *
 * @package automattic/jetpack-verbum
 */

namespace Automattic\Jetpack\Verbum\Tests;
 ...
 * Tests for the Verbum block utilities.
 *
 * @covers \Automattic\Jetpack\Verbum\Blocks\Utils
 */
#[CoversClass( Utils::class )]
class Utils_Test extends BaseTestCase {
	...
	public function test_comment_text_string_comment() {
		...
		$filtered_content = Utils::render_verbum_blocks( $comment_content );
	...
	public function test_comment_text_block_sanitization_sanity_check() {
		...
		$filtered_content = preg_replace( '/\R+/', '', Utils::render_verbum_blocks( $comment_content ) );
	...
	public function test_pre_comment_content_string_comment() {
		$comment_content  = 'This is a test comment';
		$filtered_content = Utils::remove_blocks( $comment_content );
		$this->assertEquals( $comment_content, $filtered_content );
	}
...
HTML;
		...
		$filtered_content = Utils::remove_blocks( wp_slash( $comment_content ) );
	...
	public function test_security_malformed_block_delimiters() {
		...
		foreach ( $malformed_inputs as $input ) {
			$result = Utils::remove_blocks( $input );
	...
	public function test_security_input_validation_consistency() {
		...
		foreach ( $test_inputs as $input ) {
			...
			$result        = Utils::remove_blocks( $slashed_input );
	...
	public function test_consistency_between_scanner_and_parse_blocks() {
		...
		foreach ( $test_cases as $test_case ) {
			$result = Utils::remove_blocks( $test_case );
	 ...
	 *
	 * @todo Remove once WP 7.0 is the minimum supported version.
	 *
	 ...
	 */
	private function get_version_specific_expected_html( $html_without_layout_classes, $html_with_layout_classes ) {
		global $wp_version;

		if ( ! empty( $wp_version ) && version_compare( $wp_version, '7.0-dev', '>=' ) ) {
