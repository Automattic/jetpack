<?php
/**
 * Verbum settings on the Discussion screen.
 *
...
class Settings {
	...
	protected function setup_globals() {
		...
		$this->default_greeting = __( 'Leave a Reply', 'jetpack-verbum' );
		...
		$this->color_schemes = array(
			'light'       => __( 'Light', 'jetpack-verbum' ),
			'dark'        => __( 'Dark', 'jetpack-verbum' ),
			'transparent' => __( 'Transparent', 'jetpack-verbum' ),
		);
	}
	...
	public function add_settings() {
		...
		add_settings_field(
			...
			__( 'Allow Blocks', 'jetpack-verbum' ),
		...
		add_settings_field(
			...
			__( 'Greeting Text', 'jetpack-verbum' ),
		...
		add_settings_field(
			...
			__( 'Color Scheme', 'jetpack-verbum' ),
	...
	public function comment_form_settings_section() {
		...
		<p id="jetpack-comments-settings"><?php esc_html_e( 'Adjust your Comments form with a clever greeting and color-scheme.', 'jetpack-verbum' ); ?></p>
	...
	public function verbum_field() {
		?>
			<label><input name="enable_verbum_commenting" type="checkbox" <?php checked( $this->is_verbum_enabled(), true, true ); ?> value="1" />
				<?php esc_html_e( 'Let visitors use a WordPress.com or Facebook account to comment', 'jetpack-verbum' ); ?>
			</label>
		<?php
	}
	...
	public function allow_blocks_field() {
		?>
			<label><input name="enable_blocks_comments" type="checkbox" <?php checked( $this->are_blocks_enabled(), true, true ); ?> value="1" />
				<?php esc_html_e( 'Enable blocks in comments', 'jetpack-verbum' ); ?>
			</label>
		<?php
	}
	...
	public function comment_form_greeting_setting() {
		...
		<p class="description"><?php esc_html_e( 'A few catchy words to motivate your readers to comment', 'jetpack-verbum' ); ?></p>
	...
	public function comment_form_color_scheme_setting() {
		...
		<fieldset>
			<legend class="screen-reader-text"><?php esc_html_e( 'Color Scheme', 'jetpack-verbum' ); ?></legend>
