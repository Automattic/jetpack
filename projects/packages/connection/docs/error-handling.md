...
* `useConnectionErrorNotice()` (`projects/js-packages/connection/hooks/use-connection-error-notice`) reads `connectionErrors` from the connection store, picks the effective error to show a given viewer (preferring an error with a message and an actionable, non-`'none'` action that isn't another user's to fix — see `isOtherUsersConnectionError`), and resolves it into ready-to-render `actions` via `resolveConnectionErrorActions()`. It also exposes `restoreConnection`/`isRestoringConnection` for the reconnect CTA, `connectionOwner`/`isCurrentUserConnectionOwner`, `currentUserId` for viewer-scoping, and a `severity` (`'error'` or `'warning'`) rating the errors for the viewer: `warning` when every error is the connection owner's and the viewer is not the owner, since only the owner can repair it.
* **Click tracking is built in.** The reconnect CTA, and (via `trackNoticeLinkClick`/`trackSupportLinkClick`) the Site Health and support links, record one canonical Tracks event each — `CONNECTION_ERROR_NOTICE_EVENTS` in `hooks/use-connection-error-notice/tracking.ts` — with a `{ context, error_code, audience }` payload. A consumer that renders `<ConnectionError />` (or the notice component) gets this for free; pass a `trackingContext` (e.g. `'my-jetpack'`, `'protect'`) so its events are attributable to that surface. Consumers that supply a `trackingCallback` route the events through their own analytics; everyone else records straight to Tracks (so the host must load `jp-tracks` on that screen).
* `<ConnectionError />` is a thin wrapper around the hook that renders `<ConnectionErrorNotice />` directly, for consumers that don't need the hook's raw data. It passes the hook's `severity` through, so the notice's intent matches the rating.
...
A viewer holding only `jetpack_connect_user` still gets the default reconnect CTA for their own token error. `/connection/reconnect` accepts `jetpack_connect_user`, and sends a caller without `jetpack_reconnect` to `Manager::refresh_user_token( false )` instead of `restore()`. That refreshes only their own user token: it never touches the site connection, refuses the connection owner, returns an error asking for an administrator when the blog token is unhealthy (or a retryable one when its health cannot be checked), and removes the local token only once WordPress.com confirms the unlink, so a failed unlink never leaves the user without a token.

The relink runs over the site connection, so while a displayable site-audience error is on record (other than those that set `survives_owner_promotion`, which are inbound failures and leave the outbound relink working), that viewer's own error instead gets `action = 'none'` and a message asking an administrator to restore the site connection first. A reporter-declared action is left alone. The record can be stale, so `refresh_user_token( false )` still checks the blog token live when the CTA is used.
...
An error's *action* (`error_data['action']`, e.g. `'none'` to suppress the reconnect CTA — readers treat a missing action as `'reconnect'`) can come from either side: most reporters declare it at creation time (see `build_action_error_data()`), while `get_displayable_errors()` also sets it dynamically for the non-transferable secondary-admin case and the non-admin's own error while the site connection is broken, both above. It's only ever emitted when it deviates from the default, since injecting an explicit `'reconnect'` could trip consumer code paths reserved for custom actions.
