...
Stored errors are deleted automatically when the connection is restored or torn down — on site registration, reconnection, disconnection, token deletion, user unlink, and user-token update. Additionally, a successful API request clears all `xmlrpc` and `rest` type errors via `delete_all_api_errors()`. That happens on `jetpack_get_site_data_success` and on any later signed request that returns HTTP 200 (`check_api_response_for_errors()`): outgoing errors are stored as verified without a WordPress.com round-trip, so a successful later request is what refutes them. `local_state`-type errors are deliberately kept, because a successful API round-trip says nothing about state such as a missing connection owner or WordPress.com being blocked from reaching the site. A successful reconnect (`/connection/reconnect`) deletes every stored error, including `local_state`.
