...
One case gets a different control: a viewer holding only `jetpack_connect_user` sees their own token error with `action = 'none'`, because the notice's reconnect CTA is the site-scoped one they cannot use, plus `self_service = 'connect_user'`. The JS notice turns that flag into a "Reconnect your account" CTA, which unlinks the viewer's broken token (through the blog token, so it still works) and then sends them to authorize again. Clients that predate the flag see only `'none'` and offer no CTA, never the dead reconnect.
