...
A viewer holding only `jetpack_connect_user` still gets the default reconnect CTA for their own token error. `/connection/reconnect` accepts `jetpack_connect_user`, and `Manager::restore()` limits such a caller to refreshing their own user token: it never touches the site connection, refuses the connection owner, returns an error asking for an administrator when the blog token is unhealthy, and removes the local token only once WordPress.com confirms the unlink, so a failed unlink never leaves the user without a token.

The relink runs over the site connection, so while a displayable site-audience error (user ID 0, other than `invalid_connection_owner`) is on record, that viewer's own error instead gets `action = 'none'` and a message asking an administrator to restore the site connection first. A reporter-declared action is left alone. The record can be stale, so `restore()` still checks the blog token live when the CTA is used.
