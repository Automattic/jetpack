...
| `test__wpcom_connection_test` | WordPress.com can reach the site. Also keeps the `xmlrpc_request_blocked` and `ssl_verification_failed` connection errors in sync: a result carrying one of those codes reports the matching error, a passing result clears both (see [error handling](error-handling.md)) |
...
- Runs `test__wpcom_connection_test` once a day via `do_daily_connection_check()` on the `jetpack_heartbeat` cron action, so the `xmlrpc_request_blocked` and `ssl_verification_failed` connection errors stay fresh while their conditions persist and clear within a day of being resolved. The hook is deliberately the cron-only `jetpack_heartbeat` action, not `jetpack_heartbeat_stats_array`, which also fires in synchronous request contexts (XML-RPC, REST, WP-CLI) where a remote test would be slow and an amplification vector.
...
- **`Connection_Health_Tests_Test.php`** — Tests individual health test methods: verifies skipped/pass/fail paths for each built-in test using partial mocks for helper methods, including the reporting/clearing of the `xmlrpc_request_blocked` and `ssl_verification_failed` connection errors by the WP.com connection test.
