...
This package is part of the Jetpack monorepo and contains the complete WordPress.com MCP (Model Context Protocol) implementation. It requires the `wordpress/mcp-adapter` package to function properly. The MCP adapter provides the WordPress Abilities API functionality that this registry system builds upon.

### Dependencies

- **PHP**: >= 8.1
- **wordpress/mcp-adapter**: dev-trunk (provides `wp_register_ability()` and related functions)
- **Jetpack**: Part of the Jetpack monorepo ecosystem

### Complete Implementation

This package now includes the complete MCP implementation with:
- **50+ PHP classes** covering all MCP functionality
- **Full ability registry** with configuration-driven architecture
- **Complete server implementations** (default and site-level)
- **Transport layer** for WordPress.com REST API integration
- **Error handling and observability** systems
...
src/
├── AbilitiesRegistry/
│   ├── README.md                    # Abilities Registry documentation
...
│   ├── Interfaces/
│   │   ├── AbilityInterface.php    # Contract for all abilities
│   │   └── ExecutorInterface.php   # Contract for all executors
│   ├── Abilities/
│   │   ├── User/                   # User-related ability classes
│   │   ├── Post/                   # Post-related ability classes
│   │   ├── Analytics/              # Analytics ability classes
│   │   └── ExamplePromptAbility.php
│   ├── Executors/
│   │   ├── User/                   # User-related execution logic
│   │   ├── Post/                   # Post-related execution logic
│   │   └── Analytics/              # Analytics execution logic
│   └── Helpers/
│       ├── PostQueryHelper.php     # Post query utilities
│       ├── SiteMetricsHelper.php   # Site metrics utilities
│       └── ValidationHelper.php    # Validation utilities
├── Servers/
│   ├── DefaultServer.php           # Default MCP server
│   └── SiteLevelMcpServer.php      # Site-level MCP server
├── Transport/
│   ├── WpcomRestTransport.php      # WordPress.com REST transport
│   └── WpcomSingleSiteRestTransport.php
├── ErrorHandlers/
│   └── Log2LogstashMcpErrorHandler.php
├── ObservabilityHandlers/
│   ├── ErrorLogMcpObservabilityHandler.php
│   ├── Log2LogstashMcpObservabilityHandler.php
│   └── McpObservabilityDataProcessorTrait.php
├── Infrastructure/
│   └── McpRequestContext.php       # MCP request context
├── WpcomMcp.php                    # Main plugin class
├── class-abilities-registry.php    # Jetpack registry wrapper
└── class-ability.php               # Jetpack ability wrapper
```
...
## Usage with MCP Adapter

The registry works seamlessly with the `wordpress/mcp-adapter` package. Here's how to use it:
...
if ( $registry->is_mcp_adapter_available() ) {
    // Get abilities for the default server
    $default_abilities = $registry->get_abilities_for_server( 'default' );
    
    // Get abilities for site-level server
    $site_level_abilities = $registry->get_abilities_for_server( 'site-level' );
    
    // Get raw configuration for a specific server
    $config = $registry->get_config_for_server( 'default' );
    
    // Get all available server types
    $available_servers = $registry->get_available_servers();
    
    // Filter abilities by category
    $user_abilities = $registry->get_abilities( array( 'category' => 'user' ) );
    
    // Get only enabled abilities
    $enabled_abilities = $registry->get_abilities( array( 'enabled' => true ) );
}
```

## Integration with WordPress.com MCP Plugin

This package is designed to work with the WordPress.com MCP plugin structure:

```php
// In your main plugin file or ability class
use Automattic\Jetpack\MCP_Abilities\Abilities_Registry;

// The registry automatically loads abilities from abilities-config.php
$registry = Abilities_Registry::get_instance();

// Individual ability classes use wp_register_ability() from the MCP adapter
class MyAbility {
    public function __construct() {
        wp_register_ability(
            'my-ability',
