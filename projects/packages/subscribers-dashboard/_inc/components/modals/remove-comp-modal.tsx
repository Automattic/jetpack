...
export default function RemoveCompModal( { pending, onClose }: Props ): JSX.Element | null {
	...
	const title = pending.planTitle
		? sprintf(
				...
				__( 'Remove %1$s from %2$s?', 'jetpack-subscribers-dashboard' ),
				subscriberName,
				pending.planTitle
		  )
		: sprintf(
				// translators: %s: subscriber name.
				__( 'Remove the complimentary subscription for %s?', 'jetpack-subscribers-dashboard' ),
				subscriberName
		  );
	...
	return (
		<AlertDialog.Root open onOpenChange={ handleOpenChange } onConfirm={ handleConfirm }>
			<AlertDialog.Popup
				...
				confirmButtonText={ __( 'Remove comp', 'jetpack-subscribers-dashboard' ) }
