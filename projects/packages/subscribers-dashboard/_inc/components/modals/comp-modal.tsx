...
function formatPlanLabel( product: { title: string; price?: number; currency?: string } ): string {
	...
	return sprintf(
		...
		__( '%1$s (%2$s %3$s)', 'jetpack-subscribers-dashboard' ),
		product.title,
		product.currency,
...
export default function CompModal( { subscriber, onClose }: Props ): JSX.Element | null {
	...
	const options = useMemo(
		() => [
			{
				value: '',
				label: __( 'Select a plan…', 'jetpack-subscribers-dashboard' ),
			},
			...
			<Dialog.Popup>
				<Dialog.Header>
					<Dialog.Title>
						{ sprintf(
							...
							__( 'Comp %s', 'jetpack-subscribers-dashboard' ),
				...
				<Stack direction="column" gap="md">
						...
						<Notice.Root intent="error">
							<Notice.Description>
								{ productsQuery.error?.message ||
									__( 'Could not load your paid plans.', 'jetpack-subscribers-dashboard' ) }
							</Notice.Description>
						</Notice.Root>
					) : null }
					...
					<SelectControl
						...
						label={ __( 'Plan', 'jetpack-subscribers-dashboard' ) }
						value={ planId }
						onChange={ setPlanId }
						options={ options }
					...
					<CheckboxControl
						__nextHasNoMarginBottom
						label={ __( 'Doesn’t expire', 'jetpack-subscribers-dashboard' ) }
				...
				<Dialog.Footer>
					...
					>
						{ __( 'Add comp', 'jetpack-subscribers-dashboard' ) }
					</Button>
				</Dialog.Footer>
			</Dialog.Popup>
		</Dialog.Root>
