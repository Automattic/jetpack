			...
			createSuccessNotice(
				variables.subscriberName && variables.planTitle
					? sprintf(
							...
							__( 'Comped %1$s on %2$s.', 'jetpack-subscribers-dashboard' ),
							variables.subscriberName,
							variables.planTitle
					  )
					: __( 'Comp added.', 'jetpack-subscribers-dashboard' ),
				{ type: 'snackbar' }
			);
		},
		onError: error => {
			createErrorNotice(
				error?.message || __( 'Could not comp the subscription.', 'jetpack-subscribers-dashboard' ),
				{ type: 'snackbar' }
			);
		},
	} );
		...
		onSuccess: ( _result, variables ) => {
			...
			createSuccessNotice(
				variables.subscriberName && variables.planTitle
					? sprintf(
							// translators: %1$s: subscriber name. %2$s: plan title.
							__( 'Removed %1$s from %2$s.', 'jetpack-subscribers-dashboard' ),
							variables.subscriberName,
							variables.planTitle
					  )
					: __( 'Comp removed.', 'jetpack-subscribers-dashboard' ),
				{ type: 'snackbar' }
			);
		},
		onError: error => {
			createErrorNotice(
				error?.message || __( 'Could not remove the comp.', 'jetpack-subscribers-dashboard' ),
				{ type: 'snackbar' }
			);
		},
	} );
