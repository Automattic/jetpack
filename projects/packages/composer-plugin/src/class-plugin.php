	...
	public function generateManifest( Event $event ) {
		...
		foreach ( $composer->getRepositoryManager()->getLocalRepository()->getCanonicalPackages() as $package ) {
			...
			if ( empty( $extra['textdomain'] ) ) {
				...
				// Nothing to alias: the package's strings are already in the
				// plugin's domain, and emitting a self-alias would make
				// Assets::filter_gettext() recurse infinitely on any
				// untranslated string in it. The path is still needed — it is
				// the prefix WordPress hashes to name the package's JavaScript
				// translation files — so it goes in `paths`, which registers
				// only that.
				$data['paths'][ $extra['textdomain'] ] = array(
					'path' => 'jetpack_vendor/' . $package->getPrettyName(),
					'ver'  => $ver,
				);
				$io->info( "  {$package->getName()} ($ver): textdomain {$extra['textdomain']} matches the plugin domain, recording its path without an alias" );
			} else {
