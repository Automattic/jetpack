...
class Initializer {
	...
	public static function init() {
		...
		// The admin menu and app shell register whenever the product is enabled,
		// even when the `seo-tools` module is inactive, so the product stays
		// discoverable and can be turned on from within the page itself
		// (JETPACK-1700). When the module is off, the app renders only the
		// Overview's "enable SEO tools" affordance.
		//
		...
		// The settings surface only comes online once SEO tools are active —
		// there's nothing to configure while the module is off, so we don't
		// register its REST endpoints until then. Expose the core `blog_public`
		// option to the REST settings endpoint so the Settings tab can save
		// search-engine visibility via `/wp/v2/settings` (the Jetpack settings
		// endpoint only accepts Jetpack options). Writes are still
		// capability-gated by the core settings controller.
