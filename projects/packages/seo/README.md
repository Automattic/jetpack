...
- **Settings** — search-engine indexing, the XML sitemap (with a "View sitemap" link whenever it's on, and turning it off removes the site's sitemap entirely — WordPress core's included — for a proper 404; the toggle is disabled while indexing is blocked, since a sitemap can't be served then), canonical URLs, the title structure for all page types (front page, posts, pages, tags, archives), the front-page description, site verification codes, and site-level Organization, LocalBusiness, and BreadcrumbList schema controls, plus read-only search & social previews of the home page (Google / Facebook / X).
