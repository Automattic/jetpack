...
- **Tracks instrumentation:** every event goes through `recordSeoEvent()` (`_inc/data/record-seo-event.ts`), which spreads the standard identifier set the Data team's schema expects — `blog_id` (the join key for the distinct-site launch metric), `site_type`, `is_a11n`, `is_test`, `product_version`, bootstrapped server-side by `Admin_Page::get_tracks_context()` — plus a fixed `surface` and the calling `screen`. Six events: `jetpack_seo_setting_updated` (one per *changed* key, so co-firing can't inflate the distinct-site metric) and its `jetpack_seo_setting_update_failed` counterpart, `jetpack_seo_tools_toggled`, `jetpack_seo_screen_viewed`, and — from the GEO tab — `jetpack_seo_llms_txt_enabled` and `jetpack_seo_bot_toggle_changed`. All six need registering and validating at mc.a8c.com/tracks before the data is usable; an unregistered event is accepted and then dropped. The GEO tab reports as `geo` while staying keyed `ai` internally (`screenForTab()`). **The transport is the part that's easy to miss:** `@automattic/jetpack-analytics` only queues into `window._tkq` — something has to load `jp-tracks` (`stats.wp.com/w.js`) to flush it, so `Admin_Page::enqueue_tracks_transport()` enqueues it explicitly. Without that, every event this package fires is silently dropped. It's gated on `Admin_Page::can_use_analytics()`, which short-circuits true across the whole WordPress.com platform (Simple *and* Atomic — narrowing to Simple would undercount half the metric population) and otherwise defers to `Tracking::should_enable_tracking()`, so a self-hosted site that hasn't agreed to the terms of service gets no transport at all.
