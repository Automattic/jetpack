<?php
/**
 * Search_Product_Filter_Taxonomy helper tests.
 *
...
class Search_Product_Filter_Taxonomy_Test extends TestCase {
	...
	public function test_derive_filter_key_accepts_product_taxonomies() {
		foreach ( array( 'product_cat', 'product_tag', 'product_brand' ) as $slug ) {
			$this->assertSame(
				$slug,
				Search_Product_Filter_Taxonomy::derive_filter_key( array( 'taxonomy' => $slug ) ),
	...
	public function test_derive_filter_key_rejects_out_of_list_slug() {
		...
		$this->assertSame(
			'',
			Search_Product_Filter_Taxonomy::derive_filter_key( array( 'taxonomy' => 'category' ) )
		);
		$this->assertSame(
			'',
			Search_Product_Filter_Taxonomy::derive_filter_key( array( 'taxonomy' => 'post_tag' ) )
		);
	}
	...
	public function test_derive_filter_key_rejects_empty_slug() {
		$this->assertSame(
			'',
			Search_Product_Filter_Taxonomy::derive_filter_key( array( 'taxonomy' => '' ) )
		);
		$this->assertSame( '', Search_Product_Filter_Taxonomy::derive_filter_key( array() ) );
	}
	...
	public function test_default_label_falls_back_per_slug_without_registered_taxonomy() {
		...
		$this->assertSame(
			'Category',
			Search_Product_Filter_Taxonomy::default_label( array( 'taxonomy' => 'product_cat' ) )
		);
		$this->assertSame(
			'Tag',
			Search_Product_Filter_Taxonomy::default_label( array( 'taxonomy' => 'product_tag' ) )
		);
		$this->assertSame(
			'Brand',
			Search_Product_Filter_Taxonomy::default_label( array( 'taxonomy' => 'product_brand' ) )
		);
	}
	...
	public function test_default_label_returns_empty_for_empty_slug() {
		$this->assertSame(
			'',
			Search_Product_Filter_Taxonomy::default_label( array( 'taxonomy' => '' ) )
		);
	}

	public function test_build_config_shapes_filter_type_as_taxonomy() {
		$config = Search_Product_Filter_Taxonomy::build_config(
			array(
