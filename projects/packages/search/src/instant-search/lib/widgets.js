 ...
 * array of well-formed widget objects, dropping anything malformed. Guards
 * against a missing or malformed localized configuration crashing the app.
 *
 * @param {*} widgets - localized widgets value.
 * @return {object[]} normalized widgets.
 */
export function normalizeWidgets( widgets ) {
	return Array.isArray( widgets ) ? widgets.filter( isValidWidget ) : [];
}
