...
class SearchResultMinimal extends Component {
		...
		return (
			<div
				...
				dangerouslySetInnerHTML={ {
					__html: this.props.result.highlight && typeof this.props.result.highlight === 'object'
						? Object.entries(this.props.result.highlight)
							.filter(([key, value]) => key !== 'comments' && key !== 'title' && Array.isArray(value))
	...
	render() {
		...
		const noMatchingContent = !highlight || Object.entries(highlight).every(
			([key, value]) => (key === 'comments' || key === 'title') || !Array.isArray(value) || value[0] === ''
		);
