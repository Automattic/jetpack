...
`no-results` supersedes the message region `results-list` renders from its legacy `noResultsMessage` attributes. A *sibling* can't see it at render time — `WP_Block` carries no parent pointer and `render_block_data` doesn't fire for nested blocks. A **container** can: `search-results/render.php` already sniffs `$content` for `wp-block-jetpack-search-powered-by` to decide whether to auto-append the colophon, because inner blocks render before the container. That precedent is real; it just doesn't help here, since the interactivity store is a single page-global namespace, so a container-side sniff still can't give two results regions on one page different answers (and one region per page is the documented assumption throughout this tree — see the `staticPostTypes` note under *URL format*).
...
`filterState` is deliberately two-valued (`any` | `filtered`), not three. An unscoped block already yields wherever a `filtered` one claimed the state, so the `any` + `filtered` pairing expresses every combination an author needs; a third `unfiltered` value would only say "show nothing on a filtered search", which the legacy region fills anyway. Don't add it back without a concrete case — it costs a second scoped-claim flag and a third visibility getter.
