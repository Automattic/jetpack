...
`no-results` is a container; each condition is a `no-results-slot` child (`condition`: `any` | `filtered` | `error`). Gutenberg gives a block a single `InnerBlocks` region, so per-condition *block* content has to live in child blocks — the variants are that mechanism, not something an author is meant to reason about.
...
**Front-end styles live on the container, not the variant.** A variant only ever renders inside a container, so the container's stylesheet is always present — but the reverse isn't true, and a container with no variants still needs the default copy's presentation. Putting the shared rules in `no-results-slot/style.scss` silently strips the muted/centered/min-height treatment from every stock template, since `no-results-slot.css` is only enqueued when a variant actually renders.
