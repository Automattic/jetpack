...
**All three variants exist by default.** The container's `InnerBlocks` template creates one per condition, and the shipped templates and pattern carry the same three self-closing variants, so opening a template doesn't auto-insert anything (which would mark it dirty on mere inspection). An author sees every empty state the search page can reach and edits the ones they care about; untouched variants render their localized default copy, so a stock install reads exactly as it did before the block existed. On the canvas the variants carry no custom chrome: every variant's content shows at all times, and an untouched one previews its default copy under the same `--default` class the front end styles, on the block wrapper itself to match `render.php`. The only editor-only additions are a hint under the default copy that adding blocks replaces it, and "Without filters" / "With filters" labels when an unscoped variant previews both lines (only when no `filtered` sibling exists, since that sibling takes the filtered state on the front end). Identification comes from Gutenberg's native outlines plus one block variation per condition (`conditionVariations()` in `slot/edit.jsx`, wired in `editor/register-blocks.jsx`). The active variation names the block in the settings sidebar card, List view, the breadcrumb, and the block toolbar. An `__experimentalLabel` would not reach the sidebar card, which reads only the variation. Neither reaches the canvas wrapper's `aria-label`, which core builds from `blockType.title`, so a screen reader moving through the canvas still hears "No Results Variant" three times. The variations have an empty `scope`, so they never appear in the inserter or the block switcher. Per SEARCH-341, don't reintroduce painted-on labels or a collapse-until-selected treatment. The inspector's buttons are a recovery path for a condition whose variant was deleted, not the normal way in. A variant's `condition` has no editing control for the same reason: repointing one could only duplicate a condition or vacate another. The variant carries `supports.layout` (constrained) because alignment is a property of the layout a parent provides — without it an author can't centre an image inside a message, and `alignwide`/`alignfull` never appear. `blockGap` is meaningful for the same reason. `allowedBlocks` is the variant alone and the appender is suppressed — the default appender would insert a second `any` variant, which the renderer would just stack on the first.
