...
The container binds to `state.showEmptyStateRegion` (`showNoResults || showError`) and each variant hides itself. A container whose variants don't cover the live condition therefore renders empty rather than hidden — harmless, since block supports are opt-in and an unstyled empty div has no size.
