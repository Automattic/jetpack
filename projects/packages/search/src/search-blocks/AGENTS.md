...
**Seed coverage, not a flat "a block exists" flag.** A single `no-results` scoped to "filters are active" would otherwise retire the legacy region for *both* states, leaving an unfiltered empty search with no message at all. Because `wp_interactivity_state()` deep-merges and nothing ever seeds `false`, per-case flags compose correctly across however many blocks are on the page.

`filterState` carries `any` | `filtered` | `error`. There is deliberately no `unfiltered`: an unscoped block already yields wherever a `filtered` one claimed the state, so the `any` + `filtered` pairing expresses every combination an author needs, and an `unfiltered` value would only say "show nothing on a filtered search" — which the legacy region fills anyway. Don't add it back without a concrete case; it costs a scoped-claim flag and a visibility getter.

`error` is on the same block rather than a second one because `showNoResults` and `showError` are mutually exclusive in the store (the former requires `! hasError`), so one block with one binding per option can never double-render. It is disjoint from the two no-results values in both directions: an `error` block seeds only `hasErrorBlock` (retiring `results-list`'s legacy *error* region via `showLegacyError`) and neither `hasNoResults*` flag, while an `any`/`filtered` block never seeds `hasErrorBlock`. That disjointness is what keeps `any` the safe default — a page whose only block is unscoped still shows the legacy error copy when a request fails. `error` also swaps the fallback's live region to `role="alert"`, matching the assertive/polite split `results-list` has always emitted.
