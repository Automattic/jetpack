...
**The overlay is the one render that must not seed.** `enqueue_block_template_overlay_assets()` runs `do_blocks()` over the overlay template on *every* front-end request, whether or not a visitor opens it. Seeding from that pass writes coverage into the page-global namespace and retires the legacy regions of any in-page results markup that has no `no-results` block of its own — a real message lost on a page that had one yesterday. So the pass sets `$overlay_render_coverage` from `collect_no_results_coverage()` first, `seed_no_results_coverage()` no-ops while it's set, and `legacy_no_results_getter()` / `legacy_error_getter()` / `no_results_visibility_getter()` resolve the hand-off from that known composition instead — dropping a legacy region outright where a variant covers it. Any future render pass that isn't the page's own needs the same treatment.
...
**All three variants exist by default.** The container's `InnerBlocks` template creates one per condition, and the shipped templates and pattern carry the same three self-closing variants, so opening a template doesn't auto-insert anything (which would mark it dirty on mere inspection). An author sees every empty state the search page can reach and edits the ones they care about; untouched variants render their localized default copy, so a stock install reads exactly as it did before the block existed. The inspector's buttons are a recovery path for a condition whose variant was deleted, not the normal way in. A variant offers no control over its own `condition` for the same reason — with every condition already present, repointing one could only duplicate a condition or vacate another, so the attribute is set once by whatever created the variant and never edited. `allowedBlocks` is the variant alone and the appender is suppressed — the default appender would insert a second `any` variant, which the renderer would just stack on the first.

Four helpers on `Search_Blocks` are the single implementation both renderers share: `no_results_visibility_getter()`, `no_results_live_region_attribute()`, `seed_no_results_coverage()` and `render_no_results_default_copy()`. Keep new condition logic there rather than in either `render.php`.
