...
`no-results` is a container; each condition is a `no-results-slot` child (`condition`: `any` | `filtered` | `error`). One block in the inserter, one place in the layout, and adding a condition is a button in the container's inspector rather than a second top-level block the author has to find and scope. The container's `allowedBlocks` is the variant alone and its appender is suppressed — the default appender would insert a second `any` variant, which the renderer would just stack on the first.
...
**An empty container is not a broken one.** It stands in for an unscoped variant: it renders the same filter-aware default pair and seeds the same coverage. That is what lets the shipped templates carry a self-closing `<!-- wp:jetpack-search/no-results /-->` and read exactly as they did before the block existed — don't "fix" it by giving the container an `InnerBlocks` template, which would dirty every template the moment an author clicked into it.
