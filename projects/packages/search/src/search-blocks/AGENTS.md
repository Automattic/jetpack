...
**The overlay pre-render must not seed.** It runs on every front-end request, so its coverage would retire the legacy regions of unrelated in-page markup. It goes through `No_Results::render_self_contained()` instead, which resolves the hand-off from the markup's own composition and suppresses seeding for the duration. Any render pass that isn't the page's own needs the same.
