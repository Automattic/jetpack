...
`no-results` supersedes the message regions `results-list` renders from its legacy `noResultsMessage` / `noResultsWithFiltersMessage` / `errorMessage` attributes. A *sibling* can't see it at render time — `WP_Block` carries no parent pointer and `render_block_data` doesn't fire for nested blocks. A **container** can: `search-results/render.php` already sniffs `$content` for `wp-block-jetpack-search-powered-by` to decide whether to auto-append the colophon, because inner blocks render before the container. That precedent is real; it just doesn't help here, since the interactivity store is a single page-global namespace, so a container-side sniff still can't give two results regions on one page different answers (and one region per page is the documented assumption throughout this tree — see the `staticPostTypes` note under *URL format*).

Seeded state is the escape hatch. Each `no-results/slot/render.php` seeds the conditions it covers (`hasNoResultsUnfiltered` / `hasNoResultsFiltered` / `hasErrorBlock`); `results-list` binds its legacy regions to `state.showLegacyNoResults` and `state.showLegacyError`, which show whatever no variant claimed. Because the whole interactivity state is serialized once at footer time, this resolves the same way regardless of render order — no HTML surgery, no parent walking, and the surviving region stays in its authored position.

**The overlay pre-render must not seed.** It runs on every front-end request, so its coverage would retire the legacy regions of unrelated in-page markup. It goes through `No_Results::render_self_contained()` instead, which resolves the hand-off from the markup's own composition and suppresses seeding for the duration. Any render pass that isn't the page's own needs the same. Coverage comes from `parse_blocks()`, which sees a synced pattern (`core/block`) as a leaf — a `no-results` block reachable only through one reads as uncovered, so both it and the legacy region show. Dereferencing refs would cost a post fetch on every request for a case the editor can't produce; wrap the pattern *inside* a variant instead of the block inside a pattern.

**Seed coverage, not a flat "a block exists" flag.** A single variant scoped to "filters are active" would otherwise retire the legacy region for *both* empty states, leaving an unfiltered empty search with no message at all. Because `wp_interactivity_state()` deep-merges and nothing ever seeds `false`, per-case flags compose correctly across however many variants are on the page.

### Container and variants

`no-results` is a container; each condition is a `no-results-slot` child, and it lives *inside* the parent's directory (`blocks/no-results/slot/`) rather than beside it. `register_blocks()` walks one level into each registered block directory for nested `block.json` files, so a child inherits its parent's gating for free — a skipped parent never reaches the recursion (`condition`: `any` | `filtered` | `error`). Gutenberg gives a block a single `InnerBlocks` region, so per-condition *block* content has to live in child blocks — the variants are that mechanism, not something an author is meant to reason about.

**All three variants exist by default.** The container's `InnerBlocks` template creates one per condition, and the shipped templates and pattern carry the same three self-closing variants, so opening a template doesn't auto-insert anything (which would mark it dirty on mere inspection). An author sees every empty state the search page can reach and edits the ones they care about; untouched variants render their localized default copy, so a stock install reads exactly as it did before the block existed. On the canvas the variants carry no custom chrome: every variant's content shows at all times, and an untouched one previews its default copy under the same `--default` class the front end styles, on the block wrapper itself to match `render.php`. Identification comes from Gutenberg's native outlines plus the per-condition `__experimentalLabel` (`conditionLabel()` in `slot/edit.jsx`, wired in `editor/register-blocks.jsx`), which reaches List view, the breadcrumb, and the block toolbar through `useBlockDisplayTitle`. It does **not** reach the canvas wrapper's `aria-label`, which core builds from `blockType.title`, so a screen reader moving through the canvas still hears "No Results Variant" three times. That experimental dependency is safe in a way `__experimentalUseBlockPreview` (see *Editor preview gotchas*) is not: a block type that doesn't define the property falls back to its title, so a core rename degrades to "No Results Variant" in List view rather than throwing. `packages/forms/src/blocks/option/index.jsx` registers one the same way. Per SEARCH-341, don't reintroduce painted-on labels or a collapse-until-selected treatment. The inspector's buttons are a recovery path for a condition whose variant was deleted, not the normal way in. A variant's `condition` has no editing control for the same reason: repointing one could only duplicate a condition or vacate another. The variant carries `supports.layout` (constrained) because alignment is a property of the layout a parent provides — without it an author can't centre an image inside a message, and `alignwide`/`alignfull` never appear. `blockGap` is meaningful for the same reason. `allowedBlocks` is the variant alone and the appender is suppressed — the default appender would insert a second `any` variant, which the renderer would just stack on the first.

`No_Results` (`class-no-results.php`) is the single implementation both renderers share — coverage, condition normalization, store getters, default copy. Keep new condition logic there rather than in either `render.php`.

**An empty container is not a broken one.** Saved markup that carries no variants — a self-closing `<!-- wp:jetpack-search/no-results /-->`, which is what the templates shipped before they gained variants — stands in for an unscoped variant: it renders the same filter-aware default pair and seeds the same coverage. Inner blocks that aren't variants are wrapped as one unscoped variant rather than emitted bare, which would otherwise put them behind the region-level binding and show them on a failed request too. Both are robustness floors, not migrations: a legacy per-condition scope isn't recovered, only a defined condition.

**Front-end styles live on the container, not the variant.** A variant only ever renders inside a container, so the container's stylesheet is always present — but the reverse isn't true, and a container with no variants still needs the default copy's presentation. Putting the shared rules in a stylesheet of the variant's own silently strips the muted/centered/min-height treatment from every stock template, since a variant-owned stylesheet is only enqueued when a variant actually renders. The variant ships no bundle at all for the same reason — the container's view module already registers the shared store.

There is deliberately no `unfiltered` condition: an unscoped variant already yields wherever a `filtered` one claimed the state, so `any` + `filtered` expresses every combination an author needs, and an `unfiltered` value would only say "show nothing on a filtered search" — which the legacy region fills anyway.

`error` lives on the same block because `showNoResults` and `showError` are mutually exclusive in the store (the former requires `! hasError`), so one variant per condition can never double-render. It is disjoint from the two no-results conditions in both directions: an `error` variant seeds only `hasErrorBlock`, and an `any`/`filtered` one never seeds it. That disjointness keeps `any` the safe default — a page whose only variant is unscoped still shows the legacy error copy when a request fails. `error` also swaps the fallback's live region to `role="alert"`, matching the assertive/polite split `results-list` already emitted.

The container binds to `state.showEmptyStateRegion` (`showNoResults || showError`) and each variant hides itself. A container whose variants don't cover the live condition therefore reveals with nothing visible inside it — and the container is where the block's color/spacing supports land, so that would paint a bare band. `style.scss` collapses it (`&[data-wp-bind--hidden]:not(:has(> :not([hidden])))`), keyed on the binding so the rule can't reach the editor canvas, where an empty container has to stay selectable.

All three `results-list` message attributes — `noResultsMessage`, `noResultsWithFiltersMessage`, `errorMessage` — are **deprecated**: it still renders them for content saved before `no-results` existed, but the inspector no longer offers any of them, so nothing new can set one. Don't reintroduce an editing affordance — point authors at the `no-results` block instead. They stay registered in `block.json` because dropping an attribute makes the parser discard it, which would silently blank that state on every page still relying on one.
...
`no-results`' variants take the `ButtonBlockAppender` without the outline + label (see *Container and variants*), so the package runs two shapes of this mitigation. Prefer the `no-results` shape for anything new: the appender alone fixes the boundary bug, while the outline + label is the non-standard canvas chrome SEARCH-341 set out to remove. Render the appender unconditionally, as all three of these blocks do. Gating it on selection looks like core's convention but inverts it: `shouldRenderAppender` in `@wordpress/block-editor` is `hasCustomAppender || hasSelectedRoot || …`, so core gates the *default* appender on selection and always shows a custom one.
