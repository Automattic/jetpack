...
const { state, actions } = store( NAMESPACE, {
	...
	callbacks: {
		...
		/**
		 * Reconcile materialized filter-item `<li>` children against
		 * `state.filterItems` on every state change. Defends against the same
		 * Interactivity API hydration quirk surfaced for active-filters in
		 * SEARCH-266: in a `data-wp-each` template nested inside the
		 * collapsible filters-popover, the first child that's materialized
		 * pre-ES (the URL-seeded selection) has `data-wp-text` and
		 * `data-wp-bind--hidden` bindings that don't reactively re-evaluate
		 * after buckets land — its label/count stay blank even though
		 * `data-wp-bind--value` / `data-wp-bind--checked` on the same `<li>`
		 * do update. Imperatively rewriting the affected text/attributes
		 * after each watch tick papers over it.
		 *
		 * Reads `state.filterItems` so the watch tracks it as a dep, then
		 * defers the DOM write to the next animation frame: `data-wp-each`
		 * materializes children as part of the same hydration / state-change
		 * pass, so a freshly-inserted `<li>` won't exist yet on the first
		 * watch firing — the rAF lets the materialization land first.
		 */
		reconcileFilterItems() {
			const ctx = getContext();
			const items = state.filterItems;
			const { ref } = getElement();
			if ( ! ref || ! Array.isArray( items ) || ! ctx?.filterKey ) {
				return;
			}
			const raf =
			...
			raf( () => {
				...
				for ( const li of ref.children ) {
					if ( li.tagName === 'TEMPLATE' ) {
						continue;
					}
					...
					const labelSpan = li.querySelector( '.jetpack-search-filter__label' );
					if ( labelSpan && labelSpan.textContent !== ( item.label ?? '' ) ) {
						labelSpan.textContent = item.label ?? '';
					}
					const countSpan = li.querySelector( '.jetpack-search-filter__count' );
					if ( countSpan ) {
						const desiredText = item.countLabel ?? '';
						if ( countSpan.textContent !== desiredText ) {
							countSpan.textContent = desiredText;
						}
						const shouldHide = ! item.showCount;
						if ( shouldHide && ! countSpan.hasAttribute( 'hidden' ) ) {
							countSpan.setAttribute( 'hidden', '' );
						} else if ( ! shouldHide && countSpan.hasAttribute( 'hidden' ) ) {
