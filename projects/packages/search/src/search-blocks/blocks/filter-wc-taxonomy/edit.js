/**
 * Editor preview for jetpack-search/filter-wc-taxonomy.
 *
...
// Default labels per taxonomy slug. Mirrors Filter_Wc_Taxonomy::default_label()
// so the editor preview matches what the front-end render emits when the
// block author leaves the label attribute empty.
...
/**
 * Edit component for the filter-wc-taxonomy block.
 *
 ...
 */
export default function FilterWcTaxonomyEdit( { attributes, setAttributes } ) {
	const blockProps = useBlockProps( { className: 'jetpack-search-filter-wc-taxonomy' } );
