...
require_once __DIR__ . '/admin-page/presets.php';
...
class Admin_Page {
	...
	public static function enqueue_assets() {
		$base_url  = plugins_url( 'admin-page/', __FILE__ );
		$base_path = __DIR__ . '/admin-page/';
	...
	/**
	 * Normalize an upstream episodes response into the envelope shape.
	 *
	...
	private static function bootstrap_data_wpcom(): array {
		...
		if ( function_exists( 'posts_to_podcast_get_usage' ) ) {
			$usage              = posts_to_podcast_get_usage( $blog_id );
			$bootstrap['quota'] = array(
				'quota'      => $usage,
		...
		$bootstrap['episodes'] = Endpoint::build_episodes_envelope( 1, self::EPISODES_PER_PAGE );
