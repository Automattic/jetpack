 ...
 *
 * @return CSS color token. Hex for Studio Blue and the Fresh scheme;
 * whatever `--wp-admin-theme-color` resolves to for other schemes (usually
 * hex but may be `rgb()`/`hsl()` depending on theme). Consumed by charts
 * that paint SVG directly and accept any SVG-valid color string.
 */
