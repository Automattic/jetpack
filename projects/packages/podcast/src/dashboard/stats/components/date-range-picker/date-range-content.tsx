/**
 * Popover content of the date-range picker. Mirrors the activity-log
 * port of Calypso's
...
export function DateRangeContent( props: DateRangeContentProps ) {
	...
	const defaultPresetLabel =
		...
		__( 'default range', 'jetpack-podcast' );
	...
	const startMonth = disabledBefore ? makeTZMonthFromDate( disabledBefore ) : undefined;

	const disabledMatchers = [
		disableFuture ? { after: today } : undefined,
					...
					aria-label={
						canDefaultApply
							? sprintf(
									...
									__( 'Apply %s', 'jetpack-podcast' ),
									defaultPresetLabel
							  )
							: __( 'Apply', 'jetpack-podcast' )
					}
				>
					{ canDefaultApply
						? sprintf(
