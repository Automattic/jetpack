...
export const fixerIsInError = ( fixerStatus: ThreatFixStatus ): boolean => {
	return (
		!! ( 'error' in fixerStatus && fixerStatus.error ) ||
		( 'status' in fixerStatus && fixerStatus.status === 'not_fixed' )
	);
};
...
export const fixerIsSuccess = ( fixerStatus: ThreatFixStatus ): boolean => {
	return !! ( 'status' in fixerStatus && fixerStatus.status === 'fixed' );
};
...
export const getFixerState = ( fixerStatus: ThreatFixStatus ) => {
	return {
		...
		success: fixerStatus && fixerIsSuccess( fixerStatus ),
	};
};
		...
		case 'delete':
			if ( threat.filename ) {
				if ( threat.filename.endsWith( '/' ) ) {
					return __(
						'Jetpack will delete the directory that the infected file is in.',
						'jetpack-scan'
					);
				}
				...
				if ( threat.signature === 'Core.File.Modification' ) {
					return __(
						'Jetpack will delete the unexpected file in a core WordPress directory.',
						'jetpack-scan'
					);
				}

				return __( 'Jetpack will delete the infected file.', 'jetpack-scan' );
			}

			if ( threat.extension?.type === 'plugins' ) {
				return __( 'Jetpack will delete the plugin directory to fix the threat.', 'jetpack-scan' );
			}

			if ( threat.extension?.type === 'themes' ) {
				return __( 'Jetpack will delete the theme directory to fix the threat.', 'jetpack-scan' );
			}
			break;
		case 'update':
			if ( threat.fixedIn && threat.extension?.name ) {
				return sprintf(
					...
					__( 'Jetpack will update %1$s to version %2$s.', 'jetpack-scan' ),
					threat.extension.name,
					threat.fixedIn
				);
			}
			return __( 'Jetpack will upgrade the plugin or theme to a newer version.', 'jetpack-scan' );
		case 'replace':
		case 'rollback':
			if ( threat.filename ) {
				return threat.signature === 'Core.File.Modification'
					? __(
							'Jetpack will replace the modified core WordPress file with the original clean version from the WordPress source code.',
							'jetpack-scan'
					  )
					: __(
							'Jetpack will replace the infected file with a previously backed up version that is clean.',
							'jetpack-scan'
					  );
			}
			...
			if ( threat.signature === 'php_hardening_WP_Config_NoSalts_001' ) {
				return __(
					'Jetpack will replace the default salt keys in wp-config.php with unique ones.',
					'jetpack-scan'
				);
			}
			break;
