...
import ConnectionNeeded from '../connection-needed.js';
...
const ThreatIgnoreModal = ( props: React.ComponentProps< typeof Modal > ) => {
	...
	}, [ actions, actionToConfirm, setActionToConfirm ] );
			...
			<>
				<div className={ styles[ 'threat-modal__content' ] }>
					<Text className={ styles[ 'threat-modal__section__title' ] }>
						{ __( 'Are you sure you want to ignore this threat?', 'jetpack-scan' ) }
					</Text>
					<Text>
						{ __(
	...
	return (
		<Modal title={ __( 'Ignore Threat', 'jetpack-scan' ) } children={ children } { ...props } />
	);
};
