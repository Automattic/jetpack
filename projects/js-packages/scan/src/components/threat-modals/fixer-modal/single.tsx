import { Button, Text, Spinner } from '@automattic/jetpack-components';
...
export function ThreatFixerModalContent() {
	...
	const buttonProps: React.ComponentProps< typeof Button > = useMemo( () => {
		const props = {
			children: __( 'Fix Now', 'jetpack-scan' ),
			isDestructive: false,
		};

		if ( threat.fixable && threat.fixable.fixer === 'delete' ) {
			props.children = __( 'Delete Now', 'jetpack-scan' );
	...
	const { icon, title, description } = useMemo( () => {
		if ( threat.status === 'fixed' ) {
			return {
				...
				title: __( 'This threat has been fixed', 'jetpack-scan' ),
				description: (
					<Text>
						{ sprintf(
							/* translators: placeholders are the date and time the threat was fixed. */
							__( 'Jetpack marked this threat as resolved on %1$s at %2$s.', 'jetpack-scan' ),
