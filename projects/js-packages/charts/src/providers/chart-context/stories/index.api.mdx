...
`GlobalChartsProvider` renders a wrapper element that carries the package's
`--a8c-charts-*` catalog class (see `TOKENS.md`), so every chart underneath it — and
every chart outside a provider, since the same class is applied at each chart's own
scope — resolves its colors, focus ring, motion, and border-radius roles from that
catalog. Consumers override a role by setting the matching `--a8c-charts-*` custom
