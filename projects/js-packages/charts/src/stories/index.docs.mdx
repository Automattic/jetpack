...
Colors are not among them. Every chart color is a `--a8c-charts-color-*` custom property, set in CSS anywhere inside the provider tree — see [the token catalog](?path=/docs/js-packages-charts-library-providers-globalchartsprovider--docs).
