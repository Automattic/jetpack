...
import * as BarListStories from './index.stories';

<Meta title="JS Packages/Charts Library/Charts/Bar List Chart" of={ BarListStories } />
...
<Canvas of={ BarListStories.MultiSeries } />
...
<Canvas of={ BarListStories.CustomLabelComponent } />
...
<Canvas of={ BarListStories.CustomValueComponent } />
...
<Source
	language="tsx"
	code={ `import { GlobalChartsProvider, BarList, type ChartTheme } from '@automattic/charts';
...
By default, BarList **fills its parent container's dimensions**. The parent must have an explicit height:
...
<Source
	...
	<div style={{ width: '100%' }}>
		<BarList data={data} aspectRatio={0.4} />
	</div>

	// Fixed dimensions
	<BarList data={data} width={600} height={300} />` }
/>
...
<Canvas of={ BarListStories.Animation } />
...
BarList inherits accessibility features from the underlying BarChart component, including keyboard navigation support when tooltips are enabled and proper ARIA attributes for screen readers.

## Migration from Bar Chart

If you're currently using a horizontal BarChart for list-style data, BarList provides a more optimized experience:
