...
| `tooltipPlacement` | `'auto' \| 'beside'` | `'auto'` | Beside placement flips horizontally while keeping the top anchor, clamped to viewport or clipping bounds. |
| `tooltipAnchorTop` | `number` | - | Override the top anchor for auto or beside placement in SVG coordinates; values below `margin.top` are above the plot, and negative values are above the SVG. |
