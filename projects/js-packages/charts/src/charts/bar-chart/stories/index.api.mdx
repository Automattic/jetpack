...
When `tickFormat` is not provided, categorical series render their labels as-is, and
date-based series pick a default from the data's bucket resolution and overall span —
hour ticks within a day, hour ticks dated at midnight for sub-daily data spanning up to a
week, calendar dates within a year, month names for month-or-coarser buckets, otherwise
years — shared with the line and area charts. `tickResolution` declares the bucket
resolution when the caller knows it, instead of leaving it to be inferred from point
spacing; set it on the axis that carries the dates (`x` by default, `y` when
`orientation="horizontal"`). It is ignored when `tickFormat` is set.

Default tooltip labels for date-based series name the hovered bar's bucket at the same
granularity, spelled out in full: `August 2, 2026 at 6 AM` for hourly buckets,
`August 2, 2026` for daily ones, `August 2026` for monthly, and `2026` for yearly. A
monthly bar reads `August 2026` rather than `August 1, 2026` — the day is precision the
bucket doesn't carry.
