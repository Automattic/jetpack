...
| `withCategoryHighlight` | `boolean` | `false` | Highlight the active category across the plot; requires tooltips. Uses the `--a8c-charts-color-surface-secondary` catalog role. |
| `onCategoryHighlightChange` | `(selection: CategoryHighlightSelection \| null) => void` | - | Receive active datum and `x`, `y`, `width`, `height` in SVG coordinates; requires tooltips, but not the built-in highlight. Dismissal emits null. |
