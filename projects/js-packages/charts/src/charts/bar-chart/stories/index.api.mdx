...
| `withCategoryHighlight` | `boolean` | `false` | Highlight the active category across the plot; requires tooltips. Fill uses `--a8c-charts-color-category-highlight`, falling back to the WPDS neutral weak surface. |
| `onCategoryHighlightChange` | `(selection: CategoryHighlightSelection \| null) => void` | - | Receive active datum and `x`, `y`, `width`, `height` in SVG coordinates, including when the highlight is disabled. Dismissal emits null. |
...
| `tooltipAnchorTop` | `number` | - | Override the top anchor in SVG coordinates; negative values align with content above the plot. |
