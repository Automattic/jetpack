...
| `onBandHighlightChange` | `(selection: BandHighlightSelection \| null) => void` | - | Receive active datum and `x`, `y`, `width`, `height` in SVG coordinates; requires tooltips, but not the built-in highlight. Dismissal emits null. |
...
| `tooltipPlacement` | `'auto' \| 'beside'` | `'auto'` | Offsets the tooltip horizontally without vertical flipping, and clamps to viewport or clipping bounds. |
| `tooltipAnchorTop` | `number` | - | Override the top anchor for auto or beside placement in SVG coordinates; values less than `margin.top` are above the plot, and negative values are above the SVG. |
