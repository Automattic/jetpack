...
`barClassName` receives each primary bar's datum and returns an additional CSS class. Use it to style empty or zero-value marks independently of their fill color.

<Canvas of={ BarChartStories.DatumClasses } />

**Visual Styling:**

- **`barClassName`**: A callback returning an additional CSS class for each primary bar datum
