...
Set `withCategoryHighlight` alongside `withTooltips` to shade the active category across the plot. Hover a bar or use arrow keys after focusing the chart; Escape dismisses the highlight with the tooltip.

<Canvas of={ BarChartStories.CategoryHighlight } />

For an overlay outside the chart, `onCategoryHighlightChange` reports the active datum and the category rectangle in SVG coordinates, or `null` on dismissal. The callback also works without rendering the built-in highlight.
