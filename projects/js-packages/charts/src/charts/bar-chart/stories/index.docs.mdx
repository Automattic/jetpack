...
Set `withBandHighlight` alongside `withTooltips` to shade the active band across the plot. Hover a bar or use arrow keys after focusing the chart; Escape dismisses the highlight with the tooltip.
...
Use `tooltipPlacement="beside"` to keep the tooltip beside the active bar instead of flipping vertically. It flips horizontally near an edge and stays within the viewport or clipping ancestor. `tooltipAnchorTop` sets its top anchor in SVG coordinates; negative values place the anchor above the plot.
