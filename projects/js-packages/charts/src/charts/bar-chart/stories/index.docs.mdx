import { Meta, Canvas, Story, Source } from '@storybook/addon-docs/blocks';
import * as BarChartStories from './index.stories';

<Meta title="JS Packages/Charts Library/Charts/Bar Chart" of={ BarChartStories } />

# Bar Chart

Bar Charts visualize categorical data using rectangular bars with heights or lengths proportional to the values they represent. They excel at comparing values across categories, showing distributions, and highlighting differences in data sets.

<Canvas of={ BarChartStories.Default } />

## Overview

The Bar Chart component provides a flexible, accessible, and highly customizable solution for displaying categorical data. Built on `@visx/xychart`, it supports multiple data series, both vertical and horizontal orientations, interactive tooltips, legends, and advanced visual features like pattern fills:

<Source
	language="jsx"
	code={ `import { BarChart } from '@automattic/charts';
import '@automattic/charts/style.css';

	<BarChart
		data={ data }
		orientation="vertical"
		withTooltips={ true }
		showLegend={ true }
	/>` }
/>

## API Reference

For detailed information about component props, types, and method signatures, see the [Bar Chart API Reference](./?path=/docs/js-packages-charts-library-charts-bar-chart-api-reference--docs).

## Basic Usage

### Simple Bar Chart

The simplest bar chart requires only a `data` prop with categorical data:

<Canvas of={ BarChartStories.SingleSeries } />

<Source
	language="jsx"
	code={ `<BarChart
		data={[
			{
				label: 'Olympic Medals',
				data: [
					{ label: '2016', value: 121 },
					{ label: '2020', value: 113 },
					{ label: '2024', value: 126 },
				],
				options: {}
			}
		]}
	/>` }
/>

### Required Props

- **`data`**: Array of series data, each containing `label`, `data` array, and optional `options`

### Optional Props

**Layout & Dimensions:**
- **`width`**: Chart width in pixels. When omitted, fills parent container width
- **`height`**: Chart height in pixels (defaults to 400). When omitted, fills parent container height
- **`margin`**: Custom chart margins
- **`aspectRatio`**: Height-to-width ratio (e.g. `0.5`) for responsive charts; the chart is contained within its parent on both axes. When omitted, fills the parent container's height
- **`maxWidth`**: Maximum width constraint for responsive charts (default: `1200`)
- **`resizeDebounceTime`**: Debounce delay for resize events in ms (default: `300`)

**Visual Styling:**

- **`barClassName`**: A callback returning an additional CSS class for each primary bar's datum, on top of `visx-bar`. Use it to style empty or zero-value marks independently of their fill color.
- **`tooltipStyle`**: Inline style overrides for the tooltip box
- **`orientation`**: Bar orientation (`'vertical'` or `'horizontal'`, defaults to `'vertical'`)
- **`withPatterns`**: Use pattern fills instead of solid colors (`false` by default)
- **`gridVisibility`**: Grid line visibility (`'x'`, `'y'`, `'xy'`, or `'none'`)

<Canvas of={ BarChartStories.DatumClasses } />

**Interactivity:**
- **`withTooltips`**: Enable interactive tooltips (`false` by default)

**Legend:**
- **`showLegend`**: Display chart legend (`false` by default)
- **`legend`**: Legend configuration object (`ChartLegendConfig`) for controlling orientation, position, alignment, shape, interactivity, and styling
- **`defaultHiddenSeries`**: Series labels hidden from the first defined value; later values for the same `chartId` are ignored

**Advanced:**
- **`options`**: Advanced axis and scale configuration

## Chart Orientations

### Vertical Bars (Default)

Standard vertical bar charts with categories on the x-axis and values on the y-axis:

<Canvas of={ BarChartStories.Default } />

<Source
	language="jsx"
	code={ `<BarChart
		data={data}
		orientation="vertical"
		gridVisibility="x"
	/>` }
/>

### Horizontal Bars

Horizontal bar charts with categories on the y-axis and values on the x-axis, ideal for long category names:

<Canvas of={ BarChartStories.HorizontalBarChart } />

<Source
	language="jsx"
	code={ `<BarChart
		data={data}
		orientation="horizontal"
		gridVisibility="y"
	/>` }
/>

## Multiple Series

### Multi-Series Charts

Display multiple data series with automatic color differentiation and grouped bars:

<Canvas of={ BarChartStories.Default } />

<Source
	language="jsx"
	code={ `<BarChart
		data={[
			{
				label: 'United States',
				data: olympicData.usa,
				options: {}
			},
			{
				label: 'Great Britain',
				data: olympicData.gb,
				options: {}
			},
			{
				label: 'Germany',
				data: olympicData.germany,
				options: {}
			}
		]}
	/>` }
/>

### Many Data Series

The component handles large numbers of series gracefully with automatic color cycling. Use the `seriesCount` control above to see how the chart handles many series:

<Source
	language="jsx"
	code={ `<BarChart
		data={allCountriesData}
		withTooltips={true}
		showLegend={true}
	/>` }
/>

## Comparison Mode

Add a comparison series — for example "this period vs. previous period" — by giving a series `options.type: 'comparison'`. It renders as a translucent "shadow" bar centered **behind** the primary series that shares its `group`. The shadow is wider than the primary bar (the primary is narrowed) so it peeks on each side and above, while a small gap is kept between series and a larger gap between ticks.

<Canvas of={ BarChartStories.ComparisonSingle } />

<Source
	language="jsx"
	code={ `<BarChart
		data={[
			{
				label: 'This period',
				group: 'views',
				data: thisPeriod,
			},
			{
				label: 'Previous period',
				group: 'views',
				options: { type: 'comparison' },
				data: previousPeriod,
			},
		]}
		showLegend={true}
	/>` }
/>

It composes with grouped bars: pair each primary series with its own `type: 'comparison'` series of the same `group`, and the groups render side by side, each with its own shadow.

<Canvas of={ BarChartStories.ComparisonMulti } />

**Notes:**

- A comparison series pairs with the primary series that shares its `group` (when there is a single primary series, `group` can be omitted on both).
- Comparison values are included in the value-axis domain, so a previous period taller than the current one is never clipped.
- Comparison series must reuse the primary series' category keys (the `label`/`date` of each point) so each shadow lines up with its primary bar.
- The comparison shadow isn't a separate hover/keyboard target (the primary bar drives interaction), but the tooltip shows both the current and previous period values for the hovered category.
- Each series gets its own legend item by default. Set `legend.collapseGroups` to render one item per group, labeled by its primary series; combined with `legend.interactive`, clicking that item toggles the primary and its previous-period series together. Pair it with `legend.comparisonItem` to append a static item explaining the comparison shadow once `collapseGroups` folds it away. When every series is hidden, the axes are dropped and only the empty state is shown.
- Collapsing the legend requires the primary and its comparison series to share a `group`. This is stricter than the bars themselves: rendering pairs a comparison with the sole primary even when `group` is omitted, but the legend has no single-primary fallback, so without a shared `group` the pair keeps separate legend items.
- With pattern fills enabled (`withPatterns`), the comparison shadow reuses its primary bar's pattern at the same reduced opacity, so the pair reads as one series.
- Styling comes from the theme at `barChart.barStyles.comparison`. `widthFactor` (default `1.5`) controls how much wider the comparison bar is than the primary — the primary is narrowed to `1 / widthFactor` of the slot — and `opacity` (default `0.5`) sets the shadow translucency.

## Visual Features

### Pattern Fills

Add visual accessibility and distinction with pattern fills instead of solid colors:

<Canvas of={ BarChartStories.WithPatterns } />

<Source
	language="jsx"
	code={ `<BarChart
		data={data}
		withPatterns={true}
		withTooltips={true}
	/>` }
/>

**Pattern types automatically cycle through:**
- **Lines**: Diagonal line patterns
- **Circles**: Circular dot patterns
- **Waves**: Wave line patterns
- **Hexagons**: Hexagonal patterns

### Smart Formatting

Automatic formatting handles large numbers with appropriate abbreviations:

<Canvas of={ BarChartStories.SmartFormatting } />

Values are automatically formatted as:
- **1B and above**: "1.23B"
- **1M and above**: "1.2M"
- **1K and above**: "1k"
- **Below 1K**: "1,234"

## Data Formats

### Time Series Data

Bar charts support both categorical labels and time-series data:

<Canvas of={ BarChartStories.TimeSeries } />

Date-based series pick their default tick format from the data's bucket resolution and
overall span, the same way line and area charts do. Month-or-coarser buckets follow the
resolution alone — month names with the year at January, or plain years for yearly buckets
— since they carry no day to print at any span. Daily-or-finer buckets narrow with the
span: hour ticks within a day, hour ticks dated at midnight for sub-daily data spanning up
to a week, calendar dates within a year, and years beyond that. An explicit
`options.axis.x.tickFormat` still overrides (`axis.y` on a horizontal chart).

The tick values are chosen rather than sampled evenly by index, the same selection line
and area charts use — otherwise the axis would routinely skip the bucket that prints the
year or dates the day, and could repeat a label. Over spans long
enough that nothing closer together reaches a boundary, the ticks fall on whole days or
whole years instead. `numTicks` caps how many it picks.

<Canvas of={ BarChartStories.TimeAxisTickFormats } />

Default tooltip labels name the hovered bar's bucket, spelled out in full:
`August 2, 2026 at 6 AM` for hourly buckets, `August 2, 2026` for daily ones,
`August 2026` for monthly, `2026` for yearly, and `Week of January 12, 2026` for weekly.
A monthly bar reads `August 2026` rather than `August 1, 2026` — the day is precision the
bucket doesn't carry. The tooltip always names the bucket's own granularity, so it stays
finer than the ticks whenever the span coarsens the axis.

When the bucket resolution is already known — from a granularity selector, say — declare it
with `options.axis.x.tickResolution` (`axis.y` on a horizontal chart) instead of leaving the
chart to infer it from point spacing. Two cases need it: a single-bucket series, which has
no spacing to measure, and weekly buckets, whose seven-day spacing is indistinguishable from
sparse daily data — undeclared, they are read as daily and the tooltip names one day instead
of the week.

<Canvas of={ BarChartStories.TimeAxisTickResolution } />

<Source
	language="jsx"
	code={ `// Date objects
	{ date: new Date('2024-01-01'), value: 2230 }

	// Date strings (automatically parsed)
	{ dateString: '2024-01-01', value: 2580 }
	{ dateString: '2024-01-01 14:30:00', value: 3500 }

	// Category labels
	{ label: '2024', value: 126 }` }
/>

### Custom Axis Formatting

Configure custom formatting for time-series or numerical axes:

<Source
	language="jsx"
	code={ `<BarChart
		data={timeSeriesData}
		options={{
			axis: {
				x: {
					tickFormat: (timestamp) => {
						const date = new Date(timestamp);
						return date.toLocaleDateString('en-US', { dateStyle: 'short' });
					},
				},
			},
		}}
	/>` }
/>

### Buckets With No Data

Set a point's `value` to `null` when its bucket has no reading, such as the days before a site launched. The bucket keeps its place on the axis, so the chart still spans the full range, but it draws no bar and its tooltip reads "No data" rather than 0. Use `0` only for a real reading of zero: in the example below April is a real zero, and with `showZeroValues` on it keeps a short stub where the months before it have nothing.

<Canvas of={ BarChartStories.BucketsWithNoData } />

## Zero Value Display

### Visual Enhancement for Zero Values

By default, bars with zero values have no visual height, making them difficult to identify. The `showZeroValues` feature provides better user experience by giving zero values a minimum visual height while preserving data integrity:

<Canvas of={ BarChartStories.ZeroValueComparison } />

<Source
	language="jsx"
	code={ `<BarChart
		data={dataWithZeros}
		showZeroValues={true}
		withTooltips={true}
	/>` }
/>

## Error Handling

The chart gracefully handles various error states and edge cases:

<Canvas of={ BarChartStories.ErrorStates } />

**Supported scenarios:**
- Empty data arrays
- Invalid data values (NaN, undefined)
- Buckets with no reading (null), drawn as a gap rather than rejected
- Missing data properties
- Invalid date formats
- Single data points

## Interactive Features

### Tooltips

Use `tooltipStyle` to override tooltip box styles, including padding, without CSS priority overrides.

Enable rich interactive tooltips that display detailed information on hover:

<Source
	language="jsx"
	code={ `<BarChart
		data={data}
		withTooltips={true}
		renderTooltip={({ tooltipData }) => {
			const nearestDatum = tooltipData?.nearestDatum?.datum;
			return (
				<div>
					<div>{tooltipData?.nearestDatum?.key}</div>
					<div>{nearestDatum?.label}: {nearestDatum?.value}</div>
				</div>
			);
		}}
	/>` }
/>

### Band Highlight

Set `withBandHighlight` alongside `withTooltips` to shade the active band across the plot. Hover a bar or use arrow keys after focusing the chart. Escape clears the keyboard selection and its tooltip; a hover highlight remains. Without `withTooltips`, the chart still renders, but neither the highlight nor its callback runs; development builds warn about this configuration.

<Canvas of={ BarChartStories.BandHighlight } />

For an overlay outside the chart, `onBandHighlightChange` reports the active datum, series `key`, point `index` within the rendered series, and the band rectangle in SVG coordinates. It reports the initial selection on mount (normally `null`), changes to that selection, and `null` on dismissal or unmount. The callback also works without rendering the built-in highlight.

### Beside Tooltips

Use `tooltipPlacement="beside"` to offset the tooltip horizontally from the active band center on vertical bar charts without vertical flipping. It flips horizontally near an edge and stays within the viewport or clipping ancestor. Its top follows the selected datum unless `tooltipAnchorTop` overrides it. This override also applies to `"auto"` placement, which can still flip vertically. The anchor uses SVG coordinates: values less than `margin.top` are above the plot, and negative values are above the SVG.

The horizontal anchor is the band center, not its edge, so a wide bar can be covered by the tooltip. Negative top anchors remain subject to clipping bounds and can be clamped back into view.

<Canvas of={ BarChartStories.BesideTooltip } />

<Source
	language="tsx"
	code={ `import { BarChart } from '@automattic/charts';

	<BarChart
		data={ data }
		withTooltips={ true }
		withBandHighlight={ true }
		tooltipPlacement="beside"
		tooltipAnchorTop={ 40 }
	/>` }
/>

### Keyboard Navigation

Charts are fully keyboard accessible:
- **Tab**: Focus the chart
- **Arrow Keys**: Navigate between bars
- **Enter/Space**: Activate tooltips
- **Escape**: Close active tooltips and return focus to the chart
- **Pointer**: Moving the pointer over the chart ends a keyboard selection; the next arrow key continues from the hovered bar

## Legends

Use the composition API to add a legend by placing `<BarChart.Legend />` as a child:

<Canvas of={ BarChartStories.WithCompositionLegend } />

<Source
	language="jsx"
	code={ `<BarChart data={data}>
		<BarChart.Legend />
	</BarChart>` }
/>

For full legend configuration options — positioning, orientation, shapes, interactivity, and the composition API — see the [Legend component docs](./?path=/docs/js-packages-charts-library-components-legend--docs).

Use `defaultHiddenSeries` to hide selected series initially while keeping them available through an interactive legend. The first defined list is used; later values for the same `chartId` are ignored. The defaults are seeded once per `chartId`, so a series the reader reveals stays revealed even if the chart unmounts and comes back — swapping between chart types, or a tab the chart lives in going away and returning. Only a new `chartId` seeds again.

<Canvas of={ BarChartStories.WithDefaultHiddenSeries } />

## Theming Integration

Bar Charts integrate seamlessly with the chart theming system. The default theme has neutral styling, and is automatically applied to all charts unless a custom theme is provided.

**Series colors come from the palette custom properties, not the theme object.** Set as many of the five slots as you need; an unset slot is skipped, and charts generates accessible colors beyond whatever you set:

<Source
	language="css"
	code={ `.a8c-charts-scope {
		--a8c-charts-color-series-1: #FF6B6B;
		--a8c-charts-color-series-2: #4ECDC4;
		--a8c-charts-color-series-3: #45B7D1;
	}` }
/>

Set a data point's optional `color` to override the series color for that bar. Points without a color retain the series color. When `withPatterns` is enabled, pattern fills take precedence over point colors. The shared point types expose this field, but only BarChart uses it.

<Canvas of={ BarChartStories.PerPointColors } />

For other catalog colors, declare the corresponding role. The closest declaration inside the provider tree wins, so scope the rule to one dashboard, or to `.a8c-charts-scope` to move every chart on the page:

<Source
	language="css"
	code={ `.my-dashboard {
		--a8c-charts-color-grid: #E0E0E0;
	}` }
/>

## Responsive Behavior

By default, charts **fill their parent container's dimensions**. The parent must have an explicit height:

<Canvas of={ BarChartStories.Default } />

<Source
	language="jsx"
	code={ `// Fill parent container (default) - parent needs explicit height
	<div style={{ width: '100%', height: '400px' }}>
		<BarChart data={data} />
	</div>

	// Use aspect ratio - height from width, contained if the parent is shorter
	<div style={{ width: '100%' }}>
		<BarChart data={data} aspectRatio={0.5} />
	</div>

	// Fixed dimensions
	<BarChart data={data} width={800} height={400} />` }
/>

For more details on responsive behavior, see the [Responsive Design section](./?path=/docs/js-packages-charts-library-introduction--docs#responsive-design) in the introduction.

## Animation

The Bar Chart component supports an optional entry animation that creates a smooth reveal effect when the chart first renders:

<Canvas of={ BarChartStories.Animation } />

<Source
	language="tsx"
	code={ `<BarChart
		data={ data }
		width={ 800 }
		height={ 400 }
		animation={ true }
	/>` }
/>

### Animation Behavior

- **Opt-in**: Animation is disabled by default and must be explicitly enabled with the `animation` prop
- **Accessibility**: Automatically respects the user's `prefers-reduced-motion` system setting - animation will not play for users who prefer reduced motion
- **Effect**: Creates a growing effect where bars scale from zero to their full size. Vertical bars rise from the bottom, while horizontal bars stretch from the left
- **Duration**: 1000ms (1 second) with ease-out timing

**Note**: The animation plays once when the chart initially renders and does not repeat.

## Advanced Customization

### Grid Configuration

Control grid line visibility and appearance:

<Source
	language="jsx"
	code={ `// Show only horizontal grid lines (default for vertical bars)
	<BarChart data={data} gridVisibility="x" />

	// Show only vertical grid lines (default for horizontal bars)
	<BarChart data={data} gridVisibility="y" />

	// Show both grid lines
	<BarChart data={data} gridVisibility="xy" />

	// Hide all grid lines
	<BarChart data={data} gridVisibility="none" />` }
/>

### Custom Margins

Control chart layout with precise margin settings:

<Source
	language="jsx"
	code={ `<BarChart
		data={data}
		margin={{
			top: 20,
			right: 60,
			bottom: 80,
			left: 80
		}}
	/>` }
/>

### Scale and Axis Configuration

The value axis starts at zero, so a bar's length is proportional to its value and a run of similar values reads as flat rather than as swings between empty and full. Pass `zero: false` on the value scale (`yScale`, or `xScale` for a horizontal chart) to fit the axis to the data instead; comparison mode keeps the zero baseline regardless, and an explicit `domain` is used as given.

<Source
	language="jsx"
	code={ `<BarChart
		data={data}
		options={{
			yScale: {
				type: 'linear',
				zero: false,
				nice: true
			},
			xScale: {
				type: 'band',
				padding: 0.1
			},
			axis: {
				x: {
					orientation: 'bottom',
					numTicks: 5
				},
				y: {
					orientation: 'left',
					numTicks: 6,
					tickFormat: (value) => '$' + value
				}
			}
		}}
	/>` }
/>

When every visible value is a whole number, the value axis places ticks only on whole numbers, instead of repeating a rounded label at fractional steps. Pass `options.axis.y.tickValues` to choose the ticks yourself (`axis.x` on a horizontal chart). A value domain you pin with `options.yScale.domain` (`xScale` on a horizontal chart) keeps every tick, so a percentage axis on `[ 0, 1 ]` still steps by 20%.

<Canvas of={ BarChartStories.SmallWholeNumberRange } />

## Accessibility

See [Keyboard Navigation](#keyboard-navigation) for supported keys and focus behavior.

### Screen Reader Support

- Chart container has `role="grid"` with descriptive `aria-label`
- Individual bars are navigable and announced with their values
- Interactive elements have appropriate ARIA attributes
- Color information is supplemented with patterns when `withPatterns` is enabled

### Focus Management

- Clear visual focus indicators on chart and individual bars
- Logical tab order through interactive elements
- Focus restoration after modal interactions
