...
Set a data point's optional `color` to override the series color for that bar. Points without a color retain the series color. When `withPatterns` is enabled, pattern fills take precedence over point colors.
