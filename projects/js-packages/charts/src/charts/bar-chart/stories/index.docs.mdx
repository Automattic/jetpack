...
overall span, the same way line and area charts do: hour ticks within a day, hour ticks
dated at midnight for sub-daily data spanning up to a week, calendar dates within a year,
month names for month-or-coarser buckets, otherwise years. An explicit
`options.axis.x.tickFormat` still overrides.
