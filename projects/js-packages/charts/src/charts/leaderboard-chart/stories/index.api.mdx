...
| `primaryColor` | `string` | first palette slot | Primary color for current period bars |
| `secondaryColor` | `string` | second palette slot | Secondary color for comparison period bars |
| `valueFormatter` | `(value: number) => ReactNode` | `defaultValueFormatter` | Custom formatter for values; may return an element, e.g. to attach a tooltip |
...
| `fitRows` | `boolean` | `false` | Show only complete rows that fit the chart height instead of scrolling. Needs a height-constrained container; refits are debounced on the responsive export. |
...
interface LeaderboardEntry {
	...
	previousValue?: number;      // Previous period value; omit when no comparison row exists
