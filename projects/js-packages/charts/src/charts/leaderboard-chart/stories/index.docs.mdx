...
- **`primaryColor`**: Primary color for current period bars (defaults to the first palette slot)
- **`secondaryColor`**: Secondary color for comparison period bars (defaults to the second palette slot)

**Data Formatting:**
- **`valueFormatter`**: Custom formatter function for values (a string or a React node) - defaults to `formatMetricValue` with compact notation
...
- **`fitRows`**: Show only complete rows that fit a height-constrained container instead of scrolling - `false` by default
...
<Source
	...
	function transformRawData(rawData) {
		const maxValue = Math.max(...rawData.map(item => item.current_period?.value ?? 0));

		return rawData.map(item => {
			// A row missing from either period has an unknown value there, not a
			// zero: leave previousValue undefined, or set currentValue to null,
			// and the chart draws a placeholder instead of a number.
			const currentValue = item.current_period?.value ?? null;
			...
			return {
				...
				currentValue,
				previousValue,
				currentShare: currentValue !== null ? (currentValue / maxValue) * 100 : 0,
				...
				delta: currentValue !== null && previousValue
...
Override the palette defaults with custom primary and secondary colors:
...
Leaderboard Charts integrate seamlessly with the chart theming system. The default theme has neutral styling, and is automatically applied to all charts unless a custom theme is provided.
...
By default, when content is taller than the available area, the content region scrolls and the legend remains visible without overlap. Set `fitRows` inside a height-constrained container to hide rows that do not fit instead. The example below uses `LeaderboardChartUnresponsive` because refits are debounced with the parent resize on the responsive export, which lags a live drag:

<Canvas of={ LeaderboardChartStories.FitRows } />

<Source
	language="tsx"
	code={ `import { LeaderboardChartUnresponsive } from '@automattic/charts';
...
Hidden rows also leave the tab order and the accessibility tree, so keyboard focus only ever lands on a fully visible row — tab through the example below to verify:

<Canvas of={ LeaderboardChartStories.FitRowsInteractive } />
...
- By default, the chart content area scrolls in constrained layouts, keeping focused legend controls visible and non-overlapping. With `fitRows`, rows that do not fit are removed from the focus order and accessibility tree.
