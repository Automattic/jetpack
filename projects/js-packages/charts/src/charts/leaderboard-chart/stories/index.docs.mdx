...
- **`fitRows`**: Show only complete rows that fit a fixed-height container instead of scrolling - `false` by default. Use with `LeaderboardChartUnresponsive`
...
By default, when content is taller than the available area, the content region scrolls and the legend remains visible without overlap. Set `fitRows` on `LeaderboardChartUnresponsive` inside a fixed-height container to hide rows that do not fit instead:
