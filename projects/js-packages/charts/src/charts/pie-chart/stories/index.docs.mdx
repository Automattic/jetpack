...
- **`--a8c-charts-color-label-inverse`** - Label text on a filled surface: a pie segment here, a heatmap cell elsewhere.
- **`--a8c-charts-color-label-background`** - The plate behind a label. Defaults to `transparent`, so the label reads straight off the segment. Set any color to get a rounded rectangle behind each label; a semi-transparent one (`rgba()`) keeps the segment color visible through it.
