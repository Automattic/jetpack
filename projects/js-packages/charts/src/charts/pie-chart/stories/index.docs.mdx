...
Both props are deprecated in favor of the catalog roles they now point at. Declaring the role in CSS anywhere inside the provider tree beats either prop.

- **`labelTextColor`** - Controls the color of text displayed on pie chart segments. Defaults to `var(--a8c-charts-color-label-on-fill, #FFFFFF)`, so labels are white until the role is declared.
- **`labelBackgroundColor`** - Controls the background color of labels. Defaults to `var(--a8c-charts-color-label-background, transparent)`, so a label reads straight off the slice. Any color value creates a rounded rectangle behind each label for enhanced readability, transparency included (e.g., `rgba(0, 0, 0, 0.7)`).
...
Pie Charts integrate seamlessly with the chart theming system. The default theme has neutral colors and styling, and is automatically applied to all charts unless a custom theme is provided.
