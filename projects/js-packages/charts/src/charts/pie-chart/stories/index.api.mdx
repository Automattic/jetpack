...
| `legend` | `ChartLegendConfig` | `undefined` | Legend configuration object. See [ChartLegendConfig](#chartlegendconfig-type). When `interactive` is enabled, requires `chartId` and `GlobalChartsProvider`. When segments are hidden, percentages recalculate so visible segments total 100%. **Note:** Interactive legends only work with props-based legends (`showLegend={true}`), not with composition API (`<PieChart.Legend />`) |
