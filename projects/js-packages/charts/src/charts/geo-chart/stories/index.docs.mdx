...
- **`domain`**: ISO 3166-1 alpha-2 code of the country whose viewpoint sets the disputed borders (e.g., `'IN'`). Pass the viewer's country so borders match what they expect; Google's default borders apply when omitted
...
- **No data**: Countries without values use `--a8c-charts-color-surface-secondary` (`datalessRegionColor`)
- **Low values**: Displayed with lighter colors
- **High values**: Displayed with the full first palette color

The color scale is configured via Google Charts' `colorAxis` option, from `--a8c-charts-color-series-1` and a lightened version of it.
...
Geo Charts integrate seamlessly with the chart theming system. The default theme has neutral styling, and is automatically applied to all charts unless a custom theme is provided.

**The color axis range comes from the first palette custom property, not the theme object:**
...
Google Charts takes a resolved hex rather than a `var()` chain, so `GeoChart` reads both roles at render and does not repaint on a theme change until it re-renders. Every other chart follows the cascade live.
