...
- **`scale`**: Scale factor for the map projection. Defaults to `width / 5` to fit the map within the chart bounds
...
The Geo Chart loads the world map topology asynchronously for better performance and bundle optimization. While loading, a placeholder is displayed.

### Default Loading Placeholder

By default, a simple "Loading map" text is shown while the topology loads:
...
<Source
	...
	code={ `<GeoChart
		...
		height={ 500 }
	/>
	// Shows "Loading map" until the topology is ready` }
/>
...
- **Scale**: Automatically calculated as `width / 5` to fit the map within the chart bounds. Can be customized via the `scale` prop.
...
- **Async loading**: The world topology is loaded asynchronously, which keeps the initial bundle size small. The chart displays a loading placeholder until the topology is ready.
...
- **Map complexity**: The world topology is optimized for performance
