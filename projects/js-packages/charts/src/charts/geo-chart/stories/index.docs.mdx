...
- **`displayMode`**: How data is rendered — `'auto'` (default), `'regions'`, `'markers'`, or `'text'`. In markers/text mode, identify locations by latitude/longitude columns; named locations require Google Maps geocoding (a `mapsApiKey`), which is not configured
