...
- **`scale`**: Scale factor for the map projection. Defaults to `width / (2 * Math.PI)` to fit the map exactly within the chart bounds
- **`center`**: Geographic center point as `[longitude, latitude]` for zooming into specific regions (e.g., `[-95, 40]` for USA, `[15, 50]` for Central Europe)
...
- **Center**: Can be customized via the `center` prop using `[longitude, latitude]` coordinates to zoom into specific regions
- **Scale**: Automatically calculated as `width / (2 * Math.PI)` to fit the full 360° longitude span exactly within the chart bounds. Can be customized via the `scale` prop.
...
Use `scale` and `center` together to focus on specific geographic areas:

<Source
	language="tsx"
	code={ `// Zoom into Europe
<GeoChart
	data={ ordersByCountry }
	width={ 800 }
	height={ 500 }
	scale={ 600 }
	center={ [ 10, 50 ] } // [longitude, latitude]
/>
