...
The remaining colors are catalog roles, declared in CSS. The `theme` fields that used to carry them are deprecated and now point at the same roles:

| Role | Paints | Deprecated field |
| ---- | ------ | ---------------- |
| `--a8c-charts-color-background` | Behind the map | `backgroundColor` |
| `--a8c-charts-color-surface-secondary` | Countries without data values | `geoChart.featureFillColor` |
