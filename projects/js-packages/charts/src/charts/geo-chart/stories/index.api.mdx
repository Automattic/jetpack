...
| `displayMode` | `GeoDisplayMode` | `'auto'` | How data is rendered: `'regions'` shades areas, `'markers'` places sized markers, `'text'` places text labels, `'auto'` lets Google Charts infer the mode from the data. In `'markers'`/`'text'` mode, identify locations by latitude/longitude columns — named locations require Google Maps geocoding (a `mapsApiKey` in the charts loader), which this package does not configure, so named markers render nothing |
