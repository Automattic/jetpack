...
Main component for rendering a two-dimensional heatmap.
...
| `cellGap` | `number` | `4` (`2` when compact) | Pixel gap between cells. |
| `cellRadius` | `number` | `2` | Corner radius for cells in px. |
| `withTooltips` | `boolean` | `false` | Show a tooltip on cell hover. |
| `renderTooltip` | `(data: HeatmapTooltipData) => ReactNode` | - | Custom tooltip render function. Receives `HeatmapTooltipData`. |
...
| `margin` | `{ top?: number; right?: number; bottom?: number; left?: number }` | - | Override the default inner margins. |
...
| `gap` | `GapSize` | `'md'` | Gap between chart layout regions (SVG area, legend, children). Uses WordPress design system tokens. |
| `children` | `ReactNode` | - | Composition children, e.g. `<HeatmapChart.Legend />`. |

## HeatmapChart.Legend

Sub-component that renders a sequential gradient scale from the lightest to the darkest color, annotated with the min and max values present in `data`.
...
Data passed to `renderTooltip` when a cell is hovered.
