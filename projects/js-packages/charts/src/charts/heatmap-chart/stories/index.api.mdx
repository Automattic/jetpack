...
| `range.end` | `string` | - | **Required.** Last measured day, `yyyy-MM-dd`, inclusive. Every month either bound falls in is drawn; in-month days outside the range are `placeholder` cells. An unparseable or reversed range returns an empty result with a warning. |
| `options.weekStartsOn` | `0 \| 1` | `1` | Day the week starts on. |
| `options.locale` | `string` | runtime locale | BCP-47 tag the month and day labels are written in. Always Gregorian. |
...
type MonthCalendarHeatmapResult = {
	/** Columns to pass to HeatmapChart's `data` prop: 7 per month, 6 cells each. */
	data: HeatmapColumn[];
	/** One group per month, to pass to HeatmapChart's `columnGroups` prop. */
