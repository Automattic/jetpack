...
- **`primaryColor`**: color the scale interpolates toward at the highest value (prop > `heatmapChart.primaryColor` > the first series palette color, `--a8c-charts-color-series-1`)
...
The heatmap resolves its full-intensity color in this order: the `primaryColor` prop wins, then `heatmapChart.primaryColor` from the active theme, then the first series palette color (`--a8c-charts-color-series-1`). The resolved color is fed to CSS `color-mix`, which derives the lighter shades for lower values.

Use the prop for one chart, or move the palette slot in CSS to change every chart at once. `heatmapChart.primaryColor` is deprecated and does neither better.
