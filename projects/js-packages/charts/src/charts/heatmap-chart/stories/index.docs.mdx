...
`useCalendarHeatmapData` is `buildCalendarHeatmapData` with the locale and time zone taken from `GlobalChartsProvider`, so a host that already set them for its other charts does not name them again. Call `buildCalendarHeatmapData` directly outside a component, or when the values do not come from the provider.
