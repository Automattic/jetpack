...
| `tooltipStyle` | `CSSProperties` | - | Override the container styles. The default renderer applies each color override independently, preserving its label or surface token when that property is omitted. |
| `withTooltipCrosshairs` | `{ showVertical?: boolean; showHorizontal?: boolean; verticalStyle?: CrosshairStyle; horizontalStyle?: CrosshairStyle }?` | - | Show crosshair guides with paint-only styles for each direction: stroke, strokeWidth, strokeOpacity, strokeDasharray, strokeLinecap, opacity, className, and inline style limited to the same paint properties |
...
| `renderTooltip` | `(params: RenderTooltipParams<DataPointDate> & { bucketInfo?: BucketInfo }) => ReactNode?` | - | Custom tooltip render function. `bucketInfo` carries the chart's date bucket classification |
...
type ChartOptions = {
	yScale?: {
		type?: 'linear' | 'log';
		zero?: boolean; // Start the axis at zero; the top is rounded from zero, not from the data floor. Ignored on a log scale.
