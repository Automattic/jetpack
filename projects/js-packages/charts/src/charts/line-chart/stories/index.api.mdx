...
| `rescaleYOnLegendToggle` | `boolean?` | `true` | With an interactive legend, whether the value axis rescales to the visible series. Set `false` to pin it to the full data range so the baseline doesn't move as series are toggled |
