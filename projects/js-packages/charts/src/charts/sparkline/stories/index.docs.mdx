...
**Visual Styling:**
- **`color`**: Color for the line stroke (defaults to `--a8c-charts-color-series-1`)
...
Sparklines integrate seamlessly with the chart theming system. The default theme has neutral styling, and is automatically applied to all charts unless a custom theme is provided.

**The line color comes from the first palette custom property, not the theme object.** Use the `color` prop to override a single sparkline:
