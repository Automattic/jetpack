...
Used inside a `GlobalChartsProvider`, `TrendIndicator` resolves these from the provider's catalog, so set the override on the provider wrapper or on an element between it and `TrendIndicator` — see TOKENS.md for the full precedence order. Used standalone (no provider above it), `TrendIndicator` declares its own catalog on itself, so the override must target the `TrendIndicator` element directly — via the `style` prop, or a rule matching the `className` you pass it:
...
<Source
	language="css"
	code={`/* Defaults shown — these resolve to the WPDS tokens below: */
.my-custom-trend {
...
Setting these on an ancestor of a standalone `TrendIndicator` has no effect: the component's own catalog declaration on itself always beats a value merely inherited from further up the tree. Make sure any custom colors meet contrast requirements.
