...
Display a legend to identify chart segments:

<Canvas of={ PieSemiCircleChartStories.WithLegend } />
...
### Standalone Legend for Semi-Circle Charts

For flexible layouts, use the Legend component independently:
...
{/* Position legend separately */}
<Legend chartId="gauge-chart" orientation="horizontal" />` }
/>

This is especially useful for:
- Dashboard layouts where multiple gauges share a legend area
- Mobile-responsive designs where legends need different positioning
- Complex layouts where the semi-circle chart's unique shape requires custom legend placement

See the [Legend component documentation](?path=/docs/js-packages-charts-composites-legend--docs) for advanced examples.
