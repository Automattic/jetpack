...
Interactive legends also work with standalone Legend components. A chart always honors series visibility — whether it changed from a legend click or a programmatic call like `toggleSeriesVisibility` — so `legend.interactive` on the chart does not control filtering. Set `interactive` on the standalone Legend so it renders clickable toggle controls, and give the chart and the Legend the same `chartId` so they share visibility state.
...
Series visibility can also be controlled outside the legend. This example uses `toggleSeriesVisibility` to update a chart whose legend is not interactive. The chart and legend still reflect the shared visibility state, while the legend remains non-clickable.

<Canvas of={ LegendStories.ProgrammaticVisibility } />
...
- **Grouped series** (`legend.collapseGroups`): Off by default, so every series gets its own item and toggles only itself. Turn it on and series sharing a `group` — typically a primary and its `type: 'comparison'` overlay — collapse into a single item labelled by the primary; toggling it shows or hides the whole group at once. Only available on charts built from `SeriesData` (line, bar, area); pie-style charts use `group` on their data points purely to coordinate colours, so the option is not offered there
- **Comparison item** (`legend.comparisonItem`): Off by default. Turn it on and, when a `type: 'comparison'` series has no item of its own (typically because `collapseGroups` folded it into its primary), a static "Comparison period" item is appended, drawn with the comparison series' line or bar styling. It cannot be toggled and never dims. Pass a string to replace the label
- **Accessibility**: Full ARIA support with `role="button"`, `aria-pressed`, and descriptive `aria-label`

<Canvas of={ LegendStories.ComparisonItem } />

### Requirements

Clickable legend toggle controls require:

1. **GlobalChartsProvider**: Must wrap both chart and legend
2. **chartId**: A unique identifier to connect chart and legend
3. **interactive**: Set to `true` on the Legend (or on the chart's `legend` config object, for a chart's built-in legend) so it renders clickable toggle controls

`GlobalChartsProvider` and `chartId` are also what make series visibility filtering work at all — a chart always applies the current visibility state, however it was set, regardless of `interactive`.
...
Label color is a catalog role — `--a8c-charts-color-label`, which moves every label in the chart, legend included:
...
Everything else about a legend is still set through the `legend` property on the chart theme, which applies to all legends rendered within a `GlobalChartsProvider`:
...
| --- | --- | --- |
| `legend.labelStyles` | `CSSProperties` | CSS styles spread onto every legend label element. Declare `--a8c-charts-color-label` for the color. |
