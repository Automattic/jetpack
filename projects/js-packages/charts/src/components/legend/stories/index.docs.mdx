...
For detailed information about component props, types, and method signatures, see the [Legend API Reference](./?path=/docs/js-packages-charts-composites-legend-api-reference--docs).
...
For detailed information about all optional props, see the [Legend API Reference](./?path=/docs/js-packages-charts-composites-legend-api-reference--docs).
...
Interactive legends also work with standalone Legend components when using `chartId`. The Legend automatically inherits the interactive state from the chart component via the `GlobalChartsProvider` - you only need to set `legendInteractive` on the chart itself:
