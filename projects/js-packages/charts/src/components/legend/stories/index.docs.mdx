...
- **Grouped series** (`legend.collapseGroups`): Off by default, so every series gets its own item and toggles only itself. Turn it on and series sharing a `group` — typically a primary and its `type: 'comparison'` overlay — collapse into a single item labeled by the primary; toggling it shows or hides the whole group at once. Only available on charts built from `SeriesData` (line, bar, area); pie-style charts use `group` on their data points purely to coordinate colors, so the option is not offered there
