...
- **`legendLabels`**: Custom labels for legend items - `{ current?: string; previous?: string }`
...
| `legendLabels` | `{ current?: string; previous?: string }` | `undefined` | Custom labels for legend items |
