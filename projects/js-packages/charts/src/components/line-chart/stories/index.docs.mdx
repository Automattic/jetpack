...
- **`legendAlignmentHorizontal`**: Horizontal legend alignment (`'center'` by default)
- **`legendAlignmentVertical`**: Vertical legend alignment (`'bottom'` by default)
...
Control legend placement and alignment:

<Canvas of={ LineChartStories.WithLegendTopRight } />
...
Display legends vertically for better space utilization:

<Canvas of={ LineChartStories.WithLegendVertical } />
...
### Standalone Legend Integration

For dashboard layouts or custom positioning, use the Legend component independently:
...
{/* Legend can be placed anywhere */}
<Legend chartId="metrics-chart" orientation="horizontal" />` }
/>

This enables:
- Flexible legend positioning outside chart area
- Shared legend panels for multiple charts
- Better responsive behavior on small screens

See the [Legend component documentation](?path=/docs/js-packages-charts-composites-legend--docs) for advanced customization and dashboard examples.
