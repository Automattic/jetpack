...
By default, bars with zero values have no visual height, making them difficult to identify. The `zeroValueDisplay` feature provides better user experience by giving zero values a minimum visual height while preserving data integrity:
...
<Canvas of={ LegendStories.HorizontalBars } />
...
| `zeroValueDisplay` | `boolean` | `false` | Display zero values with minimum visual height |
