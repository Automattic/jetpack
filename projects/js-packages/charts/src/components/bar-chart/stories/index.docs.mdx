...
**Note:** Legends are not enabled by default. Set `showLegend={true}` to display them.

### Legend Positioning

Control legend placement with alignment options:

<Canvas of={ BarChartStories.WithLegendTopRight } />
...
<Canvas of={ BarChartStories.WithLegendVertical } />
...
For more flexible layouts, use the Legend component separately:
...
{/* Place legend anywhere in your layout */}
<Legend chartId="sales-chart" orientation="vertical" />` }
/>

This approach enables:
- Positioning legends outside chart boundaries
- Creating centralized legend panels for dashboards
- Responsive layouts with legends that reflow independently

See the [Legend component documentation](?path=/docs/js-packages-charts-composites-legend--docs) for advanced usage and customization options.
