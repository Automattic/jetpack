...
<Meta title="JS Packages/Charts/Types/Bar List Chart" of={ BarListChartStories } />
...
- **`width`**: Chart width (handled automatically when using responsive wrapper)
- **`height`**: Chart height (handled automatically when using responsive wrapper)

### Optional Props

- **`withTooltips`**: Enable interactive tooltips on hover (default: false)
- **`margin`**: Chart margins object with top, right, bottom, left properties
- **`options`**: Configuration object for advanced customization
- **`renderTooltip`**: Custom tooltip rendering function
...
BarListChart inherits styling from the chart theme system and automatically applies appropriate colors based on series groups. Multi-series charts use distinct colors for each series, while single-series charts use the primary theme color.
