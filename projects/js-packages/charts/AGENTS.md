...
- **Package CSS variables.** Package-owned custom properties follow
  ...
  `--wpds-*` mappings). The catalog is emitted by
  `src/styles/chart-scope.module.scss` on every chart scope — that stylesheet is
  the only place a `--wpds-*` token may be named. Everywhere else, reference the
  catalog bare: `var(--a8c-charts-color-grid)`. JS theme strings add a terminal
  literal for the SSR and jsdom paths: `var(--a8c-charts-color-grid, #dbdbdb)`.
  Any JS token resolution must pass the chart scope element from
  `useChartScopeElement()` to `resolveCssVariable` — never resolve against `:root`.
