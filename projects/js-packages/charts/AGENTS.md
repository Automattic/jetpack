...
- **`var()` resolves in an SVG presentation attribute.** Measured in current Chrome, Safari 26 and Firefox 154 — four data points at the top of the range, not a support floor. `@wordpress/browserslist-config` reaches down to `chrome 118` and `ios_saf 18.5`, which nothing here has tested. An engine that misses it does not paint a slightly-off color: the attribute is invalid and ignored, so `stroke` falls to its initial `none` and the x axis line and tick marks vanish, while `fill` on text falls to black — invisible axis labels on a dark host. Much of this package's older commentary says `var()` does not resolve here; do not reintroduce a JS resolver on that basis.
