...
- **The `theme` prop carries no colors.** See `TOKENS.md` for catalog colors and the Bar Chart theming docs for point overrides. The `var()` chains that deliver them live in `providers/chart-context/private/catalog-pointers.ts` — internal, so there is no second way to say the same thing. Never add a color field back to `ChartTheme`; `catalog-pointers.test.ts` fails if one reaches `defaultTheme`.
