...
- **Paint-only colors belong in `chart-paint.scss`, not in the visx theme.** The grid and the axis tick labels are painted by CSS on visx's own selectors, and their colors are deliberately withheld from `buildChartTheme` — visx writes `gridStyles` as an inline style, which beats an author rule, and `svgLabelSmall.fill` as a presentation attribute carrying a literal that no longer follows the cascade. Adding a color back to the theme silently takes its role off the CSS cascade; `use-xychart-theme.test.tsx` guards the handover.
- **Do not add a `chart-paint.scss` rule for `--a8c-charts-color-axis` or `--a8c-charts-color-tick`.** visx gives both axes the same `.visx-axis-line` / `.visx-axis-tick` classes with no direction to select on, and the y axis renders both unstroked on purpose — a rule there draws a y axis line and a full column of tick marks that were never there. The matching theme fields are x-axis-only, so they stay JS-resolved until the x axis carries its own class.
