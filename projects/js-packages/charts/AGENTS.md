...
- **The `theme` prop carries no colors.** See [TOKENS.md](TOKENS.md) for color configuration. Never add a color field back to `ChartTheme`; `catalog-pointers.test.ts` fails if one reaches `defaultTheme`.
