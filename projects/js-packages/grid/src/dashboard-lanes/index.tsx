...
export const DashboardLanes = forwardRef< HTMLDivElement, DashboardLanesProps >(
	function DashboardLanes( props, ref ) {
		...
		const latestLayoutRef = useRef< DashboardLanesLayoutItem[] | undefined >();
		...
		const childrenCacheRef = useRef< Map< string, React.ReactElement > >( new Map() );
		...
		const mergedRootRef = useMergeRefs( [ setContainer, resizeObserverRef, ref ] );
		...
		useLayoutEffect( () => {
			...
			const parsed = Number.parseFloat( window.getComputedStyle( container ).columnGap );
		...
		const effectiveColumns = useMemo( () => {
			...
			const maxFit = Math.max( 1, Math.floor( ( containerWidth + gapPx ) / totalWidthPerColumn ) );
		...
		const minResizeWidthPx = gridSpanToPixelSize( 1, 1, columnWidth, gapPx, null ).widthPx;
		...
		const layoutKeys = useMemo( () => new Set( layout.map( item => item.key ) ), [ layout ] );
		...
		const placementItems = useMemo( () => {
			return items.map( key => {
				...
				const span =
					typeof width === 'number' ? Math.max( 1, Math.min( width, effectiveColumns ) ) : 1;
		...
		const handleResize = useEvent( ( id: string, delta: ResizeDelta ) => {
			...
			const relativeDelta = Math.round( delta.width / ( columnWidth + gapPx ) );
			...
			const newWidth = Math.max( 1, Math.min( baseline + relativeDelta, effectiveColumns ) );

			setResizeSnapPreview( {
				id,
				snap: gridSpanToPixelSize( newWidth, 1, columnWidth, gapPx, null ),
			} );

			const pendingItem = latestLayoutRef.current?.find( item => item.key === id );
			...
			activeId && activeClone ? (
				<div className={ styles[ 'drag-preview-frame' ] }>
					<div className={ styles[ 'drag-preview-frame__lift' ] }>
						{ DragPreview ? (
							<DragPreview itemId={ activeId }>{ activeClone }</DragPreview>
						) : (
							activeClone
						) }
					</div>
			...
			>
				<SortableContext items={ items } strategy={ NO_SORT_STRATEGY }>
					<div
						...
						className={ clsx(
							styles.lanes,
							layoutAnimating && layoutAnimationStyles[ 'layout-animating' ],
							className
						) }
						...
						style={
							{
								...
								'--wp-grid-lane-row-unit': `${ Math.max( 1, rowUnit ) }px`,
