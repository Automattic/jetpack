...
export function useLanePlacement(
	...
	const [ itemStyles, setItemStyles ] = useState< Map< string, React.CSSProperties > >(
		() => new Map()
	);
	...
	const itemsSignature = useMemo( () => {
		return input.items
			.map( item => `${ item.key }/${ item.span ?? 1 }/${ item.lane ?? '' }` )
			.join( '\0' );
	}, [ input.items ] );
	...
	useLayoutEffect( () => {
		...
		const recompute = () => {
			...
			rafId = requestAnimationFrame( () => {
				...
				const effectiveRowUnit = Math.max( 1, rowUnit ?? DEFAULT_ROW_UNIT );
				...
				for ( const item of itemsForPlacement ) {
					...
					const rowSpan = Math.max( 1, Math.ceil( height / effectiveRowUnit ) );
					next.set( item.key, {
		...
		const resizeObserver = new ResizeObserver( entries => {
			...
			for ( const entry of entries ) {
				const key = ( entry.target as HTMLElement ).getAttribute( GRID_ITEM_DATA_KEY );
				if ( ! key ) {
					continue;
				}
		...
		const refreshObserved = () => {
			const current = container.querySelectorAll( `[${ GRID_ITEM_DATA_KEY }]` );
	...
	}, [ container, isPolyfilled, lanes, gap, flowTolerance, rowUnit, itemsForPlacement ] );
