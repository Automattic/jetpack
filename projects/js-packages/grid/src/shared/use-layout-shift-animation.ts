 ...
 * @param container
 */
function snapshotPositions( container: HTMLElement ): Map< string, RectSnapshot > {
 ...
 * @param deltaY
 */
function playLayoutShift( element: HTMLElement, deltaX: number, deltaY: number ): void {
...
export function useLayoutShiftAnimation( {
	...
	const snapshotBeforeChangeRef = useRef< Map< string, RectSnapshot > | null >( null );
	const lastRenderedPositionsRef = useRef< Map< string, RectSnapshot > | null >( null );
	const positionsBeforeLastChangeRef = useRef< Map< string, RectSnapshot > | null >( null );
...
export function getLayoutFingerprint(
	...
	return layout
		.map(
			item =>
				`${ item.key }:${ String( item.width ?? '' ) }:${ item.height ?? 1 }:${
					item.order ?? ''
				}:${ item.lane ?? '' }`
		)
		.join( '|' );
 ...
 * @return Fingerprint string.
 */
export function getPlacementFingerprint( itemStyles: Map< string, React.CSSProperties > ): string {
	return [ ...itemStyles.entries() ]
		...
		.map( ( [ key, style ] ) => {
			...
			return `${ key }:${ String( column ) }:${ String( columnStart ) }:${ String(
				rowStart
			) }:${ String( rowEnd ) }`;
		} )
		.join( '|' );
