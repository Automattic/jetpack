 ...
 */
import { defaultDropAnimation, defaultDropAnimationSideEffects } from '@dnd-kit/core';
		...
		sideEffects( args ) {
			...
			if ( frame ) {
				...
				if ( lift instanceof HTMLElement ) {
					lift.getAnimations().forEach( animation => animation.cancel() );
				}
				frame.classList.add( exitingFrameClassName );
			}

			return () => {
				if ( typeof cleanupDefault === 'function' ) {
					cleanupDefault();
				}
				if ( frame ) {
