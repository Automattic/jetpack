 ...
 * @param interacting
 */
function getItemCursor( disabled: boolean, interacting: boolean ): React.CSSProperties[ 'cursor' ] {
	if ( disabled ) {
		return 'default';
	}
 ...
 * @param root0.interacting
 * @param root0.dragging
 * @param root0.children
 * @param root0.actionableArea
 * @param root0.onResize
 * @param root0.onResizeEnd
 * @param root0.resizeSnapPreview
 * @param root0.minResizeWidthPx
 * @param root0.minResizeHeightPx
...
export function GridItem( {
	...
	const style = {
		gridColumnEnd: `span ${
			item.width === 'full'
				? maxColumns
				: Math.min( typeof item.width === 'number' ? item.width : 1, maxColumns )
		}`,
	...
	const continuousContentStyle: React.CSSProperties | undefined =
		resizeDelta && initialContentSize
			? {
					...
					height: verticalResizable ? initialContentSize.height + resizeDelta.height : undefined,
			  }
			: undefined;
	...
	return (
		...
		>
			{ actionableArea ? (
				<div className={ actionableAreaStyles[ 'actionable-area-slot' ] }>
					<div style={ { display: 'contents' } } { ...( dragging ? { inert: '' } : {} ) }>
						{ actionableArea }
					</div>
				</div>
			) : null }
