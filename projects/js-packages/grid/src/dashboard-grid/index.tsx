...
export const DashboardGrid = forwardRef< HTMLDivElement, DashboardGridProps >(
	function DashboardGrid( props, ref ) {
		...
		const mergedGridRef = useMergeRefs( [ setGridRoot, resizeObserverRef, ref ] );
		...
		useLayoutEffect( () => {
			...
			const parsed = Number.parseFloat( window.getComputedStyle( gridRoot ).columnGap );
		...
		const minResizeHeightPx =
			rowHeightPx === null
				? undefined
				: gridSpanToPixelSize( 1, 1, columnWidth, gapPx, rowHeightPx ).heightPx ?? undefined;
		...
		const handleResize = useEvent( ( id: string, delta: ResizeDelta ) => {
			...
			if ( ! resizeBaselineRef.current ) {
				const baseItem = activeLayout.find( item => item.key === id );
			...
			const newWidth = Math.max(
				1,
				Math.min( baseline.width + relativeDelta.width, effectiveColumns )
			);
			const newHeight = Math.max( 1, baseline.height + relativeDelta.height );

			setResizeSnapPreview( {
				id,
				snap: gridSpanToPixelSize( newWidth, newHeight, columnWidth, gapPx, rowHeightPx ),
			} );
			...
			const updatedLayout = activeLayout.map( item =>
				item.key === id ? { ...item, width: newWidth, height: newHeight } : item
			);
		...
		const overlayRowHeight = typeof rowHeight === 'number' ? rowHeight : undefined;
		const overlayRows = useMemo( () => {
			if ( overlayRowHeight === undefined || containerHeight <= 0 ) {
				return undefined;
			}
			const rowTile = overlayRowHeight + gapPx;
			return Math.max( 1, Math.floor( ( containerHeight + gapPx ) / rowTile ) );
		...
		const gridOverlay = useMemo(
			...
			[ Overlay, editMode, effectiveColumns, overlayRowHeight, overlayRows ]
		);
				...
				<SortableContext items={ items } strategy={ NO_SORT_STRATEGY }>
					...
					>
						...
						{ items.map( id => (
							<GridItem
								key={ id }
								item={ resolvedItemMap.get( id ) as DashboardGridLayoutItem }
