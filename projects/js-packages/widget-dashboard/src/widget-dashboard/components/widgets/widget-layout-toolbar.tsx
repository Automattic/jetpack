...
type NamedGridWidth = Exclude< NonNullable< GridTilePlacement[ 'width' ] >, number >;
 ...
 */
export function WidgetLayoutToolbar( { widget }: WidgetLayoutToolbarProps ): React.ReactNode {
	...
	const onRemove = () => {
		onLayoutChange( layout.filter( currentWidget => currentWidget.uuid !== widget.uuid ) );
	};
	...
	return (
		<WidgetToolbar className={ styles.widgetLayoutToolbar }>
			<Menu>
				<Menu.TriggerButton
					render={
						<IconButton
							icon={ moreVertical }
							label={ __( 'Widget options', 'jetpack-widget-dashboard' ) }
							size="compact"
							variant="minimal"
							tone="neutral"
						/>
				...
				<Menu.Popover>
					<Menu.Group>
						<Menu.GroupLabel>{ __( 'Width', 'jetpack-widget-dashboard' ) }</Menu.GroupLabel>
						<Menu.Item disabled={ width === 'fill' } onClick={ () => onNamedWidthChange( 'fill' ) }>
							<Menu.ItemLabel>
								{ __( 'Use available width', 'jetpack-widget-dashboard' ) }
							</Menu.ItemLabel>
						</Menu.Item>
						<Menu.Item disabled={ width === 'full' } onClick={ () => onNamedWidthChange( 'full' ) }>
							<Menu.ItemLabel>
								{ __( 'Make full width', 'jetpack-widget-dashboard' ) }
							</Menu.ItemLabel>
						</Menu.Item>
					</Menu.Group>
				</Menu.Popover>
			...
			<IconButton
				icon={ trash }
				label={ __( 'Remove', 'jetpack-widget-dashboard' ) }
				size="compact"
				variant="minimal"
				tone="neutral"
				onClick={ onRemove }
