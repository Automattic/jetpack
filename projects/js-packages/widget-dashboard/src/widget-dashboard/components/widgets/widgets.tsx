...
import { DashboardGrid, DashboardLanes } from '@automattic/jetpack-grid';
import type {
	DashboardGridLayoutItem,
	DashboardLanesLayoutItem,
	DragPreviewRenderProps,
	ResizeHandleRenderProps,
} from '@automattic/jetpack-grid';
...
import type { DashboardWidget, GridTilePlacement, MasonryTilePlacement } from '../../types';
import type { WidgetName } from '@automattic/jetpack-widget-primitives';
 ...
 * @param widgets
 */
function toMasonryLayout( widgets: DashboardWidget[] ): DashboardLanesLayoutItem[] {
	return widgets.map( w => ( {
		key: w.uuid,
...
export const Widgets = forwardRef< HTMLDivElement, WidgetsProps >( function Widgets(
	...
	const handleMasonryChange = useCallback(
		( newMasonryLayout: DashboardLanesLayoutItem[] ) => {
			onLayoutChange( applyMasonryChange( layout, newMasonryLayout ) );
		},
		[ layout, onLayoutChange ]
	);

	const children = layout.map( ( widget, index ) => {
		const widgetType = widgetTypes.find( type => type.name === widget.type );
		...
		if ( editMode ) {
			...
			actionableArea = <WidgetSettingsToolbar widget={ widget } widgetType={ widgetType } />;
		}
	...
	const sharedRenderProps = {
		editMode,
		renderDragPreview,
		renderResizeHandle: WidgetResizeHandle as React.ComponentType< ResizeHandleRenderProps >,
	};
