	...
	return (
		<div className={ styles.preview } { ...{ inert: '' } }>
			<Suspense fallback={ null }>
				<DashboardWidgetRender widget={ exampleWidget } widgetType={ item } />
			</Suspense>
		</div>
	);
}
...
const fields: Field< WidgetType >[] = [
	{
		id: 'title',
		type: 'text',
		label: __( 'Title', 'jetpack-widget-dashboard' ),
		filterBy: false,
	},
	{
		id: 'preview',
...
export function WidgetPicker( {
	onSelect,
	itemListLabel = __( 'Widget list', 'jetpack-widget-dashboard' ),
}: WidgetPickerProps ) {
	const { widgetTypes: registeredTypes } = useDashboardInternalContext();
	const [ selection, setSelection ] = useState< string[] >( [] );
	const [ view, setView ] = useState< View >( DEFAULT_VIEW );

	const { data: widgetTypes } = filterSortAndPaginate( registeredTypes, view, fields );

	const actions = useMemo(
		() => [
			{
				id: 'select',
				label: __( 'Select', 'jetpack-widget-dashboard' ),
				isPrimary: true,
				supportsBulk: true,
