...
export function WidgetSettings(): React.ReactNode {
	...
	const { settingsWidgetUuid, setSettingsWidgetUuid, settingsDrawerSide, settingsDrawerInset } =
	...
	// frame.
	const [ lastWidgetUuid, setLastWidgetUuid ] = useState< string | null >( settingsWidgetUuid );
	useEffect( () => {
	...
	const widget = activeUuid ? layout.find( instance => instance.uuid === activeUuid ) : undefined;
	const widgetType = widget ? widgetTypes.find( type => type.name === widget.type ) : undefined;
	...
	const close = useCallback( () => setSettingsWidgetUuid( null ), [ setSettingsWidgetUuid ] );
	...
	const data = ( widget?.attributes ?? widgetType?.example?.attributes ?? {} ) as WidgetAttributes;
	...
	return (
		...
		>
			<Drawer.Popup size="medium" className={ styles.popup } style={ popupStyle }>
				<Drawer.Header>
