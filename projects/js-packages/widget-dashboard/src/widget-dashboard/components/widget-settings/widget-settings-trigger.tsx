...
export function WidgetSettingsTrigger( {
	...
	const { setSettingsWidgetUuid, setSettingsDrawerSide, setSettingsDrawerInset } =
	...
	const open = useCallback(
		( event: React.MouseEvent< HTMLElement > ) => {
			...
			const tile = event.currentTarget.closest( '[data-wp-grid-item-key]' );
		...
		[ setSettingsDrawerSide, setSettingsDrawerInset, setSettingsWidgetUuid, widget.uuid ]
	);
	...
	return (
		<IconButton
			icon={ cog }
			label={ __( 'Widget settings', 'jetpack-widget-dashboard' ) }
			variant="minimal"
			tone="neutral"
			size="compact"
			onClick={ open }
