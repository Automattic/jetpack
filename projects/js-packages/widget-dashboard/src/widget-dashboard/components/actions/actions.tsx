...
export function Actions(): React.ReactNode {
	...
	const moreActionsItems: MoreActionsDropdownItem[] = [
		{
			label: __( 'Reset to default', 'jetpack-widget-dashboard' ),
		...
		<Stack direction="row" gap="sm">
			{ isEditActionsMounted ? (
				<Stack
					direction="row"
					gap="sm"
					className={ isExitingEditActions ? styles.editActionsExit : styles.editActionsEnter }
				>
					<Button variant="minimal" tone="brand" size="compact" onClick={ insert }>
						{ ! isMobileViewport && <Button.Icon icon={ plus } /> }
						{ __( 'Add widget', 'jetpack-widget-dashboard' ) }
					</Button>

					{ canEditGridSettings && (
						<Button variant="minimal" tone="brand" size="compact" onClick={ openLayoutSettings }>
							{ ! isMobileViewport && <Button.Icon icon={ layoutIcon } /> }
							{ __( 'Layout settings', 'jetpack-widget-dashboard' ) }
						</Button>
					) }

					<div className={ styles.editActionsDivider } aria-hidden="true" />

					<Button variant="minimal" tone="brand" size="compact" onClick={ cancel }>
					...
					>
						{ __( 'Done', 'jetpack-widget-dashboard' ) }
					</Button>
				</Stack>
			) : (
				<Button variant="minimal" tone="brand" size="compact" onClick={ handleEditMode }>
					{ __( 'Customize', 'jetpack-widget-dashboard' ) }
				</Button>
			) }
			...
			>
				<AlertDialog.Popup
					intent="irreversible"
					title={ __( 'Reset dashboard to default?', 'jetpack-widget-dashboard' ) }
					description={ __(
						'All customizations will be permanently lost.',
						'jetpack-widget-dashboard'
					) }
					confirmButtonText={ __( 'Reset', 'jetpack-widget-dashboard' ) }
				/>
			</AlertDialog.Root>

			{ canEditGridSettings && (
				<LayoutSettings open={ layoutSettingsOpen } onOpenChange={ setLayoutSettingsOpen } />
			) }
		</Stack>
	);
}
