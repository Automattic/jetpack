 ...
 * @param settings
 */
function resolveGridSettings( settings: WidgetGridSettings ): GridSettingsWithColumns {
...
interface InternalDashboardContextValue {
	 ...
	 */
	cancel: ( options?: { exitEditMode?: boolean; revertLayout?: boolean } ) => void;
...
export function useDashboardInternalContext(): InternalDashboardContextValue {
	...
	if ( ! ctx ) {
		throw new Error( 'Dashboard compound used outside a WidgetDashboard subtree.' );
	}
	return ctx;
}
...
export function WidgetDashboardProvider( {
	...
	const [ stagingLayout, setStagingLayout ] = useState< DashboardWidget[] >( committedLayout );
	...
	const [ stagingGridSettings, setStagingGridSettings ] = useState< WidgetGridSettings >( () =>
		normalizeGridSettings( committedGridSettings, DEFAULT_ROW_HEIGHT )
	);

	// Same external-resync semantics as `stagingLayout`.
	useEffect( () => {
		setStagingGridSettings( normalizeGridSettings( committedGridSettings, DEFAULT_ROW_HEIGHT ) );
	}, [ committedGridSettings ] );

	const hasLayoutChanges = useMemo(
		() => ! fastDeepEqual( canonicalize( committedLayout ), canonicalize( stagingLayout ) ),
	...
	const commit = useCallback(
		( options?: { exitEditMode?: boolean } ) => {
			...
			if ( hasGridSettingsChanges ) {
				onGridSettingsChange?.( normalizeGridSettings( stagingGridSettings, DEFAULT_ROW_HEIGHT ) );
			}
	...
	const commitGridModelChange = useCallback(
		( targetModel: WidgetGridModel ) => {
			...
			onGridSettingsChange?.( normalizeGridSettings( next.gridSettings, DEFAULT_ROW_HEIGHT ) );
			onEditChange?.( false );
		},
		[ stagingLayout, stagingGridSettings, onLayoutChange, onGridSettingsChange, onEditChange ]
	);
