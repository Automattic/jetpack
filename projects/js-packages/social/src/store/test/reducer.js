...
describe( 'reducer', () => {
	it( 'returns the DEFAULT_STATE when an invalid action is passed', () => {
		expect( reducer( DEFAULT_STATE, { type: 'INVALID_ACTION_TYPE' } ) ).to.deep.equal(
			DEFAULT_STATE
		);
	} );

	it( 'returns the DEFAULT_STATE when undefined state is passed', () => {
		expect( reducer( undefined, { type: 'INVALID_ACTION_TYPE' } ) ).to.deep.equal( DEFAULT_STATE );
	} );
	...
	it( 'sets the tweets property when given the SET_TWEETS action', () => {
		...
		expect( reducer( DEFAULT_STATE, action ) ).to.deep.equal( expected );
	} );
	...
	it( 'marks existing cards as loading when given the GET_TWITTER_CARDS action', () => {
		...
		expect( reducer( startState, action ) ).to.deep.equal( expected );
	} );
} );
