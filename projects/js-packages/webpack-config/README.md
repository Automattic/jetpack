...
- `@automattic/jetpack-shared-extension-utils`: Handle `jetpack-shared-extension-utils` provided by PHP package [automattic/jetpack-assets](https://packagist.org/packages/automattic/jetpack-assets). Note: subpath imports (e.g. `/components`, `/icons`, `/site-type-utils`, `/store/wordpress-com`) are explicitly excluded from externalization and will continue to be bundled.
