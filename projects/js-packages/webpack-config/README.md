...
- `@automattic/jetpack-shared-extension-utils`: Handle `jetpack-shared-extension-utils` provided by PHP package [automattic/jetpack-assets](https://packagist.org/packages/automattic/jetpack-assets). Note: side-effect-free subpath imports (e.g. `/components`, `/icons`, `/site-type-utils`) are explicitly excluded from externalization and will continue to be bundled.
