...
- `@automattic/jetpack-shared-stores`: Handle `jetpack-shared-stores` provided by PHP package [automattic/jetpack-assets](https://packagist.org/packages/automattic/jetpack-assets). The package's `./modules-state` and `./store/wordpress-com` subpaths are mapped to the same handle so the shared data stores always resolve to one externalized bundle and register only once.
