/**
 * Floating Chat Component for Media Editor
 *
 * Provides a floating chat interface positioned on the left side
 * of the media editor, allowing users to interact with AI for image editing.
 */

import React from 'react';
import { AgentUI } from '@automattic/agenttic-ui';
import { useAgentticChat } from './provider';
import './styles.scss';

/**
 * Props for the FloatingChat component
 */
 ...
 */
export default function FloatingChat( { className, style }: FloatingChatProps ) {
	const agentChat = useAgentticChat();
	const [ isOpen, setIsOpen ] = React.useState( false );

	// Custom placeholder messages based on media editor context
	const placeholders = [
		'Ask me to crop, resize, or enhance your image...',
		'Try: "Add a hat to this cat"',
		'Try: "Change the background to sunset"',
		'Try: "Make this image more colorful"',
		'Try: "Remove the background"',
	];

	return (
		<div className={ `agenttic-media-editor-chat ${ className || '' }` } style={ style }>
			<AgentUI
				{ ...agentChat }
				variant="floating"
				...
				className="agenttic-media-editor-floating-chat"
				triggerIcon={
					<svg
						width="36"
						height="36"
