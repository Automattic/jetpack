 ...
 */
import { useMediaEditorState } from '../components/provider/with-media-editor-state-provider';
import { getUnlock } from '../utils/unlock';

// Temporary fallback for normalizeRotation
const normalizeRotation = ( rotation: number ) => ( ( rotation % 360 ) + 360 ) % 360;

// Get editMediaEntity from unlocked core-data private APIs
const unlock = getUnlock();
const coreDataUnlocked = unlock ? unlock( coreDataPrivateApis ) : ( {} as any );
...
export function useSaveImage( props?: {
	...
	const getCropperState = () => null; // Temporary fallback

	const { createErrorNotice, createSuccessNotice } = useDispatch( noticesStore );
	const { edits: attachmentEdits } = useEntityRecord( 'postType', 'attachment', id );

	// Get editMediaEntity from core-data store dispatch
	const coreDataDispatch = useDispatch( coreStore );
	const editMediaEntity = coreDataUnlocked?.editMediaEntity
		? coreDataUnlocked.editMediaEntity.bind( coreDataDispatch )
		: null;
	...
	const saveEditedImage = useCallback( async () => {
		...
		try {
			...
			if ( response && response.id ) {
				...
				createSuccessNotice( __( 'Image edited and saved.', 'media-editor' ), {
					type: 'snackbar',
			...
			createErrorNotice(
				sprintf(
					...
					__( 'Could not edit image. %s', 'media-editor' ),
