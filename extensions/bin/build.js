...
const getBaseConfig = ( options = {} ) => {
	const getConfig = require( path.join( '..', '..', 'webpack.config.blocks.js' ) );
...
const makeConfig = () => {
	...
	const presetPath = path.join( __dirname, '..', 'preset', 'index.json' );
	...
	const viewBlocksScripts = allPresetBlocks.reduce( ( viewBlocks, block ) => {
		const viewScriptPath = path.join( __dirname, '..', 'blocks', block, 'view.js' );
	...
	const editorScript = [
		editorSetup,
		...blockScripts( 'editor', path.join( __dirname, '..' ), presetBlocks ),
	];

	// Combines all the different blocks into one editor-beta.js script
	const editorBetaScript = [
		editorSetup,
		...blockScripts( 'editor', path.join( __dirname, '..' ), allPresetBlocks ),
	];
