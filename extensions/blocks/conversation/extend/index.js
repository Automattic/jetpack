...
function dialogAvailibilityHandler( OriginalBlockEdit ) {
	return ( props ) => {
		...
		const parentBlocks = useSelect( select => {
			...
			return select( 'core/block-editor' ).getBlockParentsByBlockName( selectedBlock.clientId, blockName );
		}, [] );
...
addFilter(
	'editor.BlockEdit',
	'jetpack/jetpack-dialogue-availability',
	dialogAvailibilityHandler
);