 ...
 *
 * @param array  $attr    Array containing the story block attributes.
 * @param string $content String containing the story block content.
 *
 * @return string
 */
...
function render_amp( $attr ) {
	static $wp_block_jetpack_story_id = 0;
	$wp_block_jetpack_story_id++;

	$ids      = empty( $attr['ids'] ) ? array() : $attr['ids'];
	$autoplay = empty( $attr['autoplay'] ) ? false : true;
	$extras   = array(
		'wp-amp-block',
		$autoplay ? 'wp-block-jetpack-story__autoplay' : null,
		$autoplay ? 'wp-block-jetpack-story__autoplay-playing' : null,
	);
	$classes  = Jetpack_Gutenberg::block_classes( FEATURE_NAME, $attr, $extras );

	return sprintf(
		'<div class="%1$s" id="wp-block-jetpack-story__%2$d"><div class="wp-block-jetpack-story_container swiper-container">%3$s%4$s%5$s</div></div>',
		esc_attr( $classes ),
...
function amp_carousel( $attr, $block_ordinal ) {
	...
	return sprintf(
		'<amp-carousel width="%1$d" height="%2$d" layout="responsive" type="slides" data-next-button-aria-label="%3$s" data-prev-button-aria-label="%4$s" controls loop %5$s id="wp-block-jetpack-story__amp-carousel__%6$s" on="slideChange:wp-block-jetpack-story__amp-pagination__%6$s.toggle(index=event.index, value=true)">%7$s</amp-carousel>',
		esc_attr( $width ),
		esc_attr( $height ),
	...
	return array_map(
		function( $id ) use ( $width, $height ) {
			...
			$figcaption = $caption ? sprintf(
				'<figcaption class="wp-block-jetpack-story_caption gallery-caption">%s</figcaption>',
			...
			$image      = wp_get_attachment_image(
				...
				false,
				array(
					'class'      => 'wp-block-jetpack-story_image',
					'object-fit' => 'contain',
				)
			);
			return sprintf(
				'<div class="wp-block-jetpack-story_slide"><figure>%s%s</figure></div>',
				$image,
				$figcaption
			);
		},
...
function bullets( $ids = array(), $block_ordinal = 0 ) {
	...
	return sprintf(
		'<amp-selector id="wp-block-jetpack-story__amp-pagination__%1$d" class="wp-block-jetpack-story_pagination swiper-pagination swiper-pagination-bullets amp-pagination" on="select:wp-block-jetpack-story__amp-carousel__%1$d.goToSlide(index=event.targetOption)" layout="container">%2$s</amp-selector>',
		absint( $block_ordinal ),
		implode( '', $buttons )
	);
}
...
function autoplay_ui( $block_ordinal = 0 ) {
	$block_id        = sprintf(
		'wp-block-jetpack-story__%d',
		absint( $block_ordinal )
	);
	$amp_carousel_id = sprintf(
		'wp-block-jetpack-story__amp-carousel__%d',
		absint( $block_ordinal )
	);
	$autoplay_pause  = sprintf(
		'<a aria-label="%s" class="wp-block-jetpack-story_button-pause" role="button" on="tap:%s.toggleAutoplay(toggleOn=false),%s.toggleClass(class=wp-block-jetpack-story__autoplay-playing,force=false)"></a>',
		esc_attr__( 'Pause Story', 'jetpack' ),
		esc_attr( $amp_carousel_id ),
		esc_attr( $block_id )
	);
	$autoplay_play   = sprintf(
		'<a aria-label="%s" class="wp-block-jetpack-story_button-play" role="button" on="tap:%s.toggleAutoplay(toggleOn=true),%s.toggleClass(class=wp-block-jetpack-story__autoplay-playing,force=true)"></a>',
		esc_attr__( 'Play Story', 'jetpack' ),
		esc_attr( $amp_carousel_id ),
		esc_attr( $block_id )
	);
