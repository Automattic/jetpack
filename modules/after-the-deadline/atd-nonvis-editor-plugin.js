...
if ( typeof QTags !== 'undefined' && QTags.addButton ) {
	...
	jQuery( document ).ready( function( $ ) {
		$( '#ed_AtD' ).replaceWith(
			'<input type="button" id="ed_AtD" accesskey="" class="ed_button" onclick="AtD_check(this);" value="' +
				AtD_l10n_r0ar.button_proofread +
				'" />'
		);
	} );
}
...
function AtD_restore_if_proofreading() {
	if (
		AtD_qtbutton &&
		jQuery( AtD_qtbutton ).val() === AtD.getLang( 'button_edit_text', 'edit text' )
	) {
...
function AtD_unbind_proofreader_listeners() {
	jQuery( '#save-post, #post-preview, #publish, #edButtonPreview' ).unbind(
		'focus',
		AtD_restore_if_proofreading
	);
	jQuery( '#add_poll, #add_image, #add_video, #add_audio, #add_media' ).unbind(
		'click',
		AtD_restore_if_proofreading
	);
...
function AtD_bind_proofreader_listeners() {
	jQuery( '#save-post, #post-preview, #publish, #edButtonPreview' ).focus(
		AtD_restore_if_proofreading
	);
	jQuery( '#add_poll, #add_image, #add_video, #add_audio, #add_media' ).click(
		AtD_restore_if_proofreading
	);
...
function AtD_check( button ) {
	...
	if ( jQuery( AtD_qtbutton ).val() === AtD.getLang( 'button_edit_text', 'edit text' ) ) {
		...
		jQuery( AtD_qtbutton )
			.css( { color: 'red' } )
			.val( AtD.getLang( 'button_edit_text', 'edit text' ) )
			.attr( 'disabled', true );
		...
		jQuery( AtD_qtbutton )
			.siblings( 'input' )
			.andSelf()
			.attr( 'disabled', true ); // using .arrt instead of .prop so it's compat with older WP and jQuery
		...
		AtD.check( 'atd-content', {
			...
			editSelection: function( element ) {
				var text = prompt(
					AtD.getLang( 'dialog_replace_selection', 'Replace selection with:' ),
					element.text()
				);
			...
			explain: function( url ) {
				...
				window
					.open(
						url,
						'',
						'width=480,height=380,toolbar=0,status=0,resizable=0,location=0,menuBar=0,left=' +
							left +
							',top=' +
							top
					)
