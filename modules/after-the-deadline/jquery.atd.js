...
var AtD = {
	...
	rpc_css:
		'http://www.polishmywriting.com/atd-jquery/server/proxycss.php?data=' /* you may use this, but be nice! */,
	rpc_css_lang: 'en',
	api_key: '',
	i18n: {}, // Back-compat
	listener: {},
...
AtD.checkCrossAJAX = function( container_id, callback_f ) {
	...
	if (
		( text.length > 2000 && navigator.appName === 'Microsoft Internet Explorer' ) ||
		text.length > 7800
	) {
	...
	CSSHttpRequest.get(
		AtD.rpc_css + text + '&lang=' + AtD.rpc_css_lang + '&nocache=' + new Date().getTime(),
		function( response ) {
...
AtD.editSelection = function() {
	...
	if ( AtD.errorElement.parent() !== parent ) {
		...
		if (
			AtD.counter === 0 &&
			AtD.callback_f !== undefined &&
			AtD.callback_f.success !== undefined
		) {
			AtD.callback_f.success( AtD.count );
		}
	}
...
AtD.suggest = function( element ) {
	...
	if ( errorDescription === undefined ) {
		suggest.append(
			'<strong>' + AtD.getLang( 'menu_title_no_suggestions', 'No suggestions' ) + '</strong>'
		);
		...
		for ( var i = 0; i < errorDescription[ 'suggestions' ].length; i++ ) {
			( function( sugg ) {
				suggest.append(
					'<a href="javascript:AtD.useSuggestion(\'' +
						sugg.replace( /'/, "\\'" ) +
						'\')">' +
						sugg +
						'</a>'
	...
	if (
		AtD.callback_f !== undefined &&
		AtD.callback_f.explain !== undefined &&
		errorDescription[ 'moreinfo' ] !== undefined
	) {
		suggest.append(
			'<a href="javascript:AtD.explainError()" class="spell_sep_top">' +
				AtD.getLang( 'menu_option_explain', 'Explain...' ) +
				'</a>'
		);
	...
	suggest.append(
		'<a href="javascript:AtD.ignoreSuggestion()" class="spell_sep_top">' +
			AtD.getLang( 'menu_option_ignore_once', 'Ignore suggestion' ) +
			'</a>'
	);
	...
	if ( AtD.callback_f !== undefined && AtD.callback_f.editSelection !== undefined ) {
		if ( AtD.callback_f !== undefined && AtD.callback_f.ignore !== undefined ) {
			suggest.append(
				'<a href="javascript:AtD.ignoreAll(\'' +
					AtD.container +
					'\')">' +
					AtD.getLang( 'menu_option_ignore_always', 'Ignore always' ) +
					'</a>'
			);
		} else {
			suggest.append(
				'<a href="javascript:AtD.ignoreAll(\'' +
					AtD.container +
					'\')">' +
					AtD.getLang( 'menu_option_ignore_all', 'Ignore all' ) +
					'</a>'
			);
		}

		suggest.append(
			'<a href="javascript:AtD.editSelection(\'' +
				AtD.container +
				'\')" class="spell_sep_bottom spell_sep_top">' +
				AtD.getLang( 'menu_option_edit_selection', 'Edit Selection...' ) +
				'</a>'
		);
	} else {
		if ( AtD.callback_f !== undefined && AtD.callback_f.ignore !== undefined ) {
...
AtD.initCoreModule = function() {
	...
	core.create = function( string /*, isTextNode*/ ) {
		...
		var matches = string.match(
			/\&lt;span class="hidden\w+?" pre="[^"]*"\&gt;.*?\&lt;\/span\&gt;/g
		);
		var x;
		...
		if ( matches ) {
			for ( x = 0; x < matches.length; x++ ) {
				string = string.replace(
					matches[ x ],
					matches[ x ].replace( /\&lt;/gi, '<' ).replace( /\&gt;/gi, '>' )
				);
			}
		}
		...
		if ( core.isIE() ) {
			...
			matches = string.match(
				/\&lt;span class="mceItemHidden"\&gt;\&amp;nbsp;\&lt;\/span&gt;/g,
				string
			);
