...
class Jetpack_Widget_Twitter extends WP_Widget {
	...
	function __construct() {
		parent::__construct( 'twitter', __( 'Twitter' ), array( 'classname' => 'widget_twitter', 'description' => __( 'Display your Tweets from Twitter' ) ) );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( empty( $account ) ) {
			if ( current_user_can('edit_theme_options') ) {
				...
				echo '<p>' . sprintf( __( 'Please configure your Twitter username for the <a href="%s">Twitter Widget</a>.' ), admin_url( 'widgets.php' ) ) . '</p>';
			...
			switch( $response_code ) {
				case 200 : // process tweets and display
					...
					if ( ! is_array( $tweets ) || isset( $tweets['error'] ) ) {
						...
						$the_error = '<p>' . __( 'Error: Twitter did not respond. Please wait a few minutes and refresh this page.' ) . '</p>';
				...
				case 401 : // display private stream notice
					...
					$tweets = array();
					$the_error = '<p>' . sprintf( __( 'Error: Please make sure the Twitter account is <a href="%s">public</a>.' ), 'http://support.twitter.com/forums/10711/entries/14016' ) . '</p>';
					$tweet_cache_expire = 300;
					break;
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'account' ); ?>">
				<?php esc_html_e( 'Twitter username:' ); ?> <a href="http://support.wordpress.com/widgets/twitter-widget/#twitter-username" target="_blank">( ? )</a>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show' ); ?>">
				<?php esc_html_e( 'Maximum number of Tweets to show:' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'hidepublicized' ); ?>">
				<input id="<?php echo $this->get_field_id( 'hidepublicized' ); ?>" class="checkbox" type="checkbox" name="<?php echo $this->get_field_name( 'hidepublicized' ); ?>" <?php checked( $hidepublicized, true ); ?> />
 				<?php esc_html_e( 'Hide Tweets pushed by Publicize' ); ?>
 			</label>
 		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'includeretweets' ); ?>">
				...
				<?php esc_html_e( 'Include retweets' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'followbutton' ); ?>">
				...
				<?php esc_html_e( 'Display Follow Button' ); ?>
			</label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'beforetimesince' ); ?>">
				<?php esc_html_e( 'Text to display between Tweet and timestamp:' ); ?>
