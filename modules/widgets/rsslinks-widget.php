...
class Jetpack_RSS_Links_Widget extends WP_Widget {
	...
	function _rss_link( $type = 'posts', $args ) {
		...
		if ( 'image' == $format || 'text-image' == $format ) {
			...
			$link_item = '<a rel="noopener noreferrer" target="' . $link_target . '" href="' . get_bloginfo( $rss_type ) . '" title="' . esc_attr( $subscribe_to ) . '"><img src="' . esc_url( $link_image ) . '" alt="RSS Feed" /></a>';
		}
		if ( 'text-image' == $format ) {
			$link_item .= '&nbsp;<a rel="noopener noreferrer" target="' . $link_target . '" href="' . get_bloginfo( $rss_type ) . '" title="' . esc_attr( $subscribe_to ) . '">' . esc_html__( 'RSS - ' . $type_text, 'jetpack' ). '</a>';
		}
		if ( 'text' == $format ) {
			$link_item = '<a rel="noopener noreferrer" target="' . $link_target . '" href="' . get_bloginfo( $rss_type ) . '" title="' . esc_attr( $subscribe_to ) . '">' . esc_html__( 'RSS - ' . $type_text, 'jetpack' ). '</a>';
		}
