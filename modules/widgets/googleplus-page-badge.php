 ...
 */
add_action( 'widgets_init', 'jetpack_googleplus_page_badge_init' );

function jetpack_googleplus_page_badge_init() {
	register_widget( 'WPCOM_Widget_GooglePlus_Page_Badge' );
}

/**
 * Google+ Page Badge widget class
 * Display a Google+ Page Badge as a widget
...
class WPCOM_Widget_GooglePlus_Page_Badge extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			array(
				'classname' => 'widget_googleplus_page_badge',
				'description' => __( 'Display a Google+ Page Badge to connect visitors to your Google+ page', 'jetpack' )
			)
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( ! empty( $title ) ):
			...
			/**
			 * Modify the title of Google+ Page Badge widget.
			 *
			 * @param string $html_title HTML-based title of badge widget
			 * @param string $text_title The title of badge widget
			 * @param string $href       The URL of Google+ Page
			 */
			echo apply_filters( 'jetpack_googleplus_page_badge_title', $badge_widget_title, $title, $like_args['href'] );

			echo $after_title;
		endif;

		?><script src="https://apis.google.com/js/platform.js" async defer></script>
		<g:page href="<?php echo esc_url( $like_args['href'] ); ?>" layout="<?php echo esc_attr( $like_args['layout'] ); ?>" theme="<?php echo esc_attr( $like_args['theme'] ); ?>" showcoverphoto="<?php echo esc_attr( $like_args['show_coverphoto'] ); ?>" showtagline="<?php echo esc_attr( $like_args['show_tagline'] ); ?>" width="<?php echo esc_attr( $like_args['width'] ); ?>"></g:page><?php

		echo $after_widget;

		do_action( 'jetpack_stats_extra', 'widget', 'googleplus-page-badge' );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'href' ); ?>">
				<?php _e( 'Google+ Page URL', 'jetpack' ); ?>
	...
	function get_default_args() {
		...
		/**
		 * Modify default arguments of Google+ Page Badge widget.
		 *
		 * @param array $args {
		 *     Default arguments.
		 *
		 *     @var string $href            The URL of Google+ Page.
		 ...
		 *     @var bool   $show_tagline    Whether display the page's tag line.
		 * }
		 */
		return apply_filters( 'jetpack_googleplus_page_badge_defaults', $defaults );
	}

	function normalize_googleplus_args( $args ) {
		$args = wp_parse_args( (array) $args, $this->get_default_args() );

		// Validate the Google+ Page URL
