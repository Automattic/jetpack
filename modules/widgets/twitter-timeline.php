...
class Jetpack_Twitter_Timeline_Widget extends WP_Widget {
	...
	*/
	public function __construct() {
		parent::__construct(
			'twitter_timeline',
			apply_filters( 'jetpack_widget_name', __( 'Twitter Timeline', 'jetpack' ) ),
			array(
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['widget-id']    = (int) strip_tags( $new_instance['widget-id'] );

		foreach ( array( 'link-color', 'border-color' ) as $color ) {
			$clean = preg_replace( $non_hex_regex, '', strip_tags( $new_instance[$color] ) );
			if ( $clean )
	...
	public function form( $instance ) {
		$defaults = array(
			'title'        => __( 'Follow me on Twitter', 'jetpack' ),
			'width'        => '',
			'height'       => '400',
			'widget-id'    => '',
			'link-color'   => '#f96e5b',
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'width' ); ?>"><?php _e( 'Width (px):', 'jetpack' ); ?></label>
		 ...
		 <p>
			<label for="<?php echo $this->get_field_id( 'height' ); ?>"><?php _e( 'Height (px):', 'jetpack' ); ?></label>
		...
		<p><small>
			<?php
			printf(
				__( 'You need to <a href="%1$s" target="_blank">create a widget at Twitter.com</a>, and then enter your widget id (the long number found in the URL of your widget\'s config page) in the field below. <a href="%2$s" target="_blank">Read more</a>.' ),
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'widget-id' ); ?>"><?php _e( 'Widget ID:', 'jetpack' ); ?> <a href="http://support.wordpress.com/widgets/twitter-timeline-widget/#widget-id" target="_blank">( ? )</a></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'chrome-noheader' ); ?>"><?php _e( 'Layout Options:', 'jetpack' ); ?></label><br />
			<input type="checkbox"<?php checked( in_array( 'noheader', $instance['chrome'] ) ); ?> id="<?php echo $this->get_field_id( 'chrome-noheader' ); ?>" name="<?php echo $this->get_field_name( 'chrome' ); ?>[]" value="noheader" /> <label for="<?php echo $this->get_field_id( 'chrome-noheader' ); ?>"><?php _e( 'No Header', 'jetpack' ); ?></label><br />
			<input type="checkbox"<?php checked( in_array( 'nofooter', $instance['chrome'] ) ); ?> id="<?php echo $this->get_field_id( 'chrome-nofooter' ); ?>" name="<?php echo $this->get_field_name( 'chrome' ); ?>[]" value="nofooter" /> <label for="<?php echo $this->get_field_id( 'chrome-nofooter' ); ?>"><?php _e( 'No Footer', 'jetpack' ); ?></label><br />
			<input type="checkbox"<?php checked( in_array( 'noborders', $instance['chrome'] ) ); ?> id="<?php echo $this->get_field_id( 'chrome-noborders' ); ?>" name="<?php echo $this->get_field_name( 'chrome' ); ?>[]" value="noborders" /> <label for="<?php echo $this->get_field_id( 'chrome-noborders' ); ?>"><?php _e( 'No Borders', 'jetpack' ); ?></label><br />
			<input type="checkbox"<?php checked( in_array( 'transparent', $instance['chrome'] ) ); ?> id="<?php echo $this->get_field_id( 'chrome-transparent' ); ?>" name="<?php echo $this->get_field_name( 'chrome' ); ?>[]" value="transparent" /> <label for="<?php echo $this->get_field_id( 'chrome-transparent' ); ?>"><?php _e( 'Transparent Background', 'jetpack' ); ?></label>
		</p>
