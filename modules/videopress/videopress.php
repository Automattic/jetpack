...
class Jetpack_VideoPress {
	...
	function jetpack_module_activated() {
		...
		if ( ! isset( $options['blog_id'] ) && is_array( $options['blogs'] ) && count( $options['blogs'] ) > 0 ) {
	...
	function jetpack_configuration_load() {
		...
		if ( ! empty( $_POST['action'] ) && $_POST['action'] == 'videopress-save' ) {
			...
			if ( isset( $_POST['blog_id'] ) && in_array( $_POST['blog_id'], wp_list_pluck( $this->getVideoPressEnabledBlogs(), 'blog_id' ) ) ) {
			 ...
			 * @see $this->can()
			 */
			if ( isset( $_POST['videopress-access'] ) && in_array( $_POST['videopress-access'], array(
					'',
					'read',
					'edit',
					'delete'
					...
					<tr>
						...
						<td>
							<select name="blog_id">
								...
								<?php foreach ( $this->getVideoPressEnabledBlogs() as $blog ) : ?>
									<option
	...
	function admin_menu() {
		add_media_page( __( 'VideoPress Library', 'jetpack' ), __( 'VideoPress', 'jetpack' ), 'upload_files', 'videopress-library', array(
			$this,
			'admin_menu_library'
		) );
	}
	...
	function admin_menu_library() {
		...
		<div class="wrap" style="max-width: 600px;">
			...
			<p class="hide-if-no-js"><a href="#" id="videopress-browse"
			                            class="button"><?php _e( 'Browse Your VideoPress Library', 'jetpack' ); ?></a>
			</p>
	...
	function enqueue_admin_scripts() {
		...
		wp_enqueue_script( 'videopress-admin', plugins_url( 'js/videopress-admin.js', __FILE__ ), array(
			'jquery',
			'media-views',
			'media-models'
		), $this->version );
	...
	/**
	 * Get all of the blogs that are VideoPress enabled.
	 *
	 * @return array
	 */
	protected function getVideoPressEnabledBlogs() {
		$options = $this->get_options();

		if ( ! isset ( $options['blogs'] ) || ! is_array( $options['blogs'] ) ) {
			$options['blogs'] = array();
		}

		if ( $this->isVideoPressIncludedInJetpackPlan() ) {
			array_unshift( $options['blogs'], array(
				'blog_id' => Jetpack_Options::get_option( 'id' ),
				'name'    => 'Use this blog\'s Jetpack Plan',
				'domain'  => parse_url( get_site_url(), PHP_URL_HOST ),
			) );
		}

		return $options['blogs'];
	}
}
