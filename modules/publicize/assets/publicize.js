...
jQuery( function( $ ) {
	showOptionsPage = function( service, nonce, connection, blogId ) {
		...
		$.post(
			ajaxurl,
			'action=publicize_' + service + '_options_page&_wpnonce=' + nonce + query,
			function( response ) {
				$( '#TB_load' ).remove();
				...
				try {
					...
					if ( obj && 'object' === typeof obj ) {
						if ( obj.hasOwnProperty( 'fb_redirect' ) ) {
							location.href =
								obj.fb_redirect + '&redirect_uri=' + encodeURIComponent( location.href );
							return;
						}
					}
				} catch ( err ) {
				...
				if ( response ) {
					...
					var message = $( '<div id="wpas-ajax-' + blogID + '" class="wrap"></div>' ).append(
						response
					);
					message.append(
						'<a href="#TB_inline?thickbox&height=420&width=555&inlineId=wpas-ajax-' +
							blogID +
							'" id="wpas-click-' +
							blogID +
							'" class="new-thickbox" style="display: none;"></a>'
					);
					...
					if ( content_height < tb_height ) {
						...
						var new_margin =
							parseInt( $( '#TB_window' ).css( 'margin-top' ), 10 ) +
							( tb_height - new_height ) / 2 +
							'px';
							...
							$.post(
								...
								function(/*response*/) {
									...
									if ( /inside-newdash=1/.test( document.location.search ) && frameNonce ) {
										document.location =
											'options-general.php?page=sharing&inside-newdash=1&frame-nonce' +
											frameNonce[ 1 ];
									} else {
	...
	$( '.wpas-posts' ).change( function() {
		...
		$.post(
			ajaxurl,
			inputs.serialize() + '&action=wpas_post',
			function( response ) {
	...
	publicizeConnTestStart = function() {
		$( '.pub-connection-test' ).addClass( 'test-in-progress' );
		...
		$.each( response.data, function( index, testResult ) {
			...
			if ( testResult.connectionTestPassed ) {
				...
				$( testSelector )
					...
					.html(
						'<p><span class="pub-connection-error">' +
