...
class Jetpack_Subscriptions_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( self::is_current_user_subscribed() ) {
			...
			self::render_widget_subscription_form( $instance, $args, $subscribe_email );
		}

		echo "\n" . $after_widget;
		if ( self::is_wpcom() ) {
			stats_extra( 'widget_view', 'follow_blog' ); // previously blog-sub
		}

	}

	static function render_widget_title( $instance, $args ) {
	...
	static function render_widget_subscription_form( $instance, $args, $subscribe_email ) {
		...
		if ( self::is_wpcom() && ! self::wpcom_has_status_message() ) {
			global $current_blog;
			?>
            <form action="<?php echo SUBSCRIBE_BLOG_URL; ?>" method="post" accept-charset="utf-8"
	...
	// Used to fetch subscriber count on self-hosted sites
	...
	function defaults() {
		...
		if ( self::is_jetpack() ) {
			...
			$defaults['subscribe_text']        = esc_html__( 'Subscribe to Blog via Email', 'jetpack' );
			$defaults['subscribe_placeholder'] = esc_html__( 'Subscribe to Blog via Email', 'jetpack' );
			$defaults['subscribe_button']      = esc_html__( 'Subscribe to Blog via Email', 'jetpack' );
			$defaults['success_message']       = esc_html__( 'Subscribe to Blog via Email', 'jetpack' );
		}
