...
<?php
	...
	public function get_display( $post ) {
		return $this->get_link( get_permalink( $post->ID ), _x( 'Email', 'share to', 'jetpack' ), __( 'Click to email', 'jetpack' ), 'share=email' );
	}
...
class Share_Pinterest extends Sharing_Source {
	...
	public function process_request( $post, array $post_data ) {
		...
		if ( !isset( $_GET['js_only'] ) ) {
			$pinterest_url = esc_url_raw( '//www.pinterest.com/pin/create/button/?url=' . rawurlencode( $this->get_share_url( $post->ID ) ) . '&description=' . rawurlencode( $this->get_share_title( $post->ID ) ) );
...
class Share_Skype extends Sharing_Source {
	...
	public function get_display( $post ) {
		...
		return $this->get_link( get_permalink( $post->ID ), _x( 'Skype', 'share to', 'jetpack' ), __( 'Click to share on Skype', 'jetpack' ), 'share=skype', 'sharing-skype-' . $post->ID );
	}
