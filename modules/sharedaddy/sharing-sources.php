...
class Share_Twitter extends Sharing_Source {
	...
	public function get_display( $post ) {
		...
		if ( $this->smart ) {
			...
			return '<div class="twitter_button"><iframe allowtransparency="true" frameborder="0" scrolling="no" src="' . esc_url( $this->http() . '://platform.twitter.com/widgets/tweet_button.html?url=' . rawurlencode( $share_url ) . '&counturl=' . rawurlencode( get_permalink( $post->ID ) ) . '&count=horizontal&text=' . rawurlencode( $post_title . ':' ) . $via ) . '" style="width:65px; height:20px;"></iframe></div>';
		} else {
