...
abstract class Jetpack_Tiled_Gallery_Item {
	...
	public function __construct( $attachment_image, $needs_attachment_link, $grayscale ) {
		...
		// Move [a-t]*.files.wordpress.com traffic over to only setting the crop argument when it's true, which works in
		// tandem with the new URLs generated by the change in `jetpack_photon_url`. Reference: http://wp.me/p3topS-mu
		if ( preg_match( '/^https?:\/\/([a-t][^\/]*)\.files\.wordpress\.com\/.+/', $this->orig_file ) ) {
			$img_args = array(
				'w' => $this->image->width,
				'h' => $this->image->height,
			);
			...
			// The legacy code runs for all other images...
			// If h and w are the same, there's a reasonably good chance the image will need cropping to avoid being stretched.
