...
class Grunion_Contact_Form_Plugin {
	...
	protected function __construct() {
		...
		// Add to REST API post type safelist.
...
class Grunion_Contact_Form extends Crunion_Contact_Form_Shortcode {
	 ...
	 *
	 * Important: Currently the safelisted fields are defined as follows:
	 *  `name`, `email`, `url`, `subject`, `textarea`
	 *
	 * If you need to add new fields to the Contact Form, please don't add them
	 * to the safelisted fields and leave them as extra fields.
	 *
	 ...
	 * safelisted fields without taking proper care to add them to all the
	 * other places where they accessed/used/saved.
	 *
	 * The safest way to add new fields is to add them to the dropdown and the
	 * HTML list ( @see Grunion_Contact_Form_Field::render ) and don't add them
	 * to the list of safelisted fields. This way they will become a part of the
	 * `extra fields` which are saved in the post meta and will be properly
	 * handled by the admin Feedback view and the CSV Export without any extra
	 * work.
	 *
	 * If there is need to add a field to the safelisted fields, then please
	 * take proper care to add logic to handle the field in the following places:
	 *
	 *  - Below in the switch statement - so the field is recognized as safelisted.
	 *
	...
	function get_field_ids() {
		$field_ids = array(
			...
			'extra' => array(), // array of all non-safelisted field IDs.

			// safelisted "standard" field IDs:
			// 'email'    => field_id,
			// 'name'     => field_id,
			// 'url'      => field_id,
			// 'subject'  => field_id,
		...
		foreach ( $this->fields as $id => $field ) {
			...
			if ( isset( $field_ids[ $type ] ) ) {
				// This type of field is already present in our safelist of "standard" fields for this form
				// Put it in extra
				$field_ids['extra'][] = $id;
				continue;
			}
