...
class Grunion_Contact_Form extends Crunion_Contact_Form_Shortcode {
	...
	function process_submission() {
		...
		// In addition, the @ should silence any other warnings it may get.
		$print_r_all_values = function_exists( 'print_r' ) ? @print_r( $all_values, true ) : @var_export( $all_values, true );
		
		/**
		 * Filter the results returned by print_r (or var_export when the former is disabled) for Feedback post data
		 *
		 * @module contact-form
		 *
		 * @since 4.5.1
		 *
		 * @param string|integer|float $all_values_string The string for searchability (originally generated by print_r)
		 * @param array $all_values The array with all Feedback post values, if needed.
		 */
		$all_values_string = apply_filters( 'grunion_print_all_values', $print_r_all_values, $all_values );
