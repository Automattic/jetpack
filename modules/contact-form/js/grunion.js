/* global GrunionFB_i18n:true, FB, console, alert, confirm, ajax_nonce_shortcode, ajax_nonce_json, postId, ajaxurl */
...
FB.ContactForm = (function() {
	var fbForm = { // Main object that generated shortcode via AJAX call
		...
		debug = false, // will print errors to log if true
		grunionNewCount = 0, // increment for new fields
		maxNewFields = 5,  // Limits number of new fields available
		optionsCache = {},
		optionsCount = 0, // increment for options
		shortcode;
	...
	function loadFieldEditor (id) {
		try {
			...
			if (thisType === 'select' || thisType === 'radio') {
				thisResult = '',
				thisOptions = fbForm.fields[id].options;
	...
	function removeOption (optionId) {
		try {
			var thisId = jQuery('#fb-field-id').val(),
				thisVal = jQuery('#fb-option' + optionId).val(),
				thisType = jQuery('#fb-new-type').val(),
				idx;
			// Remove from right
			...
			idx = fbForm.fields[thisId].options.indexOf(thisVal);
	...
	function sendShortcodeToEditor () {
		try {
			...
			jQuery.post(ajaxurl, fbForm, function(response) {
				...
				regexp = new RegExp('\\[contact-form\\b.*?\\/?\\](?:[\\s\\S]+?\\[\\/contact-form\\])?');
	...
	function updateLabel () {
		try {
			var thisId = jQuery('#fb-field-id').val(),
				thisLabel = jQuery('#fb-new-label').val();
			// Update preview
	...
	function updateOption (that) {
		try {
			var thisId = jQuery('#fb-field-id').val(),
				thisOptionid = that.attr('optionid'),
				thisOptionValue = that.val(),
				thisType = jQuery('#fb-new-type').val();
			// Update preview
			if (thisType === 'radio') {
	...
	function updateRequired () {
		try {
			var thisId = jQuery('#fb-field-id').val(),
				thisChecked = jQuery('#fb-new-required').is(':checked');
	...
	function updateType(thisType, thisLabelText, thisRequired) {
		try {
			var isRequired      = (thisRequired) ? '<span class="label-required">' + GrunionFB_i18n.requiredLabel + '</span>' : '',
				thisId          = jQuery('#fb-field-id').val(),
				thisRadio       = '<input type="radio" name="radio-' + thisId + '" id="fb-field' + thisId + ' "disabled="disabled" />',
				thisRadioRemove = '<div class="fb-remove fb-remove-small" id="' +  thisId + '"></div>',
				thisRemove      = '<div class="fb-remove" id="' +  thisId + '"></div>',
				thisCheckbox    = '<input type="checkbox" id="fb-field' + thisId + '" "disabled="disabled" />',
				thisText        = '<input type="text" id="fb-field' + thisId + '" "disabled="disabled" />',
				thisTextarea    = '<textarea id="fb-field' + thisId + '" "disabled="disabled"></textarea>',
				thisClear       = '<div class="clear"></div>',
				thisSelect      = '<select id="fb-field' + thisId + '" fieldid="' + thisId + '"><option id="fb-' + thisId + '-' + optionsCount + '" value="' + thisId + '-' + optionsCount + '">' + GrunionFB_i18n.firstOptionLabel + '</option></select>',
				thisLabel,
				thisRadioLabel;
			...
			thisLabel = '<label fieldid="' + thisId + '" for="fb-field' +  thisId + '"><span class="label-text">' + FB.esc_html( thisLabelText ) + '</span>' + isRequired + '</label>';
			thisRadioLabel = '<label fieldid="' + thisId + '" for="fb-field' +  thisId + '" class="fb-radio-label"><span class="label-text">' + FB.esc_html( thisLabelText ) + '</span>' + isRequired + '</label>';
	...
	return {
		resizePop: function () {
			try {
				...
				var totalWidth = jQuery('body', window.parent.document).width(),
					totalHeight = jQuery('body', window.parent.document).height(),
		...
		init: function () {
			var win, contentSource, data, $doc;
