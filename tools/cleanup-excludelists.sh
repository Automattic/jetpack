...
jq -r --arg pwd "$PWD/" '[ .[] | select( .messages[0] ) | .filePath | ltrimstr($pwd) ] | sort' "$TEMP" | yarn run -s prettier --parser=json > tools/eslint-excludelist.json
...
[[ -s "$TEMP" ]] && jq -e '.' < "$TEMP" >/dev/null || { echo "No JSON data found"; cat "$TEMP"; exit 1; }
jq -r --arg pwd "$PWD/" '[ .files | to_entries | .[] | select( .value.errors > 0 or .value.warnings > 0 ) | .key | ltrimstr($pwd) ] | sort' "$TEMP" | yarn run -s prettier --parser=json > tools/phpcs-excludelist.json
