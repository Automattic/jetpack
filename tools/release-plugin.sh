...
if ! command -v gh &> /dev/null; then
	...
	if command -v brew &> /dev/null; then
		proceed_p "Install the GitHub CLI via brew?" "" true
		brew install gh
	else
...
if ! gh auth status --hostname github.com &> /dev/null; then
	...
	proceed_p "Sign in to the GitHub CLI?" "" true
	gh auth login
fi
...
proceed_p "" "Proceed releasing above projects?" true
...
if [[ "$CURRENT_BRANCH" != "trunk" ]]; then
	proceed_p "Not currently checked out to trunk." "Check out trunk before continuing?" true
...
if git rev-parse --verify prerelease &>/dev/null; then
	proceed_p "Existing prerelease branch found." "Delete it?" true
...
if [[ -v PROJECTS["plugins/jetpack"] ]]; then
  if proceed_p "Do you want to start a new cycle for Jetpack?" "" true; then
