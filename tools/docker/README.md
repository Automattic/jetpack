...
Jetpack Docker provides two types of configurations: `dev` and `e2e`. These configurations defines list of services to start, volumes to map, etc. Both of them extends default configuration `tools/docker/docker-compose.yml` via the config file: `tools/docker/jetpack-docker-config-default.yml`. 

* `dev` configuration is used by default, and aimed for Jetpack development
...
Default config file include inline comments explaining the structure of config `tools/docker/jetpack-docker-config-default.yml`, but here's quick overview. The configuration grouped per environment type: `default`, `dev`, `e2e`. Each type could define `volumeMappings` and `extras`:
