...
async function dockerUp( argv ) {
	...
	try {
		await compose.upAll( dockerOptions( verbose ) );
	} catch ( e ) {
	...
	try {
		await compose.exec( 'wordpress', '/usr/local/bin/run', dockerOptions( verbose ) );
	} catch ( e ) {
		spinner.isSpinning && spinner.stop();
		log( e );
		log( 'There was an error. See above.' );
	...
	spinner.isSpinning &&
		spinner.succeed(
			'Initial commands finished! Check out http://localhost/ to see the test site.'
		);
}
...
async function dockerDown( argv ) {
	...
	try {
		await compose.down( dockerOptions( verbose ) );
	} catch ( e ) {
		spinner.isSpinning && spinner.stop();
		log( e );
		log( 'There was an error. See above.' );
